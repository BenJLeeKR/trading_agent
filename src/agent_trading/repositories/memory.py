...
class InMemoryCashBalanceSnapshotRepository:
    ...
    async def get_latest_with_orderable_amount(
        ...
        candidates = [
            item
            for item in self._items.values()
            if item.account_id == account_id and item.orderable_amount is not None
        ]
        if not candidates:
            return None
        candidates.sort(key=lambda item: item.snapshot_at, reverse=True)
    ...
    async def sync_loss_cut_shadow_observation(
        ...
        *,
        ...
        if existing is None:
            return None
        merged_decision_json = dict(existing.decision_json or {})
        merged_decision_json["loss_cut_shadow"] = dict(loss_cut_shadow_payload)
    ...
    async def sync_shadow_held_position_reduce_skip_observation(
        ...
        *,
        ...
        if existing is None:
            return None
        merged_decision_json = dict(existing.decision_json or {})
        merged_decision_json["shadow_held_position_reduce_skip"] = dict(
            shadow_held_position_reduce_skip_payload
        )
        updated = replace(existing, decision_json=merged_decision_json)
        self._items[trade_decision_id] = updated
        return updated
...
class InMemoryFdcQuotaRepository:
    ...
    __slots__ = ("_lock", "_attempts", "_jobs")
    ...
    async def judge_shadow_reservation(
        ...
        *,
        ...
        self._attempts[quota_scope].append(
            (datetime.now(timezone.utc), "shadow", outcome)
        )
        ...
        return ShadowJudgement(
            would_grant=would_grant, window_count=window_count, attempt_id=uuid4(),
        )
