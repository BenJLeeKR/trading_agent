    ...
    async def sync_shadow_held_position_fdc_skip_observation(
        ...
        *,
        ...
        row = await self._tx.connection.fetchrow(
            ...
            """,
            trade_decision_id,
            json.dumps(shadow_held_position_fdc_skip_payload),
        )
        if row is None:
            return None
        return row_to_entity(row, TradeDecisionEntity)
    ...
    async def sync_shadow_held_position_reduce_skip_observation(
        ...
        *,
        ...
        row = await self._tx.connection.fetchrow(
            ...
            SET decision_json = jsonb_set(
                COALESCE(decision_json, '{}'::jsonb),
                '{shadow_held_position_reduce_skip}',
                $2::jsonb,
                true
            )
            WHERE trade_decision_id = $1
            ...
            """,
            trade_decision_id,
            json.dumps(shadow_held_position_reduce_skip_payload),
        )
        if row is None:
            return None
        return row_to_entity(row, TradeDecisionEntity)
