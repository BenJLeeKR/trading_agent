...
### 2026-07-29 — `tmp/` 추적 제외

- `tmp/` tracked 파일 `55`개를 Git 추적에서 제거했다.
- 정확 참조 파일 `1`개(`tmp/measure_dschat_latency.py`)는 정식 스크립트로 승격하지 않았다.
- 해당 파일은 `.env` 기반 외부 API latency 측정용 임시 스크립트였으므로, active `scripts/` 경로로 이동하지 않고 문서에는 역사적 파일명 텍스트로만 남겼다.
- 문서 링크 `3`곳은 깨진 링크가 되지 않도록 Markdown 링크에서 코드 텍스트로 전환했다.
