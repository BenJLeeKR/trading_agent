...
**[2026-08-06 KST 후속, 가상 진입가 방법론 표준안 제안 + 시범 적용]** §34.5가 미해결로 남긴 가상 진입가 방법론을 `buy_path_variable_gate_matrix.md` §21에서 재구성했다. §34.6이 제안한 Newey-West/quintile 통계 계층은 현재 B축 표본 규모(2026-08-03+ 기준 `downgraded` 6/`suppressed` 40개 symbol×date 단위)와 맞지 않아, A/C축이 실제로 써온 **종가 기준 단순 T+1/T+2 근사**(결정일 종가=가상 진입가) 방식으로 통일할 것을 제안한다. 08-03/08-04 결정일 표본으로 시범 계산한 결과 T+1/T+2 평균이 양수였으나(§21.5), `matched` 비교군 계산이 아직 없어 "하향이 좋은 진입을 놓쳤다"는 결론은 내리지 않는다 — 다음 착수 턴 1순위는 `matched` 비교군 동기간 계산이다.
