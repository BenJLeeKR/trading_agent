...
이 시스템의 목표는 **손실을 0으로 만드는 것이 아니라, 감내 가능한 손실 제약 아래에서 최고의 기대수익률을 추구하는 것**이다. 이것은 이 문서에서 새로 만드는 원칙이 아니라, 2026-07-14 사용자가 이미 명시적으로 확정한 목표(아래 "권장 실행 순서"의 07-14 근본 재정렬 항목 — "목표 B(최고 기대수익률)": "손실 0이 목적이 아니라, 손실 최소화하며 리스크 감내 후 기대수익 추구" = 허용 손실 제약 아래 net expected return 극대화, "주문 0건은 성공이 아니라 실패")를 이 문서의 맨 앞으로 끌어올려 앞으로 모든 우선순위 판단의 공통 틀로 삼는 것이다.

### 차단 장치 검증 원칙

- 차단 장치(eligibility gate, downstream override, expected value gate 등)의 적정성은 "손실을 얼마나 줄였는가"가 아니라 **기대값을 실제로 개선했는가**로 판단한다.
- 차단률/차단 빈도가 늘었다는 사실만으로 "과잉 차단"이라 단정하지 않는다. 반대로 통과군 평균 수익률이 높다는 사실만으로 "차단이 적정하다"고 단정하지도 않는다.
- **모든 차단 장치 적정성 판단에는 반드시 사후 성과(백테스트) 검증을 포함한다** — 빈도/분포 분석만으로 결론을 내리지 않는다.
- 실제로 매수 후보가 될 수 있었던 경계 구간(`entry_score` 문턱 근접)의 사후 성과를 전체 population 평균보다 우선해서 본다 — 전체 population은 원래 매수 후보가 될 가능성이 낮은 표본이 대부분이라, 그 구간의 차단은 운영 비용이 크지 않다.
- allocation(자본 배정) 영향, `eligibility_low_relative_activity`(활동성 부족), `buy_candidate`(매수 후보 판정) 이후 downstream 차단은 모두 이 원칙 하나로 다시 읽는다 — "무조건 막는 것이 안전한가"가 아니라 **"이 차단이 기대값을 실제로 높이는가"**로 판단한다. 최근 실측(2026-08-03 이후 `BUY` 경로 기준)은 `docs/20_system_analysis/buy_path_variable_gate_matrix.md` §13.2.13/§13.2.14(allocation)와 §13.4.4(활동성 부족)에 있으며, 둘 다 이 원칙을 적용해 현재 "미확정"으로 남아 있다.

### 주문 요청 0건 수렴에 대한 해석 기준

- 주문 요청(`order_request`)이 0건에 수렴하는 상태는 안전성의 증거가 아니라 **기회비용 증가와 시스템 목적(기대수익률 추구) 훼손 가능성**으로 먼저 본다.
- 다만 이것이 "무조건 더 자주 사야 한다"는 뜻은 아니다 — 그 0건이 실제로 기대값 개선의 결과인지, 과잉 방어의 결과인지는 반드시 사후 성과로 구분한다.

### 현재 우선순위 해석 기준

- 새 검증 과제를 우선순위에 올릴 때는 "이 차단/게이트가 기대수익률 관점에서 실제로 기여하는가"를 1차 기준으로 삼는다.
- 표본이 작아 결론을 내릴 수 없는 경우 "미확정"으로 남기고, 그 상태 자체를 다음 우선순위 판단에 반영한다 — 표본이 쌓일 때까지 결론을 강요하지 않는다.
- 상세 실측·백테스트 방법론과 결과는 근거 문서(`docs/20_system_analysis/buy_path_variable_gate_matrix.md` 등)에 있다 — 이 문서(우선순위 정리)는 그 결과를 우선순위 판단에 어떻게 반영할지만 다루고, 같은 수치를 복붙하지 않는다.

### 차단 장치 3층 분석 프레임(모든 차단 장치에 공통 적용)

어떤 차단 장치를 검증하든 아래 3층을 분리한다. **1번이 참이라고 3번까지 참인 것은 아니다.**

1. **구조적으로 실제 발동하는가**(살아있는 코드/게이트인가) — 코드 추적·이력 조회로 확인 가능. 예: `eligibility_low_relative_activity`는 최근 구간에서 실제로 다수 발동한다(§13.4.4).
2. **어떤 표본을 주로 막는가**(전체 population vs 경계 고득점 구간) — 전체 population 대부분은 원래 매수 후보가 될 가능성이 낮은 표본이라, 여기를 막는 것은 운영 비용이 크지 않다. **운영 판단에서 실제로 중요한 것은 경계 고득점 구간**(`entry_score>=0.60`, 특히 `>=0.65`)이다.
3. **그 차단이 실제 기대수익률 개선에 기여하는가**(사후 성과, 백테스트) — 2번에서 식별한 구간별로 별도로 확인해야 한다. 전체 population 결과를 경계 구간에 그대로 적용하지 않는다.

### 4축 분리 관리 원칙(하나의 "막힘"으로 뭉치지 않는다)

`BUY` 경로의 차단은 발생 단계가 다른 4개 축으로 분리해서 추적한다 — 서로 다른 코드 지점, 서로 다른 근거 강도를 갖고 있어 하나로 합치면 원인 진단을 그르친다.

| 축 | 영향 단계 | 대표 필드 | 근거 문서 위치 | 현재 판정 |
|---|---|---|---|---|
...
이 절은 위 3층 프레임·4축 분리를 실제 다음 작업 순서로 구체화한다. 코드 수정 없음, 새 실측·새 결론 없음 — **후속 검증 계획만 정리**한다.

**[2026-08-06 KST 후속]** 이 4축(①~④, 아래 표의 A~D와 동일 축) 실측을 종합 재정리한 내용과 축별 factual/해석/미확정 구분, 다음 작업자용 실행 가능한 프롬프트 재료는 `buy_path_variable_gate_matrix.md` §20에 있다. 이 문서에는 우선순위 요약만 유지하고 상세 재배치는 중복 기록하지 않는다. 08-07 KST EV 게이트(④) 종가 재계산은 §20.6에서 다음 턴으로 명시 이월했다 — 이번 턴에서는 착수하지 않는다.

### 다음 1순위: 주문요청 0건 원인의 단계별 분해 표 신설(④ + 종합) — **[2026-08-06 KST 실측 완료]**

**왜 1순위인가**: ①~③ 어디가 실제 병목인지는, 하루 단위로 "어디서 몇 건이 빠지는지"를 정량화해야 비로소 비교 가능하다. 이 분해가 없으면 "차단이 과한지"도, "주문 0건이 정상인지"도 말할 수 없다.
...
1. 2026-08-03 이후 매 거래일 단위로 아래 퍼널을 집계한다.
   - 전체 판단 대상 수(`trade_decisions` 전체 행)
   - 기본 적격성 탈락 수(`eligibility_passed=false`, 사유별로도 분해: 활동성 부족 vs 기타)
   - 매수 후보(`buy_candidate=true`) 수
   - downstream 하향 수(`candidate_vs_final.alignment_status`가 `downgraded`/`suppressed`인 수)
   - 최종 `decision_type=buy`/`approve` 수
   - `expected_value_gate.passed=false`로 막힌 수
   - 실제 `order_request` 생성 수
2. 이 표를 근거 문서(`buy_path_variable_gate_matrix.md`)에 날짜별로 쌓는다.
3. "매수 후보는 있었는데 주문 요청이 0건"인 날이 있으면, 그 표에서 어느 단계 병목인지 즉시 특정 가능해야 한다(이전 사례: `035420` 08-05 유일한 `buy` 판정이 `expected_value_gate`에서 막힘 — 이 표의 첫 사례로 흡수).

### 다음 1-B순위: ② 활동성 부족 게이트 — 경계 구간 표본 축적 지속

**모집단 분리는 계속 유지한다**: 전체 population, `[0.60,0.65)`, `entry_score>=0.65`, chronic 100% 차단 종목군(현재 18개 symbol×source_type 조합)으로 계속 나눠 본다 — 하나로 합치지 않는다.

- **추적 대상 종목**: `009420`(핵심 — 직전 4거래일 +36.2% 급등 + 낮은 활동성 서지, T+1 미도착), `180640`(유일하게 계산된 경계 초과 사례, T+1 +1.74%), `073240`/`078930`/`035420`/`051900`/`008930`(경계 근접, allocation 트랙과 겹침).
- **horizon 확장 계획**: 현재 T+1/T+2까지만 계산 가능(거래일 4개뿐). 거래일이 쌓이는 대로 **T+5**까지 확장한다(2026-08-03 기준 5거래일 후 시점 도달 시 재계산).
- **가격 데이터 공백 처리 기준**(`181710` 사례로 확정): 1차 소스(`instrument_status_snapshots`)에 공백이 있으면 2차 소스(`signal_feature_snapshots`의 `sma_20*(1+price_vs_sma_20_pct/100)` 파생 종가, `009420`으로 재현성 검증됨)로 자동 대체한다. 두 소스 모두 없으면 "관측 불가"로 명시하고 표본에서 조용히 빼지 않고 별도로 카운트한다. "가격 데이터 전무"와 "해당 horizon 미도착"은 항상 구분해서 기록한다.
- **판정 갱신 기준**: 경계 구간 표본이 최소 두 자릿수(대략 n>=10)로 쌓이기 전에는 "미확정"을 유지한다.
- **[2026-08-06 KST 추가]** 종목 단위 퍼널 분해(`buy_path_variable_gate_matrix.md` §16)로 chronic 활동성 차단 상위 종목(`081660` 180건, `073240`/`138040` 각 165건)을 확정했다 — 위 추적 대상 종목 목록에 추가.

### 다음 2순위: ③ downstream 하향의 구조적 재분해

**qualitative 필드(`risk_opinion`/`evidence_strength`)에 다시 기대지 않는다** — 판별력 부족이 재검증으로 확인됐다(§13.2.14). 대신:

- `candidate_vs_final.alignment_status`(관측용 분류)는 유지하되, 그 위에 **실제 어느 guard/EV/risk/compliance 단계가 하향시켰는지**를 `decision_json`의 구조적 필드(`override_applied`, `reason_codes`, `expected_value_gate.passed`, `risk_check_passed`) 조합으로 재분해한다.
- 같은 날 실제 `buy_candidate=true` 비교군(예: `035420`/`181710` 계열)의 매 사이클을 이 단계별로 분해해 "어느 단계에서 몇 %가 걸리는지" 표로 만든다.
- 사후 성과 연결: `downgraded`/`suppressed` 표본의 "가상 진입가" 방법론(§34.5에서 이미 미해결로 남긴 선행 과제)을 확정한 뒤, 그 표본의 forward return을 계산해야 "이 downstream 차단이 기대값을 실제로 개선했는가"에 답할 수 있다 — 이번 턴에서 확정하지 않는다.

**[2026-08-06 KST 후속, 가상 진입가 방법론 표준안 제안 + 시범 적용]** §34.5가 미해결로 남긴 가상 진입가 방법론을 `buy_path_variable_gate_matrix.md` §21에서 재구성했다. §34.6이 제안한 Newey-West/quintile 통계 계층은 현재 B축 표본 규모(2026-08-03+ 기준 `downgraded` 6/`suppressed` 40개 symbol×date 단위)와 맞지 않아, A/C축이 실제로 써온 **종가 기준 단순 T+1/T+2 근사**(결정일 종가=가상 진입가) 방식으로 통일할 것을 제안한다.

**[2026-08-06 KST 재검증]** 시범 표의 `suppressed` 08-03 수치가 표본 누락으로 부정확했음을 확인해 보정했고(§21.5.1), `matched` 비교군을 추가했다. 세 버킷(`matched`/`downgraded`/`suppressed`) 모두 계산 가능 구간에서 T+1/T+2 평균이 양수로 같은 방향이었으나, **population이 버킷 간에 거의 완전히 중복된다는 더 근본적인 한계**를 발견했다(`alignment_status`는 사이클 단위로 갈리는데 forward return은 종가=일 단위로만 계산 가능해 같은 종목·같은 날이 여러 버킷에 동시 집계됨). **이 population 정의로는 "하향이 기대수익률을 개선했는지"를 판단할 수 없다** — 다음 착수 턴은 이 중복을 해소하는 비교 방식을 먼저 확정해야 한다. 상세는 `buy_path_variable_gate_matrix.md` §21.5~§21.7.

**[2026-08-06 KST 후속, 중복 해소 방법론 확정]** "지배적 상태(사이클 다수결) + 오염도 명시" 방식을 표준으로 채택했다(3안 비교, `buy_path_variable_gate_matrix.md` §22.2~22.3). 재집계 결과 08-04에는 지배적으로 `suppressed`/`downgraded`인 `(symbol, date)`가 **0건**임을 확인했다 — 이전에 "퇴화된 비교"로 봤던 08-04 경계 구간 3종목은 실제로는 그날 대부분(77.8~79.6%) `matched`였다. 08-03은 세 버킷(`matched`/`suppressed`/`downgraded`) 모두 T+1/T+2 평균이 여전히 양(+)이나 n=4~6·시장 조정 없음이라 방향성 결론은 유보한다. 다음 단계는 SPPV-2.6의 KODEX 200 벤치마크 선례를 재사용한 시장 조정 도입. 상세는 §22.4~22.6.

**[2026-08-06 KST 후속, 시장 조정 도입 결과]** KODEX 200(파생 종가) 기준 초과수익률을 계산한 결과, 이 2일 구간이 강한 상승장(T+1 +1.24~+3.96%, T+2 +5.24%)이었고 `matched` 버킷조차 시장 조정 후 초과수익률이 음(-)임을 확인했다 — 원수익률의 양(+)은 베타 효과였을 가능성이 실측으로 뒷받침됐다. `suppressed`/`downgraded`(08-03)의 초과수익률은 T+1 양(+)·T+2 음(-)으로 horizon에 따라 방향이 바뀌어 방향성 결론은 여전히 유보한다. 오염도 임계치는 30%/40% 동일 결과, 50%는 근소 다수결 사례를 오분류해 부적절함을 확인해 **40%(또는 30%) 유지**로 확정했다. `signal_feature_snapshots`가 08-06 종가까지 갱신된 사실을 새로 확인했으나 이번 턴 범위 밖으로 다음 턴에 이월했다. 상세는 §22.7~§22.10.

**[2026-08-06 KST 후속, 시장 벤치마크 비교 폐기 + 08-05/08-06 재집계]** 시장 벤치마크 비교를 기본 경로에서 폐기하고(한국 지수의 삼성전자/SK하이닉스 편중 왜곡 우려) 절대수익률+대표 라벨 비교로 전환했다. 08-05는 `matched` 14/`downgraded` 2(`035420`/`181710`)/`suppressed` 0으로 분리되며, T+1(08-06 종가, 1차+파생 혼용) 절대수익률이 `matched` +5.16%/`downgraded` -0.31%로 **08-03과 정반대 방향**이 나왔다 — 날짜 하나로 일반화 불가함을 재확인. 08-06은 `matched` 14/`downgraded` 2(`008930` 오염도 40.4%로 임계치 경계)/`suppressed` 1이나, T+1/T+2는 08-07/08-08이 미래라 계산 불가(population만 확정). 가격 소스는 1차(`instrument_status_snapshots`, 08-05까지)를 기본으로 하고, 08-06 이후 날짜나 `181710`(1차 소스 결측)만 파생 소스(`signal_feature_snapshots`)로 보완했다. 상세는 §22.11~§22.14.

**[2026-08-07 KST 후속, 4거래일 누적 재평가 — B/C축 "완화 후보" 명시]** "08-06 결정분 T+1"/"최근 7일 누적" 지시는 실측 결과 이번 턴에도 충족되지 않음을 정정했다(08-07 종가가 아직 없는 진짜 미래 날짜, 관측 가능 구간은 4거래일뿐). 대신 08-06 종가의 1차 소스 반영으로 08-05 T+1을 확정하고(파생 소스와 완전 일치, §22.11 미확인 전제 해소) 08-04 T+2(13종목 전부 양(+), +5.85%)를 새로 계산했다. **C축**은 4거래일 중 독립 비교 가능한 쌍이 1개(`001450` vs `035420`)뿐이고 성과가 거의 동일(-1.34%/-1.31%) — "판단할 표본 자체의 부재"로 완화 후보 명시. **B축**은 비교 가능한 날이 2일(08-03/08-05)뿐이고 방향이 정반대 — "있는 데이터의 방향 불안정"으로 완화 후보 명시. 두 축의 약점 성격이 달라 단순 서열화는 하지 않았고, 이번 턴은 판정 갱신까지만 하며 정책/코드 변경은 하지 않는다. 다음 트리거는 08-08 05:05 KST 이후 08-07 종가 반영. 상세는 §22.15, §23.

**[2026-08-07 KST 후속, 장 종료 전 완화 후보 설계 초안]** 실측 재계산 없이(08-07 장 미마감) B축/C축 완화 후보를 작은 완화→큰 완화 순으로 설계만 정리했다. B축은 가장 작은 후보조차 "어느 guard가 실제 하향을 만드는지" 재분해가 선행 조건임을 확인했다. `held_position` 이후 churn-control 계층과 EV 게이트 외부 하드 게이트는 논의 대상에서 제외했다. 장 종료 후 5단계 체크리스트를 문서화했다. 상세는 §24.

**[2026-08-07 KST 재정정]** 위 항목에서 C축 근소 부족(near-miss) 완화 후보를 판단할 때 단일 손실 사례(`001450`)를 "완화에 반하는 신호"로 서술한 것은 개별 손실 회피 프레이밍이라 정정했다. 이 시스템의 목표(감내 가능한 손실 제약 아래 전체 기대수익률 극대화)에 맞춰, n=1 사례는 참고 사례일 뿐이며 판단은 근소 부족 사례 모집단의 **평균** 기대값·기회비용·감내 가능한 손실 범위로 내리도록 §24.1/§24.5를 재정리했다. 상세는 §24.1, §24.5(1번), §24.7.

**[2026-08-07 KST 장 종료 후 후속]** `2026-08-07` 종가는 모든 내부 소스에 없어(외부 API는 재현성 근거로 이번 턴 미사용) forward return 계산은 여전히 불가하나, population은 확장됐다. B축: `entry_score`가 더 높은 종목들이 지배적 `downgraded`로 분류됐고 오염도 최고치(49.1%, `078930`) 신규 확인. C축: 신규 통과 사례(`068270`)가 **2026-08-03 이후 최초 실제 체결**로 이어졌고(`held_active` 최초 전환), 신규 근소 실패 사례(`000240`)도 확인해 근소 실패 참고 표본이 n=2로 늘었다. 구조적 확장만 있어 완화 후보 강도는 조정하지 않았다(B축 오염도 경계 재검토 대상만 4건으로 소폭 확대). 상세는 §25.
...
**[2026-08-08 KST 후속, rule-level 원인 분해 완료 + 재설계 후보안(A/B/C) 정리]** `downgraded`의 `reason_codes`에 orchestrator guard 전용 코드가 0건임을 확인해 하드 가드가 아니라 FDC 자체 판단임을 재확인했다. **`risk_tone`(구조적 필드)이 가장 강력한 단일 판별 변수**로 확인됐다 — `risk_off`에서 `BUY_CANDIDATE` 통과율 11.7%, `risk_on`에서 93.2%. 코드 추적 결과 `strategy_selection.py`의 `risk_tone == "risk_off"` 조건이 `regime_label`(추세)과 무관하게 `allowed_strategies`를 방어적으로 전면 제한하는 것이 이 메커니즘의 실질적 원인이다(`bullish_trend`여도 예외 없음). §28에서 확인한 `downgraded` 사후 성과(n=6, 평균 +1.29%)는 전부 `risk_off` 기반이라, 이 하향이 기대값을 개선했다는 근거는 여전히 없다. 재설계 후보안: (A) `evidence_strength`/종목 특정 이벤트 신호는 유지, (B) `risk_tone==risk_off` 단독 조건이 추세와 무관하게 모멘텀 전략을 전면 배제하는 것을 완화 후보로(`risk_off`+`bullish_trend` 조합 분리 검토), (C) 제거 후보는 아직 특정하지 않음(`matched` 사후 성과 없이는 근거 부족). `matched` 3건의 T+1은 `2026-08-10`(월) 종가가 필요해 여전히 계산 불가(거래일 미도래, 소스 문제 아님). 상세는 `buy_path_variable_gate_matrix.md` §29.

**[2026-08-08 KST 후속, `risk_off + downgraded` 표본 `08-07` 종가 고정 horizon 사후 성과]** 계산 가능한 6건의 평균 수익률 +2.81%, 양수율 66.7% — §28.2와 동일한 값으로 재확인. **신규 발견**: 표본 13개 단위 전부 `regime_label='bullish_trend'`라 §29.4(B)가 제안한 "`risk_off`+`bullish_trend` 조합 분리"는 이 구간 데이터로는 분리할 비교 대상이 없다. **[2026-08-08 KST 정정]** `strategy_policy_mismatch`가 6건 중 1건에만 지배적이라는 서술은 사실 오류였다 — 재확인 결과 계산 가능한 6건·대표 표본 13건 전부에 이 태그가 최소 1사이클 이상 존재해, 이 표본 내부에서는 분해 축으로 쓸 수 없는 공통 태그다(독립 설명력 판단 불가, 무의미하다는 뜻도 아님). §29의 "완화 후보/구조 재설계 후보" 판정 유지, "즉시 완화"로는 올리지 않음(`matched` 비교군 여전히 전무, `08-10` 이후 필요). 상세는 §30.
...
**[2026-08-06 KST 재갱신, EV 게이트 메커니즘 코드 추적]** EV 게이트(§6)의 하방/비용/슬리피지 값 자체는 임의 고정값이 아니라 종목별 실측 ATR·거래량 기반 공식으로 확인됐으나, 이번 대상 종목(`008930`/`051900`)의 ATR 성분이 상한에 포화돼 실질 통과에 필요한 `score_anchor`가 0.8~0.9대로 역산되는 반면 상류 candidate 기준선은 ≈0.65 — **상류/하류 기준선 간극**이 구조적으로 확인됐다. 상세는 `buy_path_variable_gate_matrix.md` §18, `[BACKLOG] backlog.md` 해당 후속 항목.

**[2026-08-06 KST 재정정, 목적 차원 판정 강도 상향]** 위 항목의 확신 수준을 "개별 파라미터 적정성"과 "시스템 목적 차원의 괴리"로 분리해 재판정한다. **개별 파라미터(비용/하방 공식의 계수·상한)의 사후 성과 기준 적정성은 여전히 "미확정"**이다 — 이 판단은 낮추지 않는다. 그러나 이 설계(`[DESIGN] expected_return_holding_horizon_and_churn_control_refactor.md`)의 본래 목적은 **신규 진입 억제가 아니라 보유 후 빈번한 매도/재매수(churn) 억제**이며, 실측상 EV 게이트가 상류 병목(②③)과 결합해 신규 진입 자체를 4일 연속·37건 중 32건 차단하고(`order_request` 제출 0건), 이로 인해 보유 측 churn-control 계층(§7~§10)이 발동 기회조차 얻지 못한 것이 반복 확인된 이상, **시스템 목적 차원의 괴리는 "의심이 커짐"이 아니라 "운영상 괴리 가능성 큼"으로 판정한다.** 즉 "신규 진입 억제가 보유 후 churn 억제보다 앞서 작동하는 운영상 불균형"이 확인됐다 — 개별 수식이 코드상 정확하다는 사실이 이 목적 차원의 괴리를 반박하지 않는다. 정책 변경(임계값·공식 수정) 여부는 이 진단과 별개로 여전히 미확정이며, 사후 성과 데이터가 갖춰진 뒤에 다룬다. 상세는 `[BACKLOG] backlog.md`의 동일 시각 후속 항목, `buy_path_variable_gate_matrix.md` §18.

**[2026-08-06 KST 후속, 개별 파라미터 사후 성과 검증]** 위에서 미확정으로 유지한 개별 파라미터 적정성을 사후 성과 관점에서 좁혔다. `008930`/`051900`은 결정일(08-06) 종가 자체가 없어 사후 성과 계산이 불가하고, 유일하게 계산 가능한 `001450`(08-03 유일 통과, 미체결)은 T+1/T+2 종가 기준 실현 하락(-134.0/-160.9bps)이 EV 게이트의 하방 가정(42~52bps)보다 컸다 — "과보수적"이라는 가설과 반대 방향이나 표본 1건·horizon 불일치(설계 의도 15분~2시간 vs 관측 24~48시간)로 일반화 불가. 기여도 분해상 `051900`은 하방 추정 자체가, `008930`은 비용+슬리피지 합이 주된 차단 원인이며, 민감도 시뮬레이션상 이 두 종목은 세 항목을 동시에 크게 완화해도 여전히 부족(4.54~10.35bps)해 `001450`의 근소 실패(2.36bps)와는 격차의 성격이 다르다. **결론: 개별 파라미터 적정성은 "아직 미확정"을 유지**하고, §18의 목적 차원 판정("운영상 괴리 가능성 큼")은 이와 별개로 낮추지 않는다. 상세는 `buy_path_variable_gate_matrix.md` §19, `[BACKLOG] backlog.md` 해당 후속 항목.

**[2026-08-07 KST 정책 결정, §6 "신규 BUY" 규칙 hard block 해제]** 위에서 "운영상 괴리 가능성 큼"으로 판정한 목적 차원 괴리를 근거로, §6.3의 "신규 BUY" 규칙(hard block)을 non-blocking telemetry로 전환하는 정책 결정을 반영했다 — 이 설계의 본래 목적(보유 후 churn 억제)을 §7~§10 계층이 시험받을 수 있도록, 그 무대를 없애던 진입 측 컴포넌트(§6.3 신규 BUY)를 완화했다. **§6.3의 REDUCE/EXIT 규칙과 §7~§10(하위 구현 부합 판정은 그대로 유지)은 이번 결정의 대상이 아니다.** 개별 파라미터(비용/하방 공식) 적정성은 여전히 "미확정"이며, 이번 결정은 그 적정성을 판정한 것이 아니라 "차단 비용이 개선 근거보다 크다"는 기회비용 판단이다. 상세는 `buy_path_variable_gate_matrix.md` §27, `[DESIGN] expected_return_holding_horizon_and_churn_control_refactor.md` §6.5.
...
## SPPV-3 OOS 일봉 cache 일 1회 자동 갱신 배치 설계(2026-08-25 KST, read-only 설계·조사 전용) — **설계만 완료, 구현·배치 등록 없음**

상세: `docs/40_action_plans/sppv3_oos_daily_batch_design_2026-08-25.md`.

- 위 §의 "다음 공식 판정" 조건을 사람이 수동 실행 없이 자연스럽게
  충족시키기 위한 일 1회 자동 배치의 아키텍처를 설계했다. 이번
  턴은 코드 구현, GitHub Actions 수정, cron 등록, 컨테이너 재기동,
  KIS 호출, DB write, 주문 제출을 전혀 수행하지 않았다.
- 권장안: 실행 시각 **21:00 KST**(기존 signal feature batch
  20:10과 충분히 이격, KIS 일봉 확정 시각은 실측 근거 없어 보수적
  마진 확보), 실행 주체는 **주문 스케줄러(`ops-scheduler`)와
  프로세스/장애 도메인 분리**, cache 저장은 **매일 신규 디렉토리
  방식 유지 + 성공 시 `latest` 포인터만 추가**, 수집 성공 시에만
  기존 OOS 성과 계산 도구를 후행 실행(항상 read-only, 표본 부족
  시 `PENDING_INSUFFICIENT_OOS_SAMPLE`만 기록).
- KIS 당일 일봉 확정 정확 시각은 **실측하지 못해 미확정으로
  명시**했다 — 코드상 확인 가능한 것은 `MARKET_CLOSE=15:30:30`,
  기존 `signal_feature_batch=20:10` 뿐이며, 21:00은 이로부터
  도출한 보수적 후보다.
- 실패 모드(중복 실행/휴장일/KIS 지연/부분 수집 실패/당일 데이터
  미확정)별 동작과 알림 최소 계약(민감정보 미노출 확인 포함)을
  문서에 정의했다.
- **다음 단계(사용자 승인 필요)**: 배치 실행 메커니즘 확정(host
