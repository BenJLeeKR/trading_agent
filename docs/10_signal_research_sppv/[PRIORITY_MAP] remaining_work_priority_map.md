...
- 수정내용: 신규 실행 없이 48차(SPPV-2.50)가 사용한 두 문구를
  ...
  패턴으로 격상"은 과장 — 이 3분위 재확인이 R3b/entry_score
  조합을 이미 확정하는 데 쓰인 것과 동일한 3년 in-sample 캐시에서
  수행됐고, mixed_score가 60거래일 trailing window라 인접 거래일
  버킷이 자기상관돼 634거래일이 634개의 독립 관측이 아니다.
  단조 감소·217/215/202일의 균등 분포는 그대로 사실이라 "지지
  증거 추가" 단계는 명백히 넘어섰으나, "out-of-sample로 확정된
  구조적 패턴"이라 부르는 것은 과장 — 정확한 표현은 **"강한
  구조적 정합 증거로 격상"**이다. **정정 2**: "주된 차단 요인은
  §21 게이트 하나뿐"은 "SPPV-3 착수 검토를 시작할 수 있는 유일한
  외생적 조건"이라는 뜻이지 "진입 전체에 남은 유일한 조건"이
  아니다 — 46차(§38)의 ①주된 차단 요인(§21 게이트) ②보조 잔여
  조건(entry_score 코드 반영 절차·T+5 구조적 리스크·혼합도
  모니터링) ③실거래 누적 필요 조건 3단 분류는 이번 턴에도 그대로
  유효하다. 판정: **두 정정 모두 R3b 방향성·Conditional Go를
  바꾸지 않는다** — 서술 정밀도만 회복. 신규 실행 없음, 신규 KIS
  호출 0건, 운영 코드 변경 없음, broker submit 미호출.
...
- 수정내용: §21 게이트를 **실운영 잠금 규칙**과 **paper/shadow 관측
  단계 규칙**으로 분리해 기록했다(SPPV-2.58). 현재 canonical 해석은
  다음과 같다. **production**에서는 §21 게이트를 계속 엄격 유지하며
  R3b의 실주문 반영 전제조건으로 둔다. 반면 **paper/shadow**에서는
  향후 환경 인지형 우회(config 스위치) 구현 시, 이 게이트를 shadow·
  paper 실측 자체를 막는 전역 Hold가 아니라 **실운영 승격 잠금선**으로
  해석한다. 즉 우선순위 관점에서도 다음 실무 초점은 "게이트를 이유로
  paper 관측을 멈추는 것"이 아니라 "환경별 gate 분리 구현과 그 후속
  실측 누적"이다. 이번 턴은 문서 정정만 수행했고 판정은 유지한다.
...
- 수정내용: §54(SPPV-2.65) 설계 중 "1단계: 엔진 파라미터 배선"만
  ...
  optional 파라미터 2개 추가 — §48/§49와 동일한 기본값-비활성
  backward-compat 패턴. 기존 회귀 테스트 83건 전부 통과, 활성
  경로 ad-hoc 검증(percentile=0.9 → entry_score=0.72, 기대값과
  완전 일치) 완료. cycle 단위 precompute("2단계")는 범위 밖, 별도
  승인 대상 유보. `.env` 미변경. R3b는 Conditional Go를 유지한다.
...
- 수정내용: §60이 확인한 유일한 실제 차단 요소를 실제로 해소했다
  ...
  0건→1건 실측 확인, precompute 재호출로 빈 dict 탈출 확인(`{'000810':
  1.0, '001450': 0.0}`). 회귀 20+83 passed 무손상. 판정: 실제 차단
  요소 해소 — `ENTRY_SCORE_R3B_ALPHA_ENABLED=true` 전환 시 이제
  실제 발동 가능. R3b는 Conditional Go를 유지한다. `.env` 미변경,
...
- 수정내용: §62가 확인한 실제 차단 요소를 실제로 해소했다(SPPV-
  ...
  `${VAR:-default}` 패턴 그대로 `REGIME_SWITCH_V1_GATE_OVERRIDE_
  ENABLED`/`ENTRY_SCORE_R3B_ALPHA_ENABLED` 2줄 추가(기본값 false,
  ...
  여전히 없음(compose environment 주입만으로 전달 증명), 컨테이너
  안에서 `AppSettings()` 실행 결과도 `True True`. 재생성 후 로그
  정상(비거래일 정상 판정, `submit_count=0`, 예기치 않은 주문
  없음). 판정: 실제 차단 요소 완전 해소 — R3b alpha/§21 게이트
...
> 증가: candidate_intent=buy 39→47건)**: **watch 36 / no_action
> 9 / buy 2**로 분해됐다 — §67~§68이 확정했던 "final_intent=buy
> 0건"이 이번 조회에서 **처음으로 깨졌다**. **A. 신뢰도 축
> (evidence_strength/conviction/confidence)**: conviction/
> confidence 구간 분포를 보면 watch·no_action 대다수가 동일 구간
> (conviction 0.50-0.59, confidence 0.60-0.69)에 겹쳐 있어 **뚜렷한
> threshold는 확인되지 않는다** — no_action에서만 관측되는 진짜
> 극단값은 `conviction=0.0, confidence=0.0, evidence_strength=
> 'none', opposing_evidence=[]`인 **단 1건**뿐이다. **B. 규제/
> 법률/이벤트 리스크 축**: 규제/법률 flag 보유 비율이 watch 39%
> (14/36) → no_action 89%(8/9)로 뚜렷이 상승하나, watch 그룹에도
> "weak evidence+규제flag" 조합이 14건 존재해 **전용 축은 아니고
> "약한 증거+규제flag" 조합이 함께 작용하는 보조 강도 축**으로
> 보인다. **C. 공통 배경 축**: `strategy_selection.preferred_
> strategy`(양쪽 100% defensive_low_volatility_rotation), `compliance_
> opinion`(양쪽 100% allow)은 표본 확대(47건)에서도 구분력이
> 없음을 재확인 — 우선순위를 내려도 된다. **신규 발견(핵심,
> 이번 턴 원래 질문 밖이지만 상위 질문에 직결)**: 실제 `decision_
> type='APPROVE'` 2건(2026-07-20 04:03/04:09 UTC, 000810,
> `conviction=0.7 confidence=0.78 evidence_strength='strong'` —
> watch/no_action 어느 쪽보다도 신뢰도가 높음)을 코드로 직접
> 추적한 결과, 운영 로그에 `"Decision type 'APPROVE' produced no
> order request"`가 찍혀 있었다. `src/agent_trading/services/
> translation.py:74-178`의 `build_submit_order_request_from_
> decision`→`_has_required_expected_value_anchor`를 코드로 확인한
> 결과, `expected_value_gate.passed=False`(reason_codes=
> `["expected_value_anchor_present","expected_value_edge_below_
> minimum_required"]`, **`edge_after_cost_bps(8.56) < minimum_
> required_edge_bps(10.00)`, 단 1.44bps 차이**)로 인해 `submit_
> request=None`이 반환되고, `execution_service.py`가 이를 "hold"로
> 취급해 조용히 스킵함을 확인했다. **이것은 §67~§68이 다뤄온
> 정성적 AI 최종 합성기(watch/no_action을 가르는 축)와 완전히
> 별개의, 그 이후 단계에 있는 순수 정량 게이트**다 — AI가 실제로
> "산다"고 결정해도(APPROVE) 사후 계산되는 edge_after_cost_bps
> 기준 미달 시 조용히 무산된다. 단 1건의 실측 사례(margin
> 1.44bps)이므로 이 게이트가 구조적으로 자주 근소하게 막는지,
> 이번이 우연히 근접했을 뿐인지는 이번 턴 근거만으로는 확정할 수
> 없다. **핵심 질문 답변**: (질문1) 단일 축으로 명확히 확정되지
> 않는다 — "신뢰도(극단적으로 낮은 값)와 규제 리스크 강도가 함께
> 작용할 때 no_action 쪽으로 기운다"는 것이 가장 근접한 답. (질문2)
> 단순 임계값이 아니라 "evidence_strength=weak/none + regulatory
> flag 보유"가 함께 나타날 때 no_action 비중이 높아지는 **상관관계
> 수준**이며, 단일 값의 명확한 컷오프는 확인되지 않는다. **핵심
> 판정**: **"아직 직접 분기축이라 단정 불가"** — 신뢰도+규제 조합이
> 가장 유력한 후보로 좁혀졌고, `strategy_policy_mismatch`류
> 공통 배경 축은 우선순위를 내려도 된다. 코드 변경 없음(순수 조사
> 턴), 신규 KIS 호출 0건. **다음 우선 작업**(정밀 재현 검증 중심,
> 완화안 아님): (1) evidence_strength/regulatory 조합의 재현 검증
> (최우선 — "weak+규제flag" 조합의 조건부 no_action 비율을 표본
> 누적 후 정량화); (2) expected_value_gate margin의 반복 관측
> (신규, 중요도 상승 — edge_after_cost_bps가 minimum_required_
> edge_bps에 근접하는 빈도·margin 분포 추적, 이번 1.44bps 근소
> 미달이 우연인지 구조적 패턴인지 확인); (3) 규제/이벤트 리스크
> 감지 파이프라인 자체의 데이터 근거 확인. 상세: `docs/10_signal_
> research_sppv/[DESIGN] regime_conditional_entry_signal_v1.md`
> §69.

> **📌 2026-07-20 "마지막 단계" 내부 재분해 — watch/no_action
> 두 갈래와 그 입력 패턴 차이 (작성자: Codex)**: §67
...
> decisions` 72건 중 000810 계열 36건)**: `buy_candidate=True`
> (36) → `eligibility_passed=True`(36, 무손실) → `candidate_vs_
> final.candidate_intent='buy'`(36, 무손실) → **`final_intent=
> 'buy'`(0)** → `decision_type='BUY'`(0) → order request 생성(0,
> `execution_attempts` 24시간 432건 전부 `status=non_trade`). 즉
> 손실은 **candidate_intent=buy(36) → final_intent=buy(0)의 단
> 한 지점**에서 100% 발생한다 — 여러 축에 걸쳐 조금씩 깎이는
> 구조가 아니라 날카로운 단일 병목이다. **참고(넓은 맥락)**: 같은
> 24시간 universe 전체 `trade_decisions` 432건의 `decision_type`
> 분포는 `WATCH=276, HOLD=156, BUY=SELL=REDUCE=EXIT=0` — R3b에
> 국한된 문제가 아니라 시스템 전체가 이 기간 완전히 WATCH/HOLD
> 상태였다. **넓은 병목 vs. 마지막에 누르는 병목 분리**: §66이
> 확인한 층3(`eligibility_core_risk_off_ranking_blocked` 등
> pre-AI 차단, 원시 로그 326건·distinct 11/12종목)은 candidate
> 형성 **이전** 단계에서 universe 대다수를 걸러내는 넓은 병목이며,
> **000810에는 한 번도 적용되지 않는다** — 즉 이 축과 "000810이
> 0건 통과하는 이유"는 인과관계가 없는 별개의 축이다. 000810을
> 실제로 마지막에 누르는 것은 `candidate_vs_final.alignment_
> status='downgraded'` 단 하나뿐이다. **AI 판단의 실제 근거 확인
> (추정 아님, DB 직접 조회)**: 000810의 `decision_json.ai_call_
> path.fdc_skipped=False`(final decision composer가 실제로
> 호출됨 — stub 아님, 매 cycle 실행되는 진짜 AI 판단). `conviction
> =0.5, confidence=0.6, evidence_strength='weak'`,
> `risk_flags=['high_volatility','risk_off_tone','event_
> conflict']`, `opposing_evidence`에 "시장 전반의 위험 회피
> 분위기", "고변동성 환경으로 단기 손실 가능성", "상충되는
> 이벤트로 방향성 불확실", **"선호 전략(방어적 저변동성 회전)과
> 모멘텀 매수 신호 간 상충"**, "이벤트 해석 중립 및 약한 증거"가
> **36회 거의 동일한 문구로 반복** 관측됐다. **해석**: 이 판단
> 논리(국면이 risk_off/고변동성인데 전략 선택기는 방어적 회전을
> 권고 중이고, R3b는 모멘텀 매수 신호를 올려 서로 충돌한다)는 그
> 자체로 정당한 방어 논리일 수 있다. 다만 36회 전부 거의 동일한
> 텍스트·동일한 conviction/confidence가 반복된다는 것은, 이 AI
> 판단이 매 cycle 새 시장 데이터에 실질적으로 반응하고 있다기보다
> 국면 라벨(`risk_off_tone`)이 고정된 동안 사실상 같은 결론을
> 기계적으로 재생산하고 있을 가능성도 시사한다 — "정당한 방어"와
> "국면 라벨 고착"의 경계에 있으며, 이번 턴 근거만으로는 어느
> 쪽인지 확정할 수 없다. **보정 우선순위 판정**: 층3(pre-AI,
> universe 91.7%, 000810과 인과관계 없음)=**유지**; 층2(000660
> 애초 비후보, R3b 자신의 1차 판단)=**유지**; 층1(000810 downgrade,
> candidate까지 도달한 것을 마지막에 누르는 유일한 축)=**정밀
> 보정 필요**(완전 제거·우선 완화는 시기상조 — 실제 AI 판단 근거가
> 있으므로 "명백한 과잉 억제"로 단정할 근거가 아직 부족하다). **한
> 줄 결론**: "BUY 후보는 생성되지만 마지막 단계 병목 때문에 0건
> 통과"(000810 한정, 단일 지점 100% 손실 — "후보 자체가 너무
> 적어서"도 "복합 원인이라 특정 불가"도 아니다). R3b 작동 자체
> 판정(작동하나 체감 무효, §65~§66)은 이번 턴과 무관하게 불변.
> 코드 변경 없음(순수 조사 턴), 신규 KIS 호출 0건. **다음 우선
> 작업**(부분 보정 원칙 유지, 전면 완화 아님): (1) AI 최종 결정
> 합성기 판단의 조건 민감도 확인(최우선) — `risk_off_tone` 해제나
> entry_score 변화 시 실제로 `final_intent`가 buy로 바뀌는지
> 재현 검증, "우선 완화"가 아니라 "이 AI 판단이 실제로 조건에
> 반응하는가"를 먼저 확인하는 정밀 진단; (2) §66이 남긴 층3(core
> risk-off pre-AI 차단) 조사는 별도 트랙으로 유지(이번 턴 대상과
> 인과관계 없으므로 우선순위 재조정하지 않음); (3) R3b 후보 풀
> 협소함(candidate pool 2종목뿐인 이유) 재관측. 상세: `docs/10_
> signal_research_sppv/[DESIGN] regime_conditional_entry_signal_
> v1.md` §67.

> **📌 2026-07-20 SPPV-2.76 해석 정밀 보정 — "BUY 부재" 원인의
> 3층 분리 정량화 (작성자: Codex)**: R3b 작동 여부를 다시
...
> 문제)를 사용자의 명시적 승인·상세 지시에 따라 실제로 해소했다
> (SPPV-2.74, 검증 아닌 배선 수정 턴). **수정 내용**: `docker-
> compose.yml`의 `ops-scheduler` `environment:` 블록,
> `DETERMINISTIC_TRIGGER_APPLY_CORE_RISK_OFF_TOPK` 바로 다음에
> 기존 `${VAR:-default}` 패턴 그대로 `REGIME_SWITCH_V1_GATE_
> OVERRIDE_ENABLED: "${REGIME_SWITCH_V1_GATE_OVERRIDE_ENABLED:-
> false}"`/`ENTRY_SCORE_R3B_ALPHA_ENABLED: "${ENTRY_SCORE_R3B_
> ALPHA_ENABLED:-false}"` 2줄을 추가했다 — paper/production 분기
> 로직 없음, 기본값(false)은 각 config 모듈의 기존 기본값과 동일해
> 하위 호환 유지. `api`/`reconciliation-worker` 서비스는 `Decision
> OrchestratorService`/`run_decision_loop`를 전혀 참조하지 않음을
> 코드로 확인해 배선 대상에서 제외했다(불필요한 서비스 확장 없음
> — `run_decision_loop.py`를 subprocess로 실행하는 프로세스는
> `run_ops_scheduler.py` 하나뿐임을 §62에서 이미 확인). **실제
> 검증(신규 `logs/r3b_docker_compose_env_wiring_fix_2026-07-19.
> log`, 이번 턴 직접 실행)**: (1) 수정 전 — `docker exec agent_
> trading-ops-scheduler env | grep -E "ENTRY_SCORE_R3B_ALPHA_
> ENABLED|REGIME_SWITCH_V1_GATE_OVERRIDE_ENABLED"` → 결과 없음(§62
> 와 동일 재확인); (2) `docker compose config --quiet` 문법 검증
> 통과, `docker compose config` 렌더링 결과 두 변수 모두 `"true"`
> 로 정확히 치환됨을 확인(호스트 `.env` 값 그대로); (3) `docker
> compose up -d --force-recreate --no-deps ops-scheduler`로
> `ops-scheduler`만 재생성(다른 서비스 무영향) —
> `Recreate`/`Recreated`/`Starting`/`Started` 확인; (4) 재생성 후
> `docker ps` → `Up 8 seconds (healthy)`; (5) **재생성 후 env
> 재확인** — `ENTRY_SCORE_R3B_ALPHA_ENABLED=true`, `REGIME_SWITCH_
> V1_GATE_OVERRIDE_ENABLED=true`(수정 전 없음 → 수정 후 있음을
> 직접 대조 확인); (6) `/app/.env` 파일 유무 재확인 — 여전히
> `No such file or directory`(**컨테이너 안에 `.env` 파일이 없는
> 상태 그대로, 순수 compose `environment:` 주입만으로 값이
> 전달됨**을 증명); (7) 컨테이너 안에서 직접 `AppSettings()` 실행
> — `s.entry_score_r3b_alpha_enabled`/`s.regime_switch_v1_gate_
> override_enabled` 결과 **`True True`**(실제 운영 코드가 참조하는
> 바로 그 설정 객체가 새 값을 정확히 읽음을 확인); (8) 재생성 후
> 시작 로그 — 정상 기동, 오늘(2026-07-19)도 비거래일로 정상 판정,
> `submit_count=0`(재시작 자체가 예기치 않은 주문을 유발하지
> 않았음을 확인, 실거래/주문 submit 없음 원칙 준수). **핵심 질문
> 답변**: (1) 어떤 서비스에 배선이 필요했는가 → `ops-scheduler`
> 단 하나; (2) 왜 compose 수정이 필요했고 단순 재시작으로는 안
> 됐는가 → 환경변수는 컨테이너 **생성 시점**에 고정되므로, compose
> 파일 자체를 수정하고 컨테이너를 **재생성**해야만 새
> `environment:` 블록이 적용됨(단순 `restart`는 기존 값 그대로
> 유지); (3) 재생성 후 컨테이너가 두 값을 실제로 읽는가 → **그렇다**
> (실측 확인); (4) 다음 trading day에 R3b alpha 반영이 실제 발동
> 가능한 상태가 되었는가 → **그렇다** — §58~§61의 코드/데이터
> 준비와 이번 턴의 배선 완료로, 남은 것은 다음 실제 거래일에
> cycle이 도는 것을 관측하는 것뿐; (5) 아직 남는 것의 성격 →
> **더 이상 실제 차단 요소는 없다** — 전부 "다음 거래일 관측
> 과제"로 재분류됨. **판정**: §60(벤치마크 데이터)→§62(compose
> 배선)로 이어진 두 실제 차단 요소가 모두 해소됐다. R3b는
> Conditional Go를 유지한다. paper/production 분기 로직 추가 없음,
> gate 로직 강화 없음(오히려 §21 게이트는 override로 완화 방향),
> 신규 KIS 호출 없음, 실거래/주문 submit 없음. **SPPV-3까지 남은
> 항목 재분류**: (1) 실제 차단 요소 — 없음; (2) 사용자 결정 대기
> — 없음(모두 이미 `.env`에 설정돼 있고 이제 운영 컨테이너에도
> 반영됨); (3) 다음 거래일 관측 과제 — 다음 실제 거래일(2026-07-20
> 예정) cycle에서 `trigger_r3b_alpha_percentile` reason_code 실제
> 관측 + 다음 정기 signal feature 배치 사이클에서 벤치마크 자동
> 반영 재확인 + `trigger_status` 자동화(낮은 우선순위) + T+5 +
> `portfolio_allocation` gap. 상세: `plans/[DESIGN] regime_
> conditional_entry_signal_v1.md` §63.

> **📌 2026-07-19 R3b alpha 운영 반영 여부 실제 점검 — docker-
> compose 환경변수 배선 미비 신규 발견 (작성자: Codex)**:
...
> universe(core/held_position/market_overlay/manual)뿐이었다 —
> 069500(KODEX 200 ETF)은 거래 후보가 아니라 순수 국면 분류용
> 벤치마크이므로 이 구성에 애초에 나타날 수 없었다(버그가 아니라
> 두 개념이 애초부터 분리돼 있었을 뿐). **수정**: 신규 함수
> `_with_regime_benchmark_symbol()`을 추가 — `run_decision_loop.py`
> 가 이미 두 곳(mixedness, R3b alpha)에서 쓰는 `_R3B_ALPHA_
> BENCHMARK_SYMBOL`/`_R3B_ALPHA_BENCHMARK_MARKET`("069500"/"KRX")
> 을 그대로 재사용(신규 하드코딩 아님), 거래 universe/DB freeze
> 기록은 그대로 두고 `_build_rows`/`_write_rows`에 전달되는 로컬
> tuple에만 `source_type="regime_benchmark"`로 벤치마크 1건을
> 추가한다 — 기존 core universe 구성이나 mixedness/§21 게이트
> 로직과 충돌할 여지를 원천 차단. **실제 검증(신규 `scripts/
> validate_r3b_alpha_benchmark_snapshot_fix.py`, 전체 80종목
> 재수집 없이 벤치마크 1종목만 최소 재현)**: (1) 실제 `_build_
> rows()`(운영 코드 그대로)로 벤치마크 일봉 실제 조회 — `rows=1
> errors=0`(신규 KIS 호출 1건, read-only); (2) `_write_rows()`로
> 별도 파일에 입력 JSON 저장(기존 `data/signal_feature_snapshot_
> input.json`은 손대지 않음); (3) `scripts/build_signal_feature_
> snapshots.py`를 subprocess로 **수정 없이 그대로** 실행 —
> `processed=1 persisted=1 errors=0`, 실제 069500 instrument_id
> (`22651dda-c67c-50f4-ad66-ce00a2fb37d9`, §60에서 확인한 값과
> 일치)로 snapshot 실제 upsert; (4) DB 재조회 — 069500 signal_
> feature_snapshot **0건→1건**(스크립트 종료 후 별도 프로세스로도
> 동일하게 1건 확인, 실제 커밋); (5) `_build_r3b_alpha_percentile_
> overrides_for_cycle()`(§58 코드 수정 없음)을 실제 DB의 core
> 종목 10개+벤치마크로 재호출 — 결과 **`{'000810': 1.0, '001450':
> 0.0}`**(빈 dict 아님, 실제 재발동 확인). **회귀(이번 턴 직접
> 재실행)**: `test_generate_signal_feature_snapshot_input.py`+
> `test_build_signal_feature_snapshots.py` **20 passed, 0 failed**;
> 엔진/orchestrator **83 passed, 0 failed**; `test_run_decision_
> loop.py` **8 failed, 111 passed**(§59/§68이 확정한 사전 존재
> 비결정성과 동일). **판정**: §60이 지목한 유일한 실제 차단 요소가
> 실제로 해소됐다 — 코드 수정 + DB 실측(0건→1건) + precompute
> 재발동(빈 dict→실제 percentile) 3단계 전부 실측 확인. `ENTRY_
> SCORE_R3B_ALPHA_ENABLED=true`로 전환하면 이제 실제로 alpha
> 교체가 발동할 수 있는 상태다. 다만 "1회성 실행으로 채운 것"과
> "매일 정기 배치가 앞으로 계속 포함하는 것"은 구분한다(후자는
> 이미 존재하는 ops-scheduler 스케줄이 자동 소비할 것으로 예상되나
> 시간 경과 후 재확인 권장 — 이번 턴 범위 밖). R3b는 Conditional
> Go를 유지한다. `.env` 미변경, BUY/SELL gate 로직 강화 없음, 환경
> 분기 코드 없음, compliance/VaR/broker submit 경계 미변경, 신규
> KIS 호출 1건(read-only 시세 조회, 매매/주문 없음). **SPPV-3까지
> 남은 항목 재분류**: (1) 실제 차단 요소 — 해소됨(이번 턴); (2)
> 사용자 결정 대기 — `ENTRY_SCORE_R3B_ALPHA_ENABLED=true` 전환(이제
> 의미 있는 결정); (3) 후속 검증 과제 — 다음 정기 배치 사이클
> 자동 반영 재확인(낮은 우선순위) + 기존 항목(trigger_status
> 자동화, T+5, portfolio_allocation gap). 상세: `plans/[DESIGN]
> regime_conditional_entry_signal_v1.md` §61.

> **📌 2026-07-19 R3b alpha paper 운영 전환 최종 착수 준비 상태
> 점검 (작성자: Codex)**: "지금 `ENTRY_SCORE_R3B_ALPHA_
...
> ("2단계", `run_decision_loop.py`/`decision_orchestrator.py` 수정
> 필요)은 범위 밖, 별도 승인 대상으로 유보했다. **적용 내용**:
> `settings.py`에 `_resolve_entry_score_r3b_alpha_enabled()` +
> `AppSettings.entry_score_r3b_alpha_enabled` 필드(env: `ENTRY_
> SCORE_R3B_ALPHA_ENABLED`, 기본값 False) 추가; `deterministic_
> trigger_engine.py`의 `assess_deterministic_triggers`/`_build_
> entry_score`에 `r3b_alpha_percentile: float | None = None`,
> `r3b_alpha_enabled: bool = False` optional 파라미터 2개 추가 —
> 활성 시에만 alpha 항이 `0.80 * candidate_percentile`로 교체되고
> 비활성(기본값) 시 기존 공식이 100% 그대로 유지된다(§48/§49와
> 동일한 backward-compat 패턴). **실측(수치+해석)**: (1) 기존
> 회귀 테스트 `tests/services/test_deterministic_trigger_engine.py`
> 20건 + `test_decision_orchestrator.py` 포함 총 83건 전부 통과(0건
> 실패) — 신규 optional 파라미터가 기존 호출부 동작에 영향 없음을
> 확인; (2) `AppSettings().entry_score_r3b_alpha_enabled` 기본값
> 직접 조회 → `False` 확인 — `.env` 미변경 상태에서 스위치가
> 완전히 비활성임을 확인; (3) `_build_entry_score` 직접 호출
> 비교(overall=fast=slow=0.5 고정) — 비활성 경로 `entry_score=
> 0.6000...`(기존 공식과 일치), 활성 경로(`r3b_alpha_enabled=True,
> r3b_alpha_percentile=0.9`) `entry_score=0.7200...`(기대값
> `0.80*0.9=0.72`와 완전 일치, 오차 <1e-9). **판정**: "entry_score
> PR 초안"이 "설계 완료"에서 "1단계 코드 적용·검증 완료"로
> 진전됐다. R3b는 Conditional Go를 유지한다. `.env` 미변경, BUY/
> SELL gate 로직 강화 없음, 환경 분기 코드 없음, compliance/VaR/
> broker submit 경계 미변경, 신규 KIS 호출 0건. 상세: `plans/
> [DESIGN] regime_conditional_entry_signal_v1.md` §55.

> **📌 2026-07-19 entry_score 코드 변경 PR 초안 설계 — R3b alpha
> 교체 실제 파이프라인 연결 방안 (작성자: Codex)**: **최신
> truth 재확인**: commit `aa10caee`(§21 게이트 배선), `.env`의
> override=true(paper 관측 단계 BUY 미차단), commit `4fd3ad7e`/
> `bcec9d03`(§51/§52 혼합도 모니터링 구현·연결), commit `5c977017`
> (§53 테스트 실패 무관 확정) — 모두 확인. 후속 과제 후보
> (trigger_status 자동화/entry_score 코드 변경 PR 초안/R3b alpha
> 전체 경로 재현 검증/T+5 후속 검증) 중 **entry_score 코드 변경
> PR 초안 준비**를 선택했다(SPPV-2.65) — trigger_status 자동화는
> override=true인 동안 실질 영향이 없어 여전히 급하지 않고,
> mixedness는 이미 실제 소비 위치 연결까지 끝나 같은 축 반복을
> 피했다. "R3b alpha 전체 경로 재현 검증"은 §45(SPPV-2.56, non-
> alpha 조정 항 100% 일치 58,493건 전수 확인)의 논리적 귀결이다
> — 이 세션 내내 쓰인 `_score_b(row)=clamp01(0.80*candidate_
> percentile+non_alpha)` 공식 자체가 "실제 non-alpha(§45 확인) +
> R3b alpha"의 합이므로, 이 조합이 실제 함수와 일치한다는 것을
> 다시 실측하는 것은 §45를 반복 검증하는 것일 뿐 새로운 정보가
> 없다 — 그래서 이번 턴은 재검증이 아니라 **이 세션에서 한 번도
> 명시되지 않은 아키텍처 제약**을 조사했다. **핵심 발견**: `entry_
> score`는 종목 단위로 계산되지만(`DecisionOrchestratorService.
> _derive_deterministic_context_components`는 요청(symbol) 1건
> 마다 독립적으로 호출됨) R3b의 alpha 항(`candidate_percentile
> (regime_conditional_signal)`)은 **당일 quintile 후보 집합
> 안에서의 상대 순위**로 정의된다 — 이는 종목 단위 계산만으로는
> 얻을 수 없고 하루치 전체 후보를 먼저 훑어 순위를 매기는 cross-
> sectional 사전 계산 단계가 있어야 한다. 이 세션의 모든 R3b
> shadow 스크립트는 이 사전 계산을 스크립트 안에서 이미 수행해
> 왔지만, 그 사실이 "실제 코드 반영에는 단순 공식 교체가 아니라
> 파이프라인 단계 추가가 필요하다"는 것을 뜻한다는 점은 별도로
> 정리된 적이 없었다. **기존 선례 발견**: 다행히 이 세션이 §21
> 게이트/혼합도 모니터링에서 쓴 것과 완전히 동일한 패턴이 이미
> 운영 코드에 구현돼 있다 — `scripts/run_decision_loop.py`의
> `_build_core_risk_off_apply_overrides_for_cycle(universe)`
> (1247행)이 cycle당 1회 그날의 `universe` 전체를 순회해 core
> risk-off top-k 예외 승격 대상을 미리 계산하고 `deterministic_
> trigger_override`(dict)로 종목별 주입한다 — 이것이 정확히 R3b
> alpha percentile에 필요한 것과 동일한 구조(cross-sectional
> 사전 계산 → 종목별 override 주입)다. **제안 설계(미적용 — 실제
> 코드 변경 아님, 검토용 초안)**: (1) 신규 cycle당 1회 precompute
> 함수 — 당일 universe 전체에서 quintile 상위 20% candidate만
> 골라 `candidate_percentile`을 계산(이 세션의 `_attach_
> candidate_percentile()`과 동일 로직 — 신규 로직 없음, 기존
> shadow 스크립트 로직을 운영 코드로 이식), (2) `assess_
> deterministic_triggers`에 §48/§49와 동일한 패턴의 신규 optional
> 파라미터 2개(`r3b_alpha_percentile: float | None = None`,
> `r3b_alpha_enabled: bool = False` — 기본값이면 기존 alpha 공식
> `0.45*norm(overall)+0.20*norm(fast)+0.15*norm(slow)`을 그대로
> 사용해 기존 호출부 100% 무영향), (3) 신규 config 스위치
> `AppSettings.entry_score_r3b_alpha_enabled`(env: `ENTRY_SCORE_
> R3B_ALPHA_ENABLED`, 기본값 False). **이 설계로 명확해진 것**:
> 기존 결론(§45의 non-alpha 100% 일치)은 그대로 재사용되고, R3b
> alpha를 실제로 반영하려면 (a) cycle당 1회 cross-sectional
> precompute 함수 1개, (b) 종목별 override 주입 파라미터 2개,
> (c) config 스위치 1개 — 총 3가지 신규 요소가 필요하며 이 세
> 요소 모두 이 세션에서 이미 검증된 기존 패턴의 재사용이라 새로운
> 설계 리스크는 낮다. 아직 실측하지 않은 것: 실제 적용 시 cycle당
> precompute 연산 비용(기존 core risk-off precompute와 유사한
> 크기라 추가 비용은 미미할 것으로 예상되나 실측은 안 됨). **판정:
> R3b는 Conditional Go를 유지한다.** 이번 턴은 코드를 전혀 수정
> 하지 않았다 — 순수 설계 문서 작업. "entry_score 코드 변경 PR
> 초안 작성 착수"는 §45~§46의 "shadow 계산 정합성 확보" 단계에서
> **"구체적 구현 설계 확보(실제 코드 diff 초안, 미적용)" 단계로
> 진전**했다 — 다만 이 설계를 실제로 코드에 적용하는 것은
> `deterministic_trigger_engine.py`/`run_decision_loop.py`를 다시
> 수정하는 것이므로, 이전 §48/§49와 마찬가지로 별도의 명시적 사용자
> 승인이 필요하다. compliance/VaR/broker submit 경계는 전혀
> 건드리지 않았다. 신규 KIS 호출 0건(코드 조사만 수행). 상세:
> `plans/[DESIGN] regime_conditional_entry_signal_v1.md`
> §54(SPPV-2.65).
>
> **📌 2026-07-19 SPPV-2.63 미확정 항목 확정 — `test_run_decision_
> loop.py` 10건 실패 무관 확정 (작성자: Codex)**: 직전 턴
...
> 않는다. commit `4fd3ad7e`(§51, SPPV-2.62)로 `services/regime_
> mixedness_monitor.py`가 구현·검증됐으나(§40의 217/215/202 버킷
> 재현성 확인), decision loop나 대시보드 어디에도 아직 연결되지
> 않은 상태였다 — 이번 턴이 그 마지막 gap을 메운다. 후속 과제
> 후보(trigger_status 자동화/혼합도 모니터링 실제 소비 위치 연결/
> T+5 후속 검증/SPPV-3 착수 준비) 중 **혼합도 모니터링의 실제
> 소비 위치 연결**을 최우선으로 선택했다 — trigger_status 자동화
> 는 override=true인 동안 실질적 영향이 없어 급하지 않고, T+5/
> 경로 리스크는 §41~§44에서 이미 충분히 답이 나온 반복 질문이기
> 때문이다. **구현**: `scripts/run_decision_loop.py`에 신규 함수
> `_run_mixedness_check(repos)`를 추가했다 — cycle당 1회 실행되는
> 기존 pre-check 블록(`_run_precheck()`, snapshot sync health
> 체크)과 완전히 동일한 안전 패턴(try/except로 예외 전부 흡수,
> 실패해도 사이클 진행 무영향)을 그대로 재사용: (1) 벤치마크
> (KODEX 200, 069500)의 instrument 조회, (2) `signal_feature_
> snapshots.list_by_instrument(limit=60)`로 최근 60건을 read-only
> 조회(**신규 KIS 호출 없음** — 이미 별도 스냅샷 동기화 루프가
> 채워 넣은 데이터 재사용), (3) 각 스냅샷에 실제 운영 함수
> `classify_market_regime()`을 적용해 국면 라벨 trailing 리스트
> 구성, (4) §51의 `compute_mixed_score()`/`classify_mixedness_
> bucket()`(순수 함수, 그대로 재사용)으로 버킷·reason_code 계산,
> (5) `logger.info()`로 로그 기록. **핵심 설계 원칙(반드시 지켜짐)**:
> BUY/SELL 판정에 전혀 연결하지 않았다 — `cycle_precheck`처럼
> 판정 로직에 전달되는 변수와 완전히 분리된 독립 블록이다.
> environment 분기 코드를 추가하지 않았다. `.env`는 수정하지
> 않았다. 신규 KIS 호출이 없다. **검증**(`scripts/validate_r3b_
> mixedness_decision_loop_wiring.py`, 신규, read-only, in-memory
> repos, 신규 KIS 호출 0건): `run_decision_loop.py`에 실제로
> 배선된 `_run_mixedness_check()`를 **그대로 import해 직접
> 호출**(로직 복제가 아님)해 두 시나리오를 확인했다 — 저혼합
> (벤치마크 합성 스냅샷 60건 전부 강한 bullish 고정) →
> `mixed_score=0.0, bucket=저혼합(단일 국면 지배)`, 고혼합
> (bullish/bearish 빈번 교차) → `mixed_score=0.5, bucket=고혼합`.
> **결과: 두 시나리오 모두 기대한 버킷으로 정확히 분류됨을 확인
> (`low_bucket_correct=True`, `high_bucket_correct=True`).**
> 추가로 `inspect.getsource()`로 `_run_mixedness_check()`의
> 소스를 직접 검사해 `buy_candidate`/`sell_candidate`/`assess_
> deterministic_triggers` 문자열이 전혀 등장하지 않음을 확인
> (`no_buy_sell_reference_in_mixedness_check=True`) — BUY/SELL
> 판정과의 분리를 코드 레벨로 재확인. 기존 단위 테스트(`tests/
> scripts/test_run_decision_loop.py`)도 재확인했다: 119건 중
> 10건 실패했으나, **이 변경 전(git stash로 원복) 동일 테스트를
> 재실행한 결과 동일한 10건이 동일하게 실패**함을 확인 —
> universe_selection/market_overlay 관련 사전 존재 결함으로
> 이번 변경과 무관(109건은 변경 전후 모두 통과). **해석**: §51에서
> 검증한 순수 모듈을 실제 운영 루프 안에서 실제로 실행되는 코드로
> 전환한 것이다 — "창 교체(R3b) 검증"에서 "창 교체 이후 실제 운영
> 관측"으로의 마지막 단계다. 이제 `run_decision_loop.py`가 실제로
> 실행될 때마다(cycle당 1회) 그 순간의 국면 혼합도 버킷이 로그에
> 남는다 — 사람이 로그를 보고 "오늘은 고혼합 구간이라 신호 신뢰도
> 가 낮다"는 것을 즉시 인지할 수 있다. BUY/SELL 판정 자체는 이
> 정보와 무관하게 그대로 동작한다(§40.5의 "모니터링 지표, 차단
> 사유 아님" 원칙이 코드 레벨에서도 그대로 지켜짐). **판정: R3b는
> Conditional Go를 유지한다.** §21 게이트(주된 차단 요인)나
> override 설정은 이번 턴과 무관하게 불변(override=true 유지).
> BUY/SELL 게이트 로직은 전혀 더 세지지 않았다 — 오직 관측/로깅
> 경로만 추가됐다. compliance/VaR/broker submit 경계는 전혀
> 건드리지 않았다. 신규 KIS 호출 0건. 산출: `scripts/run_decision_
> loop.py`(수정), `scripts/validate_r3b_mixedness_decision_loop_
> wiring.py`(신규), `logs/signal_ic_r3b_mixedness_decision_loop_
> wiring_2026-07-19.json`. 상세: `plans/[DESIGN] regime_
> conditional_entry_signal_v1.md` §52(SPPV-2.63).
>
> **📌 2026-07-18 국면 혼합도 모니터링 모듈 구현 및 §40 재현성
> 검증 (작성자: Codex)**: **최신 truth 확정**: commit
...
> 있게** 만드는 것이 핵심 전제였다. 착수 전 코드베이스를 전수
> 조사한 결과 **이 게이트는 지금까지 실제 운영 코드
> (`assess_deterministic_triggers`) 어디에도 연결되지 않은 순수
> 모니터링 산출물**이었음을 확인했다 — R3b shadow/paper 관측은 이
> 게이트에 의해 코드 레벨에서 전혀 막힌 적이 없다(이 세션 내내
> 실행된 모든 검증 스크립트가 그 증거). 이 세션 전체에서
> `deterministic_trigger_engine.py`는 "절대 수정하지 않는다 —
> shadow/read-only만"이라는 원칙이 반복 확립돼 있었으므로, 이번
> 요청도 그 파일은 건드리지 않고 **신규 격리 모듈**로만 구현했다.
> **구현**: `AppSettings.regime_switch_v1_gate_override_enabled`
> (env: `REGIME_SWITCH_V1_GATE_OVERRIDE_ENABLED`, 기본값 False,
> `.env` 파일 자체는 수정하지 않음) 신규 필드 + `services/regime_
> switch_gate.py`(신규)의 `assess_regime_switch_v1_gate(*, trigger_
> status, override_enabled)` 순수 함수 — **paper/real/production
> 같은 environment 값은 함수 인자로도 로직으로도 전혀 등장하지
> 않는다.** override off(기본값)면 기존 §21 해석과 100% 동일
> (TRIGGERED일 때만 열림, 기존 동작 완전 보존), override on이면
> 실제 국면 상태와 무관하게 항상 열림(강제 통과) — 어느 경로든
> reason_code(`gate_open_regime_switch_v1_triggered`/`gate_closed_
> regime_switch_v1_not_triggered`/`gate_open_config_override_
> bypass`)로 추적 가능. **검증**(`scripts/validate_regime_switch_
> gate_config_override.py`, read-only, 신규 KIS 호출 0건): (a)
> `inspect.getsource`로 `deterministic_trigger_engine.py` 소스를
> 직접 검사해 신규 모듈이 그 파일에서 import되지 않음을 확인
> (`isolation_confirmed=True`) — 파일 미수정의 코드 레벨 증거.
> (b) 실제 §21 게이트 상태 재조회 결과 여전히 `NOT_TRIGGERED`(불변,
> 벤치마크 1종목 캐시 재사용). (c) override off/on 및 TRIGGERED/
> PARTIAL/NOT_TRIGGERED 3개 상태 전부에 대한 시나리오 검증 —
> override on 시 3개 상태 모두 `gate_open=True`, override off 시
> `TRIGGERED`만 `gate_open=True`(기존 해석과 완전히 일치). **해석**:
> 단순 통과율 상승이 아니라 "게이트를 어떻게 다룰지"를 코드로
> 명시적으로 통제 가능하게 만든 것이 이번 변경의 의미다 — R3b
> shadow/paper 관측 자체는 이번 변경 전후로 바뀌지 않는다(원래도
> 가능했다), 대신 향후 게이트를 실제 파이프라인에 연결하기로
> 결정했을 때 environment 분기 없이 config 하나로 안전하게 켜고
> 끌 수 있는 준비된 인프라가 생겼다는 것이 새로운 진전이다. **판정:
> R3b는 Conditional Go를 유지한다.** §21 게이트 상태 자체는 불변,
> `deterministic_trigger_engine.py` 미수정, compliance/VaR/broker
> submit 경계 미변경. 아직 실제 파이프라인에는 연결하지 않았다 —
> 연결은 별도 승인·PR 절차 필요. 신규 KIS 호출 0건. 산출:
> `src/agent_trading/services/regime_switch_gate.py`(신규),
> `src/agent_trading/config/settings.py`(필드 추가), `scripts/
> validate_regime_switch_gate_config_override.py`(read-only),
> `logs/signal_ic_r3b_regime_switch_gate_config_override_2026-07-
> 18.json`. 상세: `plans/[DESIGN]
> regime_conditional_entry_signal_v1.md` §47(SPPV-2.58).
>
> **📌 2026-07-18 SPPV-2.56 결론 문구 정밀화 — "직접 호출" 서술
> 범위·표본 서술 정정**: 직전 턴(SPPV-2.56, §45)의 두 표현을
...
> 있었는데, 실제 코드 변경 PR 작성 전에 확인해야 할 선행 질문이
> 있었다 — **SPPV-2.46부터 이 세션 내내 R3b+entry_score
> risk_off_penalty 제거(B 시나리오)의 non-alpha 조정 부분을 계산할
> 때, 검증 스크립트마다 `_non_alpha`라는 수작업 재구현 함수를 썼을
> 뿐, 실제 운영 함수 `_build_entry_score`(`deterministic_trigger_
> engine.py:1115-1170`)를 한 번도 직접 호출한 적이 없었다.** 코드
> 대조 결과 `_build_entry_score`에는 `_non_alpha`가 구조적으로
> 담아내지 못하는 항목(portfolio_allocation 예산 보너스/차단
> 패널티 +0.10/-0.20, source_type 조정 +0.05/-0.35, 최종 clamp)이
> 있었다 — 이 세션은 항상 `source_type="core"`, `portfolio_
> allocation=None`으로 써서 이론상 no-op이었지만, **실제로 그런지는
> 검증된 적이 없었다.** 이는 (1) SPPV-3(entry_score 코드 반영)
> 착수의 직접 전제조건, (2) 이 세션 최초의 신규 검증, (3) 기존 3년
> 캐시·코드만으로 지금 당장 실측 가능, (4) 방어 논리가 아니라
> "창 교체를 실제 운영 코드 경로에 연결할 준비"를 직접 진전시킨다는
> 4가지 기준을 모두 충족했다. **3년 전체 후보 표본(87개 core
> 종목, 58,493건)에 대해 실제 `_build_entry_score`를
> `overall=fast=slow=0.0`으로 호출(alpha 항이 상수 0.40으로
> 고정되어 조정 항만 순수 분리됨)한 값과 `_non_alpha` 재구현값을
> 전수 대조했다(신규 KIS 호출 0건).** **결과: 100.0%(58,493/58,493)
> 완전 일치, 불일치 0건, 최대 절대 오차 0.0.** **해석**: 이 세션
> 내내 사용해온 수작업 재구현이 실제 운영 함수와 이 세션이 다룬
> 조건(source_type="core", portfolio_allocation=None) 안에서 소수점
> 오차 없이 완전히 일치한다 — SPPV-2.46~2.55에서 계산된 모든 B
> 시나리오 funnel·수익률 결과가 실제 운영 코드가 그대로 반영됐을
> 때의 결과와 수치적으로 동일함이 처음으로 전수 검증됐다. **판정:
> "entry_score 코드 반영 절차"는 "설계 논의 단계"에서 "shadow
> 계산 정합성 확보, 실제 코드 변경 PR 작성 가능 단계"로 격상됐다.**
> 다만 이 검증은 `source_type="core"`, `portfolio_allocation=None`
> 조건 안에서의 일치만 확인한 것이며, 이 결과 하나로 코드 변경 PR
> 자체를 승인·실행하거나 SPPV-3 확정 Go를 선언하는 것은 아니다 —
> §21 게이트(주된 차단 요인)는 여전히 불변이다. **R3b는 Conditional
> Go를 유지한다.** 운영 코드 변경 없음, broker submit 미호출. 산출:
> `scripts/validate_r3b_entry_score_shadow_fidelity.py`(read-only),
> `logs/signal_ic_r3b_entry_score_shadow_fidelity_2026-07-18.json`.
> 상세: `plans/[DESIGN] regime_conditional_entry_signal_v1.md`
> §45(SPPV-2.56).
>
> **📌 2026-07-18 손절(stop-loss) 정책 도입이 총 기대수익에 미치는
> 영향 ablation**: §42(SPPV-2.53)가 §38 보조 잔여 조건에
> 신규 추가한 "경로 리스크(MAE)·손절 정책 부재"는 지금까지 **리스크의
> 존재**만 확인했을 뿐(MAE 평균 -11.08%, 심각 손실 12.8%), "손절선을
> 도입하면 총 기대수익이 개선되는지 악화되는지"는 미답 상태였다 —
> 이는 단순 방어 논리가 아니라 **SPPV-3 설계에 손절 로직을 포함
> 시켜야 하는지를 가르는 실제 의사결정 정보**라 이번 턴 최우선으로
> 골랐다. §42/§43과 동일한 candidate 정의(R3b+entry_score
> risk_off_penalty 제거, B 시나리오, would_buy 1048건, 60거래일
> 관찰)로 baseline(손절 없음)·-15% 손절·-20% 손절 3개 변형을 한 번의
> 60일 순회로 동시 시뮬레이션했다(신규 KIS 호출 0건). **결과:
> baseline 총 기대수익 proxy=9734.7(t=5.38, 양수율 52.8%) 대비 -15%
> 손절=7024.1(약 27.8% 악화, t=4.25, 양수율 46.4%, 손절 발동률
> 28.5%), -20% 손절=9093.8(약 6.6% 악화, t=5.02, 양수율 50.7%,
> 손절 발동률 12.8%) — 두 손절 임계값 모두 총 기대수익을
> 악화시켰고, 손절이 타이트할수록(더 얕을수록) 악화 폭이 컸다.**
> **해석**: R3b candidate는 보유 기간 중 상당한 미실현 손실(MAE
> 평균 -11%)을 겪지만, 실제 청산 로직상 대부분 강제 청산 없이
> 오래 보유되며(§42의 signal-driven 청산이 T+20보다 강했던 것과
> 동일한 메커니즘) **그 조정 구간을 버텨야 이후의 회복·상승분을
> 온전히 취할 수 있는 구조**다 — -15% 손절은 candidate의 28.5%를
> 조정 국면 도중 강제로 잘라내 회복 기회를 원천 차단하며, 그 결과
> 총 기대수익이 깎인다. **판정: "경로 리스크·손절 정책 부재"는
> "미검증 공백"에서 "시험한 범위(-15%/-20%) 내에서는 손절 미도입이
> 총 기대수익 관점에서 근거 있는 선택"으로 재분류한다.** 다만 이번
> 결과가 "어떤 손절도 항상 나쁘다"는 뜻은 아니다 — 두 지점만
> 시험했고, 더 넓은 손절이나 시간 기반 손절 등은 검증하지 않았으며
> 여전히 동일 3년 in-sample 캐시의 한계도 유효하다. **R3b는
> Conditional Go를 유지한다** — 방향성 반전 아님. MAE 자체는
> 여전히 실재하므로, 포지션 사이징 등 exit 외 리스크 관리 수단은
> 낮은 우선순위로 계속 검토 대상이다. 운영 코드 변경 없음, broker
> submit 미호출. 산출: `scripts/validate_r3b_stop_loss_ablation.py`
> (read-only), `logs/signal_ic_r3b_stop_loss_ablation_2026-07-18.
> json`. 상세: `plans/[DESIGN]
> regime_conditional_entry_signal_v1.md` §44(SPPV-2.55).
>
> **📌 2026-07-18 SPPV-2.53 결론 문구 정밀화 — 20일판·60일판 표본
> 동일성 검증 + "거의 해소" 표현 재점검**: 직전 턴(SPPV-2.53,
> §42)이 서술한 **"censored 91.1%→51.3%, 평균 보유일수 19.35일→
> 48.0일"**을 신규 실행 없이 두 스크립트(`validate_r3b_signal_
> driven_exit_timing.py`, `..._extended.py`) 코드 대조로 재점검했다
> — 새 가설을 파는 턴이 아니라 이미 나온 결과가 **같은 표본
> 비교인지**를 정확히 고정하는 턴이었다. **코드 기준 판정**: 두
> 스크립트 모두 후보 스캔 범위를 `last_t = len(bars)-1-
> MAX_EXIT_OBSERVATION_DAYS`로 제한하는데, 60일판(`=60`)은 3년
> 캐시 끝에서 60거래일을 남겨야 스캔에 포함시켜 20일판(`=20`,
> 1151건)보다 좁은 표본(1048건)을 만든다 — candidate 선정 로직은
> 당일 backward-looking 데이터만 사용해 관찰 창과 무관하므로 **60
> 일판(1048건)은 20일판(1151건)의 약 91% 부분집합으로 추정된다.
> 즉 "91.1%→51.3%"는 동일 코호트의 순수 전/후 비교가 아니라 겹치지만
> 완전히 같지는 않은 두 표본의 비교**다. **확실히 말할 수 있는
> 것**: 각 판의 표본 내부 측정치(60일판 censored=51.3%, 평균 보유
> 일수=48.0일, signal-driven 청산=9.29%(t=5.38), MAE 평균=-11.08%)
> 는 유효하고, 표본 차이(~9%)가 효과 크기(censored 40%p 감소 등)를
> 설명하기엔 작아 "관찰 창을 늘리면 청산이 늦어지고 censored가
> 준다"는 방향성 자체는 신뢰 가능하다. **과장인 것**: 91.1%→51.3%
> 를 "엄밀한 페어드 전후 비교치"로 인용하는 것, 그리고 "T+5 구조적
> 리스크가 거의 해소됐다"는 것 — 60일 관찰 후에도 **과반(51.3%)이
> 여전히 censored**이기 때문이다. **판정: 정확한 표현은 "부분
> 완화"(§41)에서 "추가 완화"(§42/§43)로 하향 정정한다** — "거의
> 해소"·"완전 해소" 단계는 아직 아니다. **R3b는 Conditional Go를
> 유지한다** — 방향성 반전 아님, 60일판 표본 내부 비교(signal-
> driven 청산이 T+20보다 강하다는 것, MAE 분포)는 그대로 유효.
> 경로 리스크(MAE)·손절 정책 부재는 §38 보조 잔여 조건 목록에
> 계속 유지(변경 없음). 신규 실행 없음(§41·§42 기존 산출물 재해석
> + 코드 대조만 수행), 신규 KIS 호출 0건, 운영 코드 변경 없음,
> broker submit 미호출. 상세: `plans/[DESIGN]
> regime_conditional_entry_signal_v1.md` §43(SPPV-2.54).
>
> **📌 2026-07-18 T+5 horizon 구조적 리스크 — 20거래일 초과 구간·
> 경로 리스크(MAE) 확장 검증**: 직전 턴(SPPV-2.52, §41)이
> 20거래일 관찰 창으로 남긴 두 미확인 영역 — (a) 20일 초과 구간에서
> 실제 청산은 여전히 늦게 일어나는가, (b) 보유 중 경로 리스크(MAE)
> 는 얼마나 큰가 — 를 이번 턴에 직접 검증했다. §41과 **동일한
> candidate 정의(R3b+entry_score risk_off_penalty 제거, B 시나리오,
> would_buy 후보)를 재사용**하되 관찰 창을 **20→60거래일로 확장**
> 하고 **MAE(보유 구간 중 최대 미실현 손실)를 신규 계산**했다(1단계
> 저비용 entry scan으로 would_buy 확정 → 2단계에서 would_buy 후보
> 1048건에만 60일 exit+MAE 시뮬레이션 적용, 신규 KIS 호출 0건).
> **결과: censored 비율 91.1%(20일)→51.3%(60일)로 감소, 평균
> 보유일수=19.35일→48.0일로 증가. signal-driven 청산 수익률(9.29%,
> t_NW=5.38)이 오히려 고정 T+20(4.46%, t=3.41)보다 강하다.** **MAE:
> 평균 -11.08%, 중앙값 -10.42%, 하위 10% -21.77%, 최악값 -45.10%,
> -20% 이하 심각 손실 비율 12.8%.** **해석**: 관찰 창을 늘리면
> 실제로 더 많은 청산이 관측되지만(censored 감소), 그 청산은 T+5는
> 물론 T+20보다도 더 늦게 일어나는 경우가 많고 그 수익률은 T+20
> 보다 강하다 — **"T+5 구조적 리스크"는 "부분 완화"에서 "거의
> 해소"로 격상됐다(100% 해소는 아님, 51.3%가 여전히 60일 내
> censored).** 그러나 이 검증으로 **경로 리스크(MAE)·손절 정책
> 부재라는 신규 잔여 조건**이 드러났다 — 평균 -11%, 심각 손실
> 12.8%는 무시할 수 없는 리스크이며, 코드상 `_build_exit_score`
> 외에 별도의 손절(stop-loss) 임계값이 없음을 재확인했다. **판정:
> R3b는 Conditional Go를 유지한다** — 최종 실현 수익률은 여전히
> 강하고 방향이 일관돼 MAE가 방향성을 뒤집는 근거는 아니며, 대신
> 포지션 사이징·손절 정책 설계가 필요하다는 운영상 시사점으로
> §38 보조 잔여 조건에 신규 추가한다. 운영 코드 변경 없음, broker
> submit 미호출. 산출: `scripts/validate_r3b_signal_driven_exit_
> timing_extended.py`(read-only), `logs/signal_ic_r3b_signal_
> driven_exit_timing_extended60d_2026-07-18.json`. 상세: `plans/
> [DESIGN] regime_conditional_entry_signal_v1.md` §42(SPPV-2.53).
>
> **📌 2026-07-18 T+5 horizon 구조적 리스크 추가 정량화 — 실제
> exit_score 기반 signal-driven 청산 타이밍 시뮬레이션**:
> §38이 정리한 보조 잔여 조건 3개(entry_score 코드 반영 절차/T+5
> 구조적 리스크/혼합도 모니터링) 중 지금 당장 신규 설계 없이 기존
> 3년 캐시만으로 실측 가능한 **"T+5 구조적 리스크"만 전진시켰다**
> — entry_score 코드 반영 절차는 실제 운영 코드 변경 PR·컴플라이언스
> 검토가 전제라 이번 턴 범위를 넘고, 혼합도 모니터링은 계량치보다
> 설계 검토가 먼저 필요했다. **실제 운영 함수 `_build_exit_score`
> (`deterministic_trigger_engine.py`, 순수 함수·DB/실시간 상태
> 불필요)를 R3b+entry_score risk_off_penalty 제거(B 시나리오)
> would_buy candidate 1151건에 point-in-time으로 재호출**해 실제
> 매도 임계값(`sell_candidate_threshold=0.75`)을 처음 넘는 날을
> 20거래일 관찰 창으로 시뮬레이션했다(신규 KIS 호출 0건, 기존 3년
> 캐시로 전량 서빙). **결과: 91.1%(1049건)가 20거래일 안에 매도
> 신호를 넘지 않고 censored 처리, 평균 보유일수=19.35일(20일 상한에
> 근접). signal-driven 청산 수익률(평균 6.14%, t_NW=4.73)은 T+5
> (2.02%, t=4.18)보다 T+20(6.49%, t=3.87)에 훨씬 가깝다 — 오히려
> signal-driven 청산의 t_NW가 고정 T+20보다 높다.** **해석: 실제
> 청산 로직 기준으로는 대부분의 포지션이 T+5가 아니라 T+20 근방(또는
> 그 이상)까지 보유되며, 그 실현 수익률은 T+20과 방향·크기 모두
> 일치한다 — "T+5 평균이 약하다"는 우려가 실제 운영 리스크로 그대로
> 전이되지 않는다.** 다만 **아직 말하면 과장인 것**: 91.1%가
> censored라는 것은 20거래일 관찰 창의 한계일 뿐(20일을 넘는 실제
> 청산 분포는 미검증), 보유 기간 중 경로 리스크(MAE)도 반영하지
> 않아 "T+5 구조적 리스크가 완전히 해소됐다"고 말하는 것은 과장이다
> — 정확한 표현은 **"부분적으로 완화됐다"**. **판정: R3b는
> Conditional Go를 유지한다.** 운영 코드 변경 없음, broker submit
> 미호출. 산출: `scripts/validate_r3b_signal_driven_exit_timing.py`
> (read-only), `logs/signal_ic_r3b_signal_driven_exit_timing_2026-
> 07-18.json`. 상세: `plans/[DESIGN]
> regime_conditional_entry_signal_v1.md` §41(SPPV-2.52).
>
> **📌 2026-07-18 SPPV-2.50 결론 문구 정밀화 — 과장 없이 고정**:
> 직전 턴(SPPV-2.50)이 사용한 두 문구를 신규 실행 없이
> 재점검했다(SPPV-2.51). **정정 1**: "구조적 패턴으로 격상"은
> 과장이다 — 이 3분위 재확인이 R3b/entry_score 조합을 이미 확정
> 하는 데 쓰인 것과 **동일한 3년 in-sample 캐시**에서 수행됐고,
> mixed_score가 60거래일 trailing window라 인접 거래일 버킷이
> 서로 강하게 자기상관돼 634거래일이 634개의 독립 관측이 아니기
> 때문이다. **지금 확실히 말할 수 있는 것**: 단조 감소·217/215/
> 202일의 균등 분포는 그대로 사실이며 "지지 증거 추가" 단계는
> 명백히 넘어섰다. **아직 말하면 과장인 것**: "out-of-sample로
> 확정된 구조적 패턴"이라 부르는 것 — 정확한 표현은 **"강한
> 구조적 정합 증거로 격상"**이다. **정정 2**: "주된 차단 요인은
> §21 게이트 하나뿐"은 "SPPV-3 착수 검토를 시작할 수 있는 유일한
> 외생적 조건"이라는 뜻이지 "진입 전체에 남은 유일한 조건"이
> 아니다 — §38의 ①주된 차단 요인(§21 게이트) ②보조 잔여 조건
...
> 0건, 운영 코드 변경 없음, broker submit 미호출. 상세: `plans/
> [DESIGN] regime_conditional_entry_signal_v1.md` §40.6(SPPV-2.51).
>
> **📌 2026-07-18 "혼합 국면 약세" 가설 직접 분해 — 거래일 단위
> 혼합도 3분위 버킷화**: 직전 턴(SPPV-2.49)의 분기1 vs 분기4
...
0. **BUY 주문 0건 근본 복구 (2026-07-14 신설, 최우선)**
  ...
  이후 §2.24/§14 ablation으로 보정됨)**: `regime_conditional_
   ...
   - **SPPV-2.51(완료, 2026-07-18, SPPV-2.50 결론 문구 정밀화 —
     과장 없이 고정 — Conditional Go 유지, 방향 변경 없음)**:
     신규 실행 없이 SPPV-2.50의 두 문구를 기존 산출물만으로
     재점검했다. **정정 1**: "구조적 패턴으로 격상"은 과장 — 이
     3분위 재확인이 R3b/entry_score 조합을 이미 확정하는 데 쓰인
     것과 동일한 3년 in-sample 캐시에서 수행됐고, mixed_score가
     60거래일 trailing window라 인접 거래일 버킷이 서로 자기상관돼
     634거래일이 634개의 독립 관측이 아니다. **확실히 말할 수
     있는 것**: 단조 감소·217/215/202일의 균등 분포는 그대로
     사실이며 "지지 증거 추가" 단계는 명백히 넘어섰다. **과장인
     것**: "out-of-sample로 확정된 구조적 패턴" — 정확한 표현은
     **"강한 구조적 정합 증거로 격상"**이다. **정정 2**: "주된
     차단 요인은 §21 게이트 하나뿐"은 "SPPV-3 착수 검토를 시작할
     수 있는 유일한 외생적 조건"이라는 뜻이지 "진입 전체에 남은
     유일한 조건"이 아니다 — §38의 ①주된 차단 요인(§21 게이트)
     ②보조 잔여 조건(entry_score 코드 반영 절차·T+5 구조적
     리스크·혼합도 모니터링) ③실거래 누적 필요 조건 3단 분류는
     이번 턴에도 그대로 유효하다. **판정: 두 정정 모두 R3b
     방향성·Conditional Go를 바꾸지 않는다** — 서술 정밀도만
     회복. 신규 실행 없음, 신규 KIS 호출 0건, 운영 코드 변경 없음,
     broker submit 미호출. 상세: `plans/[DESIGN] regime_
     conditional_entry_signal_v1.md` §40.6.
   - **SPPV-2.52(완료, 2026-07-18, T+5 horizon 구조적 리스크 추가
     ...
     를 선택했다 — entry_score 코드 반영 절차는 실제 운영 코드 변경
     PR·컴플라이언스 검토가 전제, 혼합도 모니터링은 설계 검토가
     먼저 필요. 실제 운영 함수 `_build_exit_score`(순수 함수, DB/
     실시간 상태 불필요)를 R3b+entry_score risk_off_penalty 제거(B
     시나리오) would_buy candidate 1151건에 point-in-time으로
     재호출해 매도 신호(`sell_candidate_threshold=0.75`)를 처음
     넘는 날을 20거래일 관찰 창으로 시뮬레이션했다(신규 KIS 호출
     0건). **결과: 91.1%(1049건)가 20거래일 안에 매도 신호를 넘지
     않고 censored, 평균 보유일수=19.35일. signal-driven 청산
   ...
   - **SPPV-2.53(완료, 2026-07-18, T+5 horizon 구조적 리스크 —
     ...
     Go 유지, "T+5 구조적 리스크" 거의 해소·경로 리스크 신규
     잔여 조건 추가)**: §41이 20일 관찰 창으로 남긴 두 미확인
     영역(20일 초과 구간 청산 분포, 보유 중 경로 리스크)을 직접
     검증했다. §41과 동일한 candidate 정의를 재사용하되 관찰 창을
     20→60거래일로 확장하고 MAE(보유 구간 중 최대 미실현 손실)를
     추가 계산했다(1단계 저비용 entry scan → would_buy 확정 →
     2단계 would_buy 1048건에만 60일 exit+MAE 시뮬레이션 적용,
     신규 KIS 호출 0건). **결과: censored 비율 91.1%→51.3%로 감소,
     평균 보유일수=19.35일→48.0일. signal-driven 청산 수익률
     (9.29%, t=5.38)이 오히려 고정 T+20(4.46%, t=3.41)보다 강함.
     MAE 평균 -11.08%, 중앙값 -10.42%, 하위 10% -21.77%, 최악값
     -45.10%, -20% 이하 심각 손실 비율 12.8%.** **판정: 실제 청산은
     T+20보다도 더 늦게 일어나는 경우가 많고 그 수익률은 T+20보다
     강해 "T+5 구조적 리스크"는 "부분 완화"에서 "거의 해소"로
     격상됐다.** 그러나 이 검증으로 **경로 리스크(MAE)·손절 정책
     부재라는 신규 잔여 조건**이 드러났다(코드상 별도 손절 임계값
     없음 재확인). R3b는 Conditional Go를 유지한다 — 방향성 반전
     아님. 신규 KIS 호출 0건, 운영 코드 변경 없음, broker submit
     미호출. 산출: `scripts/validate_r3b_signal_driven_exit_
     timing_extended.py`(read-only), `logs/signal_ic_r3b_signal_
     driven_exit_timing_extended60d_2026-07-18.json`. 상세: `plans/
     [DESIGN] regime_conditional_entry_signal_v1.md` §42.
     **[SPPV-2.54에서 정정] "거의 해소"는 과장 — 아래 SPPV-2.54
     참고.**
   - **SPPV-2.54(완료, 2026-07-18, SPPV-2.53 결론 문구 정밀화 —
     20일판·60일판 표본 동일성 검증 + "거의 해소" 표현 재점검 —
     Conditional Go 유지, "거의 해소"→"추가 완화"로 하향 정정)**:
     신규 실행 없이 §42의 "censored 91.1%→51.3%" 비교와 "T+5
     구조적 리스크 거의 해소" 판정을 두 스크립트 코드 대조로
     재점검했다. **코드 기준 판정**: 두 스크립트 모두 후보 스캔
     범위가 `last_t = len(bars)-1-MAX_EXIT_OBSERVATION_DAYS`로
     제한되는데, 60일판은 3년 캐시 끝 약 40거래일이 스캔에서
     제외돼 20일판(1151건)보다 좁은 표본(1048건)을 만든다 —
     candidate 선정 로직 자체는 관찰 창과 무관한 당일 backward-
     looking 계산이므로 **60일판은 20일판의 약 91% 부분집합으로
     추정된다. 즉 두 결과는 동일 코호트의 순수 전/후 비교가 아니라
     겹치지만 완전히 같지는 않은 두 표본의 비교**다. **확실히 말할
     수 있는 것**: 각 판의 표본 내부 측정치는 유효하고 표본 차이
     (~9%)가 효과 크기를 설명하기엔 작아 방향성은 신뢰 가능하다.
     **과장인 것**: 91.1%→51.3%를 엄밀한 페어드 비교치로 인용하는
     것, "거의 해소"라는 표현 — 60일 관찰 후에도 과반(51.3%)이
     여전히 censored이기 때문이다. **판정: 정확한 표현은 "부분
     완화"(§41)에서 "추가 완화"(§42/§43)로 하향 정정한다.** R3b는
     Conditional Go를 유지한다(방향성 반전 아님). 신규 실행 없음,
     신규 KIS 호출 0건, 운영 코드 변경 없음, broker submit 미호출.
     상세: `plans/[DESIGN] regime_conditional_entry_signal_v1.md`
     §43.
   ...
   - **SPPV-2.56(완료, 2026-07-18, entry_score 코드 반영 절차
     ...
     "entry_score 코드 반영 절차" 착수 준비도 격상)**: `§21` 게이트
     는 외생 조건이라 반복 관측만 가능한 반면, "entry_score 코드
     반영 절차"는 실제 코드 변경 PR 작성 전 확인해야 할 선행 질문이
     있었다 — SPPV-2.46부터 이 세션 내내 B 시나리오 non-alpha
     조정을 수작업 재구현 `_non_alpha`로 계산해왔을 뿐, 실제 운영
     함수 `_build_entry_score`를 한 번도 직접 호출한 적이 없었다.
     코드 대조 결과 `_build_entry_score`에는 `_non_alpha`가 담아
     내지 못하는 portfolio_allocation·source_type 조정 항·최종
     clamp가 있었으나, 이 세션에서는 항상 `source_type="core"`,
     `portfolio_allocation=None`으로 써서 이론상 no-op이었다. 3년
     전체 후보 표본(58,493건)에서 실제 함수와 재구현을 전수
     대조했다(신규 KIS 호출 0건). **결과: 100.0%(58,493/58,493)
     완전 일치, 불일치 0건, 최대 절대 오차 0.0.** **판정: 이
     세션의 모든 B 시나리오 결과가 실제 운영 코드 동작을 정확히
     대표한다는 것이 처음으로 전수 검증됐다 — "entry_score 코드
     반영 절차"는 "설계 논의 단계"에서 "shadow 계산 정합성 확보,
     실제 코드 변경 PR 작성 가능 단계"로 격상됐다.** 다만 §21
     게이트는 불변이라 SPPV-3 확정 Go는 아니다. R3b는 Conditional
     Go를 유지한다. 신규 KIS 호출 0건, 운영 코드 변경 없음, broker
   ...
   - **SPPV-2.58(완료, 2026-07-18, `§21 gate` config 기반 gate 제어
     — mode-agnostic 신규 모듈 구현, 작성자: Codex — Conditional
     Go 유지, §21 게이트 상태 불변)**: `§21 게이트`를 문서 해석이
     아니라 코드 레벨에서 config 스위치로 제어 가능하게 만들었다 —
     단, environment(paper/real) 분기가 아니라 config 하나만 보는
     ...
     이 세션의 원칙에 따라 이번에도 수정하지 않고, 신규 격리 모듈로
     구현했다: `AppSettings.regime_switch_v1_gate_override_enabled`
     (env: `REGIME_SWITCH_V1_GATE_OVERRIDE_ENABLED`, 기본값 False)
     ...
     validate_regime_switch_gate_config_override.py`(read-only,
     신규 KIS 호출 0건)로 검증: 운영 코드 미수정을 소스 검사로
     확인(`isolation_confirmed=True`), 실제 게이트 상태 여전히
     `NOT_TRIGGERED`, override off/on 및 3개 trigger_status
     시나리오 전부 예상대로 동작. 판정: R3b는 Conditional Go를
     유지한다 — 게이트 상태 불변, `deterministic_trigger_engine.py`
     미수정, compliance/VaR/broker submit 경계 미변경, 아직 실제
     파이프라인 미연결(별도 승인 필요). 신규 KIS 호출 0건. 산출:
     `src/agent_trading/services/regime_switch_gate.py`(신규),
     `src/agent_trading/config/settings.py`(필드 추가), `scripts/
     validate_regime_switch_gate_config_override.py`(read-only),
   ...
   - **SPPV-2.62(완료, 2026-07-18, 국면 혼합도 모니터링 모듈 구현
     ...
     **혼합도 모니터링 설계**를 최우선으로 선택했다 — trigger_
     ...
     [DESIGN] regime_conditional_entry_signal_v1.md` §51. **[SPPV-
     2.63에서 진전] 실제 소비 위치 연결 완료 — 아래 SPPV-2.63
     참고.**
   - **SPPV-2.63(완료, 2026-07-19, 국면 혼합도 모니터링을 실제
     ...
     check()`를 추가 — 기존 `_run_precheck()`와 동일한 cycle당
     1회 안전 패턴으로, 벤치마크 `signal_feature_snapshots` 최근
     60건을 read-only 조회(신규 KIS 호출 없음)해 §51 모듈로 국면
     혼합도 버킷을 계산·로그에 남긴다. **BUY/SELL 판정에는 전혀
     연결하지 않았다.** `scripts/validate_r3b_mixedness_decision_
     loop_wiring.py`(read-only, in-memory repos, 신규 KIS 호출
     0건)로 `_run_mixedness_check()`를 실제 import·호출해 저혼합/
     고혼합 두 시나리오 모두 정확히 분류됨을 확인, 소스 검사로
     BUY/SELL 판정 코드가 전혀 없음도 확인. 기존 단위 테스트 10건
     실패는 변경 전에도 동일하게 실패하는 사전 존재 결함임을 stash
     ...
     미변경. 산출: `scripts/run_decision_loop.py`(수정),
   ...
   - **SPPV-2.65(완료, 2026-07-19, entry_score 코드 변경 PR 초안
     ...
     다시 실측하지 않고, 이 세션에서 한 번도 명시되지 않은
     아키텍처 제약을 조사했다. **핵심 발견**: entry_score는 종목
     단위로 계산되지만 R3b alpha(candidate_percentile)는 당일
     cross-sectional 순위가 필요해 사전 계산 단계가 있어야 한다.
     **기존 선례 발견**: `run_decision_loop.py`의 `_build_core_
     risk_off_apply_overrides_for_cycle()`(cycle당 1회 전체
     universe precompute → override 주입)이 정확히 필요한 구조로
     이미 존재함을 확인. **제안 설계(미적용)**: 신규 precompute
     함수 1개(당일 quintile 상위 20% candidate의 candidate_
     percentile 계산, 기존 shadow 로직 이식) + `assess_
     deterministic_triggers`에 §48/§49와 동일 패턴의 optional
     파라미터 2개(`r3b_alpha_percentile`, `r3b_alpha_enabled`,
     기본값 None/False) + 신규 config 스위치 `ENTRY_SCORE_R3B_
     ALPHA_ENABLED`(기본값 False). 판정: "entry_score 코드 반영
     절차"는 "shadow 정합성 확보"에서 "구체적 구현 설계 확보(diff
     초안)"로 진전됐다 — 실제 적용은 별도 승인 필요. R3b는
     Conditional Go를 유지한다. 신규 KIS 호출 0건, compliance/
   ...
   - **SPPV-2.66(완료, 2026-07-19, entry_score R3b alpha 교체 —
     ...
     Conditional Go 유지)**: §54(SPPV-2.65)의 미적용 설계 중
     "1단계: 엔진 파라미터 배선"만 선택해 실제 코드로 적용했다(cycle
     단위 precompute인 "2단계"는 범위 밖, 별도 승인 대상 유보).
     `settings.py`에 `entry_score_r3b_alpha_enabled` config 스위치
     (env: `ENTRY_SCORE_R3B_ALPHA_ENABLED`, 기본값 False) 추가,
   ...
   - **SPPV-2.72(완료, 2026-07-19, 벤치마크(069500) signal_feature_
     ...
     069500 snapshot 0건→1건 실측 확인, `_build_r3b_alpha_
     percentile_overrides_for_cycle()` 재호출 결과 `{'000810': 1.0,
     '001450': 0.0}`(빈 dict 탈출) 확인. 회귀 20+83 passed 무손상.
     `ENTRY_SCORE_R3B_ALPHA_ENABLED=true` 전환 시 이제 실제 발동
   ...
   - **SPPV-2.74(완료, 2026-07-19, docker-compose 환경변수 배선
     ...
     --no-deps`로 재생성. 재생성 후 실제 프로세스 env 확인 결과
     두 값 모두 `true`, `/app/.env` 파일은 여전히 없음(compose
     주입만으로 전달 증명), `AppSettings()` 실행 결과도 `True
     True`. 재생성 후 로그 정상(비거래일 정상 판정, submit_count=
     0). 판정: 실제 차단 요소 완전 해소 — R3b alpha/§21 게이트
     override 모두 이제 실제 paper 운영 프로세스에 도달. R3b는
     Conditional Go를 유지한다. 상세: `plans/[DESIGN] regime_
     conditional_entry_signal_v1.md` §63.
   ...
   - **SPPV-2.78(완료, 2026-07-20, BUY_CANDIDATE 최종 통과 0건의
     ...
     decision_type도 BUY=SELL=REDUCE=EXIT=0. 000810의 AI 최종
     합성기 실제 호출(`fdc_skipped=False`)과 `opposing_evidence`
     (risk_off/전략 충돌/weak evidence)가 36회 거의 동일 문구
     반복 확인 — 정당한 방어 논리일 수 있으나 국면 라벨 고착
     가능성도 배제 못 함. 판정: "BUY 후보는 생성되지만 마지막
     단계 병목 때문에 0건 통과". 보정: 층3·층2=유지, 층1(downgrade)
   ...
   - **SPPV-2.80(완료, 2026-07-20, R3b 최종 병목의 조건 민감도
     검증 + 신규 발견(expected_value_gate 정량 게이트), 작성자:
     Codex — 아직 직접 분기축 단정 불가, R3b Conditional Go
     유지)**: `candidate_intent=buy` 39→47건, **watch 36/no_
     action 9/buy 2**로 분해 — "buy 0건"이 이번 조회에서 처음
     깨짐. 신뢰도 축은 명확한 threshold 아님(no_action 극단값
     1건뿐), 규제flag 비율은 39%→89%로 상승하나 전용 축 아님
     ("weak+규제flag" 조합이 보조 강도 축). **신규 발견**: 실제
     `decision_type='APPROVE'` 2건이 `translation.py`의
     `_has_required_expected_value_anchor`에서 `expected_value_
     gate.passed=False`(edge_after_cost_bps=8.56 <
   ...
   - **SPPV-2.119(완료, 2026-07-28 KST, `0.48` 모집단 정밀 분해,
     ...
     signal_research_sppv/[DESIGN] regime_conditional_entry_
     signal_v1.md` §106.
   - **SPPV-2.120(완료, 2026-07-28 KST, `ranking_score` 산식
     구성요소 분해, 작성자: Codex, 코드 미수정, threshold/diff/
     완화안 없음, 신규 KIS 호출 0건)**: 실제 코드 공식이 설계
     문서 §7.2 제안식과 다름을 확인. 이 모집단은 정의상 `regime_
     tailwind`/`strategy_alignment` 100% 고정 0, `coverage_
     score`(1.0)/`allocation_quality`(0.25)도 완전 무분산. 실질
     변별력 있는 `entry_score`(관측 상한 0.2479)/`relative_
     activity`(관측 상한 0.6830) 관측 상한을 모두 결합해도
     이론적 상한 0.4296(고정 항목 0.05 회복 가정해도 0.4796)으로
   ...
   - **SPPV-2.126(완료, 2026-07-28 KST, `top50=002790` 문구
     ...
     research_sppv/[DESIGN] regime_conditional_entry_signal_
     v1.md` §114.
   - **SPPV-2.127(완료, 2026-07-29 KST, distinct symbol 기준
     기여도 재계산 + 반복 등장 원인 규명, `[PLAN]` §6.8 잔여
     완료, 작성자: Codex, 코드 미수정, Full pytest 미실행, 신규
     KIS 호출 0건)**: 게이트 모집단 내부(distinct symbol=25)에서
     상위/하위 10개 종목 기여도 차이의 100.0%, 일반 BUY 경로
     전체(`eligibility_path='buy'`, distinct symbol=105, row=
     35,149)에서 96.2%를 `entry_score`+`relative_activity`가
     설명 — 종목 반복 편향 제거 후에도 기존 결론 유지.
     `002790`(6일 산발, 239건)/`000720`(20일+ 연속, 761건) 반복
     원인은 intraday decision loop 5분 주기(`DEFAULT_INTERVAL_
     SECONDS=300`)+`signal_feature_snapshot` 1일 1회 갱신+게이트
     고정 상태 지속의 동일 메커니즘 — 정상 반복이며 저장/집계
     결함 아님. 최종 판정 무관하게 유지. 상세: `docs/10_signal_
     research_sppv/[DESIGN] regime_conditional_entry_signal_
     v1.md` §115.
   - **SPPV-2.128(완료, 2026-07-29 KST, `regime_tailwind`/
     `strategy_alignment` 고정 여부 설계 의도 vs 부산물 판정,
     작성자: Codex, 코드 미수정, Full pytest 미실행, 신규 KIS
     호출 0건)**: `regime_tailwind`는 `source_type` 무관하게
     `risk_tone`에만 의존(코드 확인). `strategy_alignment`는
     `strategy_selection.py`에 `source_type=='event_overlay'`
     전용 override가 있어 `risk_off`여도(`bearish_trend`만
     아니면) `event_continuation`을 강제 부여함을 확인. 전체
     이력(n=38,997)에서 `regime_tailwind=1.0`은 42건(전부
     ...
     0이 아닌 사례 없음. 판정: `strategy_alignment`(core
     기준)는 **설계 의도대로 죽어 있는 항**, `regime_tailwind`는
   ...
   - **SPPV-2.130(완료, 2026-07-29 KST, `ranking_score` 산식
     재설계 준비 — 4개 항목 역할 재분류, 작성자: Codex, 코드
     미수정, 완화안/코드 diff 없음, Full pytest 미실행, 신규
     KIS 호출 0건)**: `regime_tailwind`/`strategy_alignment`
     "고정 여부 확인" 단계(§116/§117)를 종료하고 산식 재설계
     준비 단계 진입. `coverage_score`(하드 게이트 통과 후 100%
     상수 확인)는 **다른 계층으로 이관 검토(1순위)**, `relative_
     activity`(entry_score+ranking 소프트 2중 과잉 중복)는
     **중복 정리 검토(2순위)**, `strategy_alignment`(entry_
     score+ranking 완전 동일 조건, 현재 미발동)는 **중복 정리
     검토(3순위)**, `regime_tailwind`(상류·하류에서 이미 처리,
     존치 근거 약함)는 **역할 축소 검토(4순위)**. 1·2순위는
     추가 사실 확인 없이 즉시 설계안(A/B) 비교 단계 진입 가능 —
   ...
   - **SPPV-2.134(완료, 2026-07-29 KST, `relative_activity` 1안
     ...
     이후 병합 직전 30분(n=120) vs 병합 이후 1개 사이클(n=15)을
     비교 — `ranking_score` 평균/중앙값 미세 이동(0.3358/0.3037→
     0.3319/0.2811)은 표본 극소로 해석 보류, `ranking_blocked`
     비중 46.7%→53.3%는 단일 창 변동으로 판단(의미 부여 안 함),
     `buy_candidate`(0/0)·`shadow_topk_exception_v2`(0건)는
     **변화 없음**. 관측 창이 1개 사이클(약 6분)뿐이라는 한계
     명시. 핵심 병목은 여전히 `coverage_score`+절대 threshold
     (`0.48`/`0.22`) 조합으로 재확인. **다음 1순위 결정: 2안
   ...
   - **SPPV-2.139(완료, 2026-07-30 KST, `coverage_score` A-3안
     적용 후 운영 무변화 실측 확인, 작성자: Codex, 코드 미수정,
     `.env` 미수정, Full pytest 미실행, 신규 KIS 호출 0건 —
     트랙 종료)**: 장중 예외 승인으로 2026-07-30 13:21:17 KST
     실제 배포됨을 `ranking_min_score=0.28`/`shadow_min_score=
     0.02` echo로 확인. 배포 직전 2시간(gate n=176) vs 배포 이후
     누적(~39분, gate n=64) 비교 — `ranking_blocked` 비중
     **87.5%→87.5%(소수점까지 동일)**, `buy_candidate`/`APPROVE`/
     `order_request`/`shadow_would_pass` 등은 배포 전후 모두 0.
   ...
   - **SPPV-2.141(완료, 2026-07-30 KST, `core_cap` 사전순 절단
     ...
     2~3개). `000720`(shadow 하위 10~15%, 13/20일[SPPV-2.142에서
     정정: 11/20일] 포함) vs `009150`(shadow 상위 15~30%, 0/20일
     포함) 극단 역전 사례를
   ...
   - **SPPV-2.148(완료, 2026-07-30 KST, `strategy_alignment` 직접항
     제거 threshold 영향 정량 검증, 작성자: Codex, 코드 미수정, `.env`
     미수정, Full pytest 미실행, 신규 KIS 호출 0건 — **운영 효과
     미확정**)**: 게이트 모집단(`core_risk_off_guard_active=true`)에
     `strategy_alignment=1.0`이 **최근 3거래일 0/2,401, 전체 이력
     0/11,785**로 없어 `ranking_score` 무변화, `0.28`/`0.02`/`0.26`
     판정 뒤집힘 **두 창 모두 0건**. 일반 BUY 경로는 평균만 미세 하락
     하나 `ranking_score`가 비-core 경로에서 판정에 쓰이지 않음을 코드로
     확인(`:474-486`). `sa=1.0` 2,760건은 `event_overlay`(2,718)+
     `market_overlay`(42)에만 존재하고 `core` 0건 → **`event_overlay`
     에서만 의미, `core` 게이트 영향 없음**. 범위 밖 관찰 지표
     (`0.56`)만 전체 이력 122건 이동(최근 3거래일 0건)이나 실제
     `shadow_would_pass=True` 60건 중 뒤집힘 0건. **판정: 추가 코드
   ...
   - **SPPV-3(다음 착수: [1순위] **2026-08-03(월) 08:50 KST freeze 실측** —
     ...
     판정 변화 여부 read-only 확인. **[2026-08-01 KST 갱신] `ranking_
     ...
     **3거래일/1개월/전체 이력 모두 100% 뒤집힌다** — 단순 재정규화는
     ...
     착수한다. **[2026-08-02 KST 3차 갱신] 새 threshold 산정안 비교
     (A/B/C, `buy_path_variable_gate_matrix.md` §13.1.4) 완료** —
     `entry_score`의 자체 allocation 보정항(`min(0.10, pct/100)`)이
     `ranking_score`의 `0.10*allocation_quality`와 `0<pct<=10` 구간에서
     수치까지 동일함을 확인해 R1 결론을 수치 수준에서 재확인했다.
     A안(단순 선형 치환, 이미 기각)/B안(실측 상수 보정, 재검증 부담
     있음)/C안(보조 조건 병행 — 기존 산식을 인라인으로 그대로 재현)
     ...
     **C 집합이 0건이 아니다**(전체 이력 3,692건/27종목/89 조합).
     ...
     다음 턴에 진행할 것을 권고한다. **[2026-08-03 KST 8차 갱신]
     A/B/C 설계 비교 완료**(`buy_path_variable_gate_matrix.md`
     §13.2.8) — C 집합 3,692건 중 `bullish_trend`가 3,616건(97.9%)
     으로, 하드 게이트(`bearish_trend`+`risk_off`에서만 발동)가 이
     population의 98.6%를 전혀 커버하지 못함을 확인했다. **C안(제거)
     은 권고하지 않으며, 1순위 권고안은 B안(계수 완화, 후보
     `-0.10`/`-0.05`)**이다. 다음 코드 수정 단위는 **B(추가 실측
     1건 필요)**로 좁혔다. **[2026-08-03 KST 9차 갱신] B안 후보
     계수별 영향 실측 완료**(`buy_path_variable_gate_matrix.md`
     §13.2.9) — `B1(-0.10)`/`B2(-0.05)`의 신규 통과 표본(전체 이력
     297건/2,047건)은 전부 `bullish_trend`뿐이고, 실제 override
     사례(`011070`)는 `B2`부터 해소된다. **1순위 권고 후보는
     B2(-0.05)**이며, 참고용 상한 비교 `B3(0.00)`은 `bearish_
     trend`/`range_bound`까지 끌어들여 권고하지 않는다. 다음 코드
     수정 단위는 **A(바로 초안 가능)**로 좁혔다. **[2026-08-03 KST
     10차 갱신] B2(-0.05) 코드 적용 완료**(`buy_path_variable_gate_
     matrix.md` §13.2.10) — `_build_entry_score()`의 `risk_off`
     ...
     **정당한 역할 분리**로 판정했다. `max_new_capital_pct`의 score
     ...
     작고 선택적)**로 좁혔다. **[2026-08-03 KST 13차 갱신] A 적용
     완료**(`buy_path_variable_gate_matrix.md` §13.3.2) —
     ...
     **[2026-08-03 KST 15차 갱신] `risk_off` 하드 게이트 단일 권위화
     ...
     할 포인트로 남긴다. **[2026-08-03 KST 18차 갱신] `SPPV-3` 존속
     판정 완료(read-only, 새 실측 없음)** — `BUY 경로 리팩터링`
     (R1~R4) 완료를 반영해 SPPV-3의 존속 여부를 판정했다. **판정:
     폐기가 아니라 재정의 후 지속.** 근거: 리팩터링은 경로/계약/
     병목 정리, SPPV-3는 신호/점수 예측력 검증이라 전자가 끝나도
     후자는 자동 소멸하지 않는다. 다만 §4.3~§4.5가 전제한 구
     `entry_score` 공식(allocation 항·`risk_off -0.15`)은 R2에서
     이미 바뀌었고, §4.5의 funnel 전환율 측정에는 `stale_snapshot_
     guard`(PR #119) 병목이 신호 유효성과 섞여 있었다는 점에서
     기존 SPPV-3를 그대로 이어갈 수는 없다. 재정의 목표·검증 축
     ...
     **[2026-08-04 재갱신] 검증 계획 초안 확장 완료**(`[DESIGN]
     signal_predictive_power_validation.md` §30, read-only, 새
     실측 없음) — 4개 검증 축을 질문/입력 데이터/집계 단위/비교
     기준/판정 기준/선행 전제 표로 구체화했다. 축 1(alpha 자체
     예측력)·축 3(downstream 분리 성과)은 **즉시 착수 가능**, 축
     2(정리된 gate 통과 후 성과)·축 4(funnel 전환 기여도)는 선행
     운영 관측(정리된 공식 기준 population 축적, PR #119 이후 BUY
     시도 누적)이 끝나야 시작 가능으로 분리했다. `§4.3~§4.5`의 새
     버전 뼈대(4.3′/4.4′/4.5′)와 `§7`의 새 기준선 필요성(§7′,
     ...
     **[2026-08-04 재갱신] 데이터 공백 해소 완료**(`[DESIGN] signal_
     predictive_power_validation.md` §33, read-only 실행, KIS 호출
     0건 확인) — 같은 3년 캐시·같은 함수로 `slow_momentum`을 `overall_
     score`/`slow_score`와 나란히 재계산(재현성 검증: 두 신호의 16개
     값 전부 §14.2와 정확히 일치). `slow_momentum`의 하락장(n=96)
     거동은 T+5 `t_NW=-0.63`/T+20 `t_NW=0.88` — 양 horizon 모두
     `\|t_NW\|<1`로 **무의미**(유의한 역방향도 유의한 양(+)도 아님),
     ...
     **[2026-08-06 재갱신] "판정 A(영향 미미)" 근본 원인 규명 +
     결론 강도 재조정**(`buy_path_variable_gate_matrix.md`
     §13.2.13~§13.2.14) — 3~4종목 뒤집힘의 원인은 측정 오류가 아니라
     유니버스 회전(§13.2.13, 판정 B)임을 확인했다. 후속으로 이
     종목들이 실제 주문까지 갔을 개연성을 counterfactual로 분석
     (§13.2.14)하다가, qualitative 근거로 썼던 `risk_opinion`/
     `evidence_strength`가 override 여부를 실제로 구분하지 못한다는
     것을 재검증으로 확인해 해당 해석을 철회했다. 결론을 "allocation
     제거가 주문까지 갔어야 했다는 뜻은 아니다(C)"와 "그렇다고
     downstream 차단이 과하지 않다고 단정할 수도 없다(D)"로 분리
     했다 — 어느 쪽도 확정하지 않는 것이 현재 증거 수준에 맞는
     ...
     지연 사유는 아니다(§40). §2.51에서 §2.50의 두 결론 문구를
     신규 실행 없이 재점검해 정밀화했다 — "구조적 패턴으로 격상"은
     동일 in-sample 3년 캐시 재확인 + 60일 trailing window
     자기상관 때문에 과장이며 정확히는 "강한 구조적 정합 증거로
     격상"이 맞고, "주된 차단 요인은 §21 게이트 하나뿐"은 "SPPV-3
     착수 검토를 시작할 수 있는 유일한 외생적 조건"이라는 뜻이지
     "진입 전체의 유일한 남은 조건"이 아니다(§38의 ①②③ 분류
     불변) — 두 정정 모두 R3b 방향성·Conditional Go는 바꾸지
     않는다(§40.6). §2.52에서 §38의 보조 잔여 조건 중 "T+5 구조적
     리스크"를 실제 운영 함수 `_build_exit_score`(순수 함수)로
     point-in-time 재호출해 would_buy candidate 1151건의
     signal-driven 청산 타이밍을 시뮬레이션한 결과, 91.1%가
     20거래일 안에 매도 신호를 넘지 않고 평균 보유일수=19.35일 —
     실제 청산은 T+5가 아니라 T+20 근방(6.14%, t=4.73)에서 발생해
     "T+5 평균이 약하다"는 우려가 실제 운영 리스크로 그대로
     전이되지 않는다. 20일 초과 구간·경로 리스크(MAE)는 미검증이라
     "부분 완화"이지 "완전 해소"는 아니다(§41). §2.53에서 관찰 창을
     20→60거래일로 확장해 would_buy 1048건을 재시뮬레이션한 결과,
     censored 비율 91.1%→51.3%, 평균 보유일수=48.0일, signal-driven
     청산 수익률(9.29%, t=5.38)이 오히려 T+20(4.46%, t=3.41)보다
     강해 "T+5 구조적 리스크"는 "부분 완화"에서 "거의 해소"로
     격상됐다 — 그러나 MAE 평균 -11.08%·심각 손실(-20% 이하) 12.8%·
     손절 임계값 부재가 새로 드러나 §38 보조 잔여 조건에 "경로
     리스크·손절 정책 부재"를 신규 추가했다(§42). §2.54에서 이
     "censored 91.1%→51.3%" 비교와 "거의 해소" 판정을 코드 대조로
     재점검한 결과, 20일판·60일판 스캔 범위가 서로 달라(60일판이
     20일판의 약 91% 부분집합으로 추정) 동일 코호트의 순수 전/후
     비교가 아니었고, 60일 관찰 후에도 과반(51.3%)이 여전히
     censored라 "거의 해소"는 과장임을 확인 — 정확한 표현은 "부분
     완화"에서 "추가 완화"로 하향 정정했다. R3b 방향성·Conditional
     Go는 바꾸지 않는다(§43). §2.55에서 §42가 신규 추가한 "경로
     리스크(MAE)·손절 정책 부재"에 대해 -15%/-20% 손절선을 실제
     도입하면 총 기대수익이 개선되는지 직접 검증한 결과, 두 임계값
     모두 총 기대수익 proxy를 악화시켰다(baseline 9734.7→-15%
     손절 7024.1(약 27.8% 악화)→-20% 손절 9093.8(약 6.6% 악화)) —
     R3b candidate는 조정 구간을 버텨야 이후 회복분을 취하는
     구조이기 때문이다. "손절 정책 부재"는 "미검증 공백"에서
     "시험한 범위 내에서는 손절 미도입이 총 기대수익 관점에서
     근거 있는 선택"으로 재분류했다. R3b 방향성·Conditional Go는
     바꾸지 않는다(§44). §2.56에서 이 세션 내내 B 시나리오 계산에
     써온 수작업 재구현 `_non_alpha`가 실제 운영 함수 `_build_
     entry_score`와 정확히 일치하는지 3년 전체 후보 표본(58,493건)
     에서 전수 검증한 결과, 100.0% 완전 일치(불일치 0건)를 확인 —
     이 세션의 모든 B 시나리오 결과가 실제 운영 코드 동작을 정확히
