...
**[2026-08-06 KST 재갱신, EV 게이트 메커니즘 코드 추적]** EV 게이트(§6)의 하방/비용/슬리피지 값 자체는 임의 고정값이 아니라 종목별 실측 ATR·거래량 기반 공식으로 확인됐으나, 이번 대상 종목(`008930`/`051900`)의 ATR 성분이 상한에 포화돼 실질 통과에 필요한 `score_anchor`가 0.8~0.9대로 역산되는 반면 상류 candidate 기준선은 ≈0.65 — **상류/하류 기준선 간극**이 구조적으로 확인됐다. 확신 수준은 "의심이 커짐"(사후 성과 데이터 부재로 최상위 확신까지는 아님). 상세는 `buy_path_variable_gate_matrix.md` §18, `[BACKLOG] backlog.md` 해당 후속 항목.
