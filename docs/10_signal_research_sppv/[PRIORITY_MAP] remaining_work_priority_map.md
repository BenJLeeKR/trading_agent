...
**[2026-08-07 KST 후속, 장 종료 전 완화 후보 설계 초안]** 실측 재계산 없이(08-07 장 미마감) B축/C축 완화 후보를 작은 완화→큰 완화 순으로 설계만 정리했다. B축은 가장 작은 후보조차 "어느 guard가 실제 하향을 만드는지" 재분해가 선행 조건임을 확인했고, C축은 가장 작은 후보(near-miss soft handling)의 유일한 실증 근거(`001450`)가 이미 완화에 반하는 음(-) 사후 성과를 보였다는 역설적 발견을 명시했다. `held_position` 이후 churn-control 계층과 EV 게이트 외부 하드 게이트는 논의 대상에서 제외했다. 장 종료 후 5단계 체크리스트를 문서화했다. 상세는 §24.
