...
|---|---|---|---|---|
| `000720` | 13일 | 0.0~0.0546(대부분 **0.0**, 바닥권) | **58~74위**(하위 10~15%) | **0일** |
| `002790` | 7일 | 0.1161~0.2569(000720보다 뚜렷이 높음) | 14~31위(중위권) | 1일(07-16) |
| `009150` | **0일**(core 경로로 전혀 미등장) | — | **8~24위**(자주 상위 15~30%, 최고 8위) | **6일** |

- **저신호 유지 사례**: `000720`은 20일 중 13일 실제 core로 포함
  됐지만 `entry_score`가 대부분 `0.0`(사실상 최저치)이고 shadow
  순위는 하위 10~15%에 그친다 — 사전순 10위라는 이유만으로 신호가
  거의 없는데도 유지된다.
