...
방식의 threshold 영향을 계산했다 — 案1(`ranking_score`에서
표준항 제거, `entry_score`는 그대로 유지) vs 案2(`entry_score`
내부 보너스만 제거, `ranking_score`의 표준항은 그대로 유지).

| 모집단 | 기존 `0.48` 통과 | 案1 통과 | 案2 통과 | 기존 mean | 案1 mean | 案2 mean |
|---|---:|---:|---:|---:|---:|---:|
...
- `entry_score`의 `buy_candidate_threshold=0.65` 하드 게이트에
  대한 案2의 영향도 확인했다(신규) — 전체 이력(n=35,777) 기준
  ...
  경로 자체가 없다는 뜻은 아니다(案2는 여전히 `entry_score`라는
  더 많은 곳에서 쓰이는 값을 건드리는 구조).

**비교 종합**:

| 기준 | 案1(entry에 유지) | 案2(ranking에 유지) |
|---|---|---|
...
| `relative_activity`(案1) | **가능** — threshold 재설계 없이 단독 diff 가능 | threshold 통과율 변화가 14.8%→14.3%로 미미 |

### 120.4 다음 우선 작업(완화안 확정 아님, 후속 절차로만 제시)

1. **1순위**: `relative_activity` 案1(`ranking_score`에서
...
## §122. `relative_activity` 案1 적용 후 영향 확인 + 다음 설계 분기 확정(SPPV-2.134, 2026-07-29 KST)
...
2. `coverage_score` threshold 재설계는 그 자체로 파급력이 큰 변경
   ...
   0.4~1.8%로 확인됨)이므로, 직전 diff(`relative_activity` 案1)가
...
**결론적으로 다음 턴 권고**: `relative_activity` 案1 적용 이후 최소
...
1. **1순위(갱신)**: `relative_activity` 案1 적용 이후 최소 1~2 거래일
