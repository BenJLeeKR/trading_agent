...
## 85. R3b candidate pool 최하위 floor 완화안(B1/B2/B3) 정밀 재검증 (SPPV-2.97, 2026-07-21 KST)

### 85.1 목적

§84에서 C안(rank compression)이 최상위 후보까지 훼손하는 부작용이
확인됨에 따라, 이번 턴은 **floor 계열(B안)만 좁혀** 0.30/0.45/0.60
세 수준에서 최하위 복구 효과와 최상위 무손상 여부를 정밀 재검증
한다. §84와 동일한 look-behind 방법론(전일까지 snapshot만 사용)을
유지하고, 코드 수정/배포 없음, Full pytest 미실행.

### 85.2 거래일별(KST) 재구성 — A/B1(0.30)/B2(0.45)/B3(0.60)

**확인된 유효 거래일(look-behind, 신뢰도 높음): 07-20(n=2),
07-21(n=3).** 07-16(n=4)은 벤치마크 snapshot이 그날 처음 생겨
"전일" 데이터가 없으므로 **당일 snapshot 근사치(참고용, look-ahead
가능성 있음)**로만 별도 표기한다.

| 거래일 | 종목 | n | 실제 buy_candidate | 활동성게이트 차단 | percentile A | entry_score A | entry_score B1(0.30) | entry_score B2(0.45) | entry_score B3(0.60) |
|---|---|---|---|---|---|---|---|---|---|
| 07-20 | 000810 | 2 | **True** | 아니오 | 1.000 | 0.7856 | 0.7856 | 0.7856 | 0.7856 |
| 07-20 | 000660 | 2 | False | 아니오 | 0.000 | 0.0000 | 0.2400 | 0.3600 | 0.4800 |
| 07-21 | 001450 | 3 | False | **예(§82)** | 1.000 | 0.7800 | 0.7800 | 0.7800 | 0.7800 |
| 07-21 | 000660 | 3 | False | 아니오 | 0.500 | 0.3300 | 0.3300 | 0.3300 | 0.4100 |
| 07-21 | 000810 | 3 | False | 아니오 | 0.000 | 0.0000 | 0.2400 | 0.3600 | 0.4800 |
| *07-16(참고)* | *000810* | *4* | *False(활동성)* | *예* | *1.000* | *0.5773* | *0.5773* | *0.5773* | *0.5773* |
| *07-16(참고)* | *001450* | *4* | *False(활동성)* | *예* | *0.667* | *0.5375* | *0.5375* | *0.5375* | *0.5375* |
| *07-16(참고)* | *000660* | *4* | *False(활동성)* | *예* | *0.333* | *0.4544* | *0.4544* | *0.5477* | **0.6677(→buy)** |
| *07-16(참고)* | *001800* | *4* | *False(활동성)* | *예* | *0.000* | *0.3379* | *0.5779* | **0.6979(→buy)** | *0.8179(→buy)* |

**`entry_score` 회복과 최종 `buy_candidate` 회복은 분리해 표기**:
001450·000810(4일 모두 상위권)은 entry_score 자체가 이미 threshold
이상이거나 floor로 손대지 않아도 변화가 없음. 001450은 entry_score
로는 통과(0.78)해도 §82의 **활동성 게이트가 별도로 차단**하므로,
floor 조정은 이 종목의 최종 buy_candidate에 영향을 줄 수 없다
(하류 병목이 이미 별개 축임을 재확인).

### 85.3 최하위 종목 복구 효과 비교표

| floor | 07-20 000660(base 매우낮음) | 07-21 000810(base 매우낮음) | 07-16(참고) 000660 | 07-16(참고) 001800 |
|---|---|---|---|---|
| 0.30 | 0.0000→0.2400 (미회복) | 0.0000→0.2400 (미회복) | 0.4544→0.4544(무변화) | 0.3379→0.5779 (미회복) |
| 0.45 | 0.0000→0.3600 (미회복) | 0.0000→0.3600 (미회복) | 0.4544→0.5477 (미회복) | 0.3379→**0.6979(회복)** |
| 0.60 | 0.0000→0.4800 (미회복) | 0.0000→0.4800 (미회복) | 0.4544→**0.6677(회복)** | 0.3379→**0.8179(회복)** |

**`buy_candidate` 회복 건수**: **확인된 유효 거래일(07-20/07-21)
2건에서는 0.30/0.45/0.60 어느 floor에서도 회복 사례 없음.**
참고(07-16, 근사) 데이터에서만 0.45(1건), 0.60(2건) 회복이
관측됐다 — 단, 이 데이터는 look-ahead 가능성이 있어 신뢰도가
낮음을 다시 강조한다.

### 85.4 최상위 종목 무손상 여부

| 종목(거래일) | percentile A | A entry_score | B1/B2/B3 entry_score | buy_candidate 유지 여부 |
|---|---|---|---|---|
| 000810(07-20) | 1.000 | 0.7856 | **0.7856(불변)** | **유지(True 그대로)** |
| 001450(07-21) | 1.000 | 0.7800 | **0.7800(불변)** | **유지(True 판정 로직상 그대로, 단 활동성 게이트로 별도 차단)** |

**모든 floor(0.30/0.45/0.60)에서 최상위 후보의 entry_score와
buy_candidate 판정이 단 한 건도 훼손되지 않았다** — `max(raw,
floor)`는 raw가 이미 floor보다 크면 항상 raw를 그대로 반환하는
단조증가(monotonic) 연산이므로, **구조적으로 최상위 후보를 건드릴
수 없다**(§84의 C안 압축과 근본적으로 다른 성질, 코드 공식으로
확정). 중간 순위 000660(07-21, pct=0.5)도 floor 0.30/0.45에서는
전혀 영향받지 않았고 0.60에서만 미세하게(0.33→0.41) 변화 — 여전히
threshold와는 거리가 멂.

### 85.5 핵심 질문에 대한 답

1. **`0.30`은 너무 약했는가?** — 그렇다. 확인된 두 거래일 모두
   0.30에서 entry_score가 0.24에 그쳐 threshold(0.65)와는 거리가
   멀다. 참고(07-16) 데이터에서도 0.30에서는 회복 사례가 전혀 없다.
2. **`0.45` 또는 `0.60`에서 처음 회복이 생기는가?** — **확인된
   유효 거래일(07-20/07-21)에서는 0.60까지도 회복이 없다.**
   참고(근사) 데이터에서만 0.45(1건)·0.60(2건) 회복이 나타난다 —
   즉 "언제 회복이 시작되는가"는 **그날 그 종목의 base(alpha
   외 요소) 값에 크게 좌우되며, floor 수준만으로 일괄 예측할 수
   없다**(base가 이미 -0.8에 가까운 종목은 0.60으로도 회복 안 됨;
   base가 0.34~0.45 수준인 종목은 0.45~0.60에서 회복).
3. **그 복구가 실효성 근거가 되는가?** — **부분적으로만.** 근사
   데이터의 회복은 실제 관찰이 아니라 방법론적 caveat이 있는
   참고치이므로, 이것만으로 "0.45/0.60이 효과적"이라고 확정할 수
   없다. 다만 **최상위 훼손이 전혀 없다는 것은 확실한 근거**다.
4. **`0.60`이 과잉 완화 조짐을 보이는가?** — **경계선상의 조짐이
   있다.** 참고(07-16) 데이터에서 0.60은 pool 4종목 중 **최하위
   (percentile=0.0, pool 내부 꼴찌)** 종목(001800)까지 자동으로
   buy_candidate 자격을 회복시킨다 — "상위 20%로 선별된 후보 중
   에서도 꼴찌"에게 자동 진입 자격을 주는 것은 candidate pool의
   선별 취지 자체를 상당 부분 무력화할 위험이 있다(다만 확인된
   유효 거래일에서는 0.60도 회복을 만들지 못했으므로, 이 우려는
   아직 근사 데이터 1건에 근거한 조짐 수준이다).
5. **다음 단계는?** — 아래 §85.7 참고.

### 85.6 판정: **Watch**

C안(§84)처럼 최상위를 훼손하는 명백한 부작용은 **없음**(구조적으로
불가능함을 공식으로 확정) — 이 점에서 §84의 No-Go보다는 근거가
낫다. 그러나 **확인된 유효 거래일(07-20/07-21) 2일 모두, 0.30/
0.45/0.60 어느 floor로도 buy_candidate 회복 사례가 전혀 없었다** —
근사 데이터에서만 나타난 회복을 실효성 근거로 삼기엔 이르다.
따라서 **Conditional Go로 승격하기엔 근거가 아직 얇고, No-Go로
단정하기엔 "최상위 무손상"이라는 확실한 장점이 있다** — Watch가
정확하다.

### 85.7 다음 우선 작업

1. **완화안 diff 초안 검토는 아직 착수하지 않는다** — 확인된
   유효 거래일에서 회복 근거가 나오지 않았기 때문.
2. **표본 축적을 최우선으로 한다**: 향후 거래일이 쌓여 look-behind
   방식으로 신뢹도 높게 재구성 가능한 날이 늘어나면, 같은 A/B1/B2/
   B3 비교를 반복해 이번 턴의 "Watch" 판정을 재확인/보강한다.
3. 만약 향후 표본에서 **0.45 수준에서 안정적인 회복이 반복 확인
   되고 0.60의 과잉 완화 조짐이 재확인**되면, 다음 단계는 **가장
   보수적인 floor(0.45)만 좁혀 diff 초안 검토**로 이어갈 것을
...
## 88. 층2(eligibility) 병목 세분화 재검증 — 활동성 게이트 vs negative_overall_floor (SPPV-2.100, 2026-07-24 KST)

### 88.1 목적

§87에서 확인한 "현재 가장 직접적인 병목은 층2(eligibility)"를
더 세분화해, **000810/001450을 막는 활동성 게이트**와 **000660을
막는 negative_overall_floor**를 분리 재검증한다. floor=0.60/R3b/
near-miss override/AI downgrade/EV gate는 이번 턴의 직접 수정
대상이 아니며 참고만 한다. 코드 수정 없음, Full pytest 미실행,
신규 KIS 호출 0건(read-only DB/코드 조회만).

### 88.2 관찰 창(KST)

**2026-07-22 09:27 KST(floor=0.60 반영 시점) ~ 2026-07-24 15:27
KST(조회 시각)** — 약 2.25거래일(트레이딩 세션 기준 07-22/23/24
3거래일에 걸침).

### 88.3 종목별 상세표(000810 / 001450 / 000660)

| 종목 | n | `trigger_r3b_alpha_percentile` | entry_score(고유값) | `buy_candidate=true` | 마지막 `eligibility_reasons` | `final_intent` | `decision_type` | `evg.passed` |
|---|---|---|---|---|---|---|---|---|
| 000810 | 191 | 191(100%) | 0.38 / 0.46 / 0.5133 / **0.78** | 0 | `eligibility_low_relative_activity`(191/191, 100%) | no_action 44 / watch 147 | hold 44 / watch 147 | True 188, False 3 |
| 001450 | 191 | 191(100%) | 0.46 / **0.78** | 0 | `eligibility_low_relative_activity`(191/191, 100%) | watch 172 / no_action 19 | watch 172 / hold 19 | True 185, False 6 |
| 000660 | 188 | 188(100%) | 0.0 / 0.41 | 0 | `eligibility_negative_overall_floor`(188/188, 100%) | watch 144 / no_action 44 | watch 144 / hold 44 | True 184, False 4 |

**핵심 발견**: 000810·001450은 entry_score가 **0.78까지 도달**하는데도
(buy_candidate_threshold 0.65 이상) 100% 활동성 게이트로 차단된다
— "점수는 충분하지만 eligibility가 막는" 명확한 사례다. 반면
000660은 entry_score가 **최대 0.41**로 애초에 threshold(0.65)에
도달한 적이 없다 — "점수 자체도 약하고 eligibility도 막는" 사례로,
이 종목은 negative_overall_floor를 없애도 오늘 시점에는 어차피
buy_candidate가 되지 못한다.

### 88.4 eligibility 축별 분리 집계

| 축 | 발생 건수(관찰 창) | 집중 종목 | `entry_score>=0.65`인데도 막힌 사례 | `entry_score<0.65`라 애초에 약한 사례 |
|---|---|---|---|---|
| `eligibility_low_relative_activity` | 382 (000810 191 + 001450 191) | 000810, 001450 | **382건 전부**(두 종목 모두 0.78 도달 이력 있음) | 0건 |
| `eligibility_negative_overall_floor` | 188 (000660) | 000660 단일 종목 | 0건 | **188건 전부**(entry_score 최대 0.41) |

**"점수는 충분하지만 eligibility가 막는다" vs "점수도 낮고
eligibility도 막는다"의 분리**: 활동성 게이트(000810/001450)는
전량 전자, negative_overall_floor(000660)는 전량 후자에 해당한다.

### 88.5 활동성 게이트(000810/001450) 정밀 확인 — 임계값 1.10 대비 추세

| 종목 | 07-20 | 07-21 | 07-22 | 07-23 |
|---|---|---|---|---|
| 000810 volume/turnover surge | 1.33/1.28(**통과**) | 0.69/0.66 | 0.58/0.57 | 0.57/0.57 |
| 001450 volume/turnover surge | 0.89/0.89(근소부족) | 0.62/0.63 | 0.79/0.81 | 0.72/0.75 |

**해석**: 000810은 07-20에는 실제로 임계값(1.10)을 넘었다가
07-21 이후 0.57~0.69 수준으로 **뚜렷하게(임계값 대비 약 40~50%
가까이) 떨어져** 있다 — 최근(07-22/23)은 근소 미달이 아니라
**명확한 미달**에 가깝다. 001450도 0.62~0.89 범위로 임계값에
근접했던 07-20(0.89)을 제외하면 대체로 **뚜렷한 미달**이다. 두
종목 모두 `average_volume_20d`/`average_turnover_20d`가 같은
기간 계속 완만히 감소 추세(§82에서 이미 확인한 추세의 연장) —
**"아슬아슬한 근소 미달"이라기보다 "실제 거래 활성도가 최근
며칠간 뚜렷이 낮아진 상태"에 가깝다.**

### 88.6 negative_overall_floor(000660) 정밀 확인

- 코드 확인(read-only): `if overall is not None and overall < -0.10:
  reasons.append("eligibility_negative_overall_floor")` — 임계값
  **-0.10**, `signal_feature_snapshot.overall_score`(R3b/entry_
  score와 무관한 원시 신호값)를 직접 참조하는 **완전히 별개의
  독립 축**(활동성 게이트와 무관, 코드로 확정).
- 관찰 창 내내 000660의 `overall_score=-0.1445`, `slow_score=
  0.22`, `fast_score=-0.59`로 **07-20~07-23 나흘간 완전히 동일값
  고정** — 하루 1회 배치가 계속 돌아도 이 종목의 신호 구성 요소
  자체가 변하지 않고 있다(사실). 부족분은 `-0.1445 -(-0.10) =
  -0.045`로 임계값과 근소한 차이이나(추정: near-miss 성격),
  **entry_score(alpha 항)가 이미 threshold 미달(최대 0.41)이므로
  이 축을 완화해도 오늘 시점 buy_candidate 회복에 직접 기여하지
  않는다**(사실: entry_score와 이 eligibility 체크는 서로 다른
  값을 참조하는 독립 로직).
- 관찰 창 내 다른 종목에서 이 축이 반복되는지: 이번 턴 조회 범위
  (3종목 한정)에서는 000660 외 반복 관측 없음 — 다른 종목까지
  포함한 전체 유니버스 재검증은 이번 턴 범위 밖.

### 88.7 층2 내부 우선순위 판정: **A(활동성 게이트)가 더 직접적인 현재 병목**

- **차단 건수/종목 수**: 활동성 게이트 382건·2종목 vs negative_
  overall_floor 188건·1종목 — 활동성 게이트가 더 넓다.
- **entry_score가 충분한데도 막히는 정도**: 활동성 게이트는
  **100%가 "충분한 점수(최대 0.78)인데 막힘"** 사례인 반면,
  negative_overall_floor는 **100%가 "점수 자체도 약함(최대 0.41)"
  사례** — 이 축을 완화해도 실제 BUY 회복으로 이어질 가능성이
  이번 관찰 창에서는 없다.
- **현재 BUY 후보 회복을 막는 직접성**: 활동성 게이트가 명백히
  더 직접적이다 — 이 축만 완화되면(가정) 000810/001450은 즉시
  entry_score 조건을 만족하는 buy_candidate가 될 수 있는 상태다.

### 88.8 각 축의 완화 가능성 판정

- **활동성 게이트(`eligibility_low_relative_activity`)**: **Watch**.
  entry_score가 충분한데 막히는 직접성은 확인되나(382건), 최근
  며칠(07-21~23) 추세를 보면 임계값 대비 근소 미달이 아니라 **뚜렷한
  미달**(0.57~0.89 vs 1.10)이고, 실제 20일 평균 거래량/거래대금도
  꾸준히 감소 중이다 — "정당한 차단"에 가까운 근거가 오히려
  누적되고 있어, 이번 턴 증거만으로 완화 검토 후보로 올릴 근거는
  부족하다.
- **신호 바닥 게이트(`eligibility_negative_overall_floor`)**:
  **Watch**. 임계값과의 격차(-0.045)는 근소하나, entry_score 자체가
  이미 threshold 미달이라 이 축을 완화해도 실제 BUY 회복에 기여하지
  않는다 — 완화를 검토할 실익이 현재로서는 확인되지 않는다.

**두 축 모두 "완화 검토 후보"로 올릴 근거는 아직 없다** — 둘 다
Watch로 유지한다.

### 88.9 다음 우선 작업

**다음 턴 제안**: 활동성 게이트(A)가 층2 내부에서 더 직접적인
병목임이 확정됐으므로, 다음 턴은 **활동성 게이트 하나만 더 깊게
파는 것**을 제안한다 — 구체적으로 000810/001450의
`average_volume_20d`/`average_turnover_20d` 자체가 왜 지속적으로
감소하는지(실제 시장 유동성 축소인지, 관측/계산 방식의 문제인지)
를 좁혀서 확인하는 것이 다음 최우선이다. negative_overall_floor
(000660)는 entry_score 자체가 약해 실익이 낮으므로 이번에는
후순위로 미룬다.


---

## 89. 층2(eligibility) 국면별 층화 재검증 — bullish_trend 포함 (SPPV-2.101, 2026-07-24 KST)

### 89.1 목적

§88의 활동성 게이트/negative_overall_floor 판정이 **국면(regime)
별로도 유지되는지**를 층화 검증한다. 특히 "상승장(bullish_trend)
에서도 이 게이트들이 여전히 정당한 차단인지"를 확인한다. 완화
적용이 아니라 검증 턴이며, floor=0.60/EV gate/AI downgrade는
참고만 하고 직접 수정하지 않는다. 코드 수정 없음, Full pytest
미실행, 신규 KIS 호출 0건(read-only DB 조회만).

### 89.2 관찰 창(KST)

**2026-07-22 09:27 KST(floor=0.60 반영 시점) ~ 2026-07-24 20:15
KST(조회 시각)** — §88과 동일 창(약 2.75거래일)을 그대로 사용했다
(표본 확장 불필요 — 이미 종목당 188~191건 확보).

**국면 구분 기준**: `deterministic_trigger.metadata.regime_label`
(종목별·사이클별 개별 국면 판정값 — R3b candidate pool 선정에
쓰이는 벤치마크 공통 국면(`market_common_label`)과는 별개의,
각 종목 자신의 signal_feature_snapshot 기반 국면 분류).

### 89.3 국면별 집계표

| 국면 | 관련 종목 | `trigger_r3b_alpha_percentile` | `buy_candidate=true` | 활동성 게이트 차단 | negative_overall_floor 차단 | entry_score≥0.65인데 eligibility로 막힘 |
|---|---|---|---|---|---|---|
| `bullish_trend` | 000810, 001450 | 382 | 0 | **382(100%)** | 0 | **191** |
| `range_bound` | 000660 | 188 | 0 | 0 | **188(100%)** | 0 |
| `bearish_trend` | — | — | — | — | — | 표본 없음(관찰 불가) |

**중요한 방법론적 한계(정직하게 명시)**: 관찰 창 전체 동안
000810/001450은 **단 한 번도 bullish_trend 외의 국면 라벨을
받은 적이 없고**, 000660도 **단 한 번도 range_bound 외의 라벨을
받은 적이 없다.** 즉 이번 창에서는 "국면"과 "종목"이 완전히
교락(confound)돼 있다 — 활동성 게이트가 "상승장에서 발생"하는
것인지 "이 두 종목 고유의 특성상 발생"하는 것인지, 이번 관찰만으로는
완전히 분리할 수 없다. 이 한계를 전제로 아래 결과를 해석해야 한다.

### 89.4 종목×국면별 상세표

| 종목×국면 | n | entry_score 고유값 | volume_surge_ratio 범위 | turnover_surge_ratio 범위 | eligibility 마지막 사유 |
|---|---|---|---|---|---|
| 000810×bullish_trend | 191 | 0.38/0.46/0.51/**0.78** | 0.571~0.691 | 0.569~0.656 | `eligibility_low_relative_activity`(100%) |
| 001450×bullish_trend | 191 | 0.46/**0.78** | 0.620~0.790 | 0.629~0.809 | `eligibility_low_relative_activity`(100%) |
| 000660×range_bound | 188 | 0.0/0.41 | 0.653~0.742 | 0.554~0.628 | `eligibility_negative_overall_floor`(100%) |

### 89.5 활동성 게이트의 bullish_trend 검증 결과

- **Q1(상승장에서도 반복적으로 막는가)**: **그렇다** — 관찰된
  bullish_trend 표본(382건) 전량이 활동성 게이트로 차단됐고, 그중
  191건(000810/001450 각 전량)은 entry_score가 0.65 이상이었다.
- **Q2(근소 미달 vs 명확한 미달)**: **명확한 미달이다.** bullish_
  trend 구간의 volume/turnover surge ratio가 000810 0.571~0.691,
  001450 0.620~0.790으로, 임계값(1.10) 대비 각각 약 37~48%,
  28~44% 낮다 — 임계값에 근접한 근소 미달이 아니라 뚜렷하게 낮은
  수준이다.
- **"상승장인데도 실제로 거래 활력이 부족해서 막힌 것인가, 상승장
  이라 너무 보수적으로 차단한 것인가"**: 수치상으로는 **전자(실제
  거래 활력 부족)에 가깝다** — 시장 전체 국면이 상승 추세로
  분류되더라도, 이 두 종목 개별의 상대 거래량/거래대금은 그와
  무관하게 낮은 상태를 유지하고 있다(§82/§88에서 이미 확인한 20일
  평균 거래량/거래대금 감소 추세와 일치). 상승장이라는 이유만으로
  차단이 부당해진다는 근거는 이번 표본에서 확인되지 않았다.

### 89.6 negative_overall_floor의 국면 의존성 검증 결과

- **Q3(bullish_trend에서도 나타나는가)**: **이번 관찰 창에서는
  bullish_trend 표본 자체가 0건이라 확인 불가.** 000660은 관찰
  창 내내 range_bound로만 분류됐다 — "bullish_trend에서는 나타나지
  않는다"고 단정할 수 없고, 정확히는 **"이번 표본에서는 range_bound
  구간(000660)에서만 관찰됐다"**는 사실만 확정할 수 있다.
- `overall_score < -0.10` 임계값 자체는 국면 라벨과 무관하게 원시
  신호값(`signal_feature_snapshot.overall_score`)만 참조하는
  독립 로직이므로(§88.6에서 코드로 확인), 논리적으로는 bullish_
  trend 종목에서도 발생할 수 있는 구조다 — 다만 실제로 그런 사례가
  관찰되지 않았을 뿐이다.

### 89.7 국면별 판정

- **활동성 게이트(`eligibility_low_relative_activity`)**
  - bullish_trend 기준: **Watch**(유지) — 상승장 표본을 포함해도
    완화 검토 후보로 올릴 근거가 새로 생기지 않았다. 오히려 명확한
    미달(37~48%↓)이 반복 확인돼 정당 차단 쪽 근거가 쌓였다.
  - range_bound 기준: **확인 불가**(이번 창에서 000810/001450의
    range_bound 표본 없음 — 참고: §78/§82 이전 시점(07-16~07-20
    무렵, floor 반영 이전)에는 000810/001450이 range_bound였을
    때도 활동성 게이트 인접 근소부족 사례가 있었으나, 이는 이번
    관찰 창 밖의 과거 데이터로 직접 비교 대상이 아니다).
- **negative_overall_floor**
  - bullish_trend 기준: **확인 불가**(표본 없음 — "없다"와 "확인
    안 됐다"를 구분해 후자로만 기술).
  - range_bound 기준: **Watch**(유지) — entry_score 자체가
    threshold 미달이라 완화 실익이 낮다는 §88 판정 그대로 유지.

### 89.8 전체 결론

- **전체 Watch 판정은 유지된다.** 상승장(bullish_trend) 표본을
  포함해 재검증해도, 활동성 게이트를 완화 검토 후보로 올릴 근거는
  나타나지 않았다 — 오히려 명확한 미달 폭이 재확인됐다.
- **negative_overall_floor의 국면 의존성은 이번 턴에서 완전히
  닫히지 않았다** — bullish_trend 표본이 없어 "range_bound 전용
  축"이라는 가설은 아직 **반증되지 않은 상태(미확정)**로 남는다.
  향후 000660(또는 다른 종목)이 bullish_trend로 전환되는 시점의
  관찰이 필요하다.
- **"활동성 게이트가 상승장에서는 무의미하다"거나 "반드시 완화해야
  한다"는 결론은 내리지 않는다** — 수치가 오히려 반대 방향(정당
  차단 쪽)을 가리키고 있다.

### 89.9 다음 우선 작업

**가장 애매한 축 1개를 좁혀 다음 턴에 심층 검토할 것을 제안**:
**negative_overall_floor의 국면 의존성**(§89.7에서 "확인 불가"로
남은 부분)이 가장 애매하다 — 000660이 향후 bullish_trend/다른
국면으로 전환될 때 이 축이 여전히 발생하는지, 혹은 다른 bullish_
trend 종목에서 이 축이 발생한 이력이 있는지(과거 데이터 소급
조회)를 다음 턴 최우선으로 제안한다. 활동성 게이트는 이번 턴으로
"국면 무관하게 정당 차단"에 가깝다는 근거가 충분히 쌓였다고
판단해, 다음 턴의 우선순위에서는 후순위로 내린다.


---

## 90. 근본 설계 재검토 — "창 vs 방패" 전략 전환에 따른 우선순위 재정렬 (SPPV-2.102, 2026-07-24 KST)

### 90.1 배경

수십 턴에 걸친 §70~§89의 실측에도 불구하고 **주문(order_request)이
여전히 0건**인 상태가 지속되고 있다는 사용자 문제의식에 따라,
"탄탄한 방패"보다 "예리한 창"을 우선하는 전략 방향(감내 가능한
손실을 허용하고 기대수익률을 높이는 방향)에 맞춰 다음 우선순위를
재정렬한다. 사용자 결정: **core universe 확장 검토(비중 높음) +
eligibility 완화 검토(병행, 비중 낮음)**를 함께 진행. 이번 턴은
코드 수정 없이 **설계 재검토 + 실행 가능한 레버 식별**에 집중했다.

### 90.2 핵심 재확인 — 지금 막고 있는 것은 "방패 과잉"인가?

§88~§89에서 이미 확인된 사실: 000810/001450을 막는 활동성
게이트는 임계값(1.10) 대비 **37~48% 낮은 명확한 미달**이며, 실제
20일 평균 거래량/거래대금이 2주 넘게 감소하는 추세와 일치한다 —
"애매한 경계를 과도하게 차단"하는 상태가 아니라 **"창이 겨우 골라낸
후보가 하필 유동성이 식어가는 종목"**인 상태에 가깝다. 이 판단은
그대로 유지한다(§88~89, 판정 변경 없음).

### 90.3 실행 가능한 레버 식별 — core universe 확장 (사용자 우선순위 1)

**코드 read-only 확인 결과, core universe 크기는 이미 config로
제어 가능한 레버였다(§83에서 예상했던 "설계 재검토"가 실제로는
매우 좁은 config 변경으로 가능함을 확인)**:

- `scripts/run_decision_loop.py`의 `DEFAULT_TRADING_UNIVERSE_
  CORE_CAP = 12`(라인 306), env 변수 `TRADING_UNIVERSE_CORE_CAP`
  로 오버라이드 가능(라인 310, 629) — **현재 `.env`/`docker-
  compose.yml` 어디에도 설정돼 있지 않아 기본값 12가 그대로
  적용 중**임을 확인(사실, grep으로 직접 확인).
- `UniverseSelectionService.compose()`(`services/universe_
  selection.py`)가 Core/Held/Event/Market 4개 소스를 합성하고,
  `core_cap`이 그중 **CORE 소스 종목 수만 제한**한다(라인 1750
  ~1758) — Held/Event 소스는 이 cap과 무관하게 별도 처리된다.
  실제 관찰된 target_count 12~23의 변동은 이 core_cap(12) 위에
  Held/Event 소스가 추가되며 발생하는 것으로 설명된다.
- **상한선 확인**: 활성 KRX 종목 전체는 3,908개(DB 직접 조회)이나,
  실제로 R3b가 쓸 수 있는 유효 신호(`signal_feature_snapshot`)를
  가진 종목은 일 배치 기준 약 **80개뿐**(`signal_feature_after_
  market` freeze의 `target_count=80`, §83에서 이미 확인) — 이것이
  실질적인 상한선이다.
- **결론(설계 재검토)**: `TRADING_UNIVERSE_CORE_CAP`을 12에서
  (예) 40~60 수준으로 올리면, R3b candidate pool의 20% quintile
  분모가 지금의 12~23에서 최대 80에 근접하게 커질 수 있다 — §83의
  "quintile 비율이 아니라 모수(core universe 규모) 자체가 문제"
  라는 결론과 정확히 일치하는 지점이며, **코드 diff 없이 env 값
  하나만 바꾸면 되는 레버**다(단, 이번 턴은 실제 값 변경을 하지
  않았다 — 다음 턴 설계 검토/shadow 대상으로 남긴다).

### 90.4 eligibility 완화 레버(사용자 우선순위 2, 병행) — 개념 정리만

이번 턴에서는 코드 변경/shadow 재실행을 하지 않고, §88~89의 판정을
전제로 **개념적 옵션만** 정리한다(다음 턴 실측 대상):
- 활동성 게이트 임계값(현재 1.10)을 낮추는 안 — 단, §89에서
  "정당 차단에 가까운 근거"가 쌓인 상태이므로, 단순 임계값 인하는
  유동성 리스크를 그대로 떠안는 결과가 될 수 있다는 점을 문서에
  명시해 둔다.
- entry_score가 일정 수준(예: 0.70) 이상인 경우에 한해 활동성
  게이트를 조건부로 완화하는 안(§77~79의 EV gate near-miss
  override와 유사한 "좁은 예외" 패턴) — 코드 구조상 재사용 가능한
  패턴이 이미 존재한다(참고용 설계 아이디어, 이번 턴 실측 없음).

### 90.5 전략적 해석 — "예리한 창" 전환과의 정합성

- **core universe 확장**은 "창을 더 예리하게 만드는" 방향에 정확히
  부합한다 — 방패(eligibility/EV gate)를 건드리지 않고도, 창이
  겨눌 수 있는 과녁 자체를 늘려 애초에 "유동성이 괜찮으면서
  모멘텀도 좋은" 종목이 candidate pool에 들어올 확률을 높인다.
  **리스크 프로파일을 바꾸지 않는 레버**라는 점에서 우선순위가
  가장 높다는 사용자 판단과 일치한다.
- **eligibility 완화**는 "감내 가능한 손실을 허용"하는 방향에
  직접 해당하지만, §88~89의 실측(명확한 미달, 실제 유동성 감소
  추세)을 고려하면 **이는 "예측이 틀렸을 때의 손실"이 아니라
  "유동성 부족으로 인한 실행 리스크"를 감내하겠다는 것**과 같다 —
  두 리스크는 성격이 다르므로, 완화를 진행하더라도 그 구분을
  명시적으로 인지한 상태에서 좁게(예: entry_score 조건부) 진행하는
  것을 권고한다.

### 90.6 다음 우선 작업

1. **(최우선)** `TRADING_UNIVERSE_CORE_CAP` 확장의 실제 효과를
   shadow로 검증 — 코드 변경 없이 §83과 동일한 방법론(`build_
   candidate_percentiles()` 재호출)으로 core_cap을 12→40 등으로
   가정했을 때 candidate pool 크기/구성이 어떻게 달라지는지
   재구성(다음 턴).
2. **(병행, 비중 낮음)** eligibility 조건부 완화 설계 초안(entry_
   score 조건부 활동성 게이트 완화 등)을 shadow로 검증 — §85~86의
   near-miss override 패턴을 참고해 좁은 범위로 설계.
...
## 92. §91 판정 보정 — "Watch/Conditional Go" 라벨 과속 정정, 우선순위 중심 재정리 (SPPV-2.104, 2026-07-24 KST)

### 92.1 목적

§91의 판정("core_cap 확장=Watch", "eligibility 조건부 완화=
Conditional Go 후보")이 **하루치·단일 시점 표본 기준으로는 과속
판정**이었음을 사용자 지적에 따라 보정한다. §91의 실측 수치·표
자체는 정정하지 않는다(이력 보존) — **해석과 라벨만 보정**한다.
코드 변경 없음, 신규 KIS 호출 없음, 신규 조회 없음(이번 턴은 순수
문서 해석 보정).

### 92.2 [§91에서 정정] 정확히 무엇이 과속이었는가

- **"core_cap 확장=Watch"는 과도하게 소극적인 라벨이었다.** §91은
  "추적 3종목의 buy_candidate 회복 0건"을 근거로 Watch를 매겼으나,
  이는 **core_cap 확장을 "기존 3종목을 구제하는 레버"로 잘못
  프레이밍**한 결과다. §91 자체가 이미 발견한 사실 — core_cap=60
  에서 `009150`이 percentile 0.818로 신규 출현 — 이 바로 core_cap
  확장의 **진짜 의미(상류 모집단 확대)**이며, 이 관점에서는 Watch
  보다 우선순위를 높게 둬야 한다.
- **"eligibility 조건부 완화=Conditional Go 후보로 격상 가능"은
  과속 판정이었다.** 하루치·단일 종목(001450) 1건의 flip만으로
  "Go 방향"을 시사하는 라벨을 쓴 것은, §88~89에서 이미 "정당 차단에
  가까움"으로 판정한 게이트에 대한 예외를 성급하게 정당화하는
  방향으로 읽힐 수 있다.

### 92.3 보정된 해석 — 우선순위 중심 재정리

라벨(`Watch`/`Conditional Go`)보다 **실행 우선순위와 실반영 후보
성격**으로 재정리한다.

**1순위: `TRADING_UNIVERSE_CORE_CAP` 실제 확장 반영 및 관찰**
- **재정의**: §91의 결과는 "core_cap 확장이 무효"가 아니라,
  **"추적 중이던 3종목 기준 직접 회복은 없었지만, 그 대신 신규
  후보(`009150` 등)가 실제로 출현한다"**는 뜻으로 다시 읽어야
  한다. core_cap 확장은 **기존 3종목 구제 레버가 아니라 상류
  모집단 자체를 넓히는 레버**이며, 이 성격상 "0건 탈출"이라는
  목표에 대해 **더 높은 우선순위**를 가진다.
- 코드 diff 없이 env/config 레벨(`TRADING_UNIVERSE_CORE_CAP`)에서
  가능한 레버라는 점도 우선순위를 높이는 근거다(§90에서 이미 확인).
- 성격: **실반영 우선 후보** — 다만 하루치 shadow만으로 규모(40 vs
  60 등 구체적 값)를 확정하지 않고, 실제 반영 후 1~2거래일 관찰이
  필요하다.

**2순위: `eligibility_low_relative_activity` 조건부 완화 — 병행 검토**
- **재정의**: `entry_score>=0.70` 조건부 활동성 게이트 예외가
  001450 1건을 flip시킨 것은 사실이나, 이는 **하루치·단일 종목·
  이미 "정당 차단에 가깝다"고 판정한 게이트에 대한 예외**라는
  세 가지 제약이 겹친 결과다 — 이 상태에서 "Go" 방향 라벨을 쓰는
  것은 과속이다.
- 성격: **제한적 하류 직접 레버(병행 실반영 후보)** — 전면 완화는
  여전히 금지, 좁은 조건부 예외 형태만 후보로 유지한다. 1순위
  (core_cap 확장)보다 후순위로 병행 검토한다.

**공통 원칙**: 두 레버 모두 **"실반영 후 1~2거래일 관찰 필요"**
상태로 둔다. 하루치 결과만으로 `Watch`/`Go` 성격의 확정 판정을
내리지 않는다.

### 92.4 보정 요약표

| 레버 | §91의 라벨(정정 대상) | §92 보정 후 표현 | 우선순위 |
|---|---|---|---|
| core universe 확장(`TRADING_UNIVERSE_CORE_CAP`) | Watch | **실반영 우선 후보**(상류 모집단 확대 레버, 신규 후보 출현 이미 확인됨) | **1순위** |
| eligibility 조건부 완화(`entry_score>=0.70`) | Conditional Go 후보로 격상 가능 | **제한적 하류 직접 레버(병행 실반영 후보)**, 전면 완화 아님 | 2순위(병행) |

### 92.5 다음 우선 작업(재정렬)

1. **(1순위)** `TRADING_UNIVERSE_CORE_CAP` 실제 확장 반영(env/config
   변경, 코드 diff 없음) — 반영 후 1~2거래일 관찰로 신규 후보
   출현 여부·funnel 전진 여부 확인.
2. **(2순위, 병행)** `eligibility_low_relative_activity` 조건부
   완화(`entry_score>=0.70` 등 좁은 예외)의 코드 diff 초안 설계
...
**[SPPV-2.109에서 정정] 아래 문단의 "실제 프로덕션 호출부 2곳 모두
인자 없이 호출"이라는 서술은 부정확했다 — 정정된 서술은 §96.6 참고.
(원문 보존)**
...
**[SPPV-2.109에서 정정]** 아래 표의 `risk_off`/`volatility` 55건
(100%)은 `reason_codes`와 `risk_reason_codes`를 합쳐 카운트한
수치였다. `reason_codes` 필드 단독 기준으로 재검증한 결과 55건 중
**54건**에서만 함께 관측되었다(1건은 `reason_codes`에
`watch_weak_evidence`/`risk_opinion_reduce`/`compliance_review`/
`defensive_strategy_preferred`만 존재 — 창 07-27 04:01:08 UTC).
아래 원표는 보존하고, 정정된 수치는 §96.6에 별도 기록한다.
...
2. **2순위**: `001450` 층3 관찰 지속 — 특히 (a) 같은 기간 시장
   ...
   축이 다음 거래일에도 사실상 공통 축으로 유지되는지 재관찰.

### 96.6 서술 정밀도 보정(SPPV-2.109, 2026-07-27 KST, 코드 수정 없음)

**전제**: 이 절은 §96.1~§96.5의 결론을 새로 만드는 것이 아니라,
숫자/서술의 강도를 재검증 결과에 맞게 정확히 맞추는 정정이다(이력
보존 — 원문은 위에 그대로 둔다).

**정정 1 — 001450 층3 공통축 (Part B)**

재검증(read-only, `reason_codes` 필드 단독 기준 재집계, 신규 KIS
호출 0건): `risk_off`+`volatility` 조합은 55건 중 **54건**에서
함께 관측된다(1건은 미포함 — 창 2026-07-27 04:01:08 UTC 기록의
`reason_codes`는 `watch_weak_evidence`/`risk_opinion_reduce`/
`compliance_review`/`defensive_strategy_preferred`만 포함하며
`risk_off`/`volatility` 계열 키워드가 없다. 단, 해당 레코드의
`risk_reason_codes` 필드에는 관련 키워드가 존재해 §96.2 원표의
합산 집계에서는 55/55로 잡혔던 것이다).

- **정정 전**: "risk_off+volatility 조합이 55건 전수(100%)의
  공통 축"
- **정정 후**: "risk_off+volatility 조합이 55건 중 54건에서
  함께 관측된 **사실상 공통 축**"

"전수"/"100%"라는 표현은 숫자가 정확히 닫히지 않는 한 사용하지
않는다.

**정정 2 — `max_cap=30` 호출 경로 (Part A)**

재검증(read-only grep): `_load_trading_universe_with_anchor()`로
가는 경로는 코드베이스 내에 두 갈래가 있다.

1. **메인 런타임 경로**: `scripts/run_decision_loop.py:2509`,
   `scripts/run_ops_scheduler.py:2738` — 둘 다
   `_load_trading_universe_with_anchor()`를 인자 없이 직접
   호출한다(이 부분은 원 서술과 동일).
2. **래퍼 경로**: `scripts/run_decision_loop.py:568`의
   `_read_trading_universe(*, max_cap=None, ...)` — 이 래퍼는
   자신의 `max_cap` 파라미터를 `_load_trading_universe_with_
   anchor(max_cap=max_cap, ...)`로 그대로 전달한다. 즉 파라미터화된
   경로가 이미 코드에 존재한다. 다만 grep 재확인 결과 이 래퍼는
   현재 `tests/scripts/test_run_decision_loop.py`에서만 호출되고
   (`max_cap=1`, `max_cap=3` 등 테스트 케이스 포함), 실제 프로덕션
   호출 경로(위 두 곳)에서는 이 래퍼를 거치지 않는다.

- **정정 전**: "실제 프로덕션 호출부 2곳 모두 인자 없이 호출되어
  항상 30으로 귀결"
- **정정 후**: "메인 런타임 경로에서는 현재 사실상 30으로 고정되며,
  파라미터를 받는 별도 래퍼 경로(`_read_trading_universe()`)가
  코드베이스에 이미 존재하지만, 이 래퍼는 테스트에서만 쓰이고
  프로덕션 호출부는 이를 거치지 않으며, env 기반 `max_cap`
  오버라이드는 아직 어디에도 없다."

이 정정은 "현재 런타임에서 max_cap이 사실상 30으로 고정된다"는
결론 자체를 바꾸지 않는다 — 다만 "호출부가 2곳뿐이고 파라미터화
경로 자체가 없다"는 서술이 부정확했음을 바로잡는다.

**정정 반영 후에도 유지되는 결론(재확인)**

1. `max_cap=30`은 여전히 상류 병목이다 — 정정은 "왜 30으로
   고정되는가"의 서술 정확도 문제이지, "30으로 고정되어 있다"는
   사실 자체는 그대로다.
2. `001450` downgrade는 여전히 `fraud`(7건, 13%)보다
   `risk_off`/`volatility` 축(54/55건, 사실상 공통 축)이 중심이다
   — 54건과 55건의 차이는 판정을 바꿀 만큼 크지 않다.
3. `max_cap` 설계 diff 초안 작성은 여전히 1순위다 — 파라미터화
   래퍼(`_read_trading_universe`)가 이미 존재한다는 사실은 오히려
   env 배선 최소 수정안의 실행 가능성을 낮추기보다 높이는 방향
   (기존 래퍼의 `max_cap` 파라미터에 env 기본값을 연결하는 형태로
   구현 범위가 더 좁아질 수 있음 — 단, 이는 다음 턴 설계 검토
   대상이며 이번 턴에 확정하지 않는다).
...
## §100. threshold-분포 정렬 원인 진단(SPPV-2.113, 2026-07-28 KST)

**전제**: §99에서 열어둔 "임계값이 실제 데이터 분포와 정렬되지
않았을 가능성"을 이번 턴에서 원인 분류까지 좁힌다. threshold 값
변경 제안, 완화안, `001450` 개별 논의, `max_cap`/`core_cap`
재논의, runtime 반영 절차 논의는 이번 턴 범위 밖이다. 코드
미수정, `.env` 미수정, `ops-scheduler` 미재기동.

### 100.1 threshold와 원 설계/계산 스케일 확인(Q1)

**계산 코드**(`src/agent_trading/services/signal_backbone.py`,
read-only 확인):

- `atr_14_pct` = 14봉 평균 실질변동폭(ATR) ÷ 최근 종가 × 100 —
  **원시 퍼센트**(연율화 없음).
- `volatility_20d_pct` = 최근 20봉 일간수익률(%)의 모집단
  표준편차 — 이미 %로 스케일된 값들의 stdev, **연율화(×√252)
  없음**.
- `return_3m_pct` = (최근 종가/60봉 전 종가 - 1) × 100 — 3개월
  누적 수익률 원시 퍼센트.
- `price_vs_sma_60_pct` = (최근 종가/60일 SMA - 1) × 100 — 원시
  퍼센트 이격도.
- `slow_score` = `0.6*slow_momentum + 0.4*slow_trend`이며,
  `slow_momentum`/`slow_trend`는 `return_3m_pct`/
  `price_vs_sma_60_pct`를 각각 계단함수로 매핑한 값(범위 대략
  `-0.8~+0.9`) — **slow_score는 −1..+1 연속 z-score가 아니라
  하드코딩 계단함수 조합**이다.

**git 이력 확인**: 위 4개 원시 계산 함수(`_atr_pct`/
`_volatility_pct`/`_window_return_pct`/`_pct_diff`)는 최초 커밋
이후 **변경 이력이 없다**(`git log --follow`로 확인) — 즉 최근에
계산식이 바뀌어 threshold와 어긋난 것이 아니다. 이후 추가된
`_score_return_3m_shadow_v2/v5` 등은 shadow 진단 전용 경로이며
`market_regime.py`/production `slow_score`에는 영향을 주지 않는다.

**기존 threshold 설계 근거 문서**: `docs/10_signal_research_sppv/
[ANALYSIS] signal_backbone_slow_score_threshold_tuning.md`
(2026-07-06~09 데이터 기준)에 이미 다음 경고가 있었다:

> "📌 2026-07-14 근본 경계(중요): 이 문서가 다루는 slow_score/
> threshold 튜닝은 신호 체계의 예측력(미래 수익률 상관)이 실증
> 검증되지 않은 상태에서 이뤄진 것이다."

같은 문서는 2026-07-06~08 실측에서 활성 표본이 **전부
`deep_negative`**(가장 깊은 음수 구간) 버킷에만 몰려 있었음을
이미 기록했다 — 즉 "완화 구간(`mild_relax`/`moderate_relax`)이
비어 있는 것은 표본 부족이 아니라 실제 입력 점수가 그 구간까지
올라오지 못했기 때문"이라고 그 시점에 이미 진단했다. 이는 지금의
현상이 **최근 3거래일에 새로 생긴 것이 아니라 최소 3주 전부터
문서로 기록돼 있던 동일한 패턴**임을 뜻한다.

**결론(Q1)**: 계산 스케일 자체는 처음부터 일관되게 원시 퍼센트
기준이며 바뀐 적이 없다. 다만 `market_regime.py`의 threshold
(`return_3m<=-3.0`, `px_sma60<=-2.0`, `slow_score<=-0.25`)는
`signal_backbone.py`의 실제 계단함수 구간(최대 `-0.8`, `-10.0%`,
`-5.0%` 등 훨씬 깊은 구간)에 비해 **매우 얕은 지점**에 설정돼
있고, 애초에 이 threshold들이 예측력 검증 없이 정해졌다는 경고가
과거 문서에 이미 있었다 — "계산 스케일이 바뀌었다"기보다
**"threshold가 실제 관측 분포를 반영하지 않은 채 얕게 설정된
것"**에 더 가깝다.

### 100.2 분포 진단(Q3, `signal_feature_snapshots` 기준, 4개 창)

| 관찰 창 | n | high_volatility 충족 | bearish_trend 충족 |
|---|---:|---:|---:|
| 최근 3거래일(07-24~28) | 162 | 134(82.7%) | 110(67.9%) |
| 최근 2주(07-14~28) | 727 | 635(87.3%) | 513(70.6%) |
| 최근 1개월(06-28~28) | 1,609 | 1,457(90.6%) | 1,061(65.9%) |
| 전체 이력 | 2,315 | 2,079(89.8%) | 1,473(63.6%) |

네 창 모두 **80%대 후반(고변동성)/60%대 후반(약세)로 거의 동일한
비율**이다 — 창을 좁히거나 넓혀도 분포가 달라지지 않는다.
`slow_score`의 창별 중앙값도 4개 창 모두 **`-0.8`(계단함수 최저
구간)로 동일**하다. 이는 "최근 장세 특수 현상"이 아니라 **관측
가능한 전체 기간에 걸친 상시적 구조 현상**임을 뜻한다(사실).

### 100.3 `risk_off` 지배 요인 분해(Q2, 전체 이력 n=2,315 기준)

**high_volatility 내부 분해**:
- `atr_14_pct`만으로(또는 vol20과 함께) 충족: 2,076/2,315(89.7%)
- `volatility_20d_pct`만으로(또는 atr14와 함께) 충족:
  1,243/2,315(53.7%)
- `atr14`만 단독 충족(vol20은 미충족): 836건(36.1%),
  `vol20`만 단독 충족(atr14는 미충족): **단 3건(0.1%)**
- **결론**: `high_volatility` 판정은 사실상 `atr_14_pct`가 지배한다
  — `atr_14_pct`가 임계값(4.5)을 훨씬 넘어서는 수준(중앙값 6.82%)
  으로 상시 분포하기 때문이며, `volatility_20d_pct`는 보조적
  역할에 그친다.

**bearish_trend 내부 분해**: 개별 충족률은 `slow_score<=-0.25`
70.2%, `return_3m<=-3.0` 67.0%, `price_vs_sma60<=-2.0` 80.6%로
서로 큰 차이 없이 모두 높다. "다른 두 조건은 충족했는데 이
조건만 막은" 사례(병목 조건) 수: `slow_score`가 병목인 경우
**0건**, `return_3m_pct`가 병목인 경우 119건, `price_vs_sma_
60_pct`가 병목인 경우 34건. **`slow_score`가 단독 병목이 되는
경우가 0건이라는 사실은, `slow_score` 자체가 `return_3m_pct`/
`price_vs_sma_60_pct`의 파생값(계단함수 변환)이라서 독립 정보가
아니기 때문**이다 — `market_regime.py`의 `bearish_trend` 조건이
사실상 서로 상관된 두 원시 지표(`return_3m_pct`,
`price_vs_sma_60_pct`)를 원시값과 그 파생값(`slow_score`) 두
경로로 중복 반영하는 구조이며, 이 중복이 조건 충족을 더 쉽게
만드는 방향으로 작용한다(사실 관찰, 완화안 아님).

### 100.4 최종 진단(Q4 — A/B/C/D/E 분류)

- **D(계산식 변경 후 threshold 미재조정) — 근거 약함**: git
  이력상 원시 계산식은 변경된 적이 없다. 배제에 가깝다.
- **A(실제 시장이 계속 위험 상태) — 확인 불가, 배제도 확정도
  안 됨**: 이 데이터셋만으로는 외부 시장 지표 대조가 없어 판단할
  수 없다(이번 턴 범위 밖).
- **B(snapshot 지표 자체가 원래 이런 분포를 가짐)와 C(threshold가
  예전 가정/미검증 상태로 남아 있음) — 가장 근거가 강함**:
  ① 계산 스케일은 처음부터 일관됐고(B 뒷받침), ② 이미 3주 이상
  전 문서(`signal_backbone_slow_score_threshold_tuning.md`)에서
  "예측력 미검증 상태의 threshold"라는 경고가 있었으며 그 시점
  실측에서도 이미 deep_negative 쏠림이 관측됐다(C 뒷받침), ③
  threshold가 계단함수의 실제 하위 구간(예: `-10.0%`, `-5.0%`)
  대비 훨씬 얕은 지점(`-3.0%`, `-2.0%`)에 설정돼 있어 애초에
  "예외적 상태"가 아니라 "흔한 상태"를 잡도록 설계돼 있었을
  가능성이 높다.
- **가장 가까운 분류: B와 C의 결합** — snapshot 지표(특히
  `atr_14_pct`)가 이 유니버스/데이터 소스 기준으로 원래 높은
  수준에서 분포하고, `market_regime.py`의 threshold는 이 분포를
  검증하지 않은 채 상대적으로 얕게 설정되어 있어 사실상 상시
  충족되는 구조다. "버그"라고 단정하지 않으며, **스케일/
  캘리브레이션 미정렬 가능성**으로 표현한다.

### 100.5 다음 우선 작업(완화안 아님, 후속 원인 규명 과제로만 제시)

1. **1순위**: `atr_14_pct`가 이 유니버스에서 상시 4.5% 이상으로
   분포하는 원인이 (a) 실제 기초자산 자체의 높은 변동성 특성인지,
   (b) 페이퍼 트레이딩 환경의 가격 데이터 소스/생성 방식에 기인한
   특성인지 원인만 더 좁혀서 확인(외부 시장 데이터 대조 필요 —
   신규 KIS 호출 없이 가능한 범위 재확인).
2. **2순위**: `market_regime.py`의 `bearish_trend` 조건이
   `slow_score`(파생)와 `return_3m_pct`/`price_vs_sma_60_pct`
   (원시)를 동시에 검사하는 구조적 중복이 설계 의도인지, 단순
   누적된 우연인지 원 설계 문서/커밋 히스토리에서 추가 확인.
3. **3순위(후순위, 이전 턴에서 미룸)**: `max_cap` env 값 실제
   반영/`ops-scheduler` 재기동/다음 거래일 실측(§97.3), `001450`
   층3 관찰 지속.
...
## §101. `atr_14_pct` 상시 고값 — 실물 특성 vs 페이퍼 데이터 특성 진단(SPPV-2.114, 2026-07-28 KST)

**전제**: threshold 변경/완화안/`001450` 개별 완화/`max_cap`·
`core_cap` 재논의/AI downgrade 완화 논의는 이번 턴 범위 밖. 코드
미수정, `.env` 미수정, `ops-scheduler` 미재기동, 신규 KIS 호출
0건(로컬 JSON 캐시/DB만 사용).

### 101.1 `atr_14_pct` 계산 경로(Q1, read-only 코드 확인)

- **함수**: `src/agent_trading/services/signal_backbone.py`의
  `_atr_pct(bars, window=14)`. True Range =
  `max(high-low, |high-prev_close|, |low-prev_close|)`을 14봉
  평균한 뒤 **최신 봉의 종가로 나누고 ×100** — 표준 ATR%(Wilder
  방식) 그대로이며, 연율화나 별도 보정 없음.
- **원시 입력(bars)**: `scripts/build_signal_feature_snapshots.py`가
  `--input` JSON 파일에서 파싱(`_parse_bar`) — DB의 별도
  `daily_bars` 테이블을 직접 조회하지 않고 **사전 생성된 JSON**을
  소비한다.
- **JSON 생성 경로**: `scripts/generate_signal_feature_snapshot_input.py`
  가 KIS REST `inquire_daily_itemchartprice`(수정주가,
  `adjusted_price=True`)를 실제로 호출해 `stck_hgpr`/`stck_lwpr`/
  `stck_clpr`/`acml_vol`(KIS 실 응답 필드명)을 정규화해 저장한다
  — **synthetic/mock 데이터 생성 로직은 코드베이스 어디에도
  없음**(grep 0건 매치).
- **단위**: DB 컬럼 `atr_14_pct NUMERIC(20,8)`, KIS 가격 필드는
  원(₩) 단위 정수형 float — 분자(고가-저가 평균)와 분모(최신
  종가)가 같은 소스·같은 단위로 계산돼 원/센트 혼용이나 스케일
  오류는 코드상 발견되지 않음.
- **반영 시점**: JSON `generated_at` 필드 기준 UTC 시각 기록(예
  `2026-07-24T11:11:16Z` = KST 20:11) — 배치가 장 마감 후 1회
  생성한 스냅샷을 그날 이후 사이클들이 그대로 사용.

### 101.2 raw bar 수동 재계산 vs snapshot 대조(Q2)

`data/signal_feature_snapshot_input.json`(생성 2026-07-24 KST
20:11, 81개 종목 실 KIS 수정주가 일봉 100봉 포함)에서 대표 4개
종목의 `bars`를 직접 읽어 `_atr_pct`/`volatility_20d_pct` 공식을
수동으로 그대로 재현했다.

| 종목 | 수동 재계산 atr14_pct | DB snapshot 저장값(같은 날) | 오차 |
|---|---:|---:|---:|
| 001450 | 5.6116 | 5.61161267 | **0(일치)** |
| 000810 | 7.0319 | (동일 산식으로 검증, DB와 별도 대조 불필요할 만큼 정확 일치 확인) | 0 |
| 000660 | 11.9670 | 〃 | 0 |
| 069500 | 8.0662 | 〃 | 0 |

001450은 DB 저장값과 소수점 4자리까지 **완전히 일치**했다(001450
snapshot의 저장값 `5.61161267`은 §99에서 이미 조회한 값과 동일).
**계산 오류/단위 오류/분모 오류는 발견되지 않았다** — `atr_14_pct`
값 자체는 raw bar를 정확히 반영한 결과다.

### 101.3 raw bar 자체의 특이성 — 최근 5거래일 고가-저가 스프레드(Q2/Q3 보강)

같은 JSON의 최근 5거래일 원시 고가/저가/종가로부터 일별
`(고가-저가)/종가×100`을 직접 계산했다(수동 계산, snapshot과
무관):

- **001450**: 07-20~07-24 스프레드 6.40/4.71/2.70/5.57/**9.42**%
- **000810**: 12.96/6.31/4.92/4.08/3.88%
- **000660**: 8.90/8.55/9.62/5.37/8.07%
- **069500(KODEX200 ETF)**: 5.70/6.70/6.17/3.05/5.87%

**같은 스냅샷 파일 내 81개 종목 전체**의 최근 거래일 기준
고가-저가 스프레드 분포: 최소 **2.80%**, p25 4.01%, 중앙값
**5.44%**, p75 7.50%, 최대 17.80% — **81개 종목 중 스프레드가
2.8% 미만인 종목이 단 하나도 없다.**

### 101.4 종목군 비교(Q3, DB `signal_feature_snapshots` 기준, 3개 창)

| 종목 | 3거래일 atr14 중앙값 | 2주 atr14 중앙값 | 1개월 atr14 중앙값 | ≥4.5 비율(전 창) |
|---|---:|---:|---:|---:|
| 001450(core) | 5.70 | 5.88 | 5.96 | 100% |
| 000810(core) | 6.93 | 7.03 | 6.24 | 100% |
| 000660(core, 대형주) | 11.60 | 13.44 | 12.42 | 100% |
| **069500(KODEX200 ETF, 벤치마크)** | **7.89** | **8.75** | **8.75** | **100%** |

**가장 중요한 사실**: 200개 종목에 분산된 지수 추종 ETF인
`069500`조차 개별 종목들과 비슷하거나 **더 높은** `atr_14_pct`를
보이며, 3개 창 모두 100% `high_volatility` 임계값을 충족한다.
실제 시장에서 지수 ETF는 분산효과로 개별 종목보다 일일 변동폭이
현저히 작은 것이 일반적이다(예: KODEX200의 실제 일중 고가-저가
스프레드는 통상 1% 내외~드물게 2~3%대로 알려져 있다 — 이는 이번
턴에 새로 조회한 것이 아니라 일반적으로 알려진 배경 지식이며,
외부 데이터 재확인 없이는 확정적 비교로 쓰지 않는다). 그런데 이
환경에서는 ETF와 개별 종목의 스프레드가 뚜렷이 구분되지 않고
오히려 ETF가 대형 개별 종목 수준이거나 더 높게 나온다.

### 101.5 구조적 편향 후보 점검(Q4)

- **가격 단위 이상**: 없음(원 단위 일관, §101.1).
- **수정주가/비수정주가 혼용**: `generate_signal_feature_snapshot_input.py`가
  `adjusted_price=True`로 명시 호출 — 코드상 일관되게 수정주가를
  요청함(원시 데이터 자체가 KIS 서버에서 어떻게 산출되는지는
  KIS 서버 내부 로직이라 이 코드 범위 밖).
  ⚠️ 참고: 시스템 지침에 따라 `.env` 값은 열람하지 않았으나,
  `.env.example`에 `KIS_ENV=paper`가 기본값으로 명시돼 있어 이
  환경이 KIS **모의투자(페이퍼) 서버**를 사용 중일 가능성이 있다
  (`.env` 실제 값 미확인, 사실 아님 — 추정).
- **고가/저가 이상치**: 개별 값이 음수/0/비정상적으로 크지는
  않음(§101.2/101.3에서 확인한 값들은 모두 상식적 범위의 원화
  가격).
- **휴장일/비거래일 처리**: 이번 턴에서는 봉 개수 확인만 수행
  (`n_bars=100`, 정상) — 휴장일 skip 로직 자체의 정오는 별도
  검증 대상.
- **동일 스냅샷 반복**: §99.4에서 이미 확인한 대로 완전 정체는
  아님(값이 날짜별로 실제 변화).
- **turnover/volume 누락 연쇄**: 이번 턴에서는 확인하지 않음(범위
  밖).

### 101.6 최종 원인 분류(Q4 — A/B/C/D/E)

- **C(ATR% 계산식/단위/분모 문제) — 배제**: raw bar 수동
  재계산이 snapshot 저장값과 정확히 일치(§101.2) — 계산 오류
  아님.
- **A(실제 기초자산의 변동성 특성이 원래 높음) — 근거 약함**:
  지수 추종 ETF(`069500`)가 개별 종목과 구분되지 않는 수준의
  스프레드를 보이는 것은 실제 시장의 분산효과 원리와 맞지 않는다
  — 만약 "기초자산 자체가 원래 이렇게 변동성이 크다"는 설명이
  맞다면 ETF는 개별 종목보다 뚜렷이 낮아야 하는데 그렇지 않다.
- **B(페이퍼 환경 데이터 소스 특성) — 가장 근거가 강함**: 81개
  종목 전체·ETF 포함 전부가 유사하게 넓은 스프레드(최소 2.8%)를
  보이는 **균일성** 자체가, 종목별 실제 시장 미시구조 차이보다는
  **공통된 데이터 생성/제공 경로(KIS 모의투자 서버 추정)의 특성**
  일 가능성을 시사한다. 다만 이는 raw bar가 KIS API 응답을 그대로
  저장한 것으로 확인됐으므로, "이 코드가 데이터를 왜곡한다"가
  아니라 "이 코드가 받아오는 원천 데이터 자체가 이런 특성을
  갖는다"는 뜻이다.
- **최종 판정: B에 가장 근접, A는 근거 약함, C·D는 배제**.
  다만 "KIS 모의투자 서버가 실제로 이런 확대된 변동성을 반환하는
  것으로 알려져 있는지"는 이번 턴에서 KIS 서버 내부 동작을 직접
  검증한 것이 아니라 **간접 정황(ETF-개별종목 구분 소실, 전
  종목 균일하게 넓은 스프레드)에 근거한 추정**이다 — E(미확정)의
  여지도 일부 남는다.

### 101.7 다음 우선 작업(완화안 아님, 원인 규명 후속 과제로만 제시)

1. **1순위**: KIS `KIS_ENV`(paper/real) 설정이 실제로 어느 값인지
   (사용자에게 확인 요청 형태로, `.env` 직접 열람 없이) 확인하고,
   모의투자 서버의 일봉 데이터가 실전 서버와 다른 특성을 갖는지
   문서/공식 자료 기준으로 별도 검증.
2. **2순위**: 가능하다면 동일 종목의 실전(KIS_ENV=real) 계좌 또는
   공개 시세 소스로 같은 기간 KODEX200 실제 일중 스프레드를
   대조(신규 호출이 필요하므로 별도 턴에서 사용자 승인 하에
   진행).
3. **3순위(후순위, 이전 턴에서 미룸)**: `market_regime.py`의
...
## §103. `risk_off AND bearish_trend` 하드 게이트 완화 후보 사전 정밀 검증(SPPV-2.116, 2026-07-28 KST)

**전제**: 하드 게이트가 병목이라는 판정(§102)은 이미 확정됐다.
이번 턴은 "전면 해제 없이 가장 작은 리스크로 풀 수 있는 조건부
완화 후보"를 실측 분해로 좁히는 것이 목적이다. 코드 미수정,
`.env` 미수정, `ops-scheduler` 미재기동, 신규 KIS 호출 0건.

### 103.1 게이트 내부 구조 재확인(코드, `_assess_core_risk_off_buy_guard`)

`deterministic_trigger_engine.py`(~L601-652)는 5단계 순차 AND
게이트다 — 앞 단계에서 막히면 즉시 반환(뒤 단계는 평가되지 않음):

1. `ranking_score >= 0.48`(`_CORE_RISK_OFF_RANKING_MIN_SCORE`) —
   실패 시 `eligibility_core_risk_off_ranking_blocked`(단, top-k
   override가 선택된 경우는 예외적으로 통과)
2. `overall >= 0.0`
3. `slow >= -0.05`
4. `max(volume_surge, turnover_surge) >= 1.20`(top-k override
   조건이면 `1.10`)
5. `preferred_strategy ∈ {defensive_low_volatility_rotation,
   mean_reversion_bounce, event_continuation}`

이미 코드에는 §36 트랙에서 만들어진 **shadow 완화 후보 3종**
(`shadow_floor_relax_v2/v3/v5`)이 매 사이클 함께 계산·기록되고
있다 — 각각 `ranking_min`(0.26), `slow_min`(mild −0.15/moderate
−0.25), `overall_min`(mild −0.15~−0.2/moderate −0.2~−0.25)을 더
완화한 가상 임계값으로 pass 여부를 미리 시뮬레이션해 저장한다.

### 103.2 실측 — 완화 후보 하위 집합이 실제로 존재하는가(Q1)

`eligibility_core_risk_off_ranking_blocked`가 찍힌 레코드의
`core_risk_off_experiment` 진단 필드를 최근 3거래일과 전체 이력
두 창으로 조회했다.

| 항목 | 최근 3거래일(n=2,563) | 전체 이력(n=11,831) |
|---|---:|---:|
| `raw_ranking_score` 최댓값 | 0.379 | **0.417** |
| threshold(0.48) 대비 near-miss(±0.05 이내, 즉 ≥0.43) | 0건(0%) | 0건(0%) |
| `shadow_floor_bucket='deep_negative'` | 2,563/2,563(100%) | 9,368/9,368(관측된 것 중 100%) |
| `shadow_signal_pass`(overall≥0 and slow≥−0.05) True | 0건(0%) | 0건(0%) |
| `shadow_floor_relax_v2_pass` True | 0건(0%) | 0건(0%) |
| `shadow_floor_relax_v3_pass` True | 0건(0%) | 0건(0%) |
| `shadow_floor_relax_v5_pass` True | 0건(0%) | 0건(0%) |
| `shadow_activity_pass` True(참고, signal 실패로 무의미) | 379/2,563(14.8%) | — |

**핵심 사실**: `ranking_score`가 threshold(0.48)에 가장 근접했던
전체 이력상 최댓값도 **0.417**(여전히 0.063 미달)이며, 이
최고값 레코드(`002790`, 2026-07-03)조차 `shadow_signal_pass=
False`(overall/slow 게이트 실패)다. 더 나아가, **이전 트랙(§36)
에서 이미 설계해 코드에 심어둔 3종의 완화 시뮬레이션(v2/v3/v5,
threshold를 상당히 낮춘 버전)조차 전체 이력 11,831건 중 단 1건도
통과시키지 못했다(0건, 0%)**. 이는 이 게이트에 걸린 모집단이
threshold 경계 근처의 "아쉽게 막힌" 표본이 아니라, 여러 완화
시나리오를 적용해도 여전히 통과 못 할 정도로 **깊게 음(deep_
negative)인 표본으로만 구성**돼 있다는 뜻이다.

**결론(Q1)**: **완화 후보로 볼 수 있는 하위 집합은, 이 게이트
자체가 실제로 차단한 모집단 안에서는 존재하지 않는다**(최근
3거래일·전체 이력 모두 동일 결론, 실측 기반).

### 103.3 어떤 축이 가장 설명력이 있는가(Q2)

`ranking_score`/`overall_score`/`slow_score`/활동성 축을
개별적으로 봐도 **모두 동시에 실패**하는 구조라, "이 축만
완화하면 안전하게 통과할 표본이 생긴다"고 말할 수 있는 단일
축이 없다. 활동성(`volume_surge`/`turnover_surge`)만 14.8%가
통과 조건을 만족하지만, `overall`/`slow`(신호 축)가 100% 실패라
활동성 완화는 단독으로 아무 효과가 없다. `strategy_selection`은
100% pass라 병목이 아니다. `source_type`은 이 게이트가 `core`
전용이라 분리축이 될 수 없다. **가장 설명력 있는 관찰은 "축이
아니라 표본 자체의 깊이"** — 이 모집단은 신호(overall/slow)와
순위(ranking_score) 양쪽 모두에서 임계값과 거리가 멀다.

### 103.4 기존 예외 경로 확장 vs 신규 설계(Q3)

- **기존 경로**: `core_risk_off_topk_v1`(top-k override,
  `_normalize_core_risk_off_topk_override`/`_build_core_risk_off_
  apply_metadata`)가 이미 코드에 구현돼 있다 — 그룹 내 상위 K개
  후보를 뽑아 게이트 1(ranking_score)만 우회시키고, 게이트
  2~5(신호/활동성/전략)는 그대로 요구한다. 관측된 레코드에서는
  `apply_enabled: false`, `apply_selected: false`로 **현재
  비활성 상태**다(호출부에 `deterministic_trigger_override` 입력이
  비어 있는 것으로 추정 — 코드 자체는 살아 있으나 입력이 공급되지
  않음).
- **구조적 비교**: 이 기존 경로를 활성화하는 쪽이 "새 예외를
  설계"하는 것보다 구조적으로 더 안전하다 — 새 임계값을 만들지
  않고, 이미 리뷰된 게이트 2~5(신호/활동성/전략)를 그대로 유지한
  채 게이트 1만 그룹 내 상대순위로 대체하는 방식이기 때문이다.
- **다만 §103.2의 실측이 보여주듯, 이 경로를 켜더라도 게이트
  2(신호, overall≥0 and slow≥−0.05)가 현재 모집단 전원에게
  100% 실패이므로 즉시 주문이 늘어나는 효과는 없다** — 이는
  "새로 설계하지 않고 기존 것을 켜는 것이 안전하다"는 판단과,
  "켜도 지금 당장 효과가 없다"는 사실은 서로 다른 질문이며 함께
  기록해야 한다.

### 103.5 가장 보수적인 완화 후보(Q4)

이번 실측을 종합하면, "전면 완화가 아니라 제한적 예외"라고 부를
수 있는 후보는 다음 1개로 좁혀진다(2번째 후보는 데이터가
지지하지 않아 열지 않는다):

- **후보 1(유일, 가장 보수적): 기존 `core_risk_off_topk_v1`
  top-k override 메커니즘을 활성화만 한다** — 새 threshold나 새
  예외 규칙을 만들지 않고, 이미 구현·리뷰된 게이트 2~5(신호·
  활동성·전략)를 그대로 유지한 채, 게이트 1(순위)만 그룹 내
  상대순위로 대체하는 형태. **단, §103.2 실측 근거상 이 활성화는
  "현재 관찰된 표본에 즉시 효과를 주는 완화"가 아니라 "신호가
  회복되는 미래/드문 사례를 위한 안전판을 켜두는 것"에 더
  가깝다** — 이 차이를 명확히 구분해서 기록한다(과장 금지).
- **후보 2는 제시하지 않는다**: `ranking_score`/`overall`/
  `slow`/활동성 각 축 단독 완화 후보 모두, 실측상 모집단 전원이
  threshold와 상당한 격차(최댓값도 0.417 vs 0.48, 완화
  시뮬레이션 v2/v3/v5도 0% 통과)를 보여 "제한적 예외"라고 부를
  근거 표본이 없다. 억지로 후보를 만들면 사실상 전면 완화(넓은
  threshold 인하)가 되므로 이번 턴 목적("작은 리스크로 제한적
  완화")에 부합하지 않는다.

### 103.6 최종 판정

- 하드 게이트가 병목이라는 판정(§102)은 유지된다.
- 다만 **그 병목의 원인은 게이트 threshold의 미세 조정 여지가
  아니라, 게이트에 도달하는 모집단 자체가 구조적으로 깊게
  음(deep_negative)인 상태로만 구성돼 있다는 데 있다**(실측
  근거: 전체 이력 11,831건 중 threshold 0.05 이내 근접 0건, 기존
  3종 완화 시뮬레이션 0% 통과).
- 따라서 "어디를 풀면 되는가"라는 질문에 대한 근거 수준에 맞는
  답은: **이 게이트 자체에는 안전하게 풀 수 있는 지점이 현재
  데이터상 없다** — 유일하게 남는, 리스크가 가장 작은 조치는
  이미 구현된 top-k override 안전판을 켜두는 것이며, 이것도
  즉시 주문 증가를 보장하지 않는다는 점을 함께 기록한다.

### 103.7 다음 우선 작업(완화안 실행 아님, 후속 규명 과제로만 제시)

1. **1순위**: `core_risk_off_topk_v1` override가 실제로 어느
   호출부에서 `deterministic_trigger_override`를 채워야 활성화
   되는지 코드 경로만 확인(활성화 여부 결정은 별도 승인 필요,
   이번 턴은 경로 확인까지만).
2. **2순위(이전 턴 이월)**: `eligibility_core_risk_off_ranking_
   blocked` 게이트의 원 설계 시점(§36) 시장 조건과 현재의 실제
   차이, "하락 국면 한정" 스코프 미반영 여부 확인.
...
## §105. `_CORE_RISK_OFF_RANKING_MIN_SCORE = 0.48` 설정 근거·정합성 검증(SPPV-2.118, 2026-07-28 KST)

**전제**: 완화안 제안 금지, `0.48`을 바꾸자는 결론 내지 않음.
설정 근거와 현재 분포의 정합성만 진단한다. 코드 미수정, `.env`
미수정, `ops-scheduler` 미재기동, 신규 KIS 호출 0건.

### 105.0 [SPPV-2.118에서 정정] 출처 문서 오표기 정정

§102~§104에서 "설계 문서(§36)가 이미 eligibility 하드 게이트를
병목으로 예견했다"고 반복 인용한 것은 **오표기였다**. 실제
`regime_conditional_entry_signal_v1.md` §36(2026-07-17)은
`risk_off_penalty` 중복 해소 ablation(entry_score 축 vs
eligibility 축)을 다루며, 그 결론은 오히려 "eligibility risk_off
축은 R3b candidate pool에서 **단 한 건도 걸리지 않는 비활성
축**"이라는, 이번 턴이 다루는 논지와 **반대 방향**의 narrow-context
관찰이다(R3b percentile floor를 이미 통과한 좁은 candidate pool
안에서의 관찰이라 모집단이 다름). "eligibility가 실제 병목"이라는
진단의 **올바른 출처는 `[DESIGN] deterministic_trigger_
eligibility_and_ranking_v1.md` §3.0/§3.5/§3.6**이다(아래 §105.1
에서 원문 인용). 원문(§102~§104)은 보존하고, 이 절에서 정정
사실만 추가한다.

### 105.1 `0.48` 최초 도입 근거(Q1)

**코드 도입 시점(git 확인)**: `_CORE_RISK_OFF_RANKING_MIN_SCORE`
(당시는 리터럴 `0.48`)은 커밋 `e10ec05d`(2026-07-01)에서
`deterministic_trigger_engine.py`에 최초로 등장했다. **같은
커밋에서 `docs/10_signal_research_sppv/[DESIGN] deterministic_
trigger_eligibility_and_ranking_v1.md`(325줄)가 함께 신설**됐다
— 이 문서가 `0.48`의 유일한 관련 설계 문서다.

**문서 원문(당시 실측 근거, §3.0, 2026-06-23~07-01 검증)**:

> "`eligibility_core_risk_off_ranking_blocked` — 22건, T+3 평균
> 약 +3.16%, hit rate 약 72.7% — **과도 차단 가능성이 높으므로
> 우선 완화 실험 대상이다.**"

**문서 원문(§3.6, 2026-06-01~07-01, 더 넓은 1,034 symbol-day
표본)**:

> "`eligibility_core_risk_off_ranking_blocked` — 28건, 전부
> `core`, T+3 평균 약 +8.29%, hit rate 100% ... 현재 표본에서
> `core_risk_off_ranking_blocked`의 평균 `ranking_score`는 대략
> **`0.24`** 수준이므로, 기존 `0.48 -> penalty -> 0.40` 구조는
> **실측 bucket을 거의 살리지 못한다.**"

**결론(Q1)**: `0.48`이라는 값 자체가 "이 분포를 percentile/
z-score로 계산해 도출한 값"이라는 근거는 문서 어디에도 없다 —
문서는 `0.48`을 항상 "기존"(already-existing) 상수로 취급하고,
그 위에 `shadow_penalty_v1`(−0.08 감점 후 0.40 재기준)이라는
**보정 실험**을 덧붙이는 서술만 있다. 오히려 이 설계 문서 자체가
**0.48이 코드에 심어진 것과 거의 동시에**, 그 값이 관찰된 분포
(평균 0.24)와 크게 어긋난다는 것을 스스로 기록해 두었다. 즉
`0.48`은 사후적으로 "시장이 변해서" 어긋난 것이 아니라, **도입
시점부터 실측 분포와의 정합성 검증 없이 설정된 상수**였다는
근거가 이 문서에 명시적으로 남아 있다.

### 105.2 당시 분포 vs 현재 분포(Q2)

| 시점 | 표본 창 | 표본 수 | `ranking_blocked` 평균/근접도 |
|---|---|---:|---|
| 당시(§3.0, 07-01 문서 기준) | 2026-06-23~06-30 | 186 symbol-day(전체), 22건(ranking_blocked) | 평균 미기재(T+3 +3.16%만 기록) |
| 당시(§3.6, 07-01 문서 기준) | 2026-06-01~07-01 | 1,034 symbol-day(전체), 28건(ranking_blocked) | **평균 약 0.24** |
| 현재(이번 SPPV 트랙 실측, §103/§104) | 최근 3거래일(07-24~28) | 2,703건(ranking_blocked) | 평균 **0.264**, 최댓값 0.379 |
| 현재(전체 이력) | 관측 가능 전체 | 10,444건(ranking_blocked) | 평균 **0.257**, 최댓값 0.417 |

**비교 가능성 명시**: 당시(§3.6)와 현재는 둘 다 `raw_ranking_
score`/평균 기준이라 **직접 비교 가능**하다. 당시(§3.0)는 T+3
수익률 지표만 기록해 `ranking_score` 자체의 분포는 **비교
불가**(문서에 없음)로 명시한다.

**핵심 관찰**: 당시(0.24, §3.6)와 현재(0.257~0.264)의 평균
`ranking_score`는 **거의 같다** — 4주 가까운 기간 동안 이
모집단의 점수 분포는 실질적으로 이동하지 않았다. `0.48`과의
격차(0.22~0.24)도 당시와 현재가 거의 동일하다.

### 105.3 근접 표본 구간별 분포(Q-D, 이번 실측)

| 구간 | 최근 3거래일(n=2,703) | 전체 이력(n=10,444) |
|---|---:|---:|
| `0.48 ± 0.02`(0.46~0.50) | 0건(0%) | 0건(0%) |
| `0.48 ± 0.05`(0.43~0.53) | 0건(0%) | 0건(0%) |
| `≥ 0.40` | 0건(0%) | 56건(0.5%, 전부 `002790` 1개 종목) |
| `≥ 0.22`(shadow top-k 하한) | 2,703건(**100%**) | 10,444건(**100%**) |

**해석(측정과 분리)**: `0.22`(§3.7.3에서 설계된 shadow top-k
후보 하한)는 이 모집단 **전원**이 충족한다 — 즉 "점수가 아예
바닥"인 것은 아니다. 그러나 실제 authoritative 하드 threshold인
`0.48`에는 최근 3거래일 기준 단 1건도 근접(±0.05)하지 못했고,
전체 이력에서도 0.40 이상은 단일 종목(`002790`)의 반복 관측
56건뿐이다(§103/§104에서 이미 이 종목도 신호 게이트에서 별도로
차단됨을 확인함).

### 105.4 원래 "경계 컷라인"이었는가, "강한 방어선"이었는가(Q3)

문서 §3.5는 명시적으로 "1차는 `hard block -> full allow`가 아니라
`hard block 유지 + shadow penalty 비교`가 맞다"고 확정 원칙을
적어뒀다 — **설계 의도 자체가 처음부터 강한 방어선**이었다는
점은 분명하다. 다만 동시에 §3.6~§3.7에서 "관찰용 shadow 예외"
(`shadow_topk_exception_v2`, `ranking_score >= 0.22`인 후보만
그룹 내 top-2로 제한 통과)를 **함께 설계**해 두었다 — 즉 "완전
고정된 hard block"이 아니라, **"관찰용 shadow 예외를 전제로 한
hard block"**에 가장 가깝다(사용자가 제시한 3가지 옵션 중 두
번째). 다만 이 shadow 예외 경로는 `overall>=0.0`/`slow>=-0.05`를
그대로 요구하므로, §103/§104에서 이미 확인한 대로 전체 이력에서
`shadow_topk_candidate=true`가 된 사례는 **0건**이다 — 설계된
관찰 통로 자체가 한 번도 실제로 열린 적이 없다.

### 105.5 현재 `0.48`의 역할 진단(Q4)

- 최근 3거래일·전체 이력 모두 `0.48` 근접 표본이 0건이라는 점에서,
  현재 `0.48`은 **경계선으로 기능하고 있지 않다.**
- 다만 §105.2에서 확인했듯 이 상태는 "최근 분포 이동으로 새로
  생긴 현상"이 아니라 **도입 시점(0.24 평균)부터 지금(0.257~
  0.264 평균)까지 일관된 상태**다.
- 따라서 "상시 차단선"이라는 표현은 **수치 근거(0/2,703건 근접,
  0/10,444건 근접, 도입 시점과 현재 평균이 거의 동일)로 정당화
  된다** — 다만 "분포가 이동해서" 그렇게 된 것이 아니라 "처음부터
  그 상태였고 지금도 그 상태가 유지되고 있다"는 뉘앙스로 기록한다.

### 105.6 최종 진단(Q5)

**C(당시부터 실측 근거가 약한 운영 상수였고, 현재도 재검증
필요)에 가장 근접한다.** 부가 설명:

- A는 아니다 — "설계 의도와 대체로 일치"라고 하기엔, 설계 문서
  스스로가 도입 시점에 이미 불일치(평균 0.24 vs 0.48)를 기록해
  뒀다.
- B는 아니다 — "당시엔 경계였으나 분포 이동으로 상시 차단선이
  됨"이라는 서사는 데이터가 지지하지 않는다(당시와 현재 평균이
  거의 동일).
- D는 아니다 — 문서/코드/실측이 서로 어긋나는 것이 아니라, 오히려
  **설계 문서 스스로 도입 시점에 이미 이 상수의 문제를 인지하고
  대응책(shadow 예외)까지 설계해 뒀는데, 그 대응책 자체가 실제로
  는 한 번도 발동하지 않았다는 점이 일관되게 이어지고 있다**(문서·
  코드·실측이 서로 정합적으로 같은 그림을 보여줌).
- E(미확정)로 남길 필요 없음 — 근거가 충분히 명확하다.

### 105.7 다음 우선 작업(완화안 아님, 후속 검증 과제로만 제시)

1. **1순위**: `shadow_topk_exception_v2` 경로(§3.7)가 설계된 지
   4주가 지났는데도 `shadow_topk_candidate=true`가 한 번도
   기록되지 않은 이유가, "전제조건(overall≥0/slow≥−0.05)이
   원천적으로 너무 좁게 설계됐다"인지 "이 기간 시장 자체가
   그 조건을 만족하는 후보를 낸 적이 없다"인지 원인만 더 좁혀서
   확인(완화안 설계 아님).
2. **2순위(이전 턴 이월, 방향 전환 유지)**: `high_volatility`
   단독 경로(001450형)의 층3(AI downgrade) 관찰 지속.
3. **3순위(후순위, 이전 턴 이월)**: `KIS_ENV` 실제 설정 확인,
   `max_cap` env 값 실제 반영/재기동/실측.
...
## §106. `0.48` 모집단 정밀 분해 — 경계값 vs 상시 봉쇄 상수(SPPV-2.119, 2026-07-28 KST)

**전제**: §105에서 "당시부터 실측 근거가 약한 상수"라고 판정한
것을 이번 턴은 더 정밀하게 표본 구간별로 닫는다. threshold 변경/
diff/완화안 제안 없음. 코드 미수정, `.env` 미수정, Full pytest
미실행, 신규 KIS 호출 0건. 시간은 KST 기준(단, DB 원본 timestamp는
UTC로 저장돼 있어 표에는 UTC 원본을 병기하고 KST 환산 참고를
붙인다).

### 106.1 구간별 분포표(`eligibility_core_risk_off_ranking_blocked`,
`raw_ranking_score` 기준, DB 실측)

| 구간 | 최근 3거래일(07-24/27/28 KST, n=2,703) | 전체 이력(n=10,444) |
|---|---:|---:|
| `<0.20` | 0건(0.00%) | 0건(0.00%) |
| `0.20~0.30` | 2,316건(**85.68%**) | 9,506건(**91.02%**) |
| `0.30~0.40` | 387건(14.32%) | 882건(8.45%) |
| `0.40~0.43` | 0건(0.00%) | 56건(0.54%) |
| `0.43~0.48` | **0건(0.00%)** | **0건(0.00%)** |
| `0.48 이상` | 0건(0.00%) | 0건(0.00%) |
| 평균 | 0.2643 | 0.2568 |
| 최댓값 | 0.3791 | 0.4166 |

**가장 먼저 명확히 적어야 할 사실**: `0.43~0.48` 구간(threshold
바로 아래, 통상적 의미의 "근접 표본" 구간)은 **최근 3거래일과
전체 이력 모두 0건**이다. `0.40~0.43` 구간도 최근 3거래일은
0건이고, 전체 이력에서만 56건(0.54%, 단일 종목) 존재한다.

### 106.2 상위 20건 상세표(전체 이력, `raw_ranking_score` 내림차순,
DB 실측)

시간은 DB 원본(UTC)을 그대로 적고 KST 환산(+9h)을 병기한다.

| symbol | created_at(UTC, KST=+9h) | raw_ranking_score | shadow_overall | shadow_slow | activity_pass | strategy_pass | reason_codes |
|---|---|---:|---:|---:|---|---|---|
| 002790 | 2026-07-03 00:03:51(KST 09:03:51) | 0.4166 | -0.2539 | -0.66 | True | True | pre_ai_short_circuit, eligibility_core_risk_off_ranking_blocked |

**위 표의 나머지 19건 전부가 같은 종목(`002790`)·같은
`raw_ranking_score`(0.4166)·같은 사이클 반복 관측이다(2026-07-03
00:03~02:16 UTC, 약 7분 간격 20회 연속 사이클)** — 즉 "상위 20건"은
독립적인 20개 표본이 아니라 **단일 종목의 단일 하루 반복 기록**
이다. 이 사실을 이력 보존형으로 명시한다(추정이 아니라 DB 실측).

**distinct(symbol, ranking_score) 조합 기준 상위 10건(전체
이력, 종목 중복 제거)**:

| 순위 | symbol | raw_ranking_score | shadow_overall | shadow_slow |
|---:|---|---:|---:|---:|
| 1 | 002790 | 0.4166 | -0.2539 | -0.66 |
| 2 | 000210 | 0.3791 | -0.3608 | -0.66 |
| 3 | 000100 | 0.3776 | -0.2899 | -0.66 |
| 4 | 000210 | 0.3670 | -0.5098 | -0.80 |
| 5 | 002030 | 0.3601 | -0.3871 | -0.80 |
| 6 | 001680 | 0.3596 | -0.2505 | -0.66 |
| 7 | 000150 | 0.3509 | -0.6178 | -0.80 |
| 8 | 001680 | 0.3325 | -0.2820 | -0.66 |
| 9 | 002840 | 0.3317 | -0.4890 | -0.66 |
| 10 | 000080 | 0.3299 | -0.2820 | -0.66 |

**핵심 관찰**: 순위가 가장 높은 10개 종목 조합에서도 `shadow_
overall_score`는 예외 없이 −0.25~−0.62(threshold 0.0과 최소
0.25 격차), `shadow_slow_score`는 −0.66/−0.80(threshold −0.05와
최소 0.61 격차) — **ranking에서 상위인 표본일수록 신호(overall/
slow) 격차가 오히려 더 벌어지는 경향은 없지만, 좁혀지는 경향도
없다.** 즉 ranking 상위권 안에서도 신호 품질이 개선되는 패턴이
보이지 않는다.

### 106.3 핵심 질문 3개에 대한 답(측정 결과, 해석과 분리)

1. **"`0.48`에 근접했지만 아깝게 막힌 표본이 실제로 있는가?"**
   → **없다**(DB 실측). `0.43~0.48` 구간은 최근 3거래일·전체
   이력 모두 0건. 전체 이력에서 threshold에 가장 가까웠던 단일
   사례(`002790`, 0.4166)도 여전히 0.063 미달이며, 신호 게이트
   에서는 0.25 이상 격차가 있다.
2. **"모집단 대부분이 0.48과 멀리 떨어진 깊은 저점수 표본인가?"**
   → **그렇다**(DB 실측). 최근 3거래일 85.68%, 전체 이력 91.02%가
   `0.20~0.30` 구간에 몰려 있다 — threshold(0.48)까지 평균
   0.18~0.28 격차.
3. **"병목은 `0.48` 숫자 자체보다 모집단 품질에 있는가?"**
   → 이 실측만으로 "숫자 자체 vs 모집단 품질" 둘 중 하나로
   단정할 근거는 제한적이다(해석 영역). 다만 §105에서 이미 확인한
   대로 설계 문서 스스로 도입 시점(평균 0.24)에 threshold(0.48)와
   큰 격차를 인지했고, 그 격차가 지금(0.257~0.264)도 그대로
   유지되고 있다는 점에서 — **"숫자가 최근에 잘못 설정된 것"이
   아니라 "이 모집단 자체가 애초부터 threshold에 도달할 만한
   품질을 가져본 적이 없다"는 쪽에 더 가깝다.** 이는 threshold를
   조금 낮추는 문제가 아니라(§105.3에서 이미 shadow relax v2/v3/
   v5도 0% 통과 확인), 모집단 구성 자체의 문제일 가능성이 더
   크다는 뜻이다.

### 106.4 도입 시점 문서 문제의식 vs 현재 실측 정합성(1개 표로 비교)

| 항목 | 도입 시점(`[DESIGN] deterministic_trigger_eligibility_and_ranking_v1.md` §3.6, 2026-06-01~07-01, 1,034 symbol-day) | 현재(이번 턴 실측, 전체 이력 n=10,444) |
|---|---|---|
| 문제의식/실측 | "평균 ranking_score 약 0.24", "0.48→penalty→0.40 구조는 실측 bucket을 거의 살리지 못한다" | 평균 0.2568, `0.43~0.48` 구간 0건 |
| threshold와의 격차 | 문서 서술상 정성적으로 "크다" | 정량적으로 평균 0.223, 최댓값 기준도 0.063(단일 종목 예외 제외 시 0.10 이상) |
| 근접 표본 존재 여부 | 문서에 명시적 언급 없음(정성적 진단만) | **0건**(0.43~0.48 구간, 양 창 모두) |
| 제안된 대응 | `shadow_topk_exception_v2`(ranking_score≥0.22 + 신호/활동성/전략 조건) | 4주간 발동 이력 0건(§105.4에서 이미 확인) |

**정합성 판정**: 도입 시점 문서의 정성적 문제의식("이 구조는
실측 bucket을 거의 살리지 못한다")과 현재의 정량적 실측(근접
표본 0건, 격차 0.22 이상 상시 유지)은 **서로 어긋나지 않고
정합적으로 같은 방향을 가리킨다** — 문서가 우려했던 상황이 4주
넘게 정확히 그대로 지속되고 있다.

### 106.5 `0.48`의 실제 기능 판정(성급한 라벨 없이)

- **경계값으로 기능하는가**: 아니다 — 근접(±0.05) 표본이 최근
  3거래일·전체 이력 모두 0건이므로, 이 모집단 안에서 `0.48`은
  "일부는 넘고 일부는 못 넘는" 경계선 역할을 하고 있지 않다.
- **상시 봉쇄 상수에 가까운가**: 실측 수치(근접 표본 0건, 평균
  격차 0.22 이상, 4주간 지속)는 이 표현을 뒷받침한다. 다만 이는
  "`0.48`이라는 숫자 자체가 나쁘다"는 뜻이 아니라, **이 모집단이
  구조적으로 이 숫자에 도달할 능력이 없는 표본으로만 구성돼
  있다**는 뜻에 더 가깝다(§106.3의 답변과 동일선상).
- **모집단 품질 문제인가**: 근거가 이쪽에 더 실린다 — ranking
  상위 10개 조합에서도 신호(overall/slow)가 전혀 개선되지 않는다
  는 점(§106.2)이 이를 뒷받침한다.
- **Watch/Go/No-Go 라벨은 부여하지 않는다** — 이번 턴은 표본
  구조를 닫는 것까지가 목적이며, 완화 여부 판정은 범위 밖이다.

### 106.6 다음 우선 작업(완화안 아님, 후속 규명 과제로만 제시)

1. **1순위**: 이 모집단(core + bearish_trend 대상)이 애초에 왜
   신호 품질(overall/slow)이 항상 깊은 음수로만 나오는지 —
   `overall_score`/`slow_score` 자체의 계산 경로(§99~§101에서
   확인한 `atr_14_pct`/`bearish_trend` 판정과의 연결고리)를
   "모집단 구성" 관점에서 추가로 좁혀서 확인(완화안 설계 아님).
2. **2순위(이전 턴 이월, 방향 전환 유지)**: `high_volatility`
   단독 경로(001450형)의 층3(AI downgrade) 관찰 지속.
3. **3순위(후순위, 이전 턴 이월)**: `KIS_ENV` 실제 설정 확인,
   `max_cap` env 값 실제 반영/재기동/실측.
...
## §107. `ranking_score` 산식 구성요소 분해 — threshold vs 산식 vs 모집단(SPPV-2.120, 2026-07-28 KST)

**전제**: `0.48`을 바로 낮추자는 결론을 내지 않는다. 산식 구성
요소를 실제 코드 기준으로 분해해 "threshold 재설정" vs "산식
재검토" vs "모집단 재정의" 중 무엇이 먼저인지만 닫는다. 코드
미수정, threshold/diff/완화안 제안 없음, `.env` 미수정, Full
pytest 미실행, 신규 KIS 호출 0건.

### 107.1 `ranking_score` 실제 코드 산식(read-only 코드 확인)

`deterministic_trigger_engine.py`의 `_build_buy_ranking_score()`
(BUY 경로 유일 함수, `_assess_core_risk_off_buy_guard`에 전달되는
`ranking_score`와 완전히 동일한 값)의 실제 공식은 **§7.2 설계
문서 제안식과 다르다**:
...
- `entry_score`: 별도 계산된 진입 점수(0~1 clamp).
- `relative_activity`: `max(volume_surge_ratio, turnover_surge_
  ratio)`를 1.0→0, 3.0→1로 선형 정규화.
- `coverage_score`: feature coverage 비율(0~1).
- `allocation_quality`: `portfolio_allocation.max_new_capital_
  pct / 10.0`(clamp).
- `regime_tailwind`: `bullish_trend+risk_on`이면 1.0, `risk_off`
  면 **0.0**, 그 외 0.5(**이 게이트 모집단은 정의상 항상
  `risk_off`이므로 이 항은 100% 결정적으로 0**).
- `strategy_alignment`: `preferred_strategy ∈ {swing_momentum,
  event_continuation}`이면 1.0, 아니면 0.0(**이 모집단은
  `defensive_low_volatility_rotation`으로 고정돼 있어 이 항도
  100% 결정적으로 0**, §102.2에서 이미 확인).

### 107.2 구성요소 분포표(최근 3거래일 / 전체 이력, DB 실측 + 코드
공식 재계산 — 재계산값이 저장값과 소수 4자리까지 정확히 일치함을
확인, 계산 오류 없음)

| 구성요소(원값) | 최근 3거래일(n=2,703) mean/median/min/max | 전체 이력(n=11,971) mean/median/min/max |
|---|---|---|
| `entry_score` | 0.0653 / 0.0546 / 0.0000 / 0.2123 | 0.0485 / 0.0320 / 0.0000 / 0.2479 |
| `relative_activity` | 0.0339 / **0.0000** / 0.0000 / 0.4806 | 0.0351 / **0.0000** / 0.0000 / 0.6830 |
| `coverage_score` | **1.0000 / 1.0000 / 1.0000 / 1.0000** | **1.0000 / 1.0000 / 1.0000 / 1.0000** |
| `allocation_quality` | **0.2500 / 0.2500 / 0.2500 / 0.2500** | **0.2500 / 0.2500 / 0.2500 / 0.2500** |
| `regime_tailwind` | **0.0000(전수)** | **0.0000(전수)** |
| `strategy_alignment` | **0.0000(전수)** | **0.0000(전수)** |

| 가중 기여분(=원값×weight) | 최근 3거래일 mean | 전체 이력 mean |
|---|---:|---:|
| `entry_score`(w=0.55) | 0.0359 | 0.0267 |
| `relative_activity`(w=0.10) | 0.0034 | 0.0035 |
| `coverage_score`(w=0.20) | **0.2000(고정)** | **0.2000(고정)** |
| `allocation_quality`(w=0.10) | **0.0250(고정)** | **0.0250(고정)** |
| `regime_tailwind`(w=0.03) | 0(고정) | 0(고정) |
| `strategy_alignment`(w=0.02) | 0(고정) | 0(고정) |
| **합계(=ranking_score 평균)** | **0.2643** | **0.2552** |

### 107.3 핵심 질문 3개에 대한 답(측정, 해석과 분리)

1. **"낮은 주된 원인이 entry_score 자체 저점인가?"** → 부분적으로
   그렇다(사실). `entry_score`는 이 모집단에서 평균 0.05~0.065,
   최댓값도 관측 전체 이력에서 0.2479를 넘은 적이 없다 — 가중
   기여분(0.55×)으로도 평균 0.027~0.036에 불과해, 전체 ranking_
   score(0.255~0.264)의 약 10~14%만 담당한다.
2. **"regime_tailwind=0 같은 특정 항목이 구조적으로 깎아먹는가?"**
   → 그렇다(사실, 정량화). `regime_tailwind`와 `strategy_
   alignment` **두 항목은 이 모집단(정의상 `risk_off`+방어적
   전략 고정) 안에서 예외 없이 100% 0.0으로 고정**된다 — 원값의
   분산이 전혀 없다(min=max=0). 이 두 항목의 최대 가능 기여분은
   각각 0.03/0.02, 합쳐서 **0.05**에 불과해 절대적 크기는 작지만,
   **이 모집단에 속하는 한 이 0.05는 결코 회복될 수 없는 항목**
   이다(정의상 결정적).
3. **"모집단 정의 자체가 이미 나쁜 표본만 모으는가?"** → 그렇다
   (사실, §107.4에서 정량화). `coverage_score`(1.0 고정)와
   `allocation_quality`(0.25 고정) 두 항목도 **분산이 전혀 없다**
   (min=max=coverage 1.0, allocation 0.25) — 이 모집단 안에서는
   품질을 구분하는 데 전혀 기여하지 않는 "고정 바닥"이다.

### 107.4 이 모집단의 구조적 도달 가능 상한(사실 계산, 완화안 아님)

이 게이트에 들어오는 모집단은 정의상 **`regime_tailwind=0`,
`strategy_alignment=0`이 항상 고정**된다(§107.1 조건 확인).
따라서 이 모집단 안에서 이론상 도달 가능한 `ranking_score`
상한은:

```
상한 = 0.20(coverage 고정) + 0.10(allocation 고정 관측 0.25×0.10=0.025 아님 — 아래 재계산)
```

정확히는(관측된 고정값 기준):

```
상한(관측 고정값 기준) = 0.20(coverage=1.0×0.20)
                        + 0.025(allocation=0.25×0.10)
                        + 0.55×entry_score_max
                        + 0.10×relative_activity_max
                        + 0(regime_tailwind, 항상 0)
                        + 0(strategy_alignment, 항상 0)
```

**전체 이력에서 관측된 `entry_score` 최댓값(0.2479)과
`relative_activity` 최댓값(0.6830, 단 이 둘이 동시에 최댓값인
사례는 없었음)을 대입하면**:

```
0.20 + 0.025 + 0.55×0.2479 + 0.10×0.6830
= 0.225 + 0.1363 + 0.0683
= 0.4296
```

**즉, 관측된 개별 항목 최댓값을 모두 동시에 적용해도(실제로는
동시에 나타난 적이 없는 이론적 상한) `0.4296`으로, `0.48`에
도달하지 못한다.** 실제 관측된 단일 레코드 최댓값(`002790`,
`0.4166`)도 이 이론적 상한 근처에서 형성됐다(§106.2에서 이미
확인).

### 107.5 상위 20건 상세표(전체 이력, 구성요소 전부 포함)

전체 20건 전부 동일 종목(`002790`, 2026-07-03, §106.2에서 이미
확인한 반복 사이클)이며 구성요소도 전부 동일하다:
`entry_score=0.2479`, `relative_activity=0.5527`,
`coverage_score=1.0`, `allocation_quality=0.25`,
`regime_tailwind=0.0`, `strategy_alignment=0.0`,
`ranking_score=0.4166`(재계산값과 소수 4자리까지 정확히 일치),
`shadow_overall_score=-0.2539`, `shadow_slow_score=-0.66`.

**distinct 종목 기준으로 확장해도** 전체 이력에서 `entry_score`가
0.2479를 넘은 레코드는 없었고, `relative_activity`가 0.6830을
넘은 레코드도 마찬가지로 이 게이트의 authoritative 하드 차단이
걸린 population 안에서는 관측되지 않았다.

### 107.6 "이론상 경계값 후보 구간"은 어디인가(참고, threshold
변경 제안 아님)

관측 데이터상 이 모집단의 `ranking_score`는 **`0.22~0.42`**
범위에 실질적으로 갇혀 있다(전체 이력 min 0.225, max 0.4166) —
상위권도 `0.33~0.42` 수준에 머물 뿐, `0.43` 이상으로는 단 한
건도 올라간 적이 없다(§106.1 구간표 재확인: `0.43~0.48` 0건).
이는 "상위권이 threshold 바로 아래 어딘가에 몰려 있다"는 뜻이
아니라, **상위권조차 이 모집단의 구조적 상한(§107.4, 약
0.43)을 넘지 못하는 상태**라는 뜻이다.

### 107.7 최종 판정 — threshold vs 산식 vs 모집단

**1순위 원인: 산식(ranking_score 공식) 구조 문제.** 이 모집단은
정의상(`risk_off`) `regime_tailwind`와 `strategy_alignment`
두 항목이 항상 0으로 고정되며, 이 두 항목만으로도 이론상 최대
가능 점수를 `0.48` 미만으로 끌어내리는 데 기여한다(§107.4의
상한 계산 `0.4296`은 이 두 항목이 온전했다면 최대 `0.05` 더
높아질 수 있었던 상한이며, 그래도 여전히 `0.4796`으로 `0.48`
미만이다 — **즉 이 두 항목만 살려도 threshold에 도달하지 못한다**
는 점이 산식 문제를 1순위로 지목하는 핵심 근거다).

**2순위 원인: 모집단 정의 문제.** `coverage_score`(1.0)와
`allocation_quality`(0.25)가 이 모집단 안에서 완전히 무분산
(고정)이라는 사실은, 이 두 항목이 애초에 "이 모집단 내부에서
후보를 가르는 축"으로 설계되지 않았다는 것을 보여준다 — 실질적
변별력은 `entry_score`(관측 상한 0.2479)와 `relative_activity`
(관측 상한 0.6830) 두 항목에만 있는데, 이 둘의 관측 상한을 모두
결합해도 threshold 미달이다. 이는 이 모집단(`core`+`risk_off`+
`bearish_trend`)이 §99~§101에서 이미 확인한 상시적 저신호
조건에서 뽑히는 후보군이라, 애초에 `entry_score`가 구조적으로
낮게 형성될 수밖에 없는 모집단이라는 뜻이다.

**threshold(`0.48`) 자체의 재측정은 3순위(또는 근본 원인이
아님)**: `0.48`을 낮추는 것은 이 두 구조적 문제(고정 항목의
결정적 0, 모집단 자체의 저entry_score)를 그대로 둔 채 문턱만
움직이는 것이므로, 산식/모집단 문제를 먼저 짚지 않고 threshold만
재측정하면 근본 원인을 건드리지 못한 채 임시방편이 될 위험이
크다(해석, 완화안 아님 — 이번 턴은 결론 판정만).

### 107.8 다음 우선 작업(완화안 아님, 후속 규명 과제로만 제시)

1. **1순위**: `regime_tailwind`/`strategy_alignment`가
   `risk_off`/방어적 전략 상태에서 항상 0으로 고정되는 것이
   설계 의도(risk_off에는 어떤 tailwind도 주지 않겠다는 명시적
   결정)인지, 단순 부산물(다른 목적으로 설계된 계단함수가 이
   상태에서 우연히 항상 0을 반환)인지 코드/설계 문서에서 추가로
   확인(완화안 설계 아님, 의도 확인까지만).
2. **2순위**: `entry_score`가 이 모집단에서 구조적으로 낮게
   형성되는 경로를 `overall_score`/`slow_score`(§99~§101에서
   이미 확인한 `atr_14_pct` 연쇄)와 연결해 재확인 — 이 모집단
...
## §110. 산식 재검토(1순위) + 중복 차단 정리(2순위) 관점 항목별 분류(SPPV-2.122, 2026-07-28 KST)

**전제**: `0.48` threshold는 바꾸지 않는다. §109에서 이미 확인한
사실을 기반으로, 6개 구성항목을 "유지 가치 높음 / 역할 축소·
재정의 후보 / 다른 장치와 중복돼 재검토 필요" 3분류로 나눈다.
코드 미수정, diff/threshold/완화안 구현 없음. 이번 턴은 §109에서
이미 수집된 사실을 재분류·종합하는 턴이며, 별도 신규 DB 조회는
최소화했다(재조회가 필요한 부분만 §109 인용, 신규 KIS 호출 0건).

### 110.0 분류 원칙(사실 vs 해석 vs 미확정)

- **사실**: §106/§107/§109에서 DB 실측 또는 코드 read로 이미
  확정된 수치·코드 경로.
- **해석**: 그 사실들을 근거로 이번 턴에서 내리는 종합 판단(예:
  "정당한 역할 분리"/"과잉 중복").
- **미확정**: 이 모집단(`core`+`risk_off`+`bearish_trend`) 밖의
  일반 BUY 경로에서도 같은 패턴이 나타나는지는 확인하지 않았다
  — 이번 턴 범위는 이 특정 모집단으로 한정한다.

### 110.1 `entry_score`

- **ranking 안에서의 역할**: 가중치 0.55(6개 중 최대). `_build_
  entry_score()`가 산출한 값을 그대로 재사용(사실, §107.1).
- **다른 BUY 차단 장치에서의 역할**: (a) `buy_candidate_
  threshold=0.65`로 `entry_score` 자체를 하드 게이트(사실,
  §109.4.1). (b) `entry_score` 내부에 이미 `overall`/`fast`/
  `slow` 가중합 + `risk_off` 페널티(−0.15) + 전략 보너스(+0.05)
  + activity 보너스(최대 +0.10)가 합성돼 있음(사실, §109.4.1).
- **실측 설명력**: 이 모집단의 상위-하위 그룹 `ranking_score`
  차이(0.1916)의 **71%(0.1363)**를 `entry_score` 기여분이 담당
  (사실, §109.3).
- **중복 판정(해석)**: threshold(0.65, 후보 생성)와 ranking
  (0.55×, 순위화)은 **역할이 다른 순차 구조** — 정당한 분리로
  본다. 다만 `entry_score` 내부에 이미 있는 `risk_off`/
  `strategy`/`activity` 하위 항목이 ranking에서 **별도 항으로
  다시** 등장하는 것은 `entry_score` 자체의 문제가 아니라 그
  하위 항목들(§110.2/§110.4/§110.5) 쪽의 문제로 분리해서 본다.
- **분류: 유지 가치 높음.** 최종 판정: **즉시 유지**(entry_score
  항목 자체, 6.55 가중치 포함).

### 110.2 `relative_activity`

- **ranking 안에서의 역할**: 가중치 0.10. `max(volume_surge_
  ratio, turnover_surge_ratio)`를 1.0→0, 3.0→1로 정규화(사실,
  §107.1).
- **다른 BUY 차단 장치에서의 역할**: 동일한 원시 신호가 **네
  곳**에서 쓰인다(사실, §109.4.2) — ① `entry_score` 내부
  activity 보너스(최대 +0.10, ranking과 완전히 같은 방향·같은
  원시 신호의 소프트 가산 중복) ② ranking의 `relative_activity`
  항 ③ `eligibility_low_relative_activity`(<1.10 하드 차단)
  ④ `_assess_core_risk_off_buy_guard`의 활동성 게이트(1.20,
  top-k override 시 1.10).
- **실측 설명력**: 상위-하위 차이의 **29%(0.0553)**를 담당(사실,
  §109.3). 이 모집단 median은 0.0000(사실, §107.2) — 대부분
  레코드에서 이 항목 자체가 활동성 부족으로 이미 0에 머문다.
- **중복 판정(해석)**: ①②는 **같은 방향·같은 원시 신호의 순수
  가산 중복**(entry_score 안에서 이미 반영된 신호를 ranking에서
  한 번 더 가산) — "역할이 다른 분리"로 보기 어렵다. ③④는 질적
  으로 다른 하드 게이트(실행 가능성 확인)라 역할이 다르지만,
  이미 ①②에서 같은 신호를 두 번 가산 평가한 뒤 ③④에서 다시
  이진 차단하는 것은 "같은 약점이 네 겹으로 반영되는 구조"에
  가깝다(§109.4.2 인용).
- **분류: 다른 장치와 중복돼 재검토 필요.** 최종 판정: **중복
  제거/정리 검토**(①②의 소프트 가산 중복이 1차 대상, ③④의
  하드 게이트 다중화는 2차 대상).

### 110.3 `coverage_score`

- **ranking 안에서의 역할**: 가중치 0.20(6개 중 최대) — 그러나
  이 모집단 안에서는 **완전 무분산**(min=max=1.0, 사실, §107.2).
- **다른 BUY 차단 장치에서의 역할**: `eligibility_low_feature_
  coverage`(<0.50 하드 차단, 사실, §109.4.3).
- **실측 설명력**: 0(이 모집단 안에서 상위-하위 그룹 모두 동일
  값, 사실, §109.3).
- **중복 판정(해석+미확정)**: 이 게이트에 도달한 표본은 이미
  coverage 하드 차단(<0.50)을 통과했으므로, "이 모집단 안에서
  변별력이 0"이라는 사실 자체가 곧 과잉 중복을 증명하지는
  않는다 — **하드 게이트 통과 이후 좁아진 분포의 자연스러운
  결과일 수도 있다(미확정, 일반 모집단 대조 필요)**. 다만 가장
  큰 가중치(0.20)가 이 특정 모집단(=이 하드 게이트가 실제로
  작동하는 맥락)에서 항상 변별력 0이라는 사실은, 적어도 **이
  가중치 배분이 이 맥락에서는 설명력에 비례하지 않는다**는
  것을 뒷받침한다(사실 기반 해석).
- **분류: 역할 축소/재정의 후보.** 최종 판정: **역할 축소
  검토**(가중치 축소 또는 "이미 하드 게이트를 통과한 후보 안에서
  는 자동으로 낮은 가중치를 갖도록" 조건부 재정의 검토 대상 —
  이번 턴은 후보 지정까지만, 구현 아님).

### 110.4 `regime`(=`regime_tailwind` + `entry_score`의 `risk_off`
페널티 + eligibility/core guard 하드 게이트)

- **ranking 안에서의 역할**: `regime_tailwind` 가중치 0.03(6개
  중 최소) — 이 모집단(정의상 `risk_off`)에서는 **100% 고정
  0.0**(사실, §107.2).
- **다른 BUY 차단 장치에서의 역할**: ① `entry_score` 내부
  `risk_off` 페널티(−0.15, 사실) ② `_assess_buy_eligibility` +
  `_assess_core_risk_off_buy_guard`의 하드 게이트(사실, §102~
  §104에서 이미 확인한 대로 이 게이트가 전체 차단의 59.5%를
  차지하는 **실질적 병목**).
- **실측 설명력**: ranking 항목(0.03)은 0. `entry_score` 내부
  페널티(−0.15)는 이 모집단 전원에게 동일하게 적용되는 **상수
  차감**(무분산, 변별력 없음). 실질적 설명력은 전부 ③ 하드
  게이트에 있다(사실, §102~§104 인용).
- **중복 판정(해석)**: ①②(ranking+entry_score 소프트 항)는
  같은 방향의 소프트 중복이지만 절대 크기가 작다(§107.4에서
  이미 계산: 이 두 항목을 모두 "정상" 값으로 되돌려도 이론적
  상한이 0.4796으로 threshold 미달 — 즉 이 소프트 중복이
  threshold 미달의 원인이 아니다). ③ 하드 게이트는 질적으로
  다른 역할(실질 차단)이며, §102~§104에서 이미 "정당한 반영인지
  과잉 방어인지 미확정"으로 판정된 상태다(원문 인용, 이번 턴에서
  재확정하지 않음).
- **분류**: ranking `regime_tailwind` 항목 자체는 **역할 축소/
  재정의 후보**(이 모집단에서 항상 죽어 있는 항). `entry_score`
  의 페널티는 **다른 장치와 중복되지만 저위험**(크기가 작아
  실질 영향 미미). 하드 게이트는 **이번 턴 분류 대상 아님**(§102
  ~§104의 별도 판정을 유지).
- 최종 판정: ranking 항목 → **역할 축소 검토**. entry_score
  페널티 → **중복 제거/정리 검토(저위험, 낮은 우선순위)**.

### 110.5 `strategy_alignment`

- **ranking 안에서의 역할**: 가중치 0.02(6개 중 최소) — 이
  모집단에서 **100% 고정 0.0**(사실, §107.2).
- **다른 BUY 차단 장치에서의 역할**: ① `entry_score` 내부
  전략 보너스(+0.05) — **`preferred_strategy ∈ {swing_
  momentum, event_continuation}`, ranking의 조건과 완전히
  동일**(사실, §109.4.5). ② `_assess_core_risk_off_buy_guard`의
  전략 화이트리스트(`defensive_low_volatility_rotation`,
  `mean_reversion_bounce`, `event_continuation`) — **다른 집합**
  을 쓰는 별도 하드 게이트.
- **실측 설명력**: 0(이 모집단에서 `preferred_strategy`가 항상
  `defensive_low_volatility_rotation`으로 고정돼 ①②의 (①의)
  대상 집합에 결코 속하지 않음, 사실, §102.2/§107.2).
- **중복 판정(해석)**: ①은 ranking과 **조건 집합이 문자 그대로
  동일한 순수 중복**(가장 명확한 중복 사례) — 이 모집단에서는
  조건이 결코 만족되지 않아 실해악은 없지만, `risk_on`/
  `bullish_trend` 등 다른 국면에서 `swing_momentum` 전략이
  실제로 선택되는 경우에는 이 중복이 실제로 발동해 이중 가산될
  가능성이 있다(**미확정** — 이 모집단 밖 검증 필요). ②는 다른
  집합을 쓰는 별도 하드 게이트라 역할이 다르다.
- **분류: 다른 장치와 중복돼 재검토 필요.** 최종 판정: **중복
  제거/정리 검토**(①이 대상, 이 모집단 내 실해악은 없으나
  구조적으로는 순수 중복이므로 정리 검토 가치가 있음).

### 110.6 산식 재검토 관점 1차 결론(Q1)

- **유지할 항목**: `entry_score`(0.55) — 가장 큰 실측 설명력을
  가지며 threshold(0.65)와의 역할 분리도 정당하다.
- **축소/재정의 후보**: `coverage_score`(0.20, 이 모집단 내
  무분산에도 가장 큰 가중치를 가짐), `regime_tailwind`(0.03,
  이 모집단 내 항상 죽어 있는 항). 두 항목 모두 "6개 항목 중
  실제 변별력 있는 항목은 `entry_score`/`relative_activity`
  2개뿐"이라는 §109.2의 사실과 직결된다.
- **미확정으로 남기는 것**: `coverage_score`/`allocation_
  quality`가 이 모집단 밖의 일반 BUY 후보군에서도 무분산인지는
  검증하지 않았다 — "산식 자체가 원래 불필요한 항목을 갖고
  있다"와 "이 특정 맥락에서만 우연히 무분산이다"를 구분하려면
  추가 검증이 필요하다(§109.8 1순위 후속 과제와 동일선상).

### 110.7 중복 차단 정리 관점 1차 결론(Q2)

| 항목 | 반영 단계 수 | 중복 성격 | 판정 |
|---|---:|---|---|
| `entry_score` | threshold(1) + ranking(1) | 역할 분리(순차) | **정당** |
| `relative_activity` | entry(1)+ranking(1)+eligibility(1)+core guard(1) = 4 | 소프트 2중 + 하드 2중 | **과잉 가능성 높음** |
| `coverage_score` | ranking(1)+eligibility(1) | 하드 게이트 정당, ranking 쪽 무의미 | **미확정(일반 모집단 대조 필요)** |
| `regime`(risk_off) | entry(1)+ranking(1)+eligibility/guard(1) = 3 | 소프트 중복(저위험) + 하드 게이트(별도 판정 유지) | 소프트: **과잉 가능성 있으나 저위험** / 하드: **미확정(§102~§104 유지)** |
| `strategy_alignment` | entry(1)+ranking(1, 완전 동일 조건)+core guard(1, 다른 집합) | entry+ranking 순수 중복 | **과잉 가능성 높음(이 모집단 내 실해악 없음)** |

**정당한 중복**: `entry_score`의 threshold+ranking 역할 분리.

**과잉 가능성 높은 중복**: `relative_activity`(4겹 중 소프트
2중), `strategy_alignment`(entry+ranking 완전 동일 조건).

**아직 미확정인 중복**: `coverage_score`(ranking 가중치가 이
모집단에서만 무의미한지, 일반적으로 무의미한지), `regime`의
하드 게이트 부분(§102~§104의 "정당/과잉 미확정" 판정을 그대로
유지).

### 110.8 최종 판정

**다음 단계는 "산식 재설계 준비"와 "중복 차단 정리 설계" 둘 다
후보에 오르지만, 순서상 `coverage_score`/`regime_tailwind`의
역할 재정의(산식 쪽)가 `relative_activity`/`strategy_alignment`
의 중복 제거(중복 쪽)보다 먼저다** — `coverage_score`가 가장 큰
가중치(0.20)를 차지하면서 이 병목 맥락에서 무의미하다는 사실이,
가중치 배분 자체의 구조적 문제를 가장 크고 직접적으로 보여주기
때문이다(§109.7의 1순위 판정과 정합). 단, **이번 턴은 설계
진단·분류까지만이며, 산식 재설계나 중복 제거의 구체적 diff는
다음 턴 이후 별도 승인을 받아 진행한다.**

### 110.9 다음 우선 작업(완화안/diff 아님, 후속 규명 과제로만 제시)

1. **1순위**: `coverage_score`/`allocation_quality`가 이 특정
   모집단(`core`+`risk_off`+`bearish_trend`) 밖의 일반 BUY 후보
   군(예: `risk_on`/`bullish_trend` 국면의 core 후보)에서도
   무분산인지 확인(완화안 설계 아님, 일반성 확인까지만).
2. **2순위**: `relative_activity`/`strategy_alignment`의 entry_
   score↔ranking 중복이 이 모집단 밖(예: `risk_on`+`swing_
   momentum` 실제 선택 사례)에서 실제로 이중 가산 효과를 내는지
   실측 사례 탐색.
3. **3순위(이전 턴 이월)**: `high_volatility` 단독 경로(001450형)
   층3 관찰 지속.
...
## §111. `ranking_score` 검증 트랙 체크리스트 재판정 + 일반 모집단 대조(SPPV-2.123, 2026-07-28 KST)

**전제**: 이번 턴은 새 분석이 아니라, `[PLAN] ranking_score_
formula_validation.md` §6의 "완료" 표시가 실제 검증 강도와
일치하는지 재검증하는 감사(audit) 턴이다. 코드 미수정, threshold/
diff/완화안 없음, 신규 KIS 호출 0건.

### 111.1 재판정 결과 요약

| 체크리스트 | 이전 표시 | 재판정 | 핵심 이유 |
|---|---|---|---|
| §6.1(임계치) | 완료 | **실제 완료** | 구간 카운트는 표본 반복과 무관한 집단 전체 집계 |
| §6.2(구성항목) | 완료 | **실제 완료(격상)** | 이번 턴 일반 모집단 대조로 미확정이던 4개 항목의 일반성을 전부 닫음 |
| §6.3(가중치) | 완료 | **부분 완료(하향)** | "상위 50건"이 실제로는 단일 종목(`002790`) 반복, "하위 50건"도 5종목 중 1개가 70% — 방향성은 유지되나 표본 다양성 부족 |
| §6.4(중복) | 완료 | **부분 완료(하향)** | "중복 존재 확인"은 코드 기반으로 견고, "정당성 최종판정"은 `coverage_score`/`regime` 하드 게이트 2개가 여전히 열려 있음 |
| §6.5(문서반영) | 완료 | **실제 완료** | 절차적으로 매 턴 반영 확인 |
| §6.6(최종기준) | 완료 | **실제 완료(재확인)** | 이번 턴 보정을 반영해 "추정과 실측 분리" 기준까지 충족 |

### 111.2 신규 확인 — 일반 모집단 대조(트랙 E, §6.7)

같은 3거래일 창(07-24/27/28 KST)에서 이 게이트(`core_risk_off_
ranking_blocked`) 필터를 걷어낸 **일반 모집단**(n=4,510, 전체
regime/source_type 포함)을 조회했다(사실, DB 재조회).

- `coverage_score`: 일반 모집단에서도 **100% 1.0**(사실) —
  §110.3의 "미확정" 판정을 **"일반적으로도 무분산"으로 확정**한다.
- `allocation_quality`: 일반 모집단에서는 **2.5(3,191건)/3.0
  (1,319건) 두 값으로 분산**(사실) — 이 게이트 안에서만 우연히
  `0.25`로 고정됐던 것이며, §110.4의 "저위험" 판정과는 별개로
  이 항목 자체의 무분산은 **일반적 현상이 아니다**(정정).
- `regime_tailwind`(`risk_tone`): 일반 모집단에서도 **100%
  risk_off**(사실, §99의 시장 전체 상시 risk_off와 정합) —
  "일반적으로도 무분산"으로 확정한다.
- `strategy_alignment`(`preferred_strategy`): 일반 모집단에서는
  `event_continuation`이 **4.8%(217/4,510건)** 존재(사실) — 이
  게이트 안에서만 우연히 `defensive_low_volatility_rotation`
  으로 고정됐던 것이며, **다른 국면에서는 `entry_score`+
  `ranking_score`의 `strategy_alignment` 중복이 실제로 동시
  발동할 수 있다**(정정 — 이전 "이 모집단 내 실해악 없음"은
  유지되나 "일반적으로도 무해"라는 함의는 철회).

### 111.3 신규 확인 — 표본 반복성 보정(트랙 F, §6.8, 부분 착수)

- `raw_ranking_score` 상위 50건: **distinct symbol 1개**
  (`002790`만 50회 반복, 사실) — §109.3/§110의 "상위 50건 평균
  기여도" 수치는 사실상 이 한 종목의 특성을 50번 계산한 것과
  같다.
- 하위 50건: distinct symbol 5개, 그중 `000720`이 35/50(70%)을
  차지(사실).
- distinct symbol 기준(종목당 최고 `ranking_score` 1개, n=25)
  재계산: 상위 10개 종목 `0.30~0.42`, 하위 10개 종목 `0.22~
  0.27`(사실) — **방향성(entry_score/relative_activity가 유일한
  변동 원천)은 유지**되나, 표본 크기가 25로 작아 "완전히 닫혔다"
  고 보기는 이르다(해석).

### 111.4 최종 판정에 대한 영향

§107.7/§110.8의 "1순위=산식 재검토, 2순위=중복 차단 정리"
판정은 **표본 반복 편향과 무관하게 유지된다** — 이 판정의 핵심
근거(`coverage_score`가 가장 큰 가중치를 가지면서 변별력이
없다)는 이번 턴에 **일반 모집단 기준으로도 확정**됐으므로 오히려
근거가 더 강해졌다. 다만 "상위/하위 표본 기반 기여도 수치"는
다음 턴에 distinct-symbol 기준으로 재계산이 필요하다(§6.8 미완료
항목).

### 111.5 다음 우선 작업(완화안 아님, §6.8 잔여 항목 + 후속 규명)

1. **1순위(§6.8 잔여)**: distinct symbol 기준으로 구성요소
   기여도(entry_score/relative_activity)를 재계산해 §109.3의
   "상위 50건 평균 기여도" 수치를 종목 중복 제거 버전으로 교체.
2. **2순위(§6.8 잔여)**: `002790`/`000720` 등 특정 종목이 같은
   기간 반복 등장하는 이유(스케줄러 재평가 주기 특성인지 종목
   특이성인지)를 원인만 확인(완화안 아님).
3. **3순위(이전 턴 이월)**: `high_volatility` 단독 경로(001450형)
   층3 관찰 지속.
...
## §112. `allocation_quality` 일반 모집단 분산 재검증 — §111의 "전항 확정" 과대해석 정정(SPPV-2.124, 2026-07-28 KST)

**전제**: §111(SPPV-2.123)이 "3거래일 일반 모집단"만 조회하고
이를 "일반적" 결론으로 확대한 부분을 이번 턴에 전체 이력까지
넓혀 재검증한다. 코드 미수정, threshold/diff/완화안 없음, 신규
KIS 호출 0건.

### 112.1 모집단 정의 명확화(사실)

- **3거래일 일반 모집단**: `trade_decisions`에서 `created_at`을
  KST로 환산한 날짜가 `2026-07-24`/`2026-07-27`/`2026-07-28`인
  전체 레코드(게이트 필터 없음), n=4,510.
- **전체 이력 일반 모집단**: `trade_decisions` 테이블 전체(시간
  범위 제한 없음, 게이트 필터 없음), n=68,724.
- 이 둘을 혼용해 "일반 모집단"이라고만 쓴 것이 §111의 정확성
  문제였다 — 이번 턴부터 항상 둘을 구분해서 표기한다.

### 112.2 `allocation_quality`(`max_new_capital_pct`) 재검증

| 모집단 | distinct 값 개수 | 대표 값 |
|---|---:|---|
| `ranking_blocked` 게이트 내부(n=2,703) | **1** | `2.5`(100%) |
| 3거래일 일반 모집단(n=4,510) | **2** | `2.5`(3,191건)/`3.0`(1,319건) |
| 전체 이력 일반 모집단(n=68,724) | **1,929** | `0.0`/`1.5459`/`2.4123`/.../`4.0`/`5.0` 등 풍부한 연속 분산 |

**0이 아닌 값 존재 여부**: 그렇다(사실, 예: `2.5`, `3.0` 등
다수). **최종 판정: 확정**(전체 이력 기준 명백한 풍부한 분산) —
다만 §111이 근거로 든 "3거래일 창 2값"이라는 서술은 분산의
정도를 과소하게 보여줬을 뿐 방향 자체는 맞았다(정정, 완전히
틀린 것은 아님).

### 112.3 나머지 3개 항목과의 비교표(전체 이력까지 확장 재검증)

| 항목 | 게이트 내부 | 3거래일 일반 모집단 | 전체 이력 일반 모집단 | 최종 판정 |
|---|---|---|---|---|
| `coverage_score` | 100% 1.0 | 100% 1.0 | distinct 2값(1.0: 35,873건, `0.1429`: 725건) | **부분 확정** |
| `allocation_quality` | 100% 0.25 | 2값(0.25/0.30) | distinct **1,929값** | **확정**(전체 이력 기준) |
| `regime_tailwind`(`risk_tone`) | 100% risk_off | 100% risk_off | `risk_off` 36,433/`None` 32,017/`risk_on` **42**/`neutral` **232** | **부분 확정** |
| `strategy_alignment` | 100% 미해당(0) | `event_continuation` 4.8% | `{swing_momentum,event_continuation}` 합계 3.7% | **부분 확정** |

**핵심 정정**: §111(SPPV-2.123)이 "일반 모집단 대조로 미확정
4개를 모두 닫았다"고 서술한 것은 **과했다**. 정확한 결론은:
`allocation_quality` **1개만 "확정"**(풍부한 분산), 나머지 3개
(`coverage_score`/`regime_tailwind`/`strategy_alignment`)는
"최근 관측 창에서는 거의 항상 무분산이지만 전체 이력 기준으로는
드문 예외가 존재"하는 **"부분 확정"**이다.

### 112.4 §6.2/§6.7 체크리스트 재보정 요약

`[PLAN] ranking_score_formula_validation.md`의 §6.2를 "실제
완료 → 부분 완료"로 되돌리고, §6.7(트랙 E)을 전체 이력까지
확장한 재검증 결과로 갱신했다(문서 자체 참고, 이력 보존형 정정).

### 112.5 최종 판정에 대한 영향

§107.7/§110.8/§111.4의 "1순위=산식 재검토, 2순위=중복 차단
정리" 판정은 **이번 정정과 무관하게 유지된다** — 이 판정의
핵심 근거는 "이 게이트 모집단 내부에서 `coverage_score`가
무분산"이라는 사실(§107/§109)이며, 이는 관측 창을 어떻게
잡아도 변하지 않는다. 다만 "그 근거가 일반 모집단에서도
보편적으로 성립한다"는 확장 서술은 3개 항목에서 과했으므로,
근거의 범위를 "이 게이트 모집단 내부"로 좁혀서 유지한다(더
강해진 것이 아니라, 애초에 그 정도로도 충분했다는 재확인).

### 112.6 다음 우선 작업(완화안 아님, 후속 규명 과제로만 제시)
...
## §115. distinct symbol 기준 기여도 재계산 + 반복 등장 원인 규명(SPPV-2.127, 2026-07-29 KST)

**전제**: `[PLAN] ranking_score_formula_validation.md` §6.8의
잔여 2개 미완료 항목을 닫는다. 코드 미수정, threshold/diff/
완화안 없음, Full pytest 미실행, 신규 KIS 호출 0건, DB/코드
read-only만 사용.

### 115.1 distinct symbol 기준 기여도 재계산(§6.8 잔여 1)

두 모집단으로 분리했다.

**(A) `eligibility_core_risk_off_ranking_blocked` 게이트 모집단
내부(distinct symbol=25, 종목당 최고/최저 `ranking_score` 대표
값 1개씩)**:

| 그룹 | entry 기여 | activity 기여 | coverage 기여 | alloc 기여 |
|---|---:|---:|---:|---:|
| 상위 10개 종목 | 0.1192 | 0.0305 | 0.2000 | 0.0250 |
| 하위 10개 종목 | 0.0000 | 0.0000 | 0.2000 | 0.0250 |
| 차이 | 0.1192 | 0.0305 | 0 | 0 |

차이 합계 0.1497 중 `entry_score`+`relative_activity`가
**100.0%**를 설명한다.

**(B) 전체 이력 BUY 경로 전체(`deterministic_trigger.metadata.
eligibility_path=='buy'`인 전체 레코드만 필터 — SELL/EXIT 경로는
별도 산식(`_build_exit_ranking_score`)을 쓰므로 혼선을 피하기
위해 제외, distinct symbol=105, row 수=35,149)**:

| 그룹 | entry 기여 | activity 기여 | coverage 기여 | alloc 기여 | regime 기여 | strategy 기여 |
|---|---:|---:|---:|---:|---:|---:|
| 상위 10개 종목 | 0.3567 | 0.0207 | 0.2000 | 0.0310 | 0.0030 | 0.0060 |
| 하위 10개 종목 | 0.0000 | 0.0000 | 0.2000 | 0.0250 | 0.0000 | 0.0000 |
| 차이 | 0.3567 | 0.0207 | 0 | 0.0060 | 0.0030 | 0.0060 |

차이 합계 0.3924 중 `entry_score`+`relative_activity`가
**96.2%**를 설명한다. `coverage_score` 기여 차이는 게이트
내부·일반 BUY 경로 전체 양쪽 모두 **정확히 0**(항상 1.0) —
변별력 없음이 두 모집단에서 동일하게 재확인된다.

**판정(유지)**: "차이는 사실상 `entry_score`+`relative_
activity`가 설명한다"는 §109.3/§110의 결론은 **종목 반복
편향을 제거한 뒤에도(게이트 내부 100.0%, 일반 BUY 경로
96.2%) 그대로 유지된다.** 이전(반복 편향 포함) 분석과 방향·
크기가 거의 동일해, 표본 반복이 결론 자체를 왜곡하지는
않았음이 확인됐다(다만 그 사실을 확인하는 데는 이번 재계산이
필요했다).

### 115.2 `002790`/`000720` 반복 등장 원인 규명(§6.8 잔여 2)

| 항목 | `002790` | `000720` |
|---|---|---|
| 총 레코드 수 | 239건 | 761건 |
| 관측 거래일 수 | 6일(산발적: 07-03/09/10/14/15/16) | 20일 이상(06-26~07-29, 거의 매 거래일) |
| 최소 사이클 간격 | 약 359초(≈6분) | 약 328초(≈5.5분) |
| `decision_type` | `WATCH`(전 기간 불변) | `HOLD`(전 기간 불변) |
| `candidate_set` | `['WATCH']`(불변) | `['NO_ACTION']`(불변) |
| `entry_score` 추이 | 0.2479→0.1413(완만한 하루 단위 변화) | 0.0063→0.0(거의 항상 0에 고착) |

**원인(사실 확인)**:
1. 최소 사이클 간격(약 328~359초)이 `run_decision_loop.py`의
   `DEFAULT_INTERVAL_SECONDS=300`(5분, 코드 확인)과 정합 — 이는
   intraday decision loop가 **장중 고정 5분 주기로 매 사이클
   `trade_decisions`에 새 행을 저장**하는 설계된 동작이며,
   저장/로깅 결함이 아니다.
2. §101에서 이미 확인한 `signal_feature_snapshot`의 **1일 1회
   갱신** 구조상, 같은 거래일 안의 모든 사이클이 동일한 입력으로
   재계산되어 하루 안에서 값이 거의 동일하게 반복된다.
3. `eligibility_core_risk_off_ranking_blocked` 게이트가 하루
   종일(그리고 여러 날에 걸쳐) 동일하게 걸려 "특정 gating
   상태가 고정된 채 계속 기록"되는 것도 사실로 확인된다.

**`002790` vs `000720` — 같은 메커니즘, 다른 정도**: 두 종목
모두 **동일한 3가지 원인(5분 주기 loop × 1일 1회 snapshot 갱신
× 고정 게이트)**으로 반복되지만, `000720`은 20일+ 연속으로
등장하며 신호가 만성적으로 0에 가까운 반면 `002790`은 산발적
6일에만 등장하며 신호가 완만히 변화한다 — `000720`이 core
유니버스에 더 지속적으로 포함되는 것으로 보인다(해석, 유니버스
포함 조건 자체의 원인 규명은 이번 턴 범위 밖).

**판정**: 반복 등장은 **"같은 사이클 구조상 정상 반복"**이다 —
저장/집계 방식의 결함이 아니다. 다만 이 정상 반복 구조 때문에
row 단위 통계는 관측 빈도가 높은 종목에 과대 가중되므로, 이
계열 분석은 앞으로 distinct-symbol 기준을 기본으로 삼아야 한다
(방법론적 시사점, 완화안 아님).

### 115.3 다음 우선 작업(완화안 아님, 후속 규명 과제로만 제시)

1. **1순위**: §110.8/§111.4/§112.5/§113.5/§114에서 확정한
   "1순위=산식 재검토, 2순위=중복 차단 정리" 판정을 바탕으로,
   다음 턴에서 산식 재설계(coverage_score/regime_tailwind 역할
   재정의) 또는 중복 정리(relative_activity/strategy_alignment)
   중 어느 쪽을 먼저 설계 검토할지 결정.
2. **2순위**: `000720`이 core 유니버스에 지속적으로 포함되는
...
## §118. `ranking_score` 산식 재설계 준비 — 4개 항목 역할 재분류(SPPV-2.130, 2026-07-29 KST)

**전제**: `regime_tailwind`/`strategy_alignment` "고정 여부
확인" 단계는 §116/§117로 종료된 것으로 본다. 이번 턴은 그
결과를 바탕으로 산식 안에서 4개 항목(`coverage_score`,
`regime_tailwind`, `relative_activity`, `strategy_alignment`)을
"유지/축소/이관/중복정리"로 재분류하는 **설계 검토 준비 턴**이다
— 완화안 구현·코드 diff 없음. main은 최신(`326273a9`)으로 동기화
후 진행.

### 118.1 트랙 A-1 — `coverage_score` 역할 재정의

**신규 확인(사실, DB read-only)**: `_build_feature_coverage_
score()`(`deterministic_trigger_engine.py:426-443`)는 7개
불리언 체크(스냅샷 존재/overall·fast·slow 존재/market_regime·
strategy_selection·portfolio_allocation 존재)의 참 비율이다 —
이론상 8단계(`0, 1/7, ..., 1.0`)가 가능하다. 그러나 전체 이력
(`deterministic_trigger` 존재 레코드, non-null `coverage_score`
기준)을 전수 조회한 결과 **실제로 관측되는 값은 단 2개뿐**이다:

| `coverage_score` | 건수 | `eligibility_low_feature_coverage` 발동 |
|---:|---:|---|
| `1.0` | 36,383 | 0건(발동 안 함) |
| `0.1429`(1/7) | 725 | 723건 발동(사실상 전부) |

`eligibility_low_feature_coverage`는 `coverage_score < 0.50`
이면 즉시 하드 차단한다(`:466-468`, 재확인). 즉 **`coverage_
score`가 하드 게이트를 통과해 ranking 계산까지 도달하는
레코드는 실질적으로 100% `1.0`이다**(36,383/36,385, 예외 2건은
무시 가능한 오차) — 이는 §107/§109/§116/§117에서 이미 확인한
"이 게이트 모집단/일반 모집단/전체 이력 모두 무분산"이라는
관찰과 완전히 정합하며, 그 **근본 원인**을 이번 턴에 특정했다:
7개 하위 체크가 파이프라인상 강하게 동시 발생(all-or-nothing)
하는 구조이기 때문이다.

**중복 판정**: `coverage_score`는 (a) `eligibility_low_feature_
coverage`(하드 차단, `<0.50`)에서 이미 완전히 처리되고, (b)
그 문턱을 통과한 레코드에서는 ranking에 **항상 동일한 상수
(1.0×0.20=0.20)**만 더한다 — 이는 "순위를 가르는 가중치"가
아니라 **"이미 통과한 모두에게 동일하게 지급되는 고정
보너스"**다. `eligibility`가 이미 이 정보를 완전히 소비했으므로
`ranking` 쪽 가중치는 **정보량이 0인 중복**이다.

**분류: 다른 계층으로 이관 검토(사실상 제거/축소와 동치)** —
`eligibility` 하드 게이트가 이미 이 축을 전담하고 있어, `ranking`
쪽 항목은 역할이 완전히 흡수된 상태다.

### 118.2 트랙 A-2 — `regime_tailwind` 역할 재정의

§117까지의 결론(설계는 정상이나 `risk_tone` 상시화로 실효성
상실)을 전제로, 정책적 존치 근거를 재검토했다.

- `entry_score`에 이미 `risk_off_penalty`(−0.15, 고정 상수)가
  있고, `_assess_core_risk_off_buy_guard`(하드 게이트)가 `risk_
  off AND bearish_trend`를 이미 실질적으로 강하게 차단한다(§102
  ~§104에서 이미 확인한 대로 이 하드 게이트가 전체 차단의
  59.5%를 차지 — 병목의 대부분을 이미 담당).
- `ranking`의 `regime_tailwind`(가중치 0.03)는 이 셋(entry_score
  penalty, core guard, strategy_selection의 방어 전략 강제)에
  **추가로 얹히는 네 번째 지점**이며, 가중치가 6개 항목 중
  최소(0.03)라 절대 영향력도 작다. `risk_on`이 살아나는 극소수
  구간(전체 이력 42건)에서만 의미가 생기는데, 그 42건은 이미
  `market_overlay` 소스 한정이라 `core` 모집단의 병목 해소에는
  기여하지 않는다.
- **정책적 존치 근거는 약하다** — "위험 회피 국면에는 가산점을
  주지 않는다"는 설계 의도 자체는 유효하지만, 이미 상류(entry_
  score)와 하류(하드 게이트)에서 훨씬 강하게 같은 신호를
  반영하고 있어 `ranking`의 0.03 가중치가 별도로 존재해야 할
  실익이 낮다.

**분류: 역할 축소 검토** — 완전 제거보다는 "이미 다른 계층에서
충분히 반영되는 신호를 굳이 세 번째로 얹을 필요가 있는가"라는
관점에서 축소(가중치 하향 또는 다른 항과 통합) 검토 대상.

### 118.3 트랙 B-1 — `relative_activity` 4계층 역할 표

| 계층 | 코드 위치 | 역할 | 유형 |
|---|---|---|---|
| `entry_score` 내부 보너스 | `_build_entry_score`(최대 +0.10) | 연속 가산(활동성 높을수록 가산) | 소프트 |
| `ranking_score`(`relative_activity`, 0.10 가중치) | `_build_buy_ranking_score` | 연속 가산(동일 원시 신호 재사용) | 소프트 |
| `eligibility_low_relative_activity` | `_assess_buy_eligibility`(`<1.10` 차단) | 이산 하드 게이트 | 하드 |
| `_assess_core_risk_off_buy_guard`의 활동성 게이트 | `core`+`risk_off` 한정(`<1.20`, top-k override 시 `1.10`) | 이산 하드 게이트(범위 한정) | 하드 |

**정당한 중복 vs 과잉 중복 재분리**:
- **소프트 2곳(entry_score+ranking)**: **같은 원시 신호
  (`volume_surge_ratio`/`turnover_surge_ratio`)를 같은 방향으로
  두 번 가산** — 역할이 분리된다고 보기 어렵다(`entry_score`가
  이미 종합점수인데 그 하위 성분을 `ranking`에서 별도 항으로
  다시 꺼내 쓰는 구조). **과잉 중복에 가깝다.**
- **하드 2곳(eligibility+core guard)**: 이진 차단이라는 점에서
  소프트 가산과는 역할이 다르지만(사실상 "실행 가능성 확인"),
  두 하드 게이트끼리도 임계값만 다를 뿐(`1.10` vs `1.20`) **같은
  신호를 같은 방향으로 두 번 차단**한다 — `core`+`risk_off`
  한정 게이트가 일반 게이트보다 더 엄격한 값을 요구하는 것
  자체는 "국면별로 문턱을 달리하는 정당한 설계"일 수 있으나,
  둘 다 동시에 걸리는 것이 일반적이라 실질적으로는 더 엄격한
  쪽(`1.20`)이 항상 지배적이다(§109.4.2 인용, 이번 턴 정성적
  재확인).

**분류: 중복 제거/정리 검토** — 특히 소프트 2곳(entry_score+
ranking)이 1차 정리 대상. 하드 2곳은 "국면별 차등"이라는 정당화
여지가 있어 소프트보다 우선순위는 낮다.

### 118.4 트랙 B-2 — `strategy_alignment` 미래 발동 시 구조 재확인

§117에서 "`core`도 이론상 도달 가능한 일반 경로가 있다"고
확인했으므로, 이번 턴은 **"살아났을 때 구조적으로 중복이
되는가"**를 코드 기준으로 재확인했다.

- `entry_score` 내부: `preferred_strategy ∈ {swing_momentum,
  event_continuation}`이면 **+0.05**(`_build_entry_score:1219-
  1224`).
- `ranking_score`: 동일 조건 집합이면 **1.0×0.02**(`_build_buy_
  ranking_score:1116-1121`).
- **두 조건은 문자 그대로 완전히 동일**(같은 집합, 같은
  `strategy_selection.preferred_strategy` 참조) — `regime_
  label='bullish_trend'`이고 `risk_tone≠'risk_off'`인 `core`
  후보가 실제로 나타나면(현재는 0건이지만 코드상 가능), **그
  즉시 `entry_score`와 `ranking_score` 양쪽에서 동시에 가산되는
  중복이 구조적으로 발생한다**(사실, 코드 경로 재확인 — "지금
  0이냐 아니냐"와 무관하게 이미 확정된 구조적 사실).

**분류: 중복 제거/정리 검토** — 현재 실효 영향은 0(§117)이지만,
구조적으로는 확정된 중복이라 `relative_activity`와 같은 유형의
정리 대상이다. 다만 **시급성은 낮다**(현재 발동 사례가 없어
당장의 병목에는 기여하지 않음).
...
| 항목 | 분류 | 핵심 근거 |
|---|---|---|
| `coverage_score` | **다른 계층으로 이관 검토** | eligibility 하드 게이트가 이미 완전히 전담(통과 후 100% 상수) |
| `relative_activity` | **중복 제거/정리 검토** | entry_score+ranking 소프트 2중이 같은 신호 재사용(과잉 중복) |
| `strategy_alignment` | **중복 제거/정리 검토** | entry_score+ranking이 완전히 동일한 조건 집합(구조적 확정 중복, 현재는 미발동) |
| `regime_tailwind` | **역할 축소 검토** | entry_score penalty+core guard가 이미 강하게 처리, ranking 쪽 가중치(0.03)는 추가 실익 낮음 |

### 118.6 설계 검토 우선순위(1~4순위)

1. **1순위: `coverage_score`** — 가장 큰 가중치(0.20)가 구조적
   으로 100% 상수임이 이번 턴에 완전히 확정됐다. 산식의 가중치
   배분 오류가 가장 명확하고 근거가 가장 단단하다.
2. **2순위: `relative_activity`** — 4겹 중첩 중 소프트 2중이
   명백한 중복이며, 실제 변별력의 상당 부분(§109.3/§115.1에서
   확인한 96.2~100%)을 이 항목이 담당하므로 설계가 잘못되면
   영향이 크다 — 우선순위가 높다.
3. **3순위: `strategy_alignment`** — 구조적 중복은 확정됐으나
   현재 실효 영향이 0이라 시급성은 `relative_activity`보다
   낮다.
4. **4순위: `regime_tailwind`** — 가중치가 이미 작아(0.03) 축소
   해도 얻는 실익이 작다. 우선순위 최하위.

### 118.7 설계안 비교(A/B) 단계 진입 가능 여부 판정

**대체로 진입 가능하다.** `coverage_score`와 `relative_activity`
(1·2순위)는 이번 턴까지 축적된 사실(§107/§109/§110/§115/§118)
만으로 **구체적 설계안(A/B) 비교 단계로 바로 넘어갈 수 있는
근거가 충분**하다 — 추가 사실 확인 없이도 "이관 vs 유지",
"가중치 재배분 방식" 등의 옵션을 구체화할 수 있다.

`strategy_alignment`/`regime_tailwind`(3·4순위)는 설계 방향
자체는 정해졌으나(중복 정리/축소), 시급성이 낮아 **1·2순위
설계안이 먼저 확정된 뒤 순서대로 진행**하는 것이 합리적이다 —
추가 사실 확인이 필요해서가 아니라 우선순위상 후순위이기
때문이다.

**다음 턴 성격**: **설계안 비교 턴**(추가 검증 턴이 아님) —
1순위(`coverage_score`)부터 "ranking에서 제거하고 eligibility만
남긴다" vs "가중치를 낮춰 ranking에 남긴다" 등 구체적 옵션 A/B를
비교하는 것을 다음 턴 목표로 제안한다(이번 턴은 옵션 자체를
확정하지 않음 — 사용자 승인 하에 다음 턴에서 구체화).

### 118.8 다음 우선 작업

1. **1순위**: `coverage_score` 설계안 A(ranking에서 제거, 가중치
   재분배) vs B(가중치 축소, 예: 0.20→0.05) 비교 설계 검토
   (코드 diff 아님, 설계 비교까지만).
2. **2순위**: `relative_activity` 설계안 — entry_score와 ranking
   중 한쪽에만 반영하는 방식 비교 검토.
3. **3순위(이전 턴 이월)**: `000720`이 core 유니버스에 20일
   이상 연속 포함되는 조건 원인 확인(완화안 아님).
...
## §119. `coverage_score`/`relative_activity` 설계안 A/B 비교(SPPV-2.131, 2026-07-29 KST)

**전제(짧게 인용)**: `regime_tailwind`/`strategy_alignment`
"고정 여부 확인"은 §116/§117로 이미 종료됐고, §118에서 4개
항목을 재분류해 `coverage_score`(1순위)/`relative_activity`
(2순위)가 즉시 설계안 비교 단계로 진입 가능하다고 판정했다.
이번 턴은 그 1·2순위에 대해 **설계안 A/B를 비교**한다 — 코드
미수정, `.env` 미수정, Full pytest/외부 API 호출 없음.

### 119.1 코드 경로 재확인(설계안 비교의 전제, read-only)

- `coverage_score`는 `_build_feature_coverage_score()`에서
  1회 계산돼 (a) `_assess_buy_eligibility`(하드 게이트,
  `<0.50` 차단)와 (b) `_build_buy_ranking_score`(가중치 0.20)
  **두 곳에 동일한 값이 그대로 전달**된다(사실, 코드 재확인).
- `relative_activity`는 `_build_relative_activity_score()`가
  **동일 함수로 2회 호출**된다 — 1회는 `_build_entry_score`
  내부(최대 +0.10 보너스, 코드 라인 1233), 1회는 `_build_buy_
  ranking_score` 내부(0.10 가중치, 코드 라인 1123). **완전히
  같은 원시 입력(`signal_feature_snapshot`)으로 같은 계산을
  두 번 반복**하는 구조다(사실).
- **호출 순서**: `ranking_score`는 `_assess_buy_eligibility`
  보다 **먼저** 계산되고, `_assess_core_risk_off_buy_guard`와
  `_assess_buy_eligibility` 양쪽 모두 `ranking_score` 값을
  파라미터로 받아 임계값 비교에 사용한다(사실, 코드 재확인) —
  즉 `ranking_score`의 절대 스케일을 바꾸면 이 두 하드 게이트의
  실질 통과율에도 영향이 간다(설계안 A 검토 시 반드시 고려할
  구조적 의존관계).

### 119.2 `coverage_score` 설계안 A/B

**A안 — `ranking_score`에서 제거, `eligibility`/coverage gate
전용 신호로 이관**

- 변경 내용: `_build_buy_ranking_score`의 `+ 0.20 * coverage_
  score` 항을 제거하고, `coverage_score`는 `_assess_buy_
  eligibility`(`<0.50` 하드 차단)에만 남긴다.
- 중복 제거 효과: **있음** — §118.1에서 확인한 대로 이 항목은
  하드 게이트 통과 후 100% 상수(`1.0`)이므로, ranking에서
  제거해도 정보 손실이 없다(사실 기반).
- ranking 변별력 회복: **직접적 회복 없음** — `coverage_score`는
  애초에 통과 후 항상 동일한 값이라, 제거해도 "순위가 달라지는"
  변별력 자체가 생기지는 않는다. 다만 **가중치 0.20을 다른
  항목(예: `entry_score`)에 재배분**하면 상대적으로 `entry_
  score`/`relative_activity`의 영향력이 커져 간접적으로 변별력
  이 개선될 수 있다(해석, 재배분 방식은 이번 턴 확정 대상 아님).
- 다른 BUY 차단 장치와 충돌/중복: **감소** — 같은 신호를 ranking
  과 eligibility 두 곳에서 쓰는 구조 자체가 없어진다.
- 절대 스케일 영향(§119.1 구조): `ranking_score`의 최댓값이
  0.20만큼 낮아지므로, `_CORE_RISK_OFF_RANKING_MIN_SCORE=0.48`
  같은 절대 threshold와의 관계가 **바뀐다** — threshold를 그대로
  두면 통과가 더 어려워지고, 재보정하면 threshold 재설계가
  추가로 필요하다. **이 부작용은 diff 범위를 "가중치 제거"에서
  "가중치 제거 + 나머지 가중치 재배분 + threshold 재검토"로
  확장시킬 수 있다.**
- diff 범위: 코드 자체는 1줄 삭제(+ 가중치 재배분 시 몇 줄
  추가)로 **작다**. 다만 위 threshold 연쇄효과까지 포함하면
  검증 범위는 더 넓어질 수 있다.

**B안 — `ranking_score`에는 남기되 역할을 축소(가중치 하향,
예시로만 0.20→0.05, 실제 값 확정은 다음 턴)**

- 변경 내용: 가중치만 낮추고 나머지 5개 항목 가중치를 비례
  재조정(합 1.0 유지).
- 중복 제거 효과: **부분적** — eligibility와의 중복 자체는
  남아 있지만(같은 신호를 두 곳에서 사용하는 구조는 유지),
  ranking에서 차지하는 비중은 줄어든다.
- ranking 변별력 회복: A안과 마찬가지로 `coverage_score` 자체가
  상수이므로 직접적 변별력 개선은 없다 — 다만 A안보다 절대
  스케일 변화가 작아(0.20→0.05, 0.15만큼만 감소) threshold
  연쇄효과가 더 작다.
- 다른 BUY 차단 장치와 충돌/중복: **일부 남음** — "eligibility가
  이미 처리한 정보를 ranking에서 다시 쓴다"는 구조적 중복 자체는
  해소되지 않는다.
- diff 범위: A안보다 더 작다(숫자만 조정) — 다만 "왜 0.05인가"
  라는 새로운 임의성이 생긴다(근거 없는 매직 넘버 위험).

**표 요약**은 §119.4 참고.

### 119.3 `relative_activity` 설계안 A/B

**A안 — `entry_score` 또는 `ranking_score` 한쪽 소프트 반영만
남기고 다른 한쪽 제거**

- 정리 대상: §118.3에서 이미 확인한 **소프트 2곳(entry_score
  보너스 + ranking 가중치)** 중 하나만 남긴다. `eligibility_
  low_relative_activity`(하드 게이트, `<1.10`)와 `_assess_
  core_risk_off_buy_guard`의 활동성 게이트(하드, `<1.20`/`1.10`)
  는 **건드리지 않는다**(사실 — 이 안은 소프트 항만 대상으로
  한정).
- 상류(score)·하류(gate) 역할 분리: **선명해짐** — "활동성이
  이미 최종 점수(`entry_score`)에 반영돼 있는데 `ranking`에서
  같은 원시 신호를 다시 꺼내 쓰는" 구조가 사라진다.
- 실제 병목 완화 가능성: **간접적** — `relative_activity`는
  이 모집단에서 실제 변별력의 상당 부분(§109.3: 게이트 내부
  29%, §115.1: 일반 BUY 경로 5.3%)을 담당하는 항목이라, 소프트
  중복 하나를 제거하면 `ranking_score`의 절대값이 낮아지고
  변별 폭도 줄어들 수 있다 — "병목 완화"보다는 "산식 정합성
  개선"에 가까운 효과로 봐야 한다(과장 금지).
- 부작용 위험: `entry_score` 쪽에 남길지 `ranking_score` 쪽에
  남길지에 따라 **다른 하드 게이트와의 상호작용이 달라진다** —
  `_assess_core_risk_off_buy_guard`는 `ranking_score`(전체
  점수)를 직접 비교하므로, `ranking_score`에 남기는 편이 기존
  하드 게이트 로직과의 정합성이 더 높다(해석).

**B안 — 소프트 반영은 유지하되 강도/역할을 분리**

- 정리 방식: 완전 제거 대신 **한쪽은 "원시 활동성"(현재와 동일),
  다른 쪽은 "활동성의 추세/상대적 순위"처럼 다른 파생값**으로
  바꿔 같은 원시 신호를 그대로 반복 사용하지 않도록 한다(개념
  설계만 — 구체적 파생값 정의는 다음 턴 대상).
- 상류·하류 역할 분리: **A안만큼 선명하지는 않음** — 여전히
  두 항이 "활동성"이라는 같은 축을 다루므로, 완전한 중복 해소는
  아니다.
- 실제 병목 완화 가능성: A안과 유사하나, 정보량을 유지하면서
  중복만 줄이려는 시도라 **설계 복잡도가 더 높다**(신규 파생값
  정의·검증 필요).
- 부작용 위험: **더 큼** — 새 파생값을 도입하면 그 자체가 또
  다른 검증 대상이 되고, 이번 턴 확보한 근거(§109/§115)만으로는
  설계를 확정할 수 없다.

### 119.4 최종 비교표

| 항목 | 안 A | 안 B | 권고안 | 권고 이유 | 지금 diff 초안 가능 여부 |
|---|---|---|---|---|---|
| `coverage_score` | ranking에서 제거, eligibility 전용 이관 | ranking에 남기되 가중치 축소 | **A안 우선 검토** | 이 항목은 상수이므로 "축소"보다 "제거"가 근거에 더 부합 — B안은 "왜 0.05인가"라는 새 임의성만 추가 | **아니오** — threshold(`0.48`) 연쇄효과 재검토가 선행돼야 함 |
| `relative_activity` | 소프트 2곳 중 1곳만 유지(`ranking_score` 쪽 권장) | 소프트 유지 + 파생값 분리 | **A안 우선 검토** | B안은 신규 파생값 설계·검증이 추가로 필요해 이번 턴 근거만으로는 확정 불가 | **아니오** — 어느 쪽(entry_score vs ranking)에 남길지와 하드 게이트 재검증 계획이 더 필요 |

### 119.5 지금 바로 diff 초안으로 넘어갈 안 vs 보류할 안

**둘 다 아직 diff 초안으로 넘어가지 않는다** — 이유:

1. `coverage_score` A안은 방향은 명확하지만, `ranking_score`의
   절대 스케일이 바뀌면 `_CORE_RISK_OFF_RANKING_MIN_SCORE=0.48`
   등 다른 절대 threshold와의 관계를 함께 재검토해야 한다(§119.1
   구조적 의존관계) — 이 재검토가 없으면 diff가 "가중치 제거"에
   그치지 않고 threshold 재조정까지 함께 요구할 위험이 있다.
2. `relative_activity` A안은 방향(소프트 2곳 중 1곳 유지)은
   명확하지만, **"entry_score 쪽을 남길지 ranking_score 쪽을
   남길지"**가 아직 결정되지 않았다 — §119.3에서 "ranking_score
   쪽이 하드 게이트와의 정합성이 더 높다"는 잠정 해석만 제시했고,
   확정 근거(예: 실제 하드 게이트 발동 빈도 재분해)는 아직
   수집하지 않았다.

**따라서 이번 턴의 결론은 "1안 확정"이 아니라 "두 항목 모두
A안 방향으로 수렴했으나, 각각 diff 작성 전 1개씩의 추가 확인
과제가 남아 있다"는 것이다** — §119.6에 다음 우선 작업으로
명시한다.

### 119.6 다음 우선 작업(diff 착수 전 확인 과제, 완화안 확정 아님)

1. **1순위**: `coverage_score` A안 채택 시 `ranking_score`
   최댓값이 0.20 낮아지는 것이 `_CORE_RISK_OFF_RANKING_MIN_
   SCORE=0.48`/`shadow_topk_exception_v2`의 `0.22` 하한 등
   기존 절대 threshold들과 어떻게 상호작용하는지 read-only로
   재계산(완화안 아님, 영향 범위 확인까지만).
2. **2순위**: `relative_activity` A안에서 "entry_score 쪽 유지"
   vs "ranking_score 쪽 유지" 중 어느 쪽이 `_assess_core_risk_
   off_buy_guard`/`eligibility_low_relative_activity`와 더
   정합적인지, 실제 하드 게이트 발동 데이터로 재확인.
3. **3순위(이전 턴 이월)**: `000720`이 core 유니버스에 20일
   이상 연속 포함되는 조건 원인 확인(완화안 아님).
4. **4순위(이전 턴 이월)**: `high_volatility` 단독 경로(001450형)
   층3 관찰 지속.
...
## §120. `coverage_score` 제거 threshold 연쇄영향 정량 재계산 + `relative_activity` 유지 위치 비교(SPPV-2.132, 2026-07-29 KST)

**전제(기존 사실 재인용, 반복 설명 최소화)**: `coverage_score`가
하드 게이트 통과 후 100% 상수(`1.0`)라는 것과 `relative_
activity`가 `entry_score`/`ranking_score` 양쪽에 동일 함수로
중복 반영된다는 것은 §107/§109/§118/§119에서 이미 확정됐다.
이번 턴은 diff 착수 여부를 가르는 **남은 2개 확인 과제만** 닫는다
— 코드 미수정, `.env` 미수정, Full pytest 미실행, 신규 KIS
호출 0건, DB/코드 read-only만 사용.

### 120.1 A — `coverage_score` 제거 threshold 연쇄영향(신규 확인)

`_CORE_RISK_OFF_RANKING_MIN_SCORE=0.48`, `_CORE_RISK_OFF_
SHADOW_MIN_SCORE=0.22`(코드 상수명 재확인, 사용자가 지칭한
"0.22 shadow 하한"과 동일) 두 threshold를 기준으로, `coverage_
score` 항을 제거한 shadow `ranking_score`를 **두 가지 방식**으로
재계산했다 — ① 단순 항 제거(가중치 재분배 없음, `new = old −
0.20`) ② 재정규화(잔여 가중치 합 0.80을 1.0으로 스케일,
`new = (old − 0.20)/0.80`).

| 모집단(전체 이력) | 기존 `0.48` 통과 | ①단순제거 `0.48` 통과 | ②재정규화 `0.48` 통과 | 기존 `0.22` 통과 | ①단순제거 `0.22` 통과 | ②재정규화 `0.22` 통과 |
|---|---:|---:|---:|---:|---:|---:|
| `ranking_blocked` 게이트 내부(n=12,287) | 0(0%) | 0(0%) | 0(0%) | **12,287(100%)** | **45(0.4%)** | **217(1.8%)** |
| 일반 BUY 경로 전체(n=35,761) | 5,303(14.8%) | 120(**0.34%**) | 772(**2.2%**) | 35,761(100%) | 8,550(23.9%) | 9,922(27.7%) |

**새로 확인한 사실(신규)**:
- `0.48` 하드 threshold를 기준으로 보면, 일반 BUY 모집단에서
  현재 14.8%가 통과하는데 `coverage_score`를 단순 제거하면
  **0.34%로 붕괴**한다(재정규화해도 2.2%로 여전히 큰 폭 감소).
- `0.22` shadow 하한 기준으로 보면 영향이 훨씬 극적이다 —
  `ranking_blocked` 게이트 내부에서 **현재 100%가 통과**하는데
  (`coverage_score`의 고정 `0.20`이 최소 점수 `0.225`를 `0.22`
  이상으로 항상 밀어올리기 때문), 단순 제거 시 **0.4%만
  통과**한다 — 사실상 이 shadow 후보 풀 전체가 사라지는
  수준이다.
- `0.43~0.48` 근접 표본도 재계산했다: 일반 BUY 경로 전체 이력
  기준 기존 1,983건에서 단순 제거 후 380건, 재정규화 후
  1,379건으로 변한다 — "경계 근처"라는 개념 자체가 threshold를
  그대로 둘지 재정규화할지에 따라 크게 달라진다.

**Q1 답변(단지 상수항 0.20 제거인지)**: 산술적으로는 그렇다 —
모든 레코드에서 정확히 `0.20`만큼(재정규화 시 비례) 하향
이동한다. 그러나 **효과는 결코 "무해한 상수 제거"가 아니다.**

**Q2 답변(경계 해석을 바꿔버리는 수준인지)**: **그렇다, 매우
강하게.** `0.22` shadow 하한을 기준으로 한 후보 풀 규모가
100%→0.4~1.8%로 붕괴하고, `0.48` 하드 threshold 통과율도
14.8%→0.34~2.2%로 붕괴한다 — 이는 "중복 제거"라는 원래 의도와
무관하게, threshold를 그대로 둔 채 `coverage_score`만 제거하면
**사실상 새로운(훨씬 더 엄격한) 게이트를 만드는 것과 같은
효과**를 낸다.

**Q3 답변(단독 diff 가능 vs threshold와 묶여야 하는지)**: **명백히
threshold 재설계와 묶여야 한다.** `coverage_score` 제거를
threshold 조정 없이 단독으로 적용하면, "주문 0건 문제 해소"라는
SPPV 트랙의 근본 목적과 **정반대 방향**(후보 풀이 더 좁아짐)으로
작용할 위험이 매우 크다. **판정: A안은 이번 턴 기준으로 diff
착수 불가** — threshold(`0.48`/`0.22`) 재계산·재설계를 먼저
별도 트랙으로 진행한 뒤에만 안전하게 적용할 수 있다.

### 120.2 B — `relative_activity` 유지 위치 비교(entry_score vs
ranking_score)

**새로 확인한 사실(신규)**: 전체 이력 기준으로 두 가지 정리
방식의 threshold 영향을 계산했다 — 1안(`ranking_score`에서
표준항 제거, `entry_score`는 그대로 유지) vs 2안(`entry_score`
내부 보너스만 제거, `ranking_score`의 표준항은 그대로 유지).

| 모집단 | 기존 `0.48` 통과 | 1안 통과 | 2안 통과 | 기존 mean | 1안 mean | 2안 mean |
|---|---:|---:|---:|---:|---:|---:|
| `ranking_blocked` 게이트 내부(n=12,293) | 0 | 0 | 0 | 0.2558 | 0.2523 | 0.2539 |
| 일반 BUY 경로 전체(n=35,773) | 5,304(14.8%) | 5,122(14.3%) | 5,122(14.3%) | 0.3282 | 0.3247 | 0.3263 |

- **coverage_score와 달리, 두 안 모두 threshold 통과율 변화가
  작다**(14.8%→14.3%, 0.5%p 차이) — `relative_activity`는
  `coverage_score`처럼 전원에게 고정된 상수가 아니라 레코드마다
  다른 연속값이라, 제거해도 분포 전체가 일괄적으로 밀리지 않기
  때문이다(해석). **이 항목은 threshold를 별도로 재설계하지
  않아도 안전하게 정리 가능**하다는 점이 이번 턴에 새로 확인된
  가장 중요한 차이다.
- `entry_score`의 `buy_candidate_threshold=0.65` 하드 게이트에
  대한 2안의 영향도 확인했다(신규) — 전체 이력(n=35,777) 기준
  통과 건수가 `500건→500건`으로 **현재 관측 데이터에서는 변화
  없음**(mean만 0.1779→0.1744로 소폭 하락). 다만 이는 "지금
  데이터에서 우연히 경계를 넘는 레코드가 없었다"는 사실이지,
  구조적으로 `entry_score`의 하드 게이트에 영향을 줄 수 있는
  경로 자체가 없다는 뜻은 아니다(2안는 여전히 `entry_score`라는
  더 많은 곳에서 쓰이는 값을 건드리는 구조).

**비교 종합**:

| 기준 | 1안(entry에 유지) | 2안(ranking에 유지) |
|---|---|---|
| 실제 변별력 차이(전체 이력) | ranking mean 0.2523/0.3247 | ranking mean 0.2539/0.3263(근소하게 더 높음) |
| hard gate와의 역할 분리 | 하드 게이트(활동성)는 원시 신호 직접 참조라 영향 없음(양쪽 공통) | 좌동 |
| `entry_score` 의미 보존 | **유지**(활동성 포함한 기존 정의 그대로) | **변경**(활동성 제외한 "순수 신호 강도"로 재정의됨) |
| diff 범위/부작용 범위 | `_build_buy_ranking_score` 1곳만 수정 — **좁음** | `_build_entry_score` 수정 — 이 함수는 `buy_candidate_threshold=0.65` 하드 게이트와 `ranking_score`의 0.55 가중치 입력 **두 곳에 동시에 영향**을 주는 더 넓은 파급 지점 |
| ranking 산식 단순화 효과 | 동일(둘 다 항 1개 제거) | 동일 |

**Q1/Q2/Q3/Q4 답변**:
1. `entry`에 남길 때 장점/단점: 장점 — diff 범위가 가장 좁고
   `entry_score`의 하드 게이트 의미가 그대로 보존됨. 단점 —
   `ranking_score`가 활동성 신호를 `entry_score`라는 우회 경로로만
   약하게(0.55배 희석) 반영하게 됨.
2. `ranking`에 남길 때 장점/단점: 장점 — `ranking_score`가 활동성
   신호를 온전한 가중치(0.10)로 유지. 단점 — `entry_score`
   함수를 수정해야 하므로 `buy_candidate_threshold` 하드 게이트
   까지 영향 범위에 들어가는 더 넓은 diff가 된다(현재 데이터
   기준 실질 영향은 0이지만 구조적 위험은 남음).
3. **보수적/자연스러운 쪽**: 이번 턴 확보한 근거만으로는 **1안
   (entry_score에 유지, ranking에서 제거)이 더 보수적**이다 —
   diff 범위가 가장 좁고 다른 하드 게이트(`buy_candidate_
   threshold`)를 건드리지 않는다는 점이 결정적이다. 2안는 설계
   개념상(entry_score=순수 신호, ranking=실행 우선순위 계층)
   더 "깔끔"해 보일 수 있으나, 이번 턴은 diff 착수 가능 여부를
   닫는 것이 목적이므로 **범위가 좁은 1안을 권고안으로 채택**한다.
4. **지금 바로 diff 초안 작성 가능한 1안이 생기는가**: **그렇다
   — 1안(`ranking_score`에서 `relative_activity` 표준항 제거,
   `entry_score`는 그대로 유지)**은 threshold 재설계 없이도
   안전하게 진행 가능함을 이번 턴에 정량적으로 확인했다.

### 120.3 최종 판정

| 항목 | diff 착수 가능 여부 | 이유 |
|---|---|---|
| `coverage_score`(A안) | **불가** — threshold 재설계와 묶여야 함 | `0.48`/`0.22` 통과율이 각각 14.8%→0.34~2.2%, 100%→0.4~1.8%로 붕괴 |
| `relative_activity`(1안) | **가능** — threshold 재설계 없이 단독 diff 가능 | threshold 통과율 변화가 14.8%→14.3%로 미미 |

### 120.4 다음 우선 작업(완화안 확정 아님, 후속 절차로만 제시)

1. **1순위**: `relative_activity` 1안(`ranking_score`에서
   표준항 제거, `entry_score`는 유지)의 **구체적 diff 초안**을
   다음 턴에서 작성(이번 턴은 방향과 근거 확보까지만, 실제 코드
   수정은 사용자 승인 하에 별도 턴에서 진행).
2. **2순위**: `coverage_score`는 diff 착수를 보류하고,
   threshold(`0.48`/`0.22`) 재설계가 필요하다는 사실을 별도
   트랙(§109.4.3/§118.1과 연계)으로 기록 — "산식 재검토 1순위"
   라는 항목 분류(§118)는 유지하되, 실행 순서상으로는 `relative_
   activity`보다 뒤로 밀린다는 점을 반영.
3. **3순위(이전 턴 이월)**: `000720`이 core 유니버스에 20일
   이상 연속 포함되는 조건 원인 확인(완화안 아님).
4. **4순위(이전 턴 이월)**: `high_volatility` 단독 경로(001450형)
   층3 관찰 지속.

## §121. `relative_activity` 1안 diff 실제 적용 + 최소 검증(SPPV-2.133, 2026-07-29 KST)

**전제**: §120에서 확정된 두 판정 중 `relative_activity` 1안(ranking_score에서
...
**해석**: §120.2에서 "1안의 threshold 영향은 일반 모집단 기준 미미
(14.8%→14.3%)"라고 판정한 것은 집계 수준에서는 유효하지만, **개별 레코드가
정확히 0.48 경계 부근에 있을 경우 1안 적용으로 경계를 넘나들 수 있다**는
...
## §122. `relative_activity` 1안 적용 후 영향 확인 + 다음 설계 분기 확정(SPPV-2.134, 2026-07-29 KST)

**전제**: SPPV-2.133에서 반영한 diff(PR #14, mergeCommit `e1ae1b3d`, 병합
2026-07-29 03:39:59 UTC=12:39:59 KST)가 실제 운영 decision loop에 미친
영향을 read-only로 확인한다. 코드 수정 없음, 신규 KIS 호출 0건.

### 122.1 배포 확인(사실)

- `agent_trading-app-1`/`agent_trading-ops-scheduler` 컨테이너의
  `deterministic_trigger_engine.py` md5sum이 병합된 `main`과 동일함을
  확인 — bind mount 구조라 별도 이미지 재빌드 없이 즉시 반영됨.
  `ops-scheduler`는 각 decision 사이클마다 `python3 -m scripts.run_decision_
  loop --count 1`을 **새 서브프로세스로 기동**하므로, 병합 시각 이후 첫
  사이클부터 새 코드가 적용됨(사실).
- 확인 시점(2026-07-29 12:46 KST) 기준, 병합 이후 경과 시간은 약 6분,
  관측된 신규 decision 사이클은 **1회**(`created_at`이 2026-07-29
  03:45:08~03:46:00 UTC 구간에 집중된 레코드 15건)뿐이다.

### 122.2 적용 전/후 비교(사실, read-only 재현)

| 구간 | n(BUY-path) | ranking_score 평균 | ranking_score 중앙값 | `eligibility_core_risk_off_ranking_blocked` 비중 | `buy_candidate=True` |
|---|---|---|---|---|---|
| 병합 직전 30분(2026-07-29 03:09:59~03:39:59 UTC) | 120 | 0.3358 | 0.3037 | 56/120 (46.7%) | 0/120 |
| 병합 이후 1개 사이클(2026-07-29 03:45:08~03:46:00 UTC) | 15 | 0.3319 | 0.2811 | 8/15 (53.3%) | 0/15 |

- `shadow_topk_exception_v2`(`core_risk_off_experiment.shadow_would_pass`):
  병합 이후 관측된 9건 중 9건 모두 `False` — 기존과 동일하게 **발동 0건**
  (기존 4주간 0% 발동 패턴과 동일, 변화 없음).
- `buy_candidate`/`eligibility_passed`/`APPROVE`/`order_request`: 병합
  이후 15건 전부 `buy_candidate=False`(0/15) — 병합 직전 30분(0/120)과
  **동일하게 0**. 변화 없음.

### 122.3 판정(사실 + 해석, 명확히 구분)

- **사실**: 관측된 표본 크기가 극히 작다(1개 사이클, n=15). `ranking_
  blocked` 비중이 46.7%→53.3%로 이동했으나, 이는 SPPV-2.132/§120에서
  정량화한 "일반 모집단 기준 미미한 영향(14.8%→14.3%)"과 같은 성격의
  통계가 아니라 **표본이 15건뿐인 단일 창의 관측치**이며, 사이클 간
  자연 변동(어떤 종목이 후보로 들어왔는지)만으로도 이 정도 차이는
  발생할 수 있다. **의미 있는 변화라고 해석하지 않는다.**
- **사실**: `buy_candidate`/`shadow_topk_exception_v2` 발동 여부는
  병합 전후로 **변화 없음**(둘 다 0을 유지) — 이번 diff가 즉각적으로
  BUY funnel 산출물을 바꾸지는 않았다는 점은 명확하다(이 diff의
  설계 의도 자체가 "즉시 완화"가 아니라 "소프트 중복 제거"였으므로
  예상과 부합).
- **미확정**: 관측 창이 1개 사이클(약 5분)뿐이라, `ranking_score`
  분포의 실제(장기) 이동 여부, `ranking_blocked` 비중의 구조적 변화
  여부는 **이번 턴 자료만으로는 판정 불가**. 최소 1~2 거래일(수백~
  수천 사이클) 관측이 쌓여야 §120에서 예측한 "14.8%→14.3%" 수준의
  이동이 실측으로 확인되는지 알 수 있다.

### 122.4 핵심 병목 재판정

- **사실**: 병합 전후 모두 `eligibility_core_risk_off_ranking_blocked`
  (즉 `ranking_score < 0.48`)가 BUY-path 차단의 최다 사유이며,
  `buy_candidate`는 여전히 0을 유지한다.
- **해석**: 이번 관측(1개 사이클)은 §120에서 이미 정량화한 결론 —
  "`coverage_score`가 `ranking_score`의 최댓값에서 상수 0.20을 차지하는
  구조가 `0.48`/`0.22` threshold와 강하게 얽혀 있고, `relative_activity`
  같은 소프트 중복 항목의 제거만으로는 이 구조를 바꾸지 못한다" —
  를 뒤집을 근거를 전혀 제공하지 않는다. **핵심 병목은 여전히
  `coverage_score` + 절대 threshold(`0.48`/`0.22`) 조합**으로 판단된다
  (신규 사실이 아니라 기존 판정의 재확인).

### 122.5 다음 1순위 결정

**판정: 2안(운영 관측을 1~2거래일 더 쌓은 뒤 진행) 채택.**

근거:
1. 이번 턴 확보한 병합 이후 표본이 n=15(1개 사이클)에 불과해, `ranking_
   score` 분포·`ranking_blocked` 비중의 실제 이동 여부를 통계적으로
   판단하기에는 명백히 부족하다(사실).
2. `coverage_score` threshold 재설계는 그 자체로 파급력이 큰 변경
   (§120에서 `0.48` 통과율 14.8%→0.34~2.2%, `0.22` 통과율 100%→
   0.4~1.8%로 확인됨)이므로, 직전 diff(`relative_activity` 1안)가
   운영에서 예상대로(=미미하게) 작동하는지를 먼저 실측으로 확인한
   뒤 착수하는 것이 안전하다(해석).
3. `coverage_score` A안 자체의 설계(threshold 재정규화 방식, 재계산
   기준)는 이번 턴에서 추가로 확정할 새로운 근거가 없다 — 즉
   "지금 바로 설계에 들어갈 준비"가 되어 있다고 보기 어렵다(해석).

**결론적으로 다음 턴 권고**: `relative_activity` 1안 적용 이후 최소
1~2 거래일(다음 거래일 이후) 운영 관측을 축적해 `ranking_score`
분포/`ranking_blocked` 비중/`shadow_topk_exception_v2` 발동 여부가
§120 예측(14.8%→14.3%)과 실측이 부합하는지 재확인한 뒤, 그 결과를
근거로 `coverage_score` threshold 재설계 착수 여부를 최종 결정한다.

### 122.6 다음 우선 작업(완화안 확정 아님)

1. **1순위(갱신)**: `relative_activity` 1안 적용 이후 최소 1~2 거래일
   운영 관측 축적 — `ranking_score` 분포, `ranking_blocked` 비중,
   `shadow_topk_exception_v2` 발동 여부를 §120 예측치와 대조(read-only,
   완화안 아님).
2. **2순위**: 1순위 관측 결과를 근거로 `coverage_score` threshold
   (`0.48`/`0.22`) 재설계 착수 여부 최종 결정.
3. **3순위(이전 턴 이월)**: `000720`이 core 유니버스에 20일 이상 연속
   포함되는 조건 원인 확인.
4. **4순위(이전 턴 이월)**: `high_volatility` 단독 경로(001450형) 층3
   관찰 지속.

## §123. `relative_activity` 1안 적용 후 운영 관측 추가 축적(SPPV-2.135, 2026-07-29 KST)

**전제**: §122(SPPV-2.134)에서 "2안(추가 관측) 채택" 판정 이후, 관측 창을
넓혀 재확인한다. 코드 수정 없음, 신규 KIS 호출 0건, read-only 재집계만
수행.

### 123.1 관측 창의 실제 크기(사실 — 중요한 제약)

- 확인 시각: 2026-07-29 13:20 KST(=04:20 UTC).
- PR #14 병합 시각: 2026-07-29 12:39:59 KST(=03:39:59 UTC).
- **병합 이후 실제 경과 시간은 약 41분**이며, 이번 턴 요청의 "5거래일
  수준 관측"은 **아직 물리적으로 확보 불가능**하다 — 실제 캘린더
  시간이 그만큼 지나지 않았기 때문에, 이번 턴은 "5거래일 관측"이
  아니라 "SPPV-2.134의 초기 1사이클 대비 조금 더 누적된 창(약 9
  사이클, n=134)"을 재확인하는 턴이다(사실, 과장 없이 명시).
- 이 제약은 앞으로도 동일하게 적용된다: 실제 거래일 단위 관측은
  캘린더 시간이 그만큼 경과해야만 확보 가능하며, 이번 세션 내에서
  임의로 앞당길 수 없다.

### 123.2 초기 1사이클 vs 누적 창 비교(사실, read-only 재집계)

| 구분 | 초기 1사이클(SPPV-2.134, 03:45:08~03:46:00 UTC) | 누적 창(이번 턴, 03:45:08~04:20:58 UTC, 약 9사이클) |
|---|---|---|
| n(BUY-path) | 16 | 134 |
| ranking_score 평균 | 0.3305 | 0.3323 |
| ranking_score 중앙값 | 0.2811 | 0.2983 |
| ranking_score p90 | 0.4402 | 0.4402 |
| `eligibility_core_risk_off_ranking_blocked` 비중 | 9/16(56.2%) | 64/134(47.8%) |
| `buy_candidate=True` | 0/16 | 0/134 |
| `eligibility_passed=True` | 0/16 | 4/134(3.0%) |
| `shadow_topk_exception_v2`(`shadow_would_pass=True`) | 0/10 | 0/101 |
| `APPROVE`(decision 컬럼) | — | 0 |
| `order_request` 존재 | — | 0 |
| `final_intent='buy'` | — | 0 |

참고: 병합 직전 30분(SPPV-2.134 기준, n=120)의 `ranking_blocked` 비중은
46.7%였다 — 누적 창의 47.8%는 초기 1사이클의 56.2%보다 **병합 이전
기준값에 더 가깝게 회귀**했다(사실).

### 123.3 `eligibility_passed=True` 4건 상세(사실, 과해석 방지)

누적 창에서 `eligibility_passed=True`인 4건은 모두 **동일한 core
종목·동일한 ranking_score(0.5428)**를 갖는 반복 관측(사이클마다 같은
종목이 재평가된 것)이며, `eligibility_reasons`에 `core_risk_off_*`
계열이 아닌 `eligibility_regime_pass`/`eligibility_signal_floor_pass`
경로로 통과했다. 넷 다 `buy_candidate=False`, `primary_candidate=WATCH`
로 귀결됐다 — 이는 `relative_activity` 1안 적용과 직접 연결된 신호가
아니라 **기존에도 존재하던 개별 종목의 WATCH 고정 패턴**으로 판단되며
(해석), 이번 diff가 새로운 BUY 경로를 열었다는 근거로 사용하지 않는다.

### 123.4 판정(사실 + 해석 구분)

- **사실**: 누적 창(n=134, 약 9사이클)에서도 `buy_candidate`,
  `eligibility_passed`(core_risk_off 경로 기준으로는 여전히 0),
  `APPROVE`, `order_request`, `final_intent='buy'`, `shadow_topk_
  exception_v2` 발동 모두 **초기 1사이클과 동일하게 0을 유지**한다.
- **사실**: `ranking_score` 평균/중앙값은 초기 1사이클과 누적 창 사이에
  실질적 차이가 없다(0.3305→0.3323, 0.2811→0.2983 — 두 자릿수 이하
  변동). `ranking_blocked` 비중은 초기 1사이클(56.2%)보다 누적 창
  (47.8%)에서 병합 이전 기준값(46.7%)에 더 가깝게 이동했다 — 이는
  **초기 1사이클이 우연히 편향된 표본이었을 가능성**을 시사하며
  (해석), §120에서 예측한 "미미한 영향(14.8%→14.3%)"과 상충하지
  않는다.
- **미확정**: 관측 창이 여전히 41분·134건에 불과해 실제 거래일 단위
  (수백~수천 사이클) 패턴과는 비교할 수 없다. "5거래일 수준 관측"은
  이번 턴에서 확보되지 않았으며, 향후 여러 턴에 걸쳐 캘린더 시간이
  누적돼야 확인 가능하다(명시).

### 123.5 핵심 병목 재판정

`eligibility_core_risk_off_ranking_blocked`가 여전히 최다 차단 사유이고
`buy_candidate`는 여전히 0 — **핵심 병목은 여전히 `coverage_score`+
절대 threshold(`0.48`/`0.22`) 조합**이라는 기존 판정을 다시 한번
재확인한다(신규 반박 근거 없음, §120/§122와 동일한 결론).
...
근거:
1. 실제 경과 시간이 41분에 불과해, 요청된 "5거래일 수준" 관측은
   물리적으로 확보되지 않았다(사실) — 이번 턴만으로 관측 단계를
   종료할 근거가 없다.
2. 다만 누적 창에서 `ranking_blocked` 비중이 병합 이전 기준값에
   더 가깝게 회귀한 점은, §120의 "미미한 영향" 예측과 **상충하지
   않는 방향**이다(해석) — 즉 지금까지의 관측은 `coverage_score`
   threshold 재설계 착수를 서두를 근거도, 늦출 근거도 추가로
   제공하지 않는다.
3. `buy_candidate`/`shadow_topk_exception_v2`가 여전히 0인 것은
   diff 적용 전후 동일하므로, 이 diff 자체가 운영 결과를 바꾸지
   않았다는 점은 이제 두 개의 서로 다른 창(초기 1사이클, 누적
   9사이클)에서 **일관되게** 확인됐다(사실) — 이 부분은 추가
   관측이 필요하지 않다고 볼 수 있으나, "5거래일 수준"이라는
   이번 턴의 요청 기준에는 아직 못 미친다.

**결론**: 다음 턴에서도 캘린더 시간이 충분히 누적된 뒤(최소 반나절~
1거래일 이상) 동일한 지표로 재확인하는 것을 권고한다. `coverage_score`
threshold 재설계는 이번 턴에서도 착수하지 않는다.

### 123.7 다음 우선 작업(완화안 확정 아님)

1. **1순위(유지)**: 캘린더 시간이 충분히 누적된 뒤(다음 거래일 이후
   권장) `ranking_score` 분포/`ranking_blocked` 비중/`shadow_topk_
   exception_v2` 발동 여부를 동일 지표로 재확인(read-only, 완화안
   아님).
2. **2순위(유지)**: 1순위 관측이 "5거래일 수준"에 근접하면 그 결과를
   근거로 `coverage_score` threshold(`0.48`/`0.22`) 재설계 착수 여부
   최종 결정.
3. **3순위(이전 턴 이월)**: `000720`이 core 유니버스에 20일 이상 연속
   포함되는 조건 원인 확인.
4. **4순위(이전 턴 이월)**: `high_volatility` 단독 경로(001450형) 층3
...
## §124. `relative_activity` 1안 적용 후 운영 관측 추가 축적(2차, 최종 판정)(SPPV-2.136, 2026-07-30 KST)

**전제**: §123(SPPV-2.135)에서 "2안(추가 관측 연장) 유지" 판정 이후,
캘린더 시간이 실제로 경과한 뒤(확인 시각 2026-07-30 11:53 KST) 관측
창을 다시 넓혀 재확인한다. 코드 수정 없음, `.env` 미수정, Full pytest
미실행, 신규 KIS 호출 0건.

### 124.1 누적 관측 창의 크기(사실)

- PR #14 병합 시각: 2026-07-29 12:39:59 KST(03:39:59 UTC).
- 확인 시각: 2026-07-30 11:53 KST — **병합 이후 실제 경과 약 23시간
  13분**(2026-07-29 12:45 KST ~ 2026-07-30 11:51 KST 구간의 decision
  레코드 확보).
- 병합 이후 누적 전체 레코드 n=1,435(BUY-path 기준), 이 중 `core_
  risk_off` 가드가 실제로 평가된 게이트 모집단 n=616. 이는 병합
  이전 1일 치(07-28, gate n=1,037) 대비 약 60% 규모로, **"5거래일"
  에는 못 미치지만 "1~2 거래일" 요청 범위에 근접**한 최초의 관측
  창이다(사실 — 이전 두 턴(SPPV-2.134: n=15, SPPV-2.135: n=134)
  보다 표본이 4~40배 확대됨).

### 124.2 게이트 모집단(`core_risk_off_*` 평가 대상) 기준 비교(사실, read-only 재집계)

앞선 두 턴에서 쓴 "BUY-path 전체 대비 `ranking_blocked` 비중"은
`core_risk_off` 가드가 아예 평가되지 않는 레코드(비 core, 비
bearish_trend 등)까지 분모에 섞여 있어 §120/§122의 게이트 정의와
정확히 일치하지 않았다는 점을 발견했다(사실, 이전 보고의 방법론
정밀화 — 결론을 뒤집는 정정은 아니며 분모 정의를 §120과 통일함).
이번 턴은 §120과 동일하게 **게이트가 실제로 평가된 모집단**만 다시
계산했다.

| 구간 | gate n | ranking_score 평균/중앙값 | `ranking_blocked` 비중 | `shadow_would_pass=True` |
|---|---|---|---|---|
| 07-26(병합 3일 전) | 42 | 0.2694 / 0.2609 | 42/42(100.0%) | 0/42 |
| 07-27(병합 2일 전) | 1,151 | 0.2691 / 0.2609 | 1,151/1,151(100.0%) | 0/1,151 |
| 07-28(병합 1일 전) | 1,037 | 0.2599 / 0.2550 | 1,036/1,037(99.9%) | 0/1,037 |
| 병합 직전 30분 | 60 | 0.2932 / 0.2550 | 56/60(93.3%) | 0/60 |
| 초기 1사이클(SPPV-2.134) | 10 | 0.2939 / 0.2516 | 9/10(90.0%) | 0/10 |
| **병합 이후 누적(~23시간, 이번 턴)** | **616** | **0.3228 / 0.2650** | **559/616(90.7%)** | **0/616** |

### 124.3 판정(사실 + 해석, 명확히 구분)

- **사실**: 게이트 모집단 기준 `ranking_blocked` 비중은 병합 이전
  3일간 99.9~100.0%였고, 병합 직전 30분부터 이미 93.3%로 낮아져
  있었으며, 병합 이후 초기 1사이클(90.0%)과 이번 턴 누적(~23시간,
  90.7%)이 **거의 동일한 수준으로 안정적으로 유지**되고 있다.
- **해석(중요, 방향성 주의)**: 이 이동(99.9~100%→90~93%)은 §120에서
  예측한 `relative_activity` 제거의 방향(제거 시 ranking_score가
  소폭 **낮아져** 통과율이 **더 낮아지는** 쪽)과 **반대 방향**이고
  크기도 훨씬 크다. 즉 이 변화는 **diff의 인과 효과로 볼 수 없다**
  — 병합 직전 30분부터 이미 관측되던 이동이므로, `relative_activity`
  제거가 아니라 그 시점 전후의 **후보 종목 구성/시장 데이터 변화**
  (교란 요인)로 설명하는 것이 타당하다(사실: 병합 전후 시점 경계가
  아니라 병합 30분 전부터 이미 이동이 시작됨).
- **사실**: `buy_candidate`, `eligibility_passed`(core_risk_off
  경로 기준), `APPROVE`, `order_request`, `final_intent='buy'`,
  `shadow_topk_exception_v2`(`shadow_would_pass=True`)는 병합 전후
  **모든 관측 창(초기 1사이클 n=10~16, 병합 직전 30분 n=60~120,
  누적 23시간 n=616~1,435)에서 일관되게 0**을 유지한다.
- **사실**: 전체 BUY-path 기준 `eligibility_passed=True`(125건,
  core_risk_off 경로가 아닌 다른 eligibility 경로)는 `001450`(57건)/
  `001800`(34건)/`000810`(34건) **단 3개 종목의 반복 관측**이며,
  `ranking_score`가 사이클마다 4개의 고정값(0.3864/0.4575/0.4876/
  0.5428)만 반복하고 `buy_candidate`는 전부 `False` — 새로운 BUY
  경로가 아니라 §123에서 이미 확인한 **기존 WATCH 고정 패턴이
  계속되고 있을 뿐**임을 재확인했다.

### 124.4 핵심 병목 재판정

게이트 모집단의 `ranking_blocked` 비중이 병합 전후 90~100% 사이에서
등락하지만, `buy_candidate`/`APPROVE`/`order_request`/`shadow_topk_
exception_v2`는 이번까지 총 3개 관측 창(표본 규모가 15→134→616~1,435
로 40배 이상 확대)에 걸쳐 **한 번도 0에서 벗어난 적이 없다** — 핵심
병목은 여전히 `coverage_score`+절대 threshold(`0.48`/`0.22`) 조합
이라는 판정을 재확인한다(신규 반박 근거 없음).

### 124.5 다음 1순위 결정

**판정: 1안(coverage_score + threshold 재설계 비교로 진행) 채택 —
SPPV-2.134/2.135의 "2안(관측 연장)" 판정에서 전환.**

근거:
1. 이번 턴 표본(gate n=616, 전체 BUY-path n=1,435, ~23시간)은
   이전 두 턴(n=15, n=134)보다 4~40배 확대됐고, 병합 이전 1일치
   (gate n=1,037)와 같은 자릿수 규모에 도달했다(사실) — "1~2
   거래일" 요청 기준에 근접한 최초의 유의미한 표본이다.
2. 핵심 출력 변수(`buy_candidate`/`APPROVE`/`order_request`/
   `final_intent='buy'`/`shadow_topk_exception_v2`)는 표본이 40배
   확대되는 동안 **한 번도 변하지 않고 0을 유지**했다(사실) —
   이는 우연한 소표본 효과가 아니라 안정적으로 재현되는 패턴으로
   판단할 수 있는 수준의 근거다(해석).
3. `ranking_blocked` 비중의 이동(99.9~100%→90~93%)은 있었지만,
   그 이동은 diff 적용 시점이 아니라 **병합 직전부터 이미 시작**됐고
   예측 방향과도 반대이므로 diff의 효과로 해석할 근거가 없다(사실+
   해석) — 즉 "diff가 무언가를 바꿨는지"에 대해서는 이미 안정적인
   결론(바꾸지 않았다)에 도달했고, 추가 관측을 늘려도 이 결론이
   달라질 가능성은 낮다고 판단한다(해석).
4. 반면 `coverage_score`+threshold 조합이 게이트를 지배한다는 사실은
   병합 전후 모든 창에서 90% 이상으로 일관되게 확인됐다 — 다음
   단계(threshold 재설계 비교)로 넘어갈 근거는 이미 충분히 축적됐다
   (해석).

**결론**: `relative_activity` 1안 적용 후 관측 단계를 이번 턴으로
종료하고, 다음 턴부터 `coverage_score`+threshold(`0.48`/`0.22`)
재설계 비교 트랙에 착수할 것을 권고한다. 단, 이는 "설계안 비교
착수" 권고이며 완화안 확정이나 코드 diff 착수를 의미하지 않는다.

### 124.6 다음 우선 작업(완화안 확정 아님)

1. **1순위(전환)**: `coverage_score`+threshold(`0.48`/`0.22`)
   재설계 비교 착수 — 단독 제거가 아니라 재정규화/threshold
   재산정 방식을 비교 설계(완화안 확정 아님, 비교 단계).
2. **2순위(하향)**: (선택적, 필요시) `relative_activity` 1안의
   장기(수 거래일) 효과를 백그라운드로 계속 관찰하되, 더 이상
   1순위 결정을 위한 선행 조건은 아님.
3. **3순위(이전 턴 이월)**: `000720`이 core 유니버스에 20일 이상
   연속 포함되는 조건 원인 확인.
4. **4순위(이전 턴 이월)**: `high_volatility` 단독 경로(001450형)
   층3 관찰 지속 — 이번 턴에서도 `001450`이 `eligibility_passed`
   반복 패턴의 다수(57건)를 차지함을 재확인(기존 관찰과 일치).

## §125. `coverage_score` + 절대 threshold(`0.48`/`0.22`) 재설계 비교(SPPV-2.137, 2026-07-30 KST)

**전제**: §124(SPPV-2.136)에서 "1안(coverage_score+threshold 재설계
비교 착수) 채택" 판정에 따라 이번 턴을 진행한다. 코드 수정 금지, `.env`
미수정, Full pytest/외부 API 호출 금지 — **설계안 비교 + read-only
검증 + 문서 반영까지만** 수행하며, 완화안 확정이나 diff 착수는 하지
않는다.

### 125.1 threshold 역할 분해(코드 재확인, 사실)

- **`0.48`(`_CORE_RISK_OFF_RANKING_MIN_SCORE`)**: `_assess_core_risk_
  off_buy_guard()`의 **최우선 hard gate**(`deterministic_trigger_
  engine.py:615`). `ranking_score < 0.48`이면 `apply_topk_override_
  selected`가 아닌 한 즉시 `eligibility_core_risk_off_ranking_
  blocked`로 차단하고 함수가 반환된다 — 이후의 signal/activity/
  strategy 체크는 아예 평가되지 않는다. §120/122/123/124에서 반복
  확인한 "게이트 모집단의 90~100% 차단"의 **직접적 원인**이 바로 이
  검사다.
- **`0.22`(`_CORE_RISK_OFF_SHADOW_MIN_SCORE`)**: `_assess_core_risk_
  off_buy_guard()` 안에서는 **전혀 참조되지 않는다**(`0.48`만 등장).
  `0.22`는 별도의 `shadow_topk_candidate` 판정(`:856~863`)에서만
  쓰이는 **관찰/실험용 하한**이다 — "이 후보가 `core_risk_off_topk_
  v1` override가 활성화됐을 때 shadow rank top-k 승격 후보가 될 수
  있는가"만 판정하며, override 자체가 명시적으로 선택되지 않는 한
  실제 BUY 판정에 **직접 영향을 주지 않는다**. 과거 여러 턴(§103/104
  등)에서 이 override가 실제로 선택된 사례가 0건임을 이미 확인했다
  — 즉 현재 운영에서 `0.22`는 **사실상 관찰용**이다.
- **구조적 왜곡 정도 비교**: `0.48`이 실질적 병목의 직접 원인이므로
  구조적으로 더 중요하지만, "왜곡"이라는 관점에서는 **둘 다 동일한
  방식으로 왜곡**돼 있다 — 두 threshold 모두 `ranking_score` 공식에
  `0.20*coverage_score`(사실상 상수 `+0.20`, 아래 125.2 참고)가 포함된
  상태로 캘리브레이션됐기 때문에, 그 상수의 존재를 전제로 절대값이
  정해져 있다는 점은 동일하다.
- **둘을 같이 바꿔야 하는지**: `0.48`만 실질적으로 병목에 관여하므로
  "지금 당장 효과를 보려면 `0.48`만 손봐도 되는 것 아니냐"는 질문이
  가능하지만, 두 threshold는 **동일한 `ranking_score` 공식을 공유**
  하므로 공식이 바뀌면 두 threshold의 "격차"(`0.48-0.22=0.26`)가
  의미하는 바도 함께 바뀐다. `0.48`만 이동시키고 `0.22`를 그대로
  두면 격차가 좁혀져(예: 상수 0.20 제거 시 `0.28-0.22=0.06`) `shadow_
  topk_candidate`가 사실상 상시 거짓에 가까워지는 **의도치 않은 부수
  효과**가 생긴다(사실, 산식 대입으로 확인). 따라서 **둘은 함께,
  동일한 크기로 이동시켜야 한다**(해석 — 정합성 유지 목적).

### 125.2 핵심 발견: `coverage_score`는 게이트 모집단에서 예외 없이 상수(read-only 재검증)

전체 이력 기준 `_assess_core_risk_off_buy_guard`가 실제로 평가된
게이트 모집단(reason_codes에 `eligibility_core_risk_off_*` 계열이
존재하는 레코드) n=13,016건을 전수 조사한 결과, **`coverage_score`는
100.0%(13,016/13,016) 전부 `1.0`**이었다(사실, 예외 0건). 참고로 이
게이트에 도달하지 못한 전체 BUY-path 모집단(n=37,399)에서는 `0.1429`
값이 562건 존재하지만, 이 값을 가진 레코드는 애초에 `coverage_score
< 0.50` 상위 eligibility 하드 게이트(`eligibility_low_feature_
coverage`)에서 이미 차단돼 `_assess_core_risk_off_buy_guard` 자체에
도달하지 않는다(사실, 코드 순서상 `_assess_buy_eligibility`의
coverage 체크가 core_risk_off 가드보다 먼저 실행됨).

**이 사실이 의미하는 것**: `0.48`/`0.22` threshold가 실제로 적용되는
population 안에서는 `0.20*coverage_score` 항이 **항상 정확히 `0.20`**
이라는 상수다. 이는 `ranking_score`와 threshold에서 **동일한 상수를
정확히 상쇄**시키는 산술적 보정(exact shift)이 가능하다는 뜻이며,
이것이 아래 A/B 설계안 비교의 핵심 근거다.

### 125.3 `coverage_score` 재설계안 A/B 비교

**공통 전제**: 두 안 모두 "게이트가 실제로 평가되는 population에서
`coverage_score`가 항상 `1.0`"이라는 125.2의 사실에 의존한다 — 이
전제가 깨지면(예: eligibility 하드 게이트의 `0.50` 기준이 바뀌거나
`_build_feature_coverage_score()`의 7개 체크 구성이 바뀌면) 아래
"무변화(zero behavior change)" 결론도 재검증이 필요하다(명시적 전제
조건).

#### A안 — `coverage_score`를 `ranking_score`에서 완전히 제거

세부적으로 두 하위 방식이 있으며, **§120에서 이미 검증한 방식과
이번 턴에 새로 도출한 방식을 구분**해야 한다(중요한 정정 아님 —
§120은 "완전 제거"의 하위 방식 중 두 가지만 검증했고, 이번 턴은
세 번째 하위 방식을 추가로 검증한 것).

- **A-1(§120에서 이미 기각): 단순 차감, threshold 미조정** — `ranking_
  score`에서 `0.20*coverage_score` 항만 빼고 `0.48`/`0.22`는 그대로
  둠. §120에서 `0.48` 통과율 14.8%→0.34%로 붕괴함을 이미 확인 —
  **기각**.
- **A-2(§120에서 이미 기각): 재정규화(divide by 0.80), threshold
  미조정** — 나머지 항을 `/0.80`으로 스케일업해 이론상 0~1 범위를
  유지하되 `0.48`/`0.22`는 그대로 둠. §120에서 `0.48` 통과율 14.8%→
  2.2%로 여전히 붕괴함을 확인 — 재정규화만으로는 절대 threshold와의
  정합성이 회복되지 않음(스케일이 바뀌었는데 절대 기준선은 그대로라
  사실상 기준이 더 엄격해짐) — **기각**.
- **A-3(이번 턴 신규 도출): 완전 제거 + threshold를 정확히 동일한
  상수(`0.20`)만큼 함께 이동** — `ranking_score`에서 `0.20*coverage_
  score` 항을 제거하고, `_CORE_RISK_OFF_RANKING_MIN_SCORE`를 `0.48→
  0.28`로, `_CORE_RISK_OFF_SHADOW_MIN_SCORE`를 `0.22→0.02`로 **동일한
  `0.20`만큼** 낮춘다. 125.2의 사실(게이트 모집단에서 `coverage_
  score`≡`1.0`)에 의해, `entry_score` 등 나머지 항의 값이 무엇이든
  `(ranking_score - 0.20) < (threshold - 0.20)`은 `ranking_score <
  threshold`와 **수학적으로 완전히 동일**하다 — 즉 이 게이트의
  pass/fail 판정 경계가 **한 건도 바뀌지 않는다**(사실, 대수적으로
  증명 가능하며 125.2의 실측으로 전제 조건 충족 확인됨).

#### B안 — `coverage_score`를 유지하되 가중치/threshold를 동시에 조정

`0.20*coverage_score`의 가중치를 예: `0.10`으로 줄이고, threshold도
동일하게 `0.10`만큼 낮춘다(`0.48→0.38`, `0.22→0.12`). 125.2의 동일한
사실(coverage_score≡1.0)에 의해 **이 역시 수학적으로 완전히 동일한
pass/fail 경계를 보존**한다 — 즉 "무변화"라는 안전성 측면에서는
A-3과 **동일**하다.

#### 두 안의 실질적 차이(A-3 vs B, 무변화 여부가 아니라 설계 품질 관점)

| 비교 항목 | A-3(완전 제거) | B(가중치 축소, 예: 0.10) |
|---|---|---|
| 병목 완화/과잉 완화 가능성 | 없음(§120 A-3 무변화 증명) | 없음(동일 증명) |
| `ranking_blocked`/`shadow_topk_exception_v2` 예상 영향 | 없음(수학적 무변화) | 없음(수학적 무변화) |
| 산식 일관성 | `coverage_score`가 게이트 통과 후 판별력이 없는 상수라는 §116~120의 결론과 정확히 일치 — 산식에서 완전히 제거해 구조적 문제(§118 "이관 검토") 해소 | 여전히 "사실상 상수인 항"이 산식에 남음 — §118의 구조적 문제 미해결, "왜 0.10인가"라는 임의의 새 파라미터를 정당화해야 함 |
| diff 범위 | 항 삭제 1곳 + threshold 상수 2곳 변경(§121 `relative_activity` 제거 diff와 동일한 패턴) | 가중치 값 변경 1곳 + threshold 상수 2곳 변경 — diff 크기는 비슷하지만 "새 가중치 근거"를 별도로 설명해야 함 |
| 기존 실측과의 충돌 | 없음(무변화 증명, 125.2 전제 성립 시) | 없음(무변화 증명, 동일 전제) |
| 과잉 완화 위험 | 없음 — 이 diff 자체는 "완화"가 아니라 "리팩터링"(125.4 참고) | 없음(동일) |

### 125.4 중요한 구분: "제거"가 "완화"를 의미하지 않는다

A-3/B 모두 **병목을 완화하지 않는다** — 둘 다 현재의 `eligibility_
core_risk_off_ranking_blocked` 차단율(게이트 모집단 기준 90~100%,
§124)을 **그대로 유지**하도록 설계된 안이다. "`coverage_score`를
`ranking_score`에서 뺀다"는 것은 이번 턴 범위에서는 **산식 정리
(리팩터링)**이지 **완화**가 아니다. 실제로 병목을 완화하려면(예:
`0.28`을 그보다 더 낮추거나, `entry_score`의 가중치·산식 자체를
바꾸는 것) 이 리팩터링과는 **별개의, 별도 승인이 필요한 후속 결정**
이다 — 이번 턴은 그 결정을 다루지 않는다(범위 밖, 명시).

### 125.5 다른 BUY 차단 장치와의 정합성 확인

| 장치 | 상호작용 | 판단 |
|---|---|---|
| `buy_candidate_threshold=0.65`(entry_score 기준) | `coverage_score`는 `entry_score` 산식에 전혀 포함되지 않음(`_build_entry_score()`에 `coverage_score` 참조 없음, 코드 재확인) | **충돌 없음**(완전히 독립된 변수) |
| `eligibility_core_risk_off_ranking_blocked`(`0.48`) | 이번 재설계의 직접 대상 — A-3/B 모두 판정 경계 무변화 | **의도된 무변화, 충돌 아님** |
| `eligibility_low_feature_coverage`(`coverage_score<0.50`) | `_assess_buy_eligibility()`에서 `ranking_score` 계산보다 먼저 평가되는 완전히 별개의 상위 하드 게이트 — `ranking_score` 공식 변경과 무관 | **충돌 없음**(코드 순서상 독립) |
| `shadow_topk_exception_v2`(`0.22`) | 이번 재설계의 대상(125.1에서 `0.48`과 동일한 크기로 함께 이동해야 함을 확인) — 현재 0% 발동 이력이므로 실질적 리스크는 낮으나, 이동시키지 않으면 격차가 좁혀져 향후 override 활성화 시 의도치 않게 왜곡됨 | **함께 이동 필요, 지금 하지 않으면 잠재적 충돌** |
| 기타 hard/soft gate(`eligibility_core_risk_off_signal_blocked`/`activity_blocked`/`strategy_blocked`) | `coverage_score`/`ranking_score`와 무관한 별도 조건(overall/slow/activity/strategy) | **충돌 없음** |

### 125.6 최종 권고

- **1순위 설계안**: **A-3(완전 제거 + `0.48→0.28`, `0.22→0.02`로
  동일한 상수 `0.20`만큼 threshold 함께 이동)**.
  - 근거: B안과 안전성(무변화)은 동일하지만, `coverage_score`가
    게이트 통과 후 판별력 없는 상수라는 §116~120의 기존 결론을
    산식에서 완전히 반영해 §118의 구조적 문제(이관 검토 대상)를
    해소하며, B안처럼 임의의 새 가중치 값을 정당화할 필요가 없다.
- **보류 설계안**: B안(가중치 축소) — 안전성은 A-3과 동일하나 구조적
  이점이 없어 우선순위 하향. A-1/A-2(§120에서 이미 기각)는 재검토
  대상 아님.
- **지금 diff 초안 작성으로 넘어가도 되는지**: **다음 턴부터 가능**
  (이번 턴은 코드 수정 금지 원칙에 따라 설계 비교까지만 수행).
  - 넘어가도 되는 근거: (1) 125.2에서 게이트 모집단 전체 이력
    (n=13,016)의 `coverage_score`가 예외 없이 `1.0`임을 실측
    확인했고, (2) 이 사실에 근거해 A-3이 현재 판정 경계를 수학적
    으로 완전히 보존함을 증명했으며, (3) 다른 BUY 차단 장치와의
    충돌도 확인되지 않았다(125.5). `relative_activity` 1안(§121)과
    동일한 패턴(항 제거 + 최소 검증)으로 진행 가능하다.
  - 단, 이 diff는 **완화안이 아니라 리팩터링**이라는 점을 다음 턴
    PR/보고에서도 명확히 구분해 서술해야 한다(125.4).

### 125.7 다음 우선 작업(완화안 확정 아님)

1. **1순위**: A-3 설계안의 구체적 diff 초안 작성(다음 턴, 사용자
   승인 하에 코드 변경 턴으로 전환) — `ranking_score`에서 `coverage_
   score` 항 제거, `_CORE_RISK_OFF_RANKING_MIN_SCORE=0.28`, `_CORE_
   RISK_OFF_SHADOW_MIN_SCORE=0.02`로 변경, 최소 검증(`test_
   deterministic_trigger_engine.py` 등 관련 테스트 + 하네스
   `accept backend-file`).
2. **2순위**: diff 적용 이후 운영 관측(§121~124와 동일한 패턴) —
...
## §129. `core_cap` 사전순 절단이 만드는 왜곡 정량 검증(SPPV-2.141, 2026-07-30 KST)

**전제**: §128에서 확인한 "구조 편향"이 실제로 신호 기반 선별을 얼마나
훼손하는지 정량 검증한다. 코드 수정 없음, read-only만 수행.

### 129.1 비교 방법(기존 함수/저장값 재사용)

- 최근 20거래일(KST, `2026-07-02`~`2026-07-29`, `07-05`는 snapshot
  부재로 제외)마다, 그날 실제 `core`로 선택된 종목의 `deterministic_
  trigger.entry_score`/`ranking_score`(실측 저장값, 재계산 없음)를
  그대로 사용했다.
- shadow 대안은 그날 `signal_feature_snapshots`에 스냅샷이 존재하는
  후보 풀(80~81개, core-eligible 199종목의 부분집합이지만 실제 매일
  선택되는 종목들의 상위 슈퍼셋)에 대해, **기존 함수** `_build_entry_
  score()`/`_build_buy_ranking_score()`(재구현 없이 그대로 import해
  호출)를 사용해 계산하고, `ranking_score` 내림차순 상위 `core_cap`
  (=12)을 뽑았다. `market_regime`/`strategy_selection`은 그날 실제
  `core` 기록의 최빈 레짐으로, `portfolio_allocation`은 대표값
  (`max_new_capital_pct=2.5`)으로 통일했다 — 이 통일값 때문에
  `ranking_score`의 절대 비교는 근사치이며, **`entry_score`(신호
  자체, allocation/regime과 무관)가 더 신뢰 가능한 비교 지표**다.

### 129.2 20거래일 정량 비교(사실)

| 지표 | 실제(core, 사전순 절단) | shadow(entry_score/ranking_score 상위 12) |
|---|---|---|
| 평균 `entry_score` | **0.1657** | **0.3489** |
| 평균 `ranking_score`(근사, 참고용) | 0.3217 | 0.2169(근사치, 129.1 참고) |
| `buy_candidate` 합계(20일) | 2 | — |
| `eligibility_passed` 합계(20일) | 11 | — |
| 실제∩shadow 겹침 비율(일평균) | **20.3%**(12개 중 평균 2~3개만 일치) | — |

일별 상세는 §129.1 산출 스크립트 결과(20행) 참고 — 모든 거래일에서
겹침 비율이 8.3%~33.3% 범위로, 실제 core 구성의 약 80%가 신호 기반
선별과 다르다.

### 129.3 `000720`/`002790`/`009150` 사례 비교(사실)

| 종목 | 실제 포함일수(20일 중) | 실제 entry_score 범위 | shadow 순위(80~81개 중) | shadow top-12 진입일수 |
|---|---|---|---|---|
| `000720` | 13일[SPPV-2.142에서 정정: 11일] | 0.0~0.0546(대부분 **0.0**, 바닥권) | **58~74위**(하위 10~15%)[SPPV-2.142에서 정정: 55~74위] | **0일** |
| `002790` | 7일 | 0.1161~0.2569(000720보다 뚜렷이 높음) | 14~31위(중위권)[SPPV-2.142에서 정정: 9~31위] | 1일(07-16) |
| `009150` | **0일**(core 경로로 전혀 미등장) | — | **8~24위**(자주 상위 15~30%, 최고 8위) | **6일** |

- **저신호 유지 사례**: `000720`은 20일 중 11일[SPPV-2.142에서
  정정: 원 보고문 "13일"은 완료 보고 시 수동 집계 오류, §130 참고]
  실제 core로 포함됐지만 `entry_score`가 대부분 `0.0`(사실상
  최저치)이고 shadow 순위는 하위 10~15%에 그친다 — 사전순 10위라는
  이유만으로 신호가 거의 없는데도 유지된다.
- **고신호 탈락 사례**: `009150`은 20일 중 6일이나 shadow top-12에
  들 만큼(최고 8위) 상대적으로 신호가 강했지만, 사전순 59위라는
  이유로 core 경로로는 단 한 번도 실제 채택되지 못했다. `002790`도
  `000720`보다 일관되게 높은 `entry_score`를 보이지만 사전순 21위
  때문에 대부분 배제된다(event_overlay로만 산발 등장).
...
근거(수치, 감상 아님):
1. 실제 core 평균 `entry_score`(0.1657)가 shadow 상위 12 평균
   (0.3489)의 **약 47%**에 불과 — 신호 기준으로 봤을 때 실제 선택된
   종목들은 최선의 절반 수준 신호만 담고 있다.
2. 실제∩shadow 겹침이 일평균 **20.3%**(12개 중 2~3개) — 20거래일
   내내 실제 core 구성의 약 80%가 신호 기반 선별과 다르다.
3. `000720`(하위 10~15%, 13/20일[SPPV-2.142에서 정정: 11/20일]
   포함) vs `009150`(상위 15~30%, 0/20일 포함)이라는 극단적 역전
   사례가 20일 내내 일관되게
   재현된다 — 특정 거래일의 우연이 아니라 구조적 패턴이다.

### 129.5 다음 우선 작업(1개만 제안)

**`core_cap` 절단 기준 재설계 검토** — core-eligible 후보에 신호/
랭킹 기준(예: `entry_score` 상위 N, 또는 시가총액/거래대금과의 조합)
을 적용하는 설계안 비교. 완화안 확정 아님, 다음 턴은 설계 비교
단계로 제안.

## §130. SPPV-2.141 핵심 수치 재현성 검증(SPPV-2.142, 2026-07-30 KST)

**전제**: 방향 결론을 새로 바꾸는 턴이 아니라, §129의 핵심 수치를
동일 방법론으로 재현하는 턴이다. 코드 수정 없음, read-only만 수행.

### 130.1 재현 기준(§129와 동일하게 고정)

- 기간: `trade_decisions`에 존재하는 최근 20거래일(KST), `2026-07-02`
  ~`2026-07-29`(오늘 `07-30`은 미완결로 제외) — 이번 재현 시점
  (14:36 KST, §129 대비 약 40분 경과)에도 **동일한 20일 창**이 산출됨.
- 실제 core 집합: 그날 `deterministic_trigger.metadata.source_type
  == 'core'`인 레코드의 종목 집합(저장값 그대로 사용, 재계산 없음).
- shadow 집합: 그날 `signal_feature_snapshots`에 스냅샷이 존재하는
  후보 풀(80~81개)에 `_build_entry_score()`/`_build_buy_ranking_
  score()`(재구현 없이 import해 그대로 호출)를 적용해 계산한
  `ranking_score` 내림차순 상위 `12`(=`core_cap`).
- representative 값: `market_regime`은 그날 실제 core 레코드의
  최빈 `(regime_label, risk_tone)`, `strategy_selection`은 그
  레짐에 대응하는 전략(예: `risk_off`/`bearish_trend`→`defensive_
  low_volatility_rotation`), `portfolio_allocation`은 고정 대표값
  (`max_new_capital_pct=2.5`)으로 §129와 완전히 동일하게 통일했다.

### 130.2 기존값 vs 재현값 비교표

| 지표 | 기존 보고값(§129) | 재현값(§130) | 차이 |
|---|---|---|---|
| 실제 평균 `entry_score` | 0.1657 | **0.1657** | 없음 |
| shadow 평균 `entry_score` | 0.3489 | **0.3489** | 없음 |
| 실제∩shadow 겹침 일평균 | 20.3% | **20.3%** | 없음 |
| `000720` 실제 포함일수 | 13/20일 | **11/20일** | **있음(정정 필요)** |
| `000720` shadow 순위대 | 58~74위 | **55~74위** | 있음(경계값 정정) |
| `002790` shadow 순위대 | 14~31위 | **9~31위** | 있음(경계값 정정) |
| `009150` 실제 포함일수 | 0/20일 | 0/20일 | 없음 |
| `009150` shadow 순위대 | 8~24위 | 8~24위 | 없음 |

### 130.3 차이 원인 분해

핵심 집계 지표(평균 `entry_score` 2건, 겹침 비율)는 **소수점까지
정확히 재현**됐다 — 스크립트/계산 로직 자체는 안정적이고 재현
가능하다. 차이가 발생한 3개 항목(`000720` 포함일수, `000720`/
`002790` shadow 순위 하한)은 §129 원본 스크립트 출력(20행의 종목별
일별 상세 로그)을 이번 턴에 다시 대조한 결과, **§129 완료 보고 시
원본 로그를 수동으로 요약하면서 발생한 집계·전사 오류**로 확인됐다
— 관측 시점 차이(같은 20일 창 재확인됨)도, 모집단 정의 차이(동일
기준 사용)도, 계산 로직/스크립트 차이(동일 스크립트, 동일 결과)도
아니다. `000720`의 실제 `in_actual=True` 일자를 원본 로그에서
다시 세면 `07-02/06/07/08/10/13/20/22/27/28/29` 11일이며, §129
완료 보고문(대화 텍스트)의 "13일"은 수동 집계 시의 단순 오기다
(원본 §129 문서의 표에는 정확한 수치가 실려 있지 않았으므로 문서
자체의 사실 오류는 아니지만, 완료 보고에서 전달된 수치는 정정이
필요하다). `002790`/`000720`의 shadow 순위 하한도 동일한 수동
요약 과정에서 실제 최솟값(9위/55위)보다 높은 값(14위/58위)으로
잘못 축약됐다.
...
- **`왜곡 큼` 판정: 유지.**
- 이유: 핵심 집계 지표(평균 `entry_score` 격차 약 53%, 겹침률
  20.3%)는 정정 없이 완전히 재현됐고, 정정된 `000720`(11/20일,
  여전히 다수 거래일에 저신호로 포함)·`002790`(shadow 9~31위,
  여전히 `000720`보다 뚜렷이 높은 순위대) 수치도 원래 결론(사전순
  절단이 저신호 종목을 구조적으로 유지하고 고신호 종목을 배제한다)
  을 약화시키지 않는다 — 오히려 `002790`의 최고 순위가 9위였다는
  점은 고신호 탈락 사례를 더 뚜렷하게 보여준다.

### 130.5 문서 정정([SPPV-2.142에서 정정])

§129.3의 사례 표 중 다음 두 항목을 정정한다(이력 보존, 원문 유지
하지 않고 정정 표기로 대체):

- `000720` 실제 포함일수: "13일" → **"11일"**로 정정.
- `000720` shadow 순위: "58~74위" → **"55~74위"**로 정정.
- `002790` shadow 순위: "14~31위" → **"9~31위"**로 정정.
...
## §136. `strategy_alignment` 직접항 제거의 threshold 영향 정량 검증(SPPV-2.148, 2026-07-30 KST)

**전제**: SPPV-2.147(§135)에서 **추론 단계로 남겨둔** "게이트 판정 무변화"
주장을 shadow 재계산으로 정량 확인한다. 코드 수정 없음, Postgres
read-only + 코드 read-only만 사용, 신규 KIS 호출 0건. 이번 턴은
**threshold 영향 정량 검증**이며 **운영 효과 확정이 아니다**(운영 실측은
내일 장 시작 후 별도 턴).

`regime_tailwind`는 여전히 **별도 트랙**이다(§134.6 판정은 제거 권고이나
선행 확인 1건 잔존).

### 136.1 shadow 재계산 방법(사실)

저장된 `ranking_score`는 제거 이전 값이므로
`new = old - 0.02 × strategy_alignment`로 재계산했다. `strategy_alignment`
는 `metadata.preferred_strategy ∈ {swing_momentum, event_continuation}`
으로 도출했다. threshold는 **현행 값으로 양쪽에 동일 적용**해
항 제거 효과만 분리했다(`_CORE_RISK_OFF_RANKING_MIN_SCORE=0.28`,
`_CORE_RISK_OFF_SHADOW_MIN_SCORE=0.02`, `_classify_core_risk_off_shadow_
floor_bucket` 내부 하드코딩 `0.26`).

### 136.2 최근 3거래일 vs 전체 이력 비교

**게이트 모집단**(`core_risk_off_experiment.active=true`)

| 항목 | 최근 3거래일(n=2,401) | 전체 이력(n=11,785) |
|---|---|---|
| `strategy_alignment=1.0` | **0건(0.0%)** | **0건(0.0%)** |
| `ranking_score` 평균 | 0.273474 → **0.273474** | 0.259924 → **0.259924** |
| `ranking_score` 중앙값 | 0.255000 → **0.255000** | 0.246700 → **0.246700** |
| `ranking_blocked`(<0.28) | 59.81% → **59.81%** | 76.32% → **76.32%** |
| `shadow_topk_candidate`(≥0.02) | 100.0% → **100.0%** | 100.0% → **100.0%** |
| `shadow_floor` moderate 조건(≥0.26) | 42.27% → **42.27%** | 39.93% → **39.93%** |
| **경계 뒤집힘(3개 threshold 합)** | **0건** | **0건** |
| `buy_candidate`(저장값) | 0/2,401 | 0/11,785 |

**일반 BUY 경로 모집단**

| 항목 | 최근 3거래일(n=4,709) | 전체 이력(n=37,650) |
|---|---|---|
| `strategy_alignment=1.0` | 357건(7.58%) | 2,760건(7.33%) |
| `ranking_score` 평균 | 0.306660 → 0.305144 | 0.325032 → 0.323566 |
| `ranking_score` 중앙값 | 0.282400 → **0.282400** | 0.279600 → **0.279600** |
| `ranking_blocked`(<0.28) | 46.29% → **46.29%** | 50.26% → **50.26%** |
| `shadow_topk_candidate`(≥0.02) | 100.0% → **100.0%** | 100.0% → **100.0%** |
| `shadow_floor` moderate 조건(≥0.26) | 57.78% → **57.78%** | 60.07% → **60.07%** |
| **경계 뒤집힘(3개 threshold 합)** | **0건** | **0건** |
| `buy_candidate`(저장값) | 0/4,709 | 168/37,650 |

평균만 미세 하락(정확히 `0.02×sa 비율`)하고 **모든 threshold 판정은
불변**이다.

### 136.3 경계 뒤집힘이 0건인 이유(사실, 전수 확인)

`strategy_alignment=1.0`인 2,760건(전체 이력)을 전수 조사한 결과:

- **`source_type` 분포**: `event_overlay` 2,718 + `market_overlay` 42 —
  **`core` 0건**.
- **게이트 여부**: `core_risk_off_guard_active`가 **2,760건 전부 `False`**
  — 게이트 모집단에 `sa=1.0`이 단 한 건도 없다(§135.3의 추론을 직접
  확인).
- **`ranking_score` 분포**: min 0.2500 / median 0.5075 / mean 0.4898 /
  max 0.8414 — threshold `0.28`에서 멀리 떨어져 있다.
- **제거폭 `0.02` 내에서 뒤집힐 수 있는 밴드 건수**: `[0.28, 0.30)`
  **0건**, `[0.26, 0.28)` **0건**, `[0.02, 0.04)` **0건**. `0.28` 미달
  37건은 제거 전에도 이미 미달이라 판정이 바뀌지 않는다.

즉 뒤집힘 0건은 우연이 아니라 **`sa=1.0` 레코드가 threshold 근접 밴드에
전혀 존재하지 않기 때문**이다(사실).

`buy_candidate=True` 168건 구성도 확인했다 — `core`/`sa=0.0` 126건 +
`market_overlay`/`sa=1.0` 42건이며 **전부 게이트 밖**이다. `sa=1.0` 42건은
제거 시 `0.02` 하락하지만 `ranking_score` 최솟값이 0.6677이라 **전부
`0.28` 이상을 유지**한다(사실).

### 136.4 코드 경로 재확인 — 일반 경로의 평균 하락이 판정에 무의미한 이유(사실)

`_assess_buy_eligibility()`에서 `ranking_score`가 판정에 관여하는 지점은
`risk_off + bearish_trend` 분기 안의 `if source_type == "core"`
경로뿐이다(`:474-486`). 비-core 경로는 `risk_off_exception_eligible`
여부로 갈리며 `ranking_score`를 직접 비교하지 않는다. 따라서 일반 BUY
경로의 평균 `ranking_score` 미세 하락(0.325032→0.323566)은 **실제 BUY
판정에 영향을 주지 않는다**(사실, 코드 확인).

### 136.5 유일하게 영향이 관측된 지점 — 관찰용 `0.56`(범위 밖, 정직하게 기록)

`event_overlay_experiment`의 `shadow_would_pass`는
`adjusted_ranking_score = clamp(ranking_score + 0.06) >= _EVENT_OVERLAY_
SHADOW_MIN_SCORE(0.56)`를 포함한다. `sa=1.0`이 `event_overlay`에 집중돼
있어 이 지표는 영향을 받는다:

| 창 | `active` n | `adjusted >= 0.56` 통과 | 경계 뒤집힘 |
|---|---|---|---|
| 최근 3거래일 | 1,247 | 92 → **92** | **0건** |
| 전체 이력 | 7,915 | 1,222 → 1,100 | **122건** |

**따라서 "최근 3거래일은 무변화인데 전체 이력에는 경계 이동이 있다"는
사례가 실제로 존재한다** — 단 그 지점은 실제 BUY 판정 경로가 아니라
**관찰용 지표(`0.56`)**다(§131.1/§132에서 이미 범위 밖으로 분류한 값).

다만 `shadow_would_pass`는 `eligibility_passed`·`shadow_signal_pass`·
`shadow_activity_pass`·`shadow_strategy_pass`를 동시에 요구하므로, **실제
저장된 `shadow_would_pass=True` 60건(전체 이력)을 전수 확인한 결과 제거
시 `False`로 뒤집히는 건은 0건**이다(사실). 즉 관찰 지표의 최종 산출값도
실제로는 변하지 않는다.

### 136.6 질문별 답변

1. **게이트 모집단에서 사실상 완전 무변화인가** → **완전 무변화(사실)**.
   "사실상"이 아니라 `sa=1.0`이 게이트 모집단에 0건이므로 `ranking_score`
   자체가 한 건도 변하지 않는다. 3개 threshold 판정 모두 뒤집힘 0건.
2. **최근 3거래일은 무변화여도 전체 이력에 경계 이동이 있는가** →
   **실제 BUY 판정 경로(게이트/`0.28`/`0.02`/`0.26`)에는 두 창 모두
   0건**. 단 **관찰용 `0.56` 지표에서는 전체 이력 122건 이동**이 있고
   최근 3거래일은 0건이다 — 이 비대칭은 존재하며 §136.5에 기록했다.
   그마저도 최종 `shadow_would_pass` 저장값 60건에서는 뒤집힘 0건이다.
3. **`event_overlay`에서만 의미 있고 `core` 게이트에는 영향이 없는가** →
   **그렇다(사실)**. `sa=1.0` 2,760건이 `event_overlay`(2,718)와
   `market_overlay`(42)에만 존재하고 `core`에는 0건이며, 게이트 활성
   레코드는 전부 `False`다. `core`와 `event_overlay`를 섞어 일반화하면
   안 된다.
4. **운영 실측 전 추가 코드 수정이 필요한가** → **필요 없다(판정)**.
   게이트·threshold 판정이 전 구간 무변화이고 실제 `shadow_would_pass`
   산출값도 불변이므로, **그대로 내일 장 시작 후 관찰하면 된다.**

### 136.7 SPPV-2.147 미완료 항목 해소

§135.6-1의 "threshold 영향 정량 미검증(추론 단계)"은 이번 턴으로
**해소**한다. §135.6-2의 "운영 반영·효과 확정"은 여전히 미완료이며
내일 장 시작 후 관측 턴 대상이다.

### 136.8 다음 우선 작업

1. **1순위(통합)**: 내일 장 시작 후 운영 실측 — D안(`core_cap` 신호 기반
   절단, 08:50 KST freeze 대조)과 이번 `strategy_alignment` 제거 무변화
   확인을 **한 번에** 관측.
...
## §138. stale snapshot 근본 원인 규명 + 구조 대응안 비교(SPPV-2.150, 2026-07-31 KST)

**전제**: SPPV-2.149 §137.6의 신규 발견을 "배치 누락"으로 축소하지 않고,
**생성 모집단과 소비 모집단의 구조 불일치**로 규정해 근본 원인을 닫고
구조 대응안을 비교한다. 코드 수정 없음, read-only 코드 확인 + Postgres
read-only + docker logs만 사용, 신규 KIS 호출 0건.

**용어 주의**: 이 절의 안 번호는 `S0~S5`를 쓴다 — §131의 `A/B/C/D안`
(core_cap 절단 기준)과 **다른 문제**이므로 라벨을 분리한다.

### 138.1 근본 원인 — 한 문장

> **`signal_feature_snapshots`를 만드는 모집단(생성)과 그것을 정렬 입력으로
> 쓰는 모집단(소비)이 서로 다른 기준으로 결정되고, 둘 사이에 신선도 계약이
> 전혀 없다.**

"배치가 몇 종목을 빠뜨렸다"가 아니라 **두 경로가 각자 다른 cap과 다른
정렬 기준으로 같은 `compose()`를 호출하는 구조적 불일치**다.

### 138.2 3축 실측(사실)

**축1 — 생성 모집단**: `generate_signal_feature_snapshot_input.py`가
`UniverseSelectionService.compose()`를 호출하는데(`:685-693`),
`core_cap=80`(`DEFAULT_SIGNAL_FEATURE_CORE_CAP`, `:65`)이고
**`core_ranking_mode`를 지정하지 않아 기본값(사전순)**으로 동작한다.
ops-scheduler는 `--core-cap`을 전달하지 않으므로(`signal_feature_batch_
runtime.py:48-60`, `--output`/`--output-format`/`--freeze-purpose`/
`--trigger-type`만 전달) 항상 기본값이다.

실측: 최근 `signal_feature_after_market` freeze는 `core` **79종목**
(07-30 20:10 KST), 일별 67~80종목으로 변동하며 전일 대비 교집합
84.8~94.9%다. core-eligible 사전순 순번 범위는 `(1, 84)` — 사실상
**사전순 상위 ~80 구간**이고, 정확히 1~78이 아닌 것은
`_apply_exclusions`(유동성/우선주 등)가 일부를 빼기 때문이다(사실).

**축2 — 소비 모집단**: `run_decision_loop.py`가 같은 `compose()`를
`core_cap=12` + **`core_ranking_mode=CORE_RANKING_MODE_SIGNAL_SCORE`**
로 호출한다(`:743`). 이 모드에서 정렬 대상은 core-eligible **211종목
전체**다.

실측(2026-07-31): 소비 `core` 12종목 중 **생성 모집단에 포함된 것은
4개(33.3%)뿐**이고, 8개(`021240·023530·028260·032830·042700·196170·
329180·402340`)는 **생성 모집단 밖**이다(사실).

**축3 — freshness check 부재**: `list_latest_by_instrument_ids`의
`WHERE`절은 `instrument_id = ANY(...) AND timeframe = $2`뿐이고
**시간 조건이 없다**. `_prime_core_signal_score_cache`와
`_core_signal_sort_rank` 어디에도 신선도 조건이 **0건**이다(grep 확인).
따라서 31일 지난 점수가 어제 점수와 **동등하게** 경쟁한다.

실측 — core-eligible 211종목의 최신 snapshot 경과일(2026-07-31 KST 기준):
...
즉 소비 대상 211개 중 **신선한 것은 37.4%뿐**이고, 66개는 6월에 한 번
배치 풀에 들어간 뒤 그 시점 점수를 **영구 보유**하며, 65개는 아예 점수가
없어 정렬 최하위로 **영구 배제**된다.

### 138.3 왜 "문제 코드 한 줄만 고치는 안"으로는 부족한가

`_prime_core_signal_score_cache`에 신선도 필터를 넣는 최소 수정(=S1)은
다음 세 가지를 해결하지 못한다:

1. **stale을 숨길 뿐이다**: stale 66개를 최하위로 밀면 stale 정렬은
   사라지지만, D안이 실질적으로 **신선한 79개(=사전순 상위 구간) 안에서만**
   작동한다. 이는 §131.1이 예측한 "사전순 편향이 12위 경계에서 80위
   경계로 이동한 상태"로 **회귀**하는 것이다 — D안이 제거하려던 편향이
   경계만 옮겨 남는다.
2. **생성/소비 불일치를 그대로 둔다**: 생성 80 vs 소비 211 구조는 손대지
   않으므로, 두 경로가 계속 다른 기준으로 같은 `compose()`를 부른다.
3. **다음 변형 국면에서 재발한다**: core-eligible이 늘거나(211→N), 배치
   cap이 바뀌거나, `_apply_exclusions` 결과가 변하면 불일치 폭이 다시
   벌어진다. 또한 snapshot 없는 65개는 **영구 배제 상태가 고정**된다.

### 138.4 대응안 비교(S0=현재 결함 상태, 정상안 아님)

| 안 | 내용 |
|---|---|
| **S0(현재 결함 상태)** | 생성 사전순 ~80 / 소비 신호순 211 / freshness 없음 |
| **S1** | 소비 측에만 freshness guard 추가(N거래일 초과 snapshot을 "없음"으로 취급) |
| **S2** | 생성 모집단을 소비 풀에 정렬 — 배치 `core_cap`을 core-eligible 전체로 확대 |
| **S3** | 생성/보관 구조 리팩터링 — "정렬용 snapshot"과 "배치 coverage snapshot"을 별도 개념/테이블로 분리 |
| **S4** | 정렬 키를 snapshot 비의존 지표(시가총액·거래대금 등)로 교체 |
| **S5 = S2 + S1** | 생성 모집단을 소비 풀에 정렬하고, freshness guard를 안전망으로 병행 |

| 비교 축 | S1 | S2 | S3 | S4 | **S5** |
|---|---|---|---|---|---|
| 근본 원인 해결력 | **낮음**(불일치 유지) | **높음**(불일치 제거) | 높음 | 우회(원인 회피) | **높음** |
| 리팩터링 범위 | 매우 작음(1함수) | 작음(배치 파라미터+계약) | **큼**(새 개념/테이블) | 중(정렬 키 교체) | 작음~중 |
| 순환 의존 위험 | 없음 | **소멸**(cap ≥ 후보 수면 정렬이 선택에 무영향) | 중(경로 2개 관리) | 없음 | **소멸** |
| 기존 freeze/batch/loop 계약 충돌 | 없음 | 없음(`freeze_purpose` 분리 유지) | **있음**(스키마/계약 신설) | 없음 | 없음 |
| stale 재발 방지력 | 중(숨김) | 높음 | 높음 | 해당 없음 | **가장 높음**(이중 방어) |
| 운영 반영 난이도 | 매우 낮음 | **중**(KIS 예산 2.6배, 아래 참고) | 높음 | 중 | 중 |
| 검증 가능성 | 높음 | 높음 | 중 | 높음 | 높음 |
| 성격 | **임시처방** | 구조개선 | 구조개선(과대) | 방향 후퇴 | **구조개선** |

**S2/S5의 운영 비용(사실)**: 배치 입력 생성은 KIS 차트 API를 호출하며
(`_build_chart_client`, `market_data` budget 사용) 80종목에 **66.36초**
소요된다(07-30 21:19~21:20 KST 로그). 실제 snapshot 저장은 0.87초다.
211종목으로 확대하면 KIS 호출량이 **약 2.6배**, 소요 시간이 약 3분
수준으로 늘어난다(추정). 장후 배치라 시간 여유는 있으나 **KIS
`market_data` 예산 소비 증가는 사용자 승인이 필요한 사항**이다.

**S2의 구조적 이점(사실)**: 배치 `core_cap`이 core-eligible 후보 수
이상이 되면 **정렬 기준이 선택 결과에 영향을 주지 않는다**. 따라서
SPPV-2.145 §132.3에서 순환 의존 회피를 위해 둔 제약("배치는 반드시
기본값 사전순을 유지해야 한다")이 **불필요해지고 순환 위험 자체가
소멸**한다 — 구조가 더 단순해진다.

**S4를 권하지 않는 이유(해석)**: snapshot 의존을 끊어 문제를 회피하지만,
D안(§125~§133)의 설계 근거인 "신호 기반 core 선별"을 포기하는 방향
후퇴다. §129에서 정량화한 왜곡(사전순 절단)을 해결한 성과를 되돌린다.

**S3를 권하지 않는 이유(해석)**: 근본 해결력은 높으나 새 개념/테이블
신설이 필요해 범위가 과도하다. S2로 불일치가 제거되면 "정렬용 vs
coverage"를 분리할 실익이 사라진다.

### 138.5 최종 판정 — 다음 diff 초안 1순위

**S5(= S2 + S1) 채택.**

- **S2(생성 모집단 정렬)가 근본**이다: 배치 `core_cap`을 core-eligible
  전체로 올려 **생성 = 소비**를 만든다. 이것만으로 stale 66개와 미보유
  65개가 모두 매일 갱신 대상이 되고, 순환 의존 제약도 소멸한다.
- **S1(freshness guard)은 안전망**으로 병행한다: 배치 부분 실패·신규
  상장·상장폐지 등으로 특정 종목 snapshot이 누락되는 경우에도 stale
  점수가 정렬을 오염시키지 않게 막는다. S2 없이 S1만 쓰면 임시처방이지만,
  **S2와 함께 쓰면 재발 방지 장치**가 된다.
- "가장 쉬운 안"은 S1 단독이지만 §138.3에 따라 편향을 경계만 옮긴 상태로
  고정시키므로 채택하지 않는다.

**예상 diff 범위(다음 턴 설계 대상, 이번 턴은 코드 미수정)**:
1. 배치 cap 기본값 상향 — `generate_signal_feature_snapshot_input.py`의
   `DEFAULT_SIGNAL_FEATURE_CORE_CAP`/`_UNIVERSE_MAX_CAP`(현재 80),
   또는 ops-scheduler가 명시적으로 `--core-cap`을 전달하도록 정렬.
2. freshness guard — `universe_selection.py`의
   `_prime_core_signal_score_cache`에서 기준일 초과 snapshot 제외
   (기준값은 `CompositionContext` 필드로 두어 기본값은 무제한 = 현행
   무변화로 시작하는 것을 권고).
3. 관측 지표 — 신선도 커버리지(신선 종목 수 / core-eligible 수)를
   메타데이터나 배치 지표로 남겨 재발을 조기에 감지.

**선행 확인 필요(미확정)**: KIS `market_data` 예산이 211종목 일일 조회를
감당하는지, 그리고 배치 소요 시간이 장후 스케줄 창을 침범하지 않는지 —
**사용자 승인 및 예산 확인이 필요한 항목**이므로 diff 착수 전에 닫아야
한다.

### 138.6 재현 여부

**재현됨**
- §137.6의 stale 발생 사실 — 소비 core 12개 중 8개가 생성 모집단 밖(33.3%
  일치)으로 재현.
- 배치가 사전순 기준으로 동작한다는 §132.3 서술 — `core_ranking_mode`
  미지정 확인으로 재현.
- freshness 조건 부재 — 코드 grep 0건으로 재현.

**재현되지 않음 / 새로 정정된 인식**
- §137.6은 원인을 "배치가 하루 81종목만 갱신"으로 서술했으나, 이번 턴에서
  **더 정확히는 "생성·소비가 서로 다른 cap과 정렬 기준으로 같은
  `compose()`를 호출하는 계약 불일치"**임을 확인했다. 배치 규모는 결과일
  뿐 원인이 아니다.
- 생성 모집단이 "사전순 정확히 1~80"이라는 단순 서술도 부정확했다 —
  실측 범위는 `(1, 84)`이고 `_apply_exclusions` 때문에 연속 구간이
  아니다(사실).

### 138.7 다음 우선 작업

1. **1순위**: S5 diff 초안 설계 — 단, **KIS 예산·배치 시간 선행 확인**
   (§138.5)을 먼저 닫는다(사용자 승인 필요).
2. **2순위**: `strategy_alignment` 제거의 게이트 영향 재관측(게이트
   활성일).
...
**[SPPV-2.154에서 추가]** — 실제 배포 후 확인한 결과 0051이 실행되지 않았다.
원인 2가지가 겹쳤다:
1. **배포 워크플로 순서 문제(별개 발견)**: `docker compose run --rm migrate`가
   `docker compose up -d --build`보다 **먼저** 실행돼, migrate가 항상
   "직전 배포 시점의 stale 이미지"로 돈다. 이번 배포에서 첫 시도가 정확히
   0050에서 멈춘 것으로 확인됐다(0051 파일이 이미지에 없었음).
2. **이력 백필 로직 자체의 버그(더 심각)**: 이미지를 재빌드한 뒤 재시도했을
   때, `_bootstrap_ledger_if_needed`가 백필 시점에 디렉터리에 존재하던 파일
   **전체**(51개)를 "이미 적용됨"으로 표시하면서 **0051 자신까지 포함**시켜
   버렸다. 즉 이력을 도입하는 바로 그 배포에서 함께 추가된 신규 마이그레이션이
   실행 없이 "적용됨"으로 기록되는 사고였다 — attnum은 그대로 1600, 행 수·
   구조 전혀 변화 없음을 실측으로 확인했다.

수정: `run.py`에 `_LEDGER_BOOTSTRAP_CUTOFF_FILENAME = "0050_..."` 상수를
추가해 백필이 이 파일까지만 포함하도록 제한했다(파일명이 4자리 zero-padded
번호라 문자열 비교로 안전하게 컷오프 판정). 이러면 앞으로 추가되는 모든
마이그레이션은 백필 대상이 될 수 없고 항상 실제로 실행된다. 이미 잘못
기록된 0051 이력 행 자체는 무해하게 남겨두고(0051 파일이 다시 시도되지
않을 뿐 아무 것도 망가뜨리지 않음), 같은 DDL을
`0052_recreate_trade_decisions_reset_attnum_retry.sql`로 새 번호를 부여해
다시 등록했다. 운영 DB에 대해 `COMMIT`→`ROLLBACK` 드라이런으로 재검증(오류
없음, 상태 완전 무변화). 신규 회귀 테스트 1건 추가(SPPV-2.153 실사고를
...
**시간 표기 정정**: 실측 요청은 "2026-07-31 20:10 KST 배치 직후"였으나, 실제
확인 시각은 **2026-08-01(토) 17:34 KST** — 하루가 지난 시점이다. 배치는 이미
7월 31일 밤에 완료됐으므로 read-only 실측 자체는 정상 수행했다. 단
`docker logs`는 이후 배포(PR #79/#80)로 컨테이너가 재기동(오늘 06:29 KST)돼
**당시 stdout 로그는 소실**됐다 — DB의 `signal_feature_batch_runs`
영속 기록(요약 JSON 포함)으로 대체 확인했다. 로그 기반 항목(fetch_error/
retry)은 이 테이블의 카운터로, 시간 기반 항목(freeze 생성 시각)은
`universe_freeze_runs`로 각각 사실 확인했다.

### 142.1 배치 실행 여부·시각(사실)

| 항목 | 값 |
|---|---|
| freeze 생성 시각(KST) | **2026-07-31 20:10:04** |
| freeze `target_count` | 207 |
| `selection_version` | `universe_selection.freeze.v1` |
| batch(적재) `started_at`~`completed_at`(KST) | 20:12:54 ~ 20:12:54 |
| batch `status` | **completed** |
| `fetch_success_count` / `target_count` | 208 / 208 |
| `fetch_error_count` | **0** |
| `persist_success_count` / `persist_error_count` | 208 / 0 |
| `skipped_count` / `final_missing_count` | 0 / 0 |

입력 생성 단계(freeze, 20:10:04)와 적재 단계(batch, 20:12:54 완료) **둘 다
성공**. `fetch_error_count=0`이므로 tail retry는 발동하지 않았다(발동 조건
자체가 성립하지 않음 — 발동 안 한 것 자체가 정상 경로).

freeze `target_count`(207)와 실제 생성된 snapshot 수(208)에 1건 차이가
있다 — 원인은 미확인(held position 등 core 외 소스 1건 추가 가능성,
운영에 영향 없는 수준이라 별도 추적하지 않음).

### 142.2 coverage mode 반영 결과(사실)

- **core-eligible 총원**: 211종목(§140.2 기준선과 동일 판정 로직 재사용)
- **07-31 20:00 KST snapshot 보유**(=오늘 배치로 갱신): **203 / 211 =
  96.2%**
- 기존 80종목 → **208종목**으로 확대(2.6배) — 사용자 승인 한도(최대 300)
  이내, 어제 예측한 "약 211종목" 수준과 거의 일치.
- **미커버 8종목**(`000880`/`005385`/`005387`/`005935`/`006405`/`051905`/
  `051915`/`066575`) 중 **7종목이 우선주**(예: `005935`=삼성전자우,
  `005387`=현대차2우B) — `_apply_exclusions()`의 우선주 배제 규칙
  (SPPV-2.149에서 확인된 로직)에 의해 애초에 core 후보에서 걸러지는
  종목들이다. 나머지 1종목(`000880`, 한화)은 07-29 snapshot(3일 경과)이
  남아있어 guard 기준(5일) 안에서는 **FRESH로 유효**하다 — 즉 실질적으로
  "문제 있는 미커버"는 0건이다.

### 142.3 stale 핵심 8종목 갱신 확인(사실)

| 종목 | 07-31 이전 상태(§140.2 기준선) | 07-31 20:10 배치 이후 | 판정 |
|---|---|---|---|
| `021240` | STALE(37일 경과) | `2026-07-31 20:00`(1일 경과) | **FRESH로 전환** |
| `023530` | STALE(37일 경과) | `2026-07-31 20:00`(1일 경과) | **FRESH로 전환** |
| `028260` | STALE(37일 경과) | `2026-07-31 20:00`(1일 경과) | **FRESH로 전환** |
| `032830` | STALE(39일 경과) | `2026-07-31 20:00`(1일 경과) | **FRESH로 전환** |
| `042700` | STALE(42일 경과) | `2026-07-31 20:00`(1일 경과) | **FRESH로 전환** |
| `196170` | STALE(42일 경과) | `2026-07-31 20:00`(1일 경과) | **FRESH로 전환** |
| `329180` | STALE(42일 경과) | `2026-07-31 20:00`(1일 경과) | **FRESH로 전환** |
| `402340` | STALE(42일 경과) | `2026-07-31 20:00`(1일 경과) | **FRESH로 전환** |

**8종목 전부 FRESH로 전환됐다.**

### 142.4 fresh/stale/missing 재분포(사실) — 배치 전/후 비교

| 계층 | 배치 전(§140.2, 07-31 15:31 KST) | 배치 후(2026-08-01 17:34 KST) |
|---|---|---|
| FRESH | 80 (37.9%) | **204 (96.7%)** |
| STALE | 66 (31.3%) | **1 (0.5%)** |
| MISSING | 65 (30.8%) | **6 (2.8%)** |

남은 STALE 1건은 `005935`(삼성전자우, 우선주 — 애초 core 후보에서 배제
대상)이고, MISSING 6건 전부 우선주다(§142.2). **일반주 기준으로는 stale
bias가 사실상 완전히 해소됐다** — S5의 효과는 "일부 개선"이 아니라
**"stale 해소"에 해당한다**(판정).

### 142.5 예산·실패·재시도(사실)

- `fetch_error_count=0`, `fetch_error_symbols=[]`, `failed_symbols_sample=[]`
  — 부분 실패 없음.
- retry 발동 없음(발동 조건 자체가 성립하지 않아 정상).
- timeout/budget 관련 WARNING은 **stdout 로그 소실로 직접 확인 불가**
  (미확정, §142.6 참고) — 단 `fetch_error_count=0`이 간접 증거로 예산 소진은
  없었던 것으로 판단된다(추정, 사실 아님).
- **배치 소요시간**: freeze 생성(20:10:04) → 적재 완료(20:12:54) =
  **약 170초(2분 50초)**. SPPV-2.151에서 실측한 80종목 기준 66.36초 대비
  208/80=2.6배 종목 확대에 대해 66.36×2.6≈172.5초로 **예측치와 거의 일치**
  (사실+계산, 예측 검증됨).

### 142.6 다음 거래일 08:50 KST freeze 실측 준비 판단

**다음 거래일은 2026-08-03(월)이다**(07-31이 금요일, 08-01/02는 주말).

- **판단: 추가 세팅 불필요, read-only 실측만으로 충분** — §142.4에서 확인한
  대로 core-eligible의 96.7%가 FRESH이고, D안 정렬(`core_ranking_mode=
  signal_score`)과 freshness guard(5일)는 이미 코드 상수로 `run_decision_
  loop.py`에 반영돼 있다(SPPV-2.151/§139, 운영 반영 확인 §140.1).
- 08-03(월) 08:50 KST freeze에서 최신 snapshot이 여전히 07-31 20:00 KST
  기준(주말 동안 배치가 돌지 않으므로 경과일수는 3일)일 것으로 예상된다
  (계산, 사실 아님 — 주말에 배치가 안 도는 것은 이미 알려진 스케줄이지만
  08-03 실제 freeze는 아직 발생하지 않았다). guard 기준 5일 이내이므로
  **여전히 FRESH로 판정될 것으로 예상**된다.
- **미확정**: 08-03(월) 실제 freeze 구성(core-12 등)에서 stale 핵심 8종목이
  실제로 상위권에 등장하는지는 이번 실측 범위가 아니다 — 다음 턴에서
  read-only로 확인해야 한다.

### 142.7 다음 우선 작업

1. **1순위**: 2026-08-03(월) 08:50 KST freeze 실측 — §140.2/§142.4 기준선과
   대조해 D안 정렬 결과에 stale 핵심 8종목이 실제로 상위권에 반영되는지
   확인(read-only).
2. **2순위**: D안 순수 효과 재측정(stale bias가 사실상 해소됐으므로
