...
**[2026-08-06 KST 추가]** 이후 "뒤집힘 270건"으로 재보고된 수치를
검증한 결과, 270은 C 집합(뒤집힌 건수)이 아니라 **구 공식 기준
`buy_candidate=true` 총합**(108+162)이었다 — 실제 뒤집힘은 위에서
확인한 162건/3종목이 맞다. 또한 §13.2.3의 원래 방법론(loose `entry_
score>=0.65` 기준, `eligibility_passed` 미필터)을 오늘 표본에 그대로
재적용하면 216건/4종목(`073240` 추가, activity 게이트로 별도 차단되는
표본까지 포함)이 나온다는 것도 확인했다. 근본 원인은 이 4개 종목이
§13.2.3 측정 기간(06-20~07-31)에 거래 이력이 전혀 없던 **완전히 새로운
유니버스 편입 종목**이라는 것 — 측정 오류가 아니라 모집단(유니버스
...
**[2026-08-06 KST 추가]** `buy_path_variable_gate_matrix.md`
§13.2.14의 counterfactual 후속 분석에서, `buy_candidate` 통과 후
downstream(`risk_opinion`/`evidence_strength`)이 override 여부를
실제로 구분해주는지 재검증한 결과 **판별력이 확인되지 않았다**
(같은 신호 조합이 override된 표본과 안 된 표본 양쪽에 동일하게
나타남, 비교군 2종목·54사이클로 표본도 작음). 이는 축 2("정리된
gate 통과 후 성과")를 실제로 착수할 때, 통과 표본과 차단 표본을
가르는 downstream 메커니즘 자체의 신뢰도를 별도로 검증해야 한다는
선행 조건을 추가한다 — "gate를 통과한 표본이 우월한 성과를 보이는가"
라는 축 2의 질문과, "차단 메커니즘이 실제로 유의미한 근거로
작동하는가"라는 질문은 서로 다른 질문이며, 후자가 약하게 확인된
상태로는 전자의 해석에 주의가 필요하다.

## 36. `SPPV-3` 축 1 Hold 이후 신호 재설계 후보 탐색 + 후보군 동결(2026-08-24 KST, read-only)

이 절은 코드/정책/DB를 전혀 바꾸지 않는다 — read-only 조사와 후보군
"동결"(사전 계약)만 다룬다. §31~33에서 축 1(핵심 대상 3개 신호)이
**Hold로 종합 판정 완료**된 뒤, 다음 신호 재설계 시도가 §17~23의
반복(같은 유형의 절대 기술지표를 계속 변형)에 빠지지 않도록, 과거
10개 후보의 최종 상태를 먼저 못박고 그 위에 최대 3개의 새 후보만
동결한다.

### 36.1 재현 기반 점검

**bar cache**: `logs/_bars_cache_core87_3y_2026-07-14/`(87종목,
2023-07-10~2026-07-14, 733거래일, OHLCV+거래대금 전부 존재
— `stck_clpr`/`stck_oprc`/`stck_hgpr`/`stck_lwpr`/`acml_vol`/
`acml_tr_pbmn`)와 `logs/_bars_cache_core88_2026-07-14/`(88종목,
2025-06-09~2026-07-14, 270거래일, KODEX 200 포함 — 국면 라벨용).
**두 캐시 모두 2026-07-14 이후로 갱신되지 않았다** — 오늘
(2026-08-24) 기준 약 40거래일 가까이 정체돼 있고, 갱신에는 신규
KIS 호출이 필요하다(이번 턴 범위 밖, 뒤 §36.4에서 별도 항목으로
분리).

**검증 스크립트**(`scripts/validate_signal_predictive_power*.py`,
v1~v10): 전부 **CLI 인자가 없다** — 기간/캐시 경로/신호 목록이
모듈 상단 상수로 하드코딩돼 있고(`DIRECT_SIGNALS`, `_BARS_CACHE_
DIR_3Y`, `RECENT_WINDOW_CALENDAR_DAYS` 등), 새 후보를 추가하려면
새 계산 함수를 작성해 기존 함수(`_collect_symbol_samples`/
`_cross_sectional_ic_by_date`/`_newey_west_se_of_mean`/`_quintile_
spread_series`)에 꽂아 넣어야 한다 — "인자만 바꿔 재실행"은 안
되고 코드 추가가 필요하다(이번 턴에는 하지 않음, §36.5에서 다음
작업으로 분리).

**새 후보 계산에 필요한 데이터 중 없는 항목**: 이번에 동결하는 3개
후보(§36.2) 전부 `stck_clpr`/`stck_oprc`/기존 `volatility_20d_pct`
파생만으로 계산 가능해, 캐시 밖의 새 raw 데이터는 필요 없다.
호가창 깊이, 외국인/기관 수급, 시가총액 등급 같은 데이터는 이
캐시에 없고 이번 후보 3개에는 필요하지도 않다 — 향후 다른 후보를
고려할 때 그런 데이터가 필요하면 "데이터 가용성 확인 필요"로 별도
분리한다(이번 턴엔 해당 없음).

**재현성 위험 점검**(PR #341이 확정한 원칙을 그대로 계승):
- **look-ahead**: `market_regime.py`의 `classify_market_regime()`
  을 KODEX 200(069500) 단독 스냅샷에만 적용해 "거래일 하나당 국면
  라벨 하나"를 만드는 방식(§12.2 이후 표준)은 유지한다. 다만 §9.1이
  스스로 밝힌 한계 — "point-in-time universe(당시 편입·편출 종목
  포함)는 시도하지 않았다, 현재 승인된 core universe 리스트로 과거
  데이터를 조회한다" — 는 survivorship bias를 완전히 제거하지
  못한다는 뜻이라 그대로 재확인·인용한다(교정하지 않음, 범위 밖).
- **백필**: bar cache는 KIS 응답을 그대로 저장한 정적 JSON이라
  PR #341이 우려한 "DB 스냅샷 backfill" 문제와는 다른 층이다 —
  다만 캐시 갱신 시점(2026-07-14) 이후 이미 확정된 과거 시세가
  사후에 정정(수정 종가 등)될 가능성은 이 조사에서 확인하지
  않았다(미확인 사항으로 남김).
- **가격 availability/중복**: 캐시는 종목당 파일 하나에 날짜별
  단일 레코드라 PR #341류의 (종목, 거래일) 중복 문제는 구조적으로
  발생하지 않는다(JSON 딕셔너리 키가 날짜라 자연히 dedupe됨).
- **as-of-at**: 이 연구용 스크립트들은 PR #341의 축 3 도구와 별도
  실행 환경(오프라인 bar cache, DB 미접속)이라 같은 `--as-of-at`
  계약을 직접 쓰지 않는다. 대신 §36.3에서 "캐시 as-of 고정일
  (2026-07-14)"과 "스크립트 실행 시각"을 결과에 항상 병기하는
  약한 버전의 동일 원칙을 새 계약으로 못박는다.

### 36.2 후보군 동결안

**기존 10개 후보의 최종 상태**(과거 실측 재확인, 새 판정 없음 —
§17~23):

| 후보 | 정의 | 최종 판정 | 비고 |
|---|---|---|---|
| `fast_score`/`slow_trend` | 운영 정의 | 비교군(Go 후보 아님) | §31.1 |
| `fast_trend`(SMA20 이격) | `fast_score` 성분 | 하락장 역전 최대 원인으로 재확인 | §19.2 |
| `rsi_signal` | `fast_score` 성분 | 상승장 역전(예외 패턴) | §23.2 |
| `fast_score_v2_drop`/`_flip` | `rsi_signal` 제거/반전 | **No-Go** | §18.5 |
| `rsi_mean_reversion` | `-(rsi_14-50)` | **No-Go(범용), 하락장 조건부만 유의** | §20.4 |
| `sma5_over_sma20_gap` | `(sma_5/sma_20-1)×100` | **No-Go**(하락장 역전 오히려 확대) | §20.4 |
| `money_flow_5d` | `Σsign(r_d)·turnover_d / Σturnover_d`(5일) | **No-Go**(전 구간 무신호) | §22.4 |
| `relative_strength_rank_1m` | `return_1m_pct`의 cross-sectional 순위 | **No-Go(범용), 규칙성 확증 참고자료** | §22.4 |
| `reversal_1m` | `-return_1m_pct` | **Hold**(하락장 T+5 방향 일관, 반분 유의 미달) | §18.4 |
| `risk_adj_momentum_3m` | `return_3m_pct / max(volatility_20d_pct, 1.0)` | **Watch(marginal)**, 확정 Go 아님(1차 t=1.47<2) | §17.4/§18.3/§19.5 |

**이번 턴 동결 후보(최대 3개)**:

| 후보명 | 수식/의사코드 | 사용 feature·데이터 출처 | 경제적 가설 | 기존 후보와의 차별점 | 기존 Hold/No-Go와의 관계 | cache로 계산 가능? | 예상 약점·과적합 위험 | 분류 |
|---|---|---|---|---|---|---|---|---|
| **A. `regime_switch_v1`** | 시장 공통 국면이 `bearish_trend`인 거래일 → 신호값 = `reversal_1m`; 그 외(bullish/range/event) → 신호값 = `risk_adj_momentum_3m`(가중 결합 아닌 국면별 전환) | `return_1m_pct`, `return_3m_pct`, `volatility_20d_pct`(캐시 종가로 직접 계산), KODEX 200 공통 국면 라벨 | "상승/횡보장은 모멘텀, 하락장은 역추세"라는 국면 조건부 가설 — 8/10 기존 후보가 재현한 "추세형=상승·횡보 전용, 되돌림형=하락장 전용" 규칙성(§23.2)을 명시적으로 결합 | 새 raw feature 없음, 기존 두 구성요소(`reversal_1m` Hold, `risk_adj_momentum_3m` Watch)를 국면으로 전환 결합한 것만 새로움 | `reversal_1m`(Hold)과 `risk_adj_momentum_3m`(Watch/marginal)을 구성요소로 그대로 흡수 — **이 자체를 새 후보로 포장하지 않고, §19~20에서 이미 "가장 유망한 Watch, 확정 Go 아님"으로 판정된 것의 독립 재검증으로만 취급**(사용자 지시 원칙 그대로 적용) | 즉시 가능(캐시에 이미 구현된 로직, `v8`~`v10` 스크립트가 이미 계산 완료) | 1차창(최근 12개월) 하락장 표본이 구조적으로 0일이라 §16.2 게이트를 근본적으로 통과 못 할 수 있음(관찰 유예 규칙 A로 대기 중, 모니터링 스크립트 최근 판정 `NOT_TRIGGERED`). Rule C(적응형 최소창) 유형의 데이터 스누핑은 §20.2에서 이미 명시적으로 거부됨 — 같은 함정 재발 금지 | **재검증 후보** |
| **B. `overnight_intraday_split_momentum_v1`**(신규) | `overnight_ret_Nd = Σ_{i=1..N} ln(open_i/close_{i-1})`, `intraday_ret_Nd = Σ_{i=1..N} ln(close_i/open_i)`(N=5 기본); 후보 신호 = 두 성분을 각각 독립적으로 IC 검증(단일 스칼라로 합치지 않음, "divergence = overnight_ret − intraday_ret"는 보조 지표로만) | `stck_oprc`(시가)/`stck_clpr`(종가) — 캐시에 이미 존재, 신규 raw 데이터 없음 | 오버나이트(장 마감~다음 시가)는 기관/뉴스/해외 시장 정보가 반영되는 구간, 장중(시가~종가)은 국내 개인·모멘텀 매매가 우세한 구간이라는 마이크로스트럭처 가설(고전적 overnight/intraday return 분해 문헌과 동일 계열) — 두 구간의 예측력이 서로 다를 것 | 기존 10개 후보 전부 종가-대-종가(close-to-close) 수익률만 썼다 — 시가를 분리해 쓴 후보는 이번이 처음(조사로 확인, §36.1) | 기존 후보와 직접 겹치지 않음(구성요소가 다름) — 다만 "단기 되돌림"류(`reversal_1m`)와 유사한 반응이 intraday leg에서 재현될 위험은 배제하지 않고 결과 해석 시 명시 | 즉시 가능(시가/종가 전부 캐시에 있음, 신규 계산 함수만 필요) | 저유동성 종목의 시가 결정(단일가매매) 노이즈가 오버나이트 leg에 과대 반영될 위험, 상/하한가 이벤트 처리 미정(제외 규칙 사전 필요), 지수/개별 로그수익률 합산 시 누적 오차 | **신규 재설계 후보** |
| **C. `low_volatility_rank_20d`**(신규) | 그날 표본의 `volatility_20d_pct`(20거래일 realized vol)를 cross-sectional 오름차순 순위로 매기고 `[-1,1]`로 스케일링(낮은 변동성이 +1에 가까움) — `relative_strength_rank_1m`(§22.1)과 같은 순위 스케일링 방식을 재사용하되 대상 변수만 변동성으로 교체 | `volatility_20d_pct`(캐시 종가로 직접 계산 가능, `signal_feature_snapshots`에도 이미 존재) | 저변동성 이상현상(low-volatility anomaly) — 과거 수익률 방향과 무관하게, 변동성이 낮은 종목이 위험조정 기준으로 더 나은 forward 성과를 보인다는 가설(학계에서 반복 확인된 anomaly, 이 시스템에서는 미시도) | 기존 10개 후보 전부 수익률(모멘텀/되돌림/자금흐름/상대강도) 기반이었다 — 이 후보는 **수익률 방향 성분이 전혀 없는 순수 리스크 기반 순위**라는 점에서 유일하게 구조적으로 다른 축 | `risk_adj_momentum_3m`(수익률/변동성 비율, 분자에 수익률 있음)과 달리 분자 자체가 없다 — 개념적으로 독립이나, "짧은 horizon 시스템에 저변동성 anomaly가 존재하는가"라는 전제 자체가 학계 문헌과 다른 horizon(T+1/5)이라 이식성 불확실 | 즉시 가능(종가만으로 20일 realized vol 계산) | 저변동성=대형/고유동성 종목 편향 가능성(이 시스템의 universe 선정 필터와 상관될 위험), anomaly가 통상 1~12개월 horizon에서 보고돼 T+1/5의 짧은 horizon에는 약하거나 없을 수 있음 | **신규 재설계 후보** |

**동결 원칙 재확인**: 위 3개가 이번 턴에 사전 고정하는 후보군
**전체**다 — 실측 결과를 본 뒤 후보를 추가하지 않는다. `risk_adj_
momentum_3m`은 후보 A의 구성요소로 이미 포함되므로 별도 4번째
후보로 세지 않는다.

**R3b/`regime_switch_v1`에 대한 별도 트랙 관계(factual, 이번 턴
범위 확인용)**: `[PRIORITY_MAP]`을 조회한 결과 `regime_switch_v1`
신호는 이미 "R3b"라는 별도의 대규모 운영 검증 트랙(gate wiring
검증, shadow cycle 실행기, stop-loss/exit timing ablation 등 다수
스크립트: `validate_r3b_*.py`, `run_regime_conditional_shadow_
cycle.py`, `shadow_regime_conditional_entry_signal.py`)을 갖고
있으며, 그 트랙에서는 "Conditional Go"라는 **다른 축의 판정**을
이미 받은 상태다. 이는 이번 SPPV-3 축 1 스타일 Newey-West 통계
게이트(§16.2/§31.3)와는 **별개 평가 축**이며, "SPPV-3(운영 코드
반영) 진입은 R3b의 성과와 무관한 §3 게이트 미충족으로 아직 이르다"
는 서술이 `[PRIORITY_MAP]`에 이미 남아있다. **이번 턴은 R3b
운영 트랙을 전혀 건드리지 않는다** — 후보 A는 순수하게 SPPV-3
축 1식 통계 게이트 관점의 독립 재검증만 다루며, R3b 트랙의 상세
현황은 이번 조사에서 전부 재확인하지 않았다(미확인 사항, §36.6).

### 36.3 사전 고정 평가 계약

결과를 보기 전에 다음을 고정한다 — 실측 후 이 계약을 소급 수정하지
않는다.

- **평가 horizon**: T+1/T+5/T+20 3종 병행(기존 스크립트는 T+5/T+20만
  focus했으나, PR #341 축 3 도구와의 일관성을 위해 T+1도 추가한다).
- **primary metric**: 거래일별 cross-sectional Spearman IC(종목×
  거래일) → Newey-West(Bartlett 가중치) 보정 pooled t-통계.
- **보조 지표**: hit-rate(quintile spread 부호 일치율), 표본
  거래일 수·종목-일 수, 비용 차감 quintile spread — 기존 §16.2/
  §31.3 Go 게이트와 동일한 산출물을 그대로 재사용.
- **train/validation/OOS 분리**: 이 후보들은 파라미터를 데이터에
  적합(fit)한 모델이 아니라 경제적 가설로 사전 설계한 수식이므로
  "학습/검증 분리"보다는 **"수식을 본 뒤 데이터를 보았는가"**가
  핵심 스누핑 위험이다. 계약: 아래 1차/2차 두 창을 **동일한 이
  frozen 수식**으로 각각 한 번씩만 계산하고, 결과가 기준 미달이면
  같은 후보의 수식을 변형해 재시도하지 않는다(변형이 필요하면
  "새 후보"로 별도 턴에 다시 동결 절차를 거친다).
  - **1차(최근 12개월, 캐시 as-of 2026-07-14 기준 롤링)**: §16.2와
    동일한 최근성 우선 창.
  - **2차(3년, 국면 커버리지 게이트)**: §14와 동일한 시장 공통
    국면 분해.
- **walk-forward**: 전통적 ML의 rolling walk-forward 재학습은
  적용하지 않는다(파라미터 적합 모델이 아니므로) — 대신 1차/2차
  이원 창 자체가 "최근 구간에서도 유지되는가 + 장기 국면 전체에서
  깨지지 않는가"라는 이중 시점 검증 역할을 한다(§16.2 원안 그대로
  계승, 새로 설계하지 않음).
- **층화**: `risk_tone`, `regime_label`(KODEX 200 공통 국면:
  `bullish_trend`/`bearish_trend`/`range_bound`/`event_driven_
  unstable`) 기준 층화를 유지한다.
- **최소 표본 조건**: 국면 계층 최소 표본 `MIN_REGIME_TRADING_DAYS
  = 30`(§20.2 상수 그대로 재사용) — 미달 국면은 판정에서 제외하고
  "표본 부족으로 판정 보류"로 명시한다.
- **Go/Watch/Hold/No-Go 판정 기준**(§16.2/§31.3을 그대로 계승,
  완화 없음):
  - **Go**: 1차(`|t_NW|≥2`, 부호 정확) **그리고** 2차(하락장 포함
    필수 국면에서 유의한 역전 없음)를 **모두** 충족.
  - **Watch**: 둘 중 하나만 통과하거나, 한쪽이 marginal(`1.5≤|t_NW|
    <2`)하지만 방향이 두 창에서 일관될 때.
  - **Hold**: 어느 창도 유의성이 없으나 부호 역전도 없을 때.
  - **No-Go**: 필수 국면(특히 하락장)에서 유의한 방향 역전이
    확인될 때.
- **하락장·횡보장 표본 부재 시 제한 규칙**: 해당 국면 표본이
  `MIN_REGIME_TRADING_DAYS` 미달이면 그 국면에 대해서는 Go/No-Go
  승격을 금지하고 "판정 보류"만 허용한다(§20.2 규칙 A 계승,
  Rule C류 적응형 창 축소는 데이터 스누핑으로 재차 금지).
- **재현 실행 cutoff 계약**: 이 오프라인 bar-cache 스크립트들은
  DB에 접속하지 않아 PR #341의 `--as-of-at` 파라미터를 직접 쓰지
  않는다. 대신 모든 실행 로그·산출 JSON에 **(1) 캐시 as-of 고정일
  (`2026-07-14`, 캐시가 갱신되지 않는 한 불변)과 (2) 스크립트
  실행 시각(KST)**을 항상 함께 기록한다 — 캐시가 갱신되기 전까지는
  "몇 번을 다시 실행해도 같은 입력"이라는 게 이 cutoff 계약의
  재현성 보장이다.
...
| 후보 | 판정 | 근거 |
|---|---|---|
| A. `regime_switch_v1` | **즉시 cache 재검증 가능**(같은 캐시로 재현) — 단, 진짜 out-of-sample(2026-07-14 이후 신규 거래일)은 **데이터 계약 확인 후 가능**(캐시 갱신에 신규 KIS 호출 필요, 이번 턴 범위 밖) | 계산 로직이 이미 `v8`~`v10`에 구현돼 있어 재실행만 하면 됨. 새 데이터가 없으면 §19~20과 같은 결과가 그대로 재현될 뿐 "독립" 재검증의 의미가 약함 — 진짜 독립성은 캐시 갱신 이후에만 확보 |
| B. `overnight_intraday_split_momentum_v1` | **즉시 cache 재검증 가능** | `stck_oprc`/`stck_clpr` 전부 캐시에 존재, 계산 함수만 신규 작성하면 됨 |
| C. `low_volatility_rank_20d` | **즉시 cache 재검증 가능** | 종가만으로 20일 realized vol 계산 가능, 순위 스케일링 로직은 `relative_strength_rank_1m`(§22.1) 구현을 그대로 재사용 가능 |

**이번 후보 탐색이 Stage B 착수 조건을 바꾸는가— 아니다.**
후보 3개는 아직 **동결됐을 뿐 한 번도 실행되지 않았다** — 실측
결과가 전혀 없는 상태다. §16.7/§18.7의 원칙(축 1의 새로운 Go
증거 없이는 Stage B를 열지 않는다)은 이번 턴으로 전혀 바뀌지
않는다. 설령 다음 턴에 이 중 하나가 Go로 판정되더라도, 그것은
"새 신호 하나가 축 1을 통과했다"는 뜻일 뿐 — 축 2/3/4가 여전히
미완료·관측 중인 상태이므로 Stage B 착수에는 추가로 그 축들의
결과가 필요하다.

### 36.5 다음 구현/분석 작업 단위

| 단계 | 내용 | 코드 변경 | DB write | 운영 위험 | 필요한 검증 |
|---|---|---|---|---|---|
| 1 | 재현 스크립트 보강(§36.1의 기존 함수를 재사용하는 신규 계산 함수 추가 — 후보 B/C용) | 있음(연구용 read-only 스크립트, 운영 코드 아님) | 없음 | 없음(read-only, 로컬 캐시만 사용) | `accept script-file`, 신규 함수 단위 테스트 |
| 2 | 후보별 계산 구현 — A는 기존 재사용, B/C는 신규 함수를 §36.3 계약대로 1차/2차 창에 연결 | 있음 | 없음 | 없음 | 계산 결과가 §36.1 캐시 범위와 일치하는지 스팟체크 |
| 3 | cache 기반 이원 게이트(1차/2차) 실행 | 없음(스크립트 실행만) | 없음 | 없음(신규 KIS 호출 없음) | 실행 로그+JSON 산출물 보존, §36.3 cutoff 계약 기록 |
| 4 | 독립 구간 재검증(캐시 갱신 후 2026-07-14 이후 신규 거래일 포함) | 없음(캐시 갱신 스크립트는 기존) | 없음 | **신규 KIS read-only 호출 필요 — 사용자 승인 필요** | 캐시 갱신 후에도 §14/§16과 동일한 point-in-time 한계(§9.1) 재확인 |
| 5 | shadow 관측 설계(Go/Watch로 격상된 후보가 있을 경우, PR #341 축 3 도구와 유사한 패턴으로 운영 DB 대비 read-only shadow 계산 스크립트 설계) | 있음 | 없음 | 없음(read-only) | 신규 스크립트에 대한 `accept` 계열 전체 |
| 6 | 운영 정책 반영 판단(축 1 Go 확정 + 축 2/3/4 결과 충분히 축적된 뒤) | 이번 계획 범위 밖 | 이번 계획 범위 밖 | 이번 계획 범위 밖 | Stage B 착수 게이트(§16.7)와 동일 |

### 36.6 사실 / 해석 / 미확인 사항

- **사실**: §36.1의 캐시 범위·스크립트 구조, §36.2 표의 과거
  10개 후보 판정(원문 인용), R3b 관련 트랙 존재 자체.
- **해석**: 후보 A/B/C의 경제적 가설, 예상 약점, "이번 3개가
  기존 10개와 충분히 다르다"는 차별화 판단.
- **미확인**: R3b 트랙의 최신 상세 수치(이번 조사에서 전체
  재확인하지 않음), bar cache의 2026-07-14 이후 수정 종가
  정정 가능성, 저변동성 anomaly의 실제 이식 가능성(후보 C, 실행
  전이라 당연히 미확인).

## 37. §36 정정 — "1차/2차 이원 게이트"는 독립 표본이 아니다(2026-08-24 KST 후속, read-only 문서 정정)

이 절은 §36의 서술을 지우거나 덮어쓰지 않고, "독립 표본"으로
오해될 수 있는 표현을 정정하는 append-only 절이다. 코드/테스트/
DB/캐시/운영 설정/컨테이너는 이번 정정에서 전혀 건드리지 않았다.

### 37.1 정정해야 할 핵심 사실

§36.3("사전 고정 평가 계약")의 1차(최근 12개월)/2차(3년) 이원
창은 **둘 다 같은 정적 bar cache** `logs/_bars_cache_core87_3y_
2026-07-14`**를 그대로 사용한다** — §36.1이 이미 이 캐시 하나만
존재한다고 명시했음에도, §36.3의 "train/validation/OOS 분리"라는
소제목과 "이중 시점 검증"이라는 서술이 마치 두 창이 서로 다른
독립 데이터를 본다는 뜻으로 읽힐 수 있었다.

실제로는:
- **1차(최근 12개월) 창은 2차(3년) 창에 완전히 포함되는 부분
  집합이다** — 같은 733거래일 캐시에서 가장 최근 약 245거래일만
  잘라 쓴 것이지, 별도로 수집한 표본이 아니다.
- 두 창은 **같은 캐시에서 기간이 겹치는 최근성·장기 국면 재검증
  창**이며, 서로 독립된 out-of-sample 표본이 아니다.
- §36.2 후보 A 행의 "§19~20에서 이미 '가장 유망한 Watch, 확정
  Go 아님'으로 판정된 것의 **독립 재검증**으로만 취급"이라는
  표현, §36.4 표의 "'독립' 재검검증의 의미가 약함 — 진짜 독립성은
  캐시 갱신 이후에만 확보"라는 표현, §36.5 4단계의 "독립 구간
  재검증"이라는 표현 — 이 중 §36.4와 §36.5는 이미 "캐시 갱신
  이후에만 진짜 독립"이라고 정확히 단서를 달아뒀으나, §36.2와
  §36.3은 그 단서 없이 "독립"이라는 단어만 나열돼 오해 위험이
  있었다. 이 절에서 그 위험을 명시적으로 닫는다.

### 37.2 표현 구분(정정)

| 구분 | 현재 상태 |
|---|---|
| **현재 가능** | 사전 고정 수식(§36.2에서 동결한 그대로, 실측 후 변형 금지)을 같은 캐시의 최근 구간(1차)과 전체 3년(2차)에 **각각** 적용하는 **최근성·장기 국면 재검증** — 같은 모집단 안에서 "최근에도 유지되는가"와 "장기 국면 전체에서 깨지지 않는가"를 나눠 보는 것일 뿐, 두 창이 서로 모르는 별개 데이터를 보는 것은 아니다. |
| **현재 불가능** | 진정한 independent out-of-sample 검증 — 1차 창이 2차 창의 부분집합이므로, 1차에서 통과했다고 해서 "2차가 보지 못한 새 데이터에서도 통과했다"는 근거가 되지 않는다. |
| **독립 검증이 가능해지는 조건** | bar cache 기준일 `2026-07-14` **이후의 거래일을 포함하도록 캐시를 갱신**한 뒤(신규 KIS read-only 호출 필요, 사용자 승인 필요 — §36.5 4단계와 동일), **동결된 수식을 한 글자도 바꾸지 않고** 그 신규 구간에 재실행해야 한다. 그래야 "이미 관찰한 데이터에서 도출한 후보"가 "아직 보지 않은 데이터"를 통과하는지 확인하는 진짜 out-of-sample 검증이 성립한다. |

### 37.3 후보 동결과 Stage B 판단에 미치는 영향

- **후보 동결의 효용은 부정하지 않는다** — §36.2에서 후보를
  최대 3개로 사전 고정하고 실측 후 수식 변형·후보 추가를 금지한
  장치는 그대로 유효하다. 이는 "이후 데이터 스누핑(결과를 본
  뒤 후보를 계속 바꿔가며 유의성을 찾는 행위)"을 막는 절차이며,
  이번 정정과는 다른 문제를 다룬다.
- **다만 이미 관찰한 캐시에서 후보를 도출했다는 사실은 남는다**
  — 후보 B(`overnight_intraday_split_momentum_v1`)와 C(`low_
  volatility_rank_20d`)의 경제적 가설은 §36.1에서 이 캐시의
  구조(시가/종가 존재, 20일 변동성 계산 가능)를 이미 확인한
  뒤 설계됐다. 즉 "수식을 먼저 정하고 데이터를 나중에 봤다"는
  강한 사전등록(pre-registration)이 아니라, "이 데이터로 무엇을
  계산할 수 있는지 안 상태에서 경제적으로 그럴듯한 수식을 골랐다"
  는 약한 형태의 사전 고정이다. **따라서 현재 cache 결과만으로는
  일반 Go 판정이나 운영 반영을 허용하지 않는다** — §36.3의
  Go/Watch/Hold/No-Go 판정 기준 자체는 유지하되, 이 캐시(1차/2차
  모두 포함)에서 나온 결과는 "같은 관찰 캐시 내 최근성·국면
  재검증 통과"로만 해석하고, §37.2의 조건(캐시 갱신 + 수식 불변
  재실행)을 충족하는 진짜 out-of-sample 재현이 확인되기 전까지는
  운영 정책 반영(§16.7의 Stage B 게이트, §36.5 6단계)의 근거로
  쓰지 않는다.
- **`regime_switch_v1`의 R3b `Conditional Go` 구분은 변경 없음**
  — §36.2가 이미 명시한 대로, R3b의 "Conditional Go"는 별도
  운영 검증 트랙의 판정이며 SPPV-3 축 1식 통계 게이트의 Go와
  다른 축이다. 이 정정 이후에도 그 구분은 그대로 유지된다.
- **Stage B 보류 판정은 변경 없음** — §16.7/§18.7/§36.4의 "축
  1의 새로운 Go 증거 없이는 Stage B를 열지 않는다"는 원칙은
  이번 정정으로 더 강화될 뿐 약화되지 않는다(같은 캐시 재검증
  통과는 "새로운 독립 Go 증거"에 해당하지 않으므로).

### 37.4 다음 진짜 독립 검증을 위한 조건(요약)

1. bar cache를 `2026-07-14` 이후 거래일까지 갱신한다(신규 KIS
   read-only 호출, 사용자 승인 필요).
2. §36.2에서 동결한 후보 A/B/C의 수식을 **한 글자도 바꾸지 않고**
   그대로 재사용한다.
3. 갱신된 캐시의 신규 구간(2026-07-15 이후)에 대해서만 §36.3의
   판정 기준을 적용해, 그 결과가 기존 캐시 결과와 방향이
   일치하는지 확인한다 — 이것이 이 트랙에서 확보할 수 있는 가장
...
## 39. 후보 B/C cache 기반 1차 결과 판정(2026-08-24 KST, read-only)

이 절은 §36.3/§37을 **변경 없이 그대로 적용**해 §38의 실행 결과를
판정한다. **이 판정은 §37이 정한 "같은 cache 기반 최근성·장기
국면 재검증"일 뿐이며, independent out-of-sample 검증이 아니다
— 어떤 후보가 여기서 Go/Watch로 나와도 운영 반영이나 Stage B
착수 근거로 쓰지 않는다.**

### 39.1 실행 환경과 cache cutoff

- `bash scripts/harness/docker_dev_exec.sh python3 scripts/
  validate_signal_predictive_power_v11_candidate_bc_freeze.py`
  (`network_mode=none`) — 정적 cache만 읽었음을 확인.
- 실행 시각: 2026-08-24 14:51:50 KST. cache as-of:
  `2026-07-14`(§36.2 동결 시점, 갱신 없음).
- 87종목, 653거래일(2023-10-10~2026-06-16).
- 원본 결과 JSON: `logs/signal_ic_sppv3_candidate_bc_freeze_
  2026-07-14.json`(`.gitignore` 대상, 커밋하지 않음 — 기존
  `logs/signal_ic_sppv2_*.json` 관례와 동일).
- `load_exclusion_counts`(`close_missing_or_nonpositive`=0,
  `open_missing_or_nonpositive`=0), `compute_exclusion_counts`
  (빈 dict — `candidate_c_signal_snapshot_failed`/`regime_label_
  unavailable`/`fwd_*_nonfinite` 전부 0건) — **예외가 0건이었다는
  사실만 기록한다. 이것이 "예외 처리 설계가 완벽함"을 뜻하지는
  않는다** — 이번 cache(87종목·653거래일)가 우연히 깨끗했을
  뿐이며, 결측/실패 경로 자체는 §38.4/§38.8~38.10에서 단위
  테스트로만 검증됐지 이번 실행에서 실제로 발동한 적은 없다.
- 국면 표본: `bearish_trend` 96일, `range_bound` 206일,
  `bullish_trend` 351일 — 전부 `MIN_REGIME_TRADING_DAYS=30` 충족.
  `event_driven_unstable`은 이번 cache에 0일(관측 자체가 없음).

### 39.2 후보 A(`regime_switch_v1`) 재현 가능성 확인(실행하지 않음)

`scripts/validate_signal_predictive_power_v8_fast_score_teardown.py`
/`_v9_gate_and_fast_features.py`/`_v10_new_fast_features.py`
(regime_switch_v1이 정의·검증된 스크립트들)는 소스를 확인한 결과
**bar cache가 이미 있어도 `main()`이 무조건 `agent_trading.
runtime.bootstrap._build_kis_live_quote_client(settings)`를 호출**
한다 — cache hit 여부와 무관하게 KIS 실거래 설정(`KIS_LIVE_INFO_
*`)이 없으면 즉시 `SystemExit`으로 실패한다. 이는 **외부 호출 없이
재현 불가능**하다는 뜻이라, 이번 턴에서는 실행하지 않았다(재실행
불가를 후보 실패나 성공으로 해석하지 않는다 — 지시사항 그대로).

대신 이미 문서에 확정된 결과(§19.5, §20.2~20.3, §21.3)를 참조값
으로만 인용한다: **`regime_switch_v1`은 2차(3년) pooled에서
T+5 t_NW=2.60/T+20 t_NW=2.36으로 이 신호 재설계 트랙 전체에서
가장 강한 2차 결과를 냈으나, 1차(최근 12개월)는 하락장 표본
0일로 §16.2 게이트를 완전히 통과하지 못해 "가장 유망한 Watch,
확정 Go 아님"으로 남아 있다.** 관찰 유예(규칙 A) 모니터링
(`scripts/monitor_regime_switch_v1_gate.py`)의 마지막 확인 판정은
`NOT_TRIGGERED`(2026-07-14 기준, 최근 12개월 bearish_trend 0일)
였다 — 이번 턴에 재확인하지 않았다(그러려면 벤치마크 KIS 조회가
필요).

### 39.3 후보 B(`overnight_intraday_split_momentum_v1`) 결과

| horizon | 1차(12개월) t_NW | 2차(3년) 전체 t_NW | 하락장(96일) t_NW | range_bound(206일) t_NW | bullish(351일) t_NW |
|---|---|---|---|---|---|
| **overnight_ret_5d** T+1 | -1.21 | **-3.01** | -1.06 | **-2.17** | -1.86 |
| T+5 | -0.85 | **-2.46** | -1.04 | **-2.71** | -1.18 |
| T+20 | 0.14 | -1.07 | -0.62 | **-2.05** | -0.02 |
| **intraday_ret_5d** T+1 | 0.57 | **-2.84** | **-3.70** | -1.32 | -1.07 |
| T+5 | 1.04 | -1.21 | **-2.51** | -0.26 | -0.41 |
| T+20 | 0.71 | 0.02 | -0.43 | 1.39 | -0.92 |

(모든 값 Newey-West 보정 t-통계. **굵게 표시한 값이 `|t|≥2`를
충족.** 두 신호 모두 원자재는 `n=225`(1차)/`n=653`(2차) 거래일,
`valid_row_count`는 결측 0건이라 전체 표본과 동일 — §39.1 참고.)

- **1차 창은 두 신호 모두 모든 horizon에서 `|t_NW|<1.5`** —
  §16.2/§36.3의 "marginal"(1.5~2.0) 문턱조차 넘지 못한다.
- **2차 창(전체 cache)에서 `overnight_ret_5d`는 T+1/T+5가 유의**
  (부호는 음수 — "최근 5거래일 오버나이트 수익률이 높을수록
  향후 수익률이 낮다"는, 사전 가설(정보 확산 방향)과는 반대
  방향). 국면별로는 `range_bound`(206일)에서 세 horizon 모두
  유의하고, `bearish_trend`(96일)에서는 방향은 같은 음수이지만
  유의하지 않다(`|t|≤1.06`) — **부호가 국면 간에 뒤집히지
  않으므로(reversal 아님) §36.3의 No-Go 기준("필수 국면에서 유의한
  방향 역전")에는 해당하지 않는다.**
- **`intraday_ret_5d`는 2차 T+1만 유의**하고, 그 유의성의 대부분이
  `bearish_trend`(96일, t_NW=-3.70)에서 나온다 — `range_bound`/
  `bullish_trend`는 같은 방향(음수)이나 유의하지 않다(`|t|≤1.32`).
  이 또한 부호 반전(reversal)은 아니지만, **"특정 국면(특히
  하락장)에서만 유의하고 그 방향이 pooled와 같다"는 패턴은
  §20.4/§22.4가 `rsi_mean_reversion`/`relative_strength_rank_1m`을
  "범용 대체 근거 없음"으로 No-Go 처리한 것과 구조적으로 유사하다**
  — 이번 턴은 이 유사성을 근거로 규칙을 확장 적용해 No-Go를
  선언하지 않는다(§36.3이 명시한 기준은 "유의한 방향 역전"이지
  "특정 국면 집중"이 아니다). 대신 해석상 주의사항으로만 명시한다.

**판정(§36.3 규칙 그대로 적용)**:
- `overnight_ret_5d`: 1차 미달(marginal 문턱도 못 넘음), 2차 T+1/
  T+5 통과(`|t_NW|≥2`), 방향은 1차·2차 모두 음수로 일관, 필수
  국면(하락장)에서 유의한 역전 없음 → **Watch**("한쪽만 통과" 요건
  충족, 확정 Go 아님).
- `intraday_ret_5d`: 1차 미달, 2차 T+1만 통과, 방향은 1차·2차
  대체로 일관(양쪽 다 약한 음수 경향), 필수 국면(하락장)에서
  유의한 **역전**은 없음(같은 방향이 더 강하게 나타날 뿐) →
  **Watch**, 단 위에서 서술한 "하락장 집중" 패턴을 낮은 신뢰도
  사유로 명시.

### 39.4 후보 C(`low_volatility_rank_20d`) 결과

| horizon | 1차(12개월) t_NW | 2차(3년) 전체 t_NW | 하락장(96일) t_NW | range_bound(206일) t_NW | bullish(351일) t_NW |
|---|---|---|---|---|---|
| T+1 | 1.28 | **3.14** | -0.91 | **3.59** | **2.08** |
| T+5 | 0.11 | 1.96 | -0.48 | **2.67** | 1.11 |
| T+20 | -0.51 | 1.28 | 0.85 | 1.75 | 0.60 |

(`valid_row_count`는 결측 0건이라 전체 표본과 동일, `excluded_
row_count_volatility_missing`도 모든 horizon에서 0 — §39.1.)

- **1차 창은 세 horizon 모두 `|t_NW|<1.5`**(marginal 미달) — T+20
  은 부호까지 음수로 바뀐다(`-0.51`, 유의하지 않음).
- **2차 창(전체)은 T+1이 유의**(3.14, "낮은 변동성일수록 forward
  return이 높다"는 가설과 같은 방향), T+5는 경계선(1.96, `|t|≥2`
  문턱을 0.04 차이로 미달), T+20은 유의하지 않다(1.28).
- 국면별로는 `range_bound`(T+1=3.59, T+5=2.67)와 `bullish_trend`
  (T+1=2.08)가 유의하게 가설 방향과 일치하지만, **`bearish_
  trend`(96일)에서는 부호가 반대(-0.91, -0.48, 단 유의하지
  않음)** — 방향이 국면 간에 뒤집히는 조짐은 있으나 통계적으로
  유의한 역전은 아니다(`|t|<1`). 이 96일 표본은 §14가 `overall_
  score`의 유의한 하락장 역전을 확인한 것과 같은 표본 크기·같은
  국면 구간이라, 같은 데이터로도 이번엔 유의한 역전이 나오지
  않았다는 점은 참고할 만하다.

**판정**: 1차 미달, 2차 T+1 통과(T+5는 경계선 marginal), 방향은
1차 T+1/T+5와 2차가 대체로 일관(둘 다 비음수 경향)하나 T+20에서는
1차(-0.51)/2차(+1.28) 방향이 어긋난다. 필수 국면(하락장)에서
**유의한** 역전은 없음(부호만 반대, 유의성 없음) → **Watch**
("한쪽만 통과" + 단기 horizon에서 방향 일관 요건 충족), 단 T+20
방향 불일치와 하락장 비유의 반전 조짐을 해석상 주의사항으로
명시.

### 39.5 예외/관측성 점검

`build_signal_snapshot()` 예외 경로(`candidate_c_signal_snapshot_
failed`)와 `regime_label_unavailable`은 이번 실행에서 **0건**이었다
— `compute_exclusion_counts`가 빈 dict로 반환됐다(§39.1). 이
스크립트의 예외 처리 자체가 완전무결하다는 뜻은 아니다 — 87종목·
653거래일이라는 이번 cache의 특정 범위에서 예외가 발생하지
않았다는 사실만을 보고한다. cache가 갱신돼 표본이 늘어나면 이
경로들이 실제로 발동할 가능성은 여전히 열려 있다.

### 39.6 요약표

| 후보 | 1차 | 2차 최고 t_NW | 판정 | 비고 |
|---|---|---|---|---|
| A. `regime_switch_v1` | 참조값(하락장 0일, 게이트 미충족) | 참조값 T+5=2.60 | **Watch(기존 확정값 인용, 이번 턴 재실행 안 함)** | 외부 KIS 설정 없이 재현 불가 확인 |
| B-overnight `overnight_ret_5d` | 미달(`\|t\|<1.5`) | T+1=-3.01 | **Watch** | 방향 일관, 역전 없음, range_bound 주도 |
| B-intraday `intraday_ret_5d` | 미달(`\|t\|<1.5`) | T+1=-2.84 | **Watch** | 하락장 집중 유의성 — 낮은 신뢰도 사유로 명시 |
| C `low_volatility_rank_20d` | 미달(`\|t\|<1.5`) | T+1=3.14 | **Watch** | T+20 방향 불일치, 하락장 비유의 반전 조짐 |

**이번 턴 어느 후보도 Go가 아니다** — 1차 창이 세 신호 전부(6개
horizon×신호 조합) `|t_NW|<1.5`로, §16.2/§36.3의 marginal 문턱조차
넘지 못했다. No-Go로 격하할 만한 **유의한** 방향 역전도 발견되지
않았다(부호 반전은 관찰되나 전부 비유의).

### 39.7 해석 제한(§37 계승, 재확인)

이 절의 모든 "1차"/"2차" 비교는 **같은 정적 cache**(`_bars_
cache_core87_3y_2026-07-14`) 안에서의 **최근성·장기 국면
재검증**이다 — 1차(225일)는 2차(653일)의 부분집합이며, 서로
독립된 out-of-sample 표본이 아니다. 이번 턴에서 나온 Watch
판정은 "같은 cache를 다시 봐도 특정 신호가 일정 방향으로 유의하게
나타난다"는 관측일 뿐, 미래 데이터에 대한 예측력을 검증한 것이
아니다. **진짜 independent out-of-sample 검증은 cache를
`2026-07-14` 이후 거래일까지 갱신한 뒤, 이 절의 수식·기간·판정
기준을 한 글자도 바꾸지 않고 그 신규 구간에만 재실행해야만
확보된다**(§37.4) — 이 절은 그 갱신을 수행하지 않았다.

### 39.8 Stage B/운영 반영 판정

**변경 없음** — 이번 실행·판정으로 어떤 후보가 Watch로 나왔다는
사실이 Stage B(정책 threshold/gate 최적화) 착수나 운영 반영의
근거가 되지 않는다. §16.7/§36.4의 원칙(축 1의 새로운 Go 증거
없이는 Stage B를 열지 않는다) 그대로 유지한다 — 이번 후보들은
**Go가 아니라 Watch**이고, Watch 자체도 §37의 정정에 따라 "같은
cache 재검증 통과"일 뿐 독립 Go 증거가 아니다.

## 40. §39 정정 — 후보 B(`overnight_intraday_split_momentum_v1`)의 방향 계약 누락(2026-08-24 KST 후속, read-only 문서 정정)

이 절은 §39의 후보 B 판정에서 발견된 **방향성 계약 누락 + 사후
해석 문제**를 정정하는 append-only 절이다. 과거 §39 서술은 삭제·
수정하지 않는다. 코드/cache/DB/외부 API/운영 설정/주문 경로는
전혀 건드리지 않았다.

### 40.1 먼저 확인한 사실 — §36.2 원문에 방향 계약이 있었는가

PR #342(병합 커밋 `a7d74ac4`)가 확정한 §36.2 후보 B 행의
"경제적 가설" 칸 원문을 그대로 인용한다:

> "오버나이트(장 마감~다음 시가)는 기관/뉴스/해외 시장 정보가
> 반영되는 구간, 장중(시가~종가)은 국내 개인·모멘텀 매매가
> 우세한 구간이라는 마이크로스트럭처 가설(고전적 overnight/
> intraday return 분해 문헌과 동일 계열) — **두 구간의 예측력이
> 서로 다를 것**"

그리고 "기존 후보와의 차별점"/"사용 feature" 칸에도 방향(부호)에
대한 언급은 없다. §36.2 어디에도 "overnight_ret_5d가 양(+)의
방향으로 forward return을 예측할 것"이나 "intraday_ret_5d가
음(-)의 방향일 것" 같은 **구체적 부호 기대**가 명시돼 있지
않다 — 명시된 것은 오직 "두 구간의 예측력이 서로 **다를 것**"
(방향 자체가 아니라 "두 leg가 서로 구분되는 패턴을 보일 것"이라는
정성적 기대)뿐이다.

**판정: 이것은 "사전 기대 방향이 명시돼 있었는데 결과가 반대로
나온" 경우(경우 A)가 아니라, §36.2 동결 당시 방향 계약 자체가
애초에 고정되지 않았던 경우(경우 B)다.**

§39가 "사전 가설(정보 확산 방향)과는 반대 방향"이라고 서술한
부분은 **§36.2 원문에 근거가 없는, 결과를 본 뒤 §39 작성 시점에
새로 붙은 사후 해석**이었다 — §36.2의 마이크로스트럭처 가설은
"두 leg가 다르게 움직일 것"이라고만 했지 "오버나이트가 양의
모멘텀으로 이어질 것"이라고 특정한 적이 없다. 이 사후 해석을
바로잡는다.

### 40.2 정정 규칙 적용(경우 B)

- **B(`overnight_intraday_split_momentum_v1`, `overnight_ret_5d`/
  `intraday_ret_5d`)는 §39의 Watch 판정을 취소하고 "방향 계약
  미고정으로 판정 보류"로 정정한다.** §36.3의 Go/Watch/Hold/No-Go
  네 판정 전부 "특정 부호가 정확한가"를 전제로 설계됐는데, 애초에
  "정확한 부호가 무엇인지"가 동결되지 않은 신호에는 그 네 판정
  중 어느 것도 원칙적으로 적용할 수 없다 — Watch로 판정한 것 자체가
  범주 오류였다.
- §39.3에 기록된 2차(3년) 유의한 음수 `t_NW`(overnight T+1=-3.01/
  T+5=-2.46, intraday T+1=-2.84)는 **삭제하지 않는다** — 방향
  계약이 없는 상태에서 관측된 **탐색적(exploratory) 관찰값**으로만
  남긴다. 이 값은 어떤 방향의 신호도 "검증 통과"시키지 않는다.
- **음(-) 방향을 실제로 신호로 쓰려면**(예: "overnight_ret_5d가
  낮을수록 forward return이 높다"는 반전 가설), 그것은 이 문서가
  동결한 기존 B 후보의 검증 결과가 아니라 **완전히 새로운 후보**다
  — §36.2와 동일한 절차(새 후보명, 명시적 수식과 **명시적 기대
  방향**, horizon, 판정 기준을 실측 전에 사전 고정)를 처음부터
  다시 거쳐야 한다(§40.4).
- B의 이번 결과를 근거로 한 운영 반영, Stage B 착수, shadow 승격은
  **어떤 방향으로도 허용하지 않는다.**

### 40.3 최종 상태표(정정 후)

| 후보 | §39 원판정 | 정정 후 최종 상태 | 비고 |
|---|---|---|---|
| A. `regime_switch_v1` | Watch(참조값 인용) | **변경 없음 — Watch(참조값 인용)** | 외부 KIS 없이 재현 불가 확인, 재실행 안 함(§39.2) |
| B `overnight_ret_5d` | Watch | **방향 계약 미고정으로 판정 보류** | 2차 t_NW=-3.01(T+1) 등은 탐색적 관찰값으로만 기록, 어떤 방향의 검증 통과도 아님 |
| B `intraday_ret_5d` | Watch | **방향 계약 미고정으로 판정 보류** | 2차 t_NW=-2.84(T+1)도 동일하게 탐색적 관찰값 |
| C `low_volatility_rank_20d` | Watch | **변경 없음 — Watch** | §39.4 그대로, 방향 계약("낮은 변동성=높은 점수")은 §36.2에 명시돼 있었고 결과 부호가 그와 일치 |

### 40.4 역방향 B 후보를 다루려면(새 후보 동결 조건)

이번 §39 실행에서 관찰된 음(-) 방향을 향후 정식으로 다루려면,
다음을 **결과를 다시 보기 전에** 전부 새로 고정해야 한다 —
§36.2/§36.3과 동일한 절차이며, 기존 B 후보의 연장이 아니라
독립적인 신규 동결이다.

1. **새 후보명**(예: `overnight_reversal_v1` 등, 기존 `overnight_
   intraday_split_momentum_v1`과 구분되는 이름).
2. **명시적 수식과 명시적 기대 방향** — "overnight_ret_5d가
   낮을수록(또는 음의 방향일수록) forward return이 높다"처럼
   부호까지 사전에 못박는다.
3. **horizon**(T+1/T+5/T+20 중 어느 것을 primary로 볼지),
4. **§36.3과 동일한 Go/Watch/Hold/No-Go 판정 기준**(1차/2차
   이원 게이트, 최소 국면 표본, 완화 없음),
5. 이 신규 동결 이후에만 §39와 같은 cache 재실행을 수행하고,
   §37의 원칙(같은 cache 재검증 ≠ independent OOS)을 그대로
   적용한다.

### 40.5 이번 정정이 다른 판정에 미치는 영향

- **C(`low_volatility_rank_20d`)의 Watch 판정은 변경하지 않는다**
  — §36.2가 "낮은 변동성일수록 높은 점수"라는 방향을 이미 명시했고,
  §39.4의 양(+)의 유의한 결과가 그 방향과 일치하므로 방향 계약
  누락 문제가 없다.
- **A(`regime_switch_v1`)의 참조 Watch도 변경하지 않는다** — §39.2
  그대로, 이번 턴에 재실행하지 않았고 인용만 했다.
- **Stage B 보류 판정은 변경 없음** — B가 Watch에서 "판정 보류"로
  낮아진 것은 오히려 Stage B 착수 조건을 더 엄격하게 만들 뿐,
  완화하지 않는다.
- **독립 OOS 필요성도 변경 없음** — C/A의 Watch(참조)도 여전히
  §37이 정한 "같은 cache 재검증"일 뿐이며, cache 갱신 후 동결
  수식 그대로의 재실행이 있어야 진짜 independent OOS가 된다.

## 41. 후보 B 역방향 신규 후보 2개 동결 — `overnight_reversal_v1`/`intraday_reversal_v1`(2026-08-24 KST, read-only 설계·문서화 전용)

이 절은 **계산 구현·기존 cache 재실행·cache 갱신·외부 API/KIS
호출·DB write·운영 코드 변경을 전혀 하지 않는다** — 후보 동결과
독립 OOS 검증 계약 설계·문서화만 다룬다.

### 41.1 §40과의 관계 — 기존 B는 건드리지 않는다

§40에서 `overnight_intraday_split_momentum_v1`(`overnight_ret_5d`/
`intraday_ret_5d`)을 **"방향 계약 미고정으로 판정 보류"**로 정정
했다. 이 절은 그 판정을 수정·승격·재해석하지 않는다 — 기존 B는
계속 판정 보류 탐색 후보로 남는다.

이 절이 동결하는 두 신규 후보(`overnight_reversal_v1`/
`intraday_reversal_v1`)는 **§39가 관측한 기존 B의 탐색적 음수
결과(overnight T+1 t_NW=-3.01, intraday T+1 t_NW=-2.84 등)를 보고
그 반대 부호를 가설로 세운 사후(post-hoc) 신규 후보**다 — §40.4가
요구한 절차(새 후보명·명시적 수식·명시적 기대 방향·horizon·판정
기준을 결과를 보기 전에 다시 고정) 그대로 이번 절에서 수행한다.
**이 가설 자체가 기존 static cache(`~2026-07-14`)의 탐색 결과에서
유래했다는 사실을 숨기지 않는다** — 그래서 같은 cache로는 이
가설을 "검증"할 수 없다(같은 데이터를 보고 만든 가설을 같은
데이터로 확인하면 순환 논증이 된다). 반드시 `2026-07-15` 이후의
**신규** 거래일에서만 첫 유효 검증을 수행한다.

### 41.2 후보 동결안

| 후보명 | 원재료 | 후보 점수 | 명시적 기대 방향 | 경제적 가설 | 기존 B와의 관계 |
|---|---|---|---|---|---|
| **`overnight_reversal_v1`** | `overnight_ret_5d = Σ_{i=1..5} ln(open_i/close_{i-1})`(§36.2와 동일 원재료 정의, 재계산 로직 재사용 가능) | `overnight_reversal_score_5d = -overnight_ret_5d` | **점수가 높을수록(=`overnight_ret_5d`가 낮을수록) 향후 forward return이 높다** — 양(+)의 IC, 양(+)의 비용 차감 quintile spread를 기대 | 단기 오버나이트 과열/과매도 뒤 되돌림(mean reversion) 가능성 | `overnight_ret_5d`와 원재료는 동일하지만 부호를 반전한 **완전히 별도의 신규 후보**다 — §39가 관측한 탐색적 음수 결과에서 유래한 사후 가설(§41.1) |
| **`intraday_reversal_v1`** | `intraday_ret_5d = Σ_{i=1..5} ln(close_i/open_i)`(§36.2와 동일 원재료 정의) | `intraday_reversal_score_5d = -intraday_ret_5d` | **점수가 높을수록(=`intraday_ret_5d`가 낮을수록) 향후 forward return이 높다** — 양(+)의 IC, 양(+)의 비용 차감 quintile spread를 기대 | 단기 장중 과열/과매도 뒤 되돌림 가능성 | `intraday_ret_5d`와 원재료는 동일하지만 부호를 반전한 **완전히 별도의 신규 후보**다 — 같은 사후 유래(§41.1), 특히 §39.3이 지적한 "하락장(96일) 집중 유의성"(t_NW=-3.70) 패턴이 이 가설의 직접적 계기였음을 명시 |

두 후보 모두 **원재료 계산 함수(`overnight_ret_Nd`/`intraday_
ret_Nd`)는 §36.2/`compute_overnight_intraday_split_momentum()`과
동일하다** — 새로 만드는 것은 부호 반전과 그에 따른 기대 방향·
판정 계약뿐이다. 이번 절은 이 계산조차 구현하지 않는다(§41.5
"다음 작업"으로 분리).

### 41.3 사전 고정 평가 계약(결과 확인 전 고정, §36.3/§37 원칙 계승)

- **평가 시작일**: cache 갱신 후 **`2026-07-15` 이후 신규 거래일만**
  사용한다. `2026-07-14` 이전(기존 cache) 표본은 이 두 후보의
  검증에 **전혀** 쓰지 않는다(가설이 그 구간에서 유래했으므로).
- **lookback**: 5거래일 고정(§36.2와 동일, `N=5`).
- **horizon**: T+1/T+5/T+20 **모두 병행**한다.
- **primary metric**: 거래일별 cross-sectional Spearman IC →
  Newey-West 보정 pooled t-statistic(§36.3/v2·v4 함수 재사용
  원칙 계승).
- **기대 방향**: 두 후보 모두 **양(+)의 IC, 양(+)의 비용 차감
  quintile spread**를 기대한다 — 이 부호가 검증 대상이다(§41.2).
- **층화**: `risk_tone`, 시장 공통 `regime_label`.
- **국면별 최소 표본**: `MIN_REGIME_TRADING_DAYS = 30`(§20.2/§36.3
  상수 그대로).
- **결측 처리**: 시가/종가/forward return이 결측이거나 비유한
  값이면 분석에서 완전히 제외하고 사유별로 집계한다(§38.4/§38.8~
  38.10이 확립한 계약 그대로 — `None`을 0으로 채우거나 정상 신호로
  섞지 않는다).
- **동점 처리**: 이 두 후보는 순위 스케일링을 쓰지 않는 연속형
  점수라 동점 규칙이 직접 적용되지는 않지만, 향후 이 후보를
  cross-sectional 순위로 바꾸는 변형이 나온다면 §38.10이 확립한
  평균 순위(average rank, 입력 순서 무관) 원칙을 그대로 따른다.
- **다중 horizon 해석 규칙**: **T+1을 primary horizon으로 고정**
  한다(§36.2의 lookback·후보 설계가 5거래일 단기 되돌림을 가정하는
  것과 가장 정합적인 창이며, `overnight_ret_5d`/`intraday_ret_5d`
  둘 다 §39.3에서 T+1이 가장 강한 신호를 보였다). T+5/T+20은
  **보조 horizon**으로만 취급한다 — **primary(T+1)가 Go 게이트를
  충족하지 못하면, T+5나 T+20 중 하나가 우연히 높게 나왔다는
  이유만으로 승격하지 않는다.** 보조 horizon의 역할은 오직 "T+1
  결과가 짧은 horizon에 국한된 우연인지, 조금 더 긴 창에서도
  방향이 유지되는지"를 참고하는 것뿐이다.
- **Go/Watch/Hold/No-Go**(§36.3 기준을 그대로 계승, 이 역방향
  후보에 맞게 "정확한 부호"만 명시적으로 재확인 — 완화 아님):
  - **Go**: 1차와 2차 모두에서 primary(T+1)의 `|t_NW|≥2`**이고
    부호가 양(+)**이며, 필수 국면(하락장 포함)에서 유의한 역전이
    없어야 한다.
  - **Watch**: 한쪽 창만 통과하거나 marginal(`1.5≤|t_NW|<2`)이지만
    부호가 **일관되게 양(+)**일 때.
  - **Hold**: 유의성은 없으나 부호 역전(음으로 유의)도 없을 때.
  - **No-Go**: 부호가 **음(-)으로 유의**하게 나오거나(=기대 방향과
    반대 방향으로 유의), 필수 국면에서 유의한 역전이 확인될 때.
  - 이 후보들은 **1차/2차 이원 게이트 자체를 신규 구간(`2026-07-15`
    이후) 안에서 시간 순으로 다시 나눠 구성**해야 한다(예: 신규
    구간 중 최근 N개월을 1차, 신규 구간 전체를 2차) — §37이 지적한
    "1차가 2차의 부분집합"이라는 한계는 이 신규 구간에서도 구조적
    으로 동일하게 남으므로, 이 역시 여전히 **같은(신규) cache
    안의 최근성 재검증**일 뿐 진정한 독립 표본 분리는 아니다. 다만
    "`~2026-07-14`" 구간과는 완전히 분리된, 가설을 만들 때 보지
    않았던 데이터라는 점에서 §36.2/§39의 원래 B 후보보다는 훨씬
    강한 out-of-sample 성격을 갖는다.
- **표본 부족 시**: 신규 OOS 구간(`2026-07-15` 이후)의 거래일
  수가 통계적으로 의미 있는 최소치(국면별 30일 등)에 못 미치면
  **"판정 보류"만 허용**한다 — 기간을 과거 cache(`~2026-07-14`)
  쪽으로 확장해 표본을 채우는 것은 **금지**한다(그렇게 하면 다시
  가설이 유래한 구간을 섞어 순환 논증이 된다).

### 41.4 cache 갱신 전제조건(이번 턴에는 수행하지 않음)

- **cache 갱신에는 신규 KIS read-only 호출이 필요하다** — 이번
  절은 그 호출을 수행하지 않으며, 실제 갱신은 **별도 턴에서 사용자
  승인 후에만** 진행한다.
- **갱신 대상 기간**: 최소 `2026-07-15`(기존 `_bars_cache_core87_
  3y_2026-07-14` 마지막 날짜의 다음 거래일)부터 갱신 실행 시점의
  최신 거래일까지.
- **대상 universe**: 기존 §36.1의 87종목(`_bars_cache_core87_3y_
  2026-07-14`와 동일 종목 집합, 벤치마크 069500 별도) — universe
  자체를 이번에 확장하거나 축소하지 않는다(신규 후보의 결과가
  universe 변경 효과와 뒤섞이는 것을 방지).
- **기존 cache 보존**: `logs/_bars_cache_core87_3y_2026-07-14/`는
  **덮어쓰지 않는다.** 갱신은 **새 디렉터리**(예: `logs/_bars_
  cache_core87_3y_<갱신실행일>/` 형식, 기존 명명 관례 그대로
  "수집 기준일"을 디렉터리명에 남긴다)에 저장해, 기준일·수집
  시각·원본 KIS 응답을 기존 cache와 완전히 분리 보존한다 — 이렇게
  해야 "어느 cache로 어느 결과가 나왔는지"를 사후에도 재현·감사
  가능하다.
- **갱신 후 수식 불변 원칙**: cache가 갱신된 뒤에도 §41.2/§41.3에서
  동결한 수식·기대 방향·horizon·판정 기준을 **한 글자도 바꾸지
  않고** 그대로 적용해 신규 구간만 채점한다 — 결과를 먼저 훔쳐보고
  계약을 조정하는 것은 금지한다(§36.3/§37/§40이 반복적으로 명시한
  원칙과 동일).
- **후보 C(`low_volatility_rank_20d`)의 OOS 검증도 같은 신규
  cache 구간에서 병행 가능하다** — C는 §36.2가 방향(낮은 변동성=
  높은 점수)을 이미 명시했고 §39.4에서 Watch로 판정됐으므로, 별도
  가설 재설계 없이 **같은 신규 구간(`2026-07-15` 이후)에 §36.3의
  기존 수식을 그대로 재실행**하면 된다 — 이는 §37.4가 요구한 "진짜
  independent OOS 검증"의 첫 실제 기회다. `regime_switch_v1`(후보
  A)도 cache 갱신 후 같은 신규 구간에서 재실행 가능하나, 그 실행
  경로(v8~v10 스크립트)는 여전히 KIS 실거래 클라이언트 생성이
  필요하므로(§39.2) 별도로 그 부분의 실행 가능성을 재확인해야
  한다.

### 41.5 신규 구간 결과가 나오기 전까지의 제약(재확인)

- `overnight_reversal_v1`/`intraday_reversal_v1` 둘 다 **신규
  구간 결과가 나오기 전까지 Watch/Go/운영 반영/Stage B 착수를
  절대 허용하지 않는다** — 지금은 "동결된 가설"일 뿐 어떤 실측도
  없다.
- 이번 절은 계산 구현을 포함하지 않는다 — 다음 턴에서 (1) 두 신규
  후보의 계산 함수(원재료는 기존 `compute_overnight_intraday_
  split_momentum()` 재사용, 부호 반전만 추가) 구현, (2) cache
  갱신(사용자 승인), (3) §41.3 계약 그대로 신규 구간 실행, (4)
  결과 판정 순으로 진행한다.

### 41.6 §16.7/§36.4/§40.5 원칙 재확인
...
## 49. PR #352 병합 후 발견 — systemd unit이 표준 배포 래퍼를 우회하던 결함 수정(2026-08-26 KST, read-only 최종 점검 후속, 새 PR)

§47이 병합(PR #352, merge commit `8e0b5cff`)된 뒤, 운영 활성화 전
read-only 최종 점검(별도 턴)에서 다음 결함이 발견됐다: `ops/systemd/
sppv3-oos-batch.service`의 `ExecStart`가 `docker compose`를 **직접**
호출하고 있어, 저장소 표준 배포 래퍼 `scripts/harness/docker_
compose_env.sh`를 거치지 않았다. 이 래퍼가 `/etc/agent_trading/*.env`
(KIS live-info 자격증명 포함)를 `--env-file`로 명시 전달해야만
Compose가 `${KIS_LIVE_INFO_APP_KEY}` 등을 올바르게 치환하는 구조라,
**그대로 설치·활성화했다면 자격증명이 빈 문자열로 치환돼 배치가 매일
조용히 `skip_market_calendar_unavailable`만 반복하고 실제로는 절대
동작하지 않았을 것이다.** 위험한 실패는 아니지만("빈 자격증명 →
`build_authoritative_holiday_provider()`가 안전 skip" 그대로 작동)
표본이 영원히 쌓이지 않는 조용한 무동작 상태였다.

### 49.1 수정 내용

- `ops/systemd/sppv3-oos-batch.service`의 `ExecStart`를
  `/bin/bash __AGENT_TRADING_REPO_ROOT__/scripts/harness/docker_
  compose_env.sh --profile sppv3-oos-batch run --rm sppv3-oos-batch`로
  변경 — `docker compose`를 직접 부르지 않고 반드시 이 래퍼를
  경유한다. `--profile sppv3-oos-batch`를 명시해야 profile로 격리된
  이 서비스가 `run` 대상으로 인식된다.
- `Environment=PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:
  /sbin:/bin`(systemd.exec(5) 기본값과 동일)을 명시 추가 — systemd
  서비스가 최소 PATH로 뜰 때 `docker`를 못 찾는 사고를 구조적으로
  방지한다.
- `__DOCKER_COMPOSE_BIN__` 자리표시자를 템플릿과 설치 스크립트에서
  제거했다 — `docker_compose_env.sh`가 내부적으로 PATH에서 `docker`를
  찾아 `exec`하므로 이 unit이 compose 바이너리 경로를 별도로 알
  필요가 없어졌다.
- `ops/systemd/install_sppv3_oos_batch_systemd.sh`에 렌더링 직후
  자체 안전장치를 추가했다 — `ExecStart`가 `docker_compose_env.sh`를
  포함하지 않거나 `docker compose `를 직접 호출하는 것으로 보이면
  `--yes`를 줬더라도 설치를 즉시 중단한다(향후 템플릿을 손으로 고치다
  이 결함을 되돌리는 회귀를 설치 시점에 잡기 위함). `scripts/harness/
  docker_compose_env.sh`가 대상 `REPO_ROOT`에 존재하는지도 사전 검증
  한다(운영 checkout이 이 파일을 포함한 커밋까지 동기화됐는지 확인).

### 49.2 검증

dry-run(`--yes` 없이, `AGENT_TRADING_REPO_ROOT=/workspace/agent_trading`)
으로 렌더링된 unit의 `ExecStart`가 `docker_compose_env.sh` 경유임을
확인했다. 신규/수정 테스트(`tests/ops/test_sppv3_oos_batch_ops_
contracts.py`, 실제 설치 스크립트를 fake repo 대상 서브프로세스로
1회 dry-run 실행해 출력을 검증하는 테스트 포함) 전부 PASS, DB/네트워크
미사용. 076 장애 시 안전 skip, 주문 scheduler 분리, lock/idempotency
계약은 이번 수정에서 전혀 변경하지 않았고 기존 테스트가 그대로 통과함을
재확인했다. `accept env`/`style`/`no-bypass`/`architecture`/`docs`
전부 PASS.
...
재기동, 주문 제출 — 전부 이번 후속 PR에서도 수행하지 않았다. 운영
활성화 전 남은 선행 조건(운영 checkout이 아직 PR #352를 반영하지
않음)은 이전 read-only 점검에서 이미 보고된 그대로 유효하다.
