...
| 6 | 비용 차감 후 기대값 승인 | `expected_value_gate`(`edge_after_cost_bps >= minimum_required_edge_bps`), `risk_check_passed` | 거래비용·슬리피지를 뺀 뒤에도 기대값이 남는지, 계좌 위험 한도 안인지 | 7단계(주문 요청 생성)로 진행 | 여기서 최종 승인 후보였어도 주문 요청이 생성되지 않음 | 어떤 날은 유일한 매수(`buy`) 판정마저 이 단계에서 막혀 그날 주문 요청이 0건이었던 사례가 실제로 확인됨(근거 문서 §13.2.14) |
