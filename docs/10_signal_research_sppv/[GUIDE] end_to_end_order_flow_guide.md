...
### 이 문서가 답하는 질문, 이 문서가 답하지 않는 질문

이 문서는 **"오늘 어떤 종목이 왜 그 단계에서 멈췄는가"를 업무담당자가 스스로 되짚어볼 수 있게 하는 설명 문서**다. "이 차단 장치가 옳은가/과한가"에 대한 근거는 이 문서가 아니라 `docs/20_system_analysis/buy_path_variable_gate_matrix.md`(근거 문서, 실측·백테스트 상세)에 있다. 후속 검증 과제는 `docs/99_meta_handover/[BACKLOG] backlog.md`(후속 작업 정리 문서)에 있다. 이 문서는 세 문서 중 "설명" 역할만 맡고, 같은 수치를 여기에 복붙하지 않는다 — 필요하면 위 문서로 연결한다.

이 문서는 "정답을 확정해서 알려주는 문서"가 아니라 **"지금까지 검증된 범위와 아직 검증이 끝나지 않은 범위를 구분해서 보여주는 문서"**다. 아래에서 "확정된 사실"과 "아직 미확정"이라고 쓴 부분은 그 구분을 그대로 따른다.

---

## 0. 업무 흐름 7단계 한눈에 보기

업무담당자가 가장 자주 던지는 질문은 "이 종목은 왜 매수까지 안 갔는가"다. 이 질문에 답하려면 아래 7단계 중 **어디서 멈췄는지**를 확인하면 된다. 각 단계는 이전 단계를 통과한 것만 넘어오므로, 뒤 단계에서 멈췄다는 것은 앞 단계는 이미 통과했다는 뜻이다.

| 단계 | 업무 관점 이름 | 내부 필드/판정명 | 무엇을 확인하는 단계인가 | 통과 시 다음 단계 | 차단 시 어떤 결과가 되는가 | 최근 검증에서 왜 중요했는가 |
|---|---|---|---|---|---|---|
| 1 | 오늘의 관찰 대상 선정 | `source_type`(`core`/`held_position`/`reconciliation_overlay`/`event_overlay`/`market_overlay`/`manual`) | 오늘 이 종목을 아예 판단 대상으로 올릴 것인가 | 2단계(기본 적격성 판단)로 진행 | 이 cycle에서 판단 자체가 생성되지 않음(기록조차 남지 않음) | 최근 유니버스 구성이 바뀌면서 새로 편입된 종목(예: `009420`)이 이후 단계 논의의 핵심이 됨 — 자세한 내용은 근거 문서 §13.2.13 |
| 2 | 기본 적격성 판단(지금 살 수 있는 상태인가) | `eligibility_passed`, `eligibility_reasons`(예: `eligibility_low_relative_activity`=활동성 부족, `eligibility_negative_overall_floor`=신호 자체가 나쁨) | 유동성·활동성·위험장·참여율 같은 "실행 가능성" 조건을 통과하는지 | 3단계(점수 산출)로 진행 | 점수가 아무리 높아도 매수 후보가 될 수 없음 — `WATCH`/`HOLD`에 그대로 머무름 | 2026-08-03 이후 `eligibility_low_relative_activity`(활동성 부족) 차단 비중이 급증해 별도 재검증 진행 중 — **결론은 아직 미확정**(근거 문서 §13.4.4) |
| 3 | 매력도 점수 산출 | `entry_score`(진입 판단 점수), `overall_score`/`fast_score`/`slow_score` | 신호 강도를 하나의 점수로 압축 | `0.65` 이상이면 4단계(매수 후보 판정) 문턱 통과 | 점수가 낮으면 매수 후보에서 자연히 배제 | 자본 배정(allocation) 보정항 제거 이후 이 점수 계산 자체가 바뀌어, 경계 근처 종목의 후보 판정이 실제로 바뀐 사례가 확인됨(근거 문서 §13.2.13) |
| 4 | 매수 후보 판정 | `buy_candidate`(매수 후보 판정), `primary_candidate` | 기본 적격성 + 점수 문턱 + 자본 배정 여유를 모두 만족하는지 | 5단계(AI/후속 판단)로 진행 | `WATCH` 등 관찰 상태에 머무름 — **아직 주문과는 무관** | "매수 후보가 됐다"와 "실제 주문까지 간다"는 서로 다른 질문이라는 것이 여기서부터 갈린다 |
| 5 | AI/후속 판단(승인 유지 또는 하향) | `candidate_vs_final.alignment_status`(`matched`/`downgraded`/`upgraded`/`suppressed`), `decision_type` | AI 4단 체인이 앞선 매수 후보 판정을 그대로 받아들이는지, 낮추는지 | 6단계(리스크/기대값 승인)로 진행 | 매수 후보였어도 하향(`downgraded`)되어 최종 `WATCH`/`HOLD`로 내려갈 수 있음 | 실측 비교군에서 매수 후보의 80% 이상이 이 단계에서 하향됨을 확인 — 다만 그 판단을 설명하는 것으로 봤던 일부 근거는 재검증에서 판별력이 확인되지 않아 **표현을 낮춤**(근거 문서 §13.2.14) |
| 6 | 비용 차감 후 기대값 확인 | `expected_value_gate`(`edge_after_cost_bps >= minimum_required_edge_bps`), `risk_check_passed` | 거래비용·슬리피지를 뺀 뒤에도 기대값이 남는지, 계좌 위험 한도 안인지 | 7단계(주문 요청 생성)로 진행 | **[2026-08-07 정책 변경]** 신규매수(`BUY`/`APPROVE`)는 이 EV 판정에서 불통과여도 더 이상 주문 요청 생성을 막지 않는다 — 참고 지표로는 계속 기록된다. 매도/축소(`SELL`/`EXIT`/`REDUCE`)는 기존과 동일하게 이 단계에서 막힐 수 있다 | 2026-08-03~07 실측에서 이 단계의 신규매수 차단이 기대값 개선으로 이어졌다는 근거가 확인되지 않아, 차단보다 관찰로 전환했다(근거 문서 `buy_path_variable_gate_matrix.md` §27) |
| 7 | 주문 요청 생성 | `order_request` 생성 여부, `compliance_validator_v1`, `VaR`, sizing | 규정 위반·계좌 위험·수량 산정까지 최종 통과하는지 | KIS 제출 | 여기까지 왔어도 규정/한도 위반이면 주문 자체가 만들어지지 않음 | 최근 구간 전체에서 `order_request` 생성 0건인 날이 확인됨 — "후보가 여럿이어도 주문은 0건일 수 있다"는 사례 |

### 0-1. 점수 산출 이후 차단 장치를 특히 주의해서 봐야 하는 이유

3단계(점수 산출)를 통과했다고 해서 나머지가 자동으로 진행되는 것이 아니다. 4~7단계는 각각 독립적인 차단 지점이고, 최근 검증에서 반복적으로 논의된 4가지가 모두 이 구간에 몰려 있다.

1. **자본 배정(allocation) 영향** — 3단계 점수 계산 자체에 영향을 준다.
2. **활동성 부족(`eligibility_low_relative_activity`)** — 2단계에서 이미 걸러지는 경우가 있고, 4단계 문턱을 넘었어도 이 사유로 걸리는 경우가 있다.
3. **매수 후보 이후 downstream 하향** — 5단계에서 발생한다.
4. **주문 요청 미생성** — 6~7단계에서 발생한다.

이 4가지를 **하나의 "차단이 과하다"는 결론으로 뭉치지 않는다** — 각각 서로 다른 단계, 서로 다른 근거 강도를 갖고 있고, 근거 문서에도 서로 다른 절로 분리돼 있다.

### 0-2. 차단 장치 적정성 판단 원칙(가장 중요한 원칙)

**어떤 차단 장치든 "적정한가/과한가"를 판단하려면, 차단된 표본과 통과된 표본의 사후 성과(실제 이후 가격 변화, 백테스트) 비교가 반드시 필요하다.** 아래 어느 쪽도 단독으로는 결론이 되지 않는다.

- 차단 비중이 늘었다는 사실만으로 "과잉 차단"이라고 말하지 않는다.
- 통과군 평균 수익률이 차단군보다 높다는 사실만으로 "장치가 적정하다"고 말하지 않는다.
- 특히 **실제 매수 후보가 될 수 있었던 경계 구간**(점수가 문턱 근처인 표본)의 사후 성과는 전체 population 평균보다 운영 판단에 더 중요하게 다뤄야 한다 — 전체 population은 원래부터 매수 후보가 될 가능성이 낮은 표본이 대부분이라, 그 구간의 차단은 운영상 비용이 크지 않기 때문이다.
- **장중(실시간) 가격 움직임과 종가 기준 백테스트는 별개 층위다.** 장중에 어떤 종목이 크게 움직였다는 관찰은 참고할 추가 신호일 뿐, 그 자체로 종가 기준 백테스트 결론을 대체하거나 뒤집지 않는다. 이 시스템은 실시간 시세를 저장하는 테이블이 없어(별도 확인됨), 장중 관찰은 항상 "아직 검증되지 않은 추가 의심 신호"로만 다룬다.
- 표본이 작으면(특히 경계 구간은 대부분 표본이 매우 작다) 강한 결론을 내리지 않는다.

이 원칙에 따른 상세 수치·개별 종목 사례·판정 근거는 이 가이드에 옮겨 적지 않는다 — `buy_path_variable_gate_matrix.md`의 §13.2.13(자본 배정 영향 재검증), §13.2.14(매수 후보 이후 downstream 하향, 백테스트 포함), §13.4.4(활동성 부족 게이트, 백테스트 포함)를 근거 문서로 참조한다.

### 0-3. 자주 생기는 오해 4가지

1. **"점수가 높으면 결국 산다"는 오해** — 아니다. 점수(3단계)가 높아도 4~7단계 중 어디서든 멈출 수 있고, 실제로 점수가 매수 문턱을 넘긴 표본 대부분이 이후 단계에서 다시 관찰 상태로 내려간다.
2. **"매수 후보(`buy_candidate`)가 됐으면 주문이 나간 것"이라는 오해** — 아니다. 매수 후보 판정(4단계)과 실제 주문 요청 생성(7단계)은 서로 다른 질문이다. 매수 후보였지만 5~7단계에서 막혀 주문이 전혀 생성되지 않은 날도 실제로 있었다.
3. **"오늘 주문 요청이 0건이면 상류에도 후보가 없었다"는 오해** — 아니다. 주문 요청이 0건인 날에도 1~4단계까지는 후보가 존재할 수 있다. "후보가 없었다"와 "후보는 있었지만 뒤 단계에서 막혔다"는 서로 다른 상황이고, 운영 판단에서는 반드시 구분해야 한다.
4. **"같은 종목이면 항상 같은 결과"라는 오해** — 아니다. 같은 종목도 날짜나 `source_type`(편입 사유)이 다르면 점수·적격성·최종 판정이 달라질 수 있다. 어제 차단됐던 종목이 오늘은 통과할 수 있고, 반대도 가능하다.
...
8. 상대 활동성(평소 대비 오늘 거래량/거래대금이 얼마나 살아 있는지, `eligibility_low_relative_activity`로 미달 시 차단)
...
**참고**: 8번(상대 활동성) 차단은 2026-08-03 이후 차단 비중이 크게
늘어 별도로 백테스트 포함 재검증이 진행 중이며, **적정성 결론은
아직 미확정**이다(§0-2 원칙, 상세는 근거 문서 §13.4.4).
