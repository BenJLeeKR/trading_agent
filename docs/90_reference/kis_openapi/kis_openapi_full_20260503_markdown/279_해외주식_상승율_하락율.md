...
|모의 Domain|미지원||||||
|URL 명|/uapi/overseas-stock/v1/ranking/updown-rate||||||
|개요|||||||
|개요|해외주식 상승율/하락율 API입니다._x000D_<br>한국투자 HTS(eFriend Plus) &gt; [7629] 상승율/하락율 화면 기능을 API로 개발한 사항으로, 해당 화면을 참고하시면 기능을 이해하기 쉽습니다.||||||
|Layout|||||||
...
|Request Query Parameter|KEYB|NEXT KEY BUFF|string|Y|8|공백|
