...
| 레거시 호환 | 기존 `CORORDBST001`, `MMTCOLBSM001` 등의 물리명은 `LEGACY_TBL_NM` 또는 `PHYS_TBL_NM`으로 유지 가능하도록 매핑 기준 보강 |
...
| v0.5.2 | 2026-06-21 | ChatGPT | `05. 핵심 테이블 컬럼 메타` 이하의 절 제목, 대표 테이블명, 예시, 명명 규칙을 12자리 표준테이블명 중심으로 일괄 정합화하고 레거시 물리명은 보조 속성으로 분리 | 데이터설계총괄 |
...
### 00.3 v0.5.2 명명규칙 정합화 기준

| 항목 | 정합화 기준 |
| --- | --- |
| 본문 대표 테이블명 | 12자리 표준테이블명 `STD_TBL_NM`을 우선 표기 |
| 기존 `TB_*`, `MT_*` 명칭 | 공식 기준명이 아니라 `LEGACY_TBL_NM` 또는 `PHYS_TBL_NM`으로만 보관 |
| 컬럼 메타 절 제목 | `논리테이블명` + `STD_TBL_NM` + Legacy 명칭 순서로 표기 |
| 컬럼 설명 내 참조 테이블 | 표준테이블명 사용. 필요 시 괄호로 Legacy 명칭 병기 |
| 명명 규칙 장 | Prefix/Suffix 기반 테이블명 규칙을 폐기하고 12자리 위치 기반 규칙만 공식 규칙으로 유지 |
...
| 테이블 | 컬럼 | 포함 | CORORDBST001 contains ORD_DT, ORD_NO |
| 테이블 | 인터페이스필드 | 매핑 | IF_EXEC_RCV.EXEC_QTY → CEXEXCDTT001.EXEC_QTY |
| 배치 | 테이블 | 입출력 | BAT_BAL_EOD reads CEXEXCDTT001, writes CBAPOSBLB001 |
...
| --- | --- | --- | --- | --- | --- |
| 고객 | 고객기본 | CCUMSTBSM001 | CUST_NO | 고객 식별, 실명확인, 고객상태, 투자자 구분 | 계좌, KYC, AML, 권한 |
| 고객 | 고객KYC | CCUKYCBSM001 | CUST_NO | 투자성향, 위험등급, 고객확인의무, 적합성/적정성 | 상품판매, 주문제한, 컴플라이언스 |
| 고객 | 고객주소/연락처 | CCUADRDTM001 | CUST_NO+ADDR_TP_CD | 주소, 휴대폰, 이메일, 통지 수단 | 우편, SMS, 알림톡, 전자문서 |
| 계좌 | 계좌기본 | CACMSTBSM001 | ACNO | 위탁/연금/파생/CMA 등 계좌 속성 | 고객, 주문, 잔고, 예수금 |
| 계좌 | 계좌권한 | CACAUTMPM001 | ACNO+USER_ID | 대리인, 직원, 온라인 사용자 권한 | 채널, 인증, 감사 |
| 계좌 | 계좌제한 | CACRSTBSM001 | ACNO+RSTR_CD | 사고, 압류, 거래정지, 미수동결 등 제한 | 주문, 출금, 대체 |
| 상품/종목 | 종목기본 | CISMSTBSM001 | ISU_CD | 주식/ETF/ETN/ELW/채권/파생 공통 종목 | 주문, 시세, 권리 |
| 상품/종목 | 주식종목 | CISSTKBSM001 | ISU_CD | 상장주식, 우선주, 관리종목, 투자경고 | 주문, 잔고, 대용 |
| 상품/종목 | 파생종목 | CDRDRVBSM001 | ISU_CD | 선물/옵션 만기, 행사가, 승수, 결제월 | 주문, 증거금, 정산 |
| 상품/종목 | 채권발행 | CBDISSBSM001 | ISU_CD | 발행일, 만기일, 표면금리, 이자지급주기 | 채권매매, 이자, 상환 |
| 시세 | 현재가/호가 | CMKQUOSNB001 | MKT_DT+ISU_CD | 현재가, 기준가, 상한가, 하한가, 호가단위 | 주문가능금, 평가, HTS/MTS |
| 주문 | 주문원장 | CORORDBST001 | ORD_DT+ORD_NO | 주문 접수, 정정, 취소, 상태관리 | 채널, 계좌, 체결 |
| 주문 | 주문이력 | CORORDHSH001 | ORD_DT+ORD_NO+HIST_SEQ | 주문 상태 변경 및 정정/취소 이력 | 감사, 민원, 장애분석 |
| 체결 | 체결내역 | CEXEXCDTT001 | EXEC_DT+EXEC_NO | 거래소 체결 결과, 부분체결, 비용 산정 | 주문, 잔고, 결제 |
| 잔고 | 종목잔고 | CBAPOSBLB001 | BAS_DT+ACNO+ISU_CD+BAL_TP_CD | 계좌별 상품 보유수량, 평가금액, 매입단가 | 체결, 평가, 권리 |
| 예수금 | 예수금잔고 | CCACSHBLB001 | BAS_DT+ACNO+CURR_CD | 예수금, 주문가능금, 출금가능금, 미수금 | 주문, 결제, 출납 |
| 결제 | 결제예정 | CSTSCHSTS001 | STL_DT+ACNO+ISU_CD+STL_SEQ | 매수/매도 결제 예정 금액·수량 | 체결, 예수금, 잔고 |
| 신용공여 | 신용약정 | CCRAGRBSM001 | ACNO | 신용거래 한도, 만기, 약정상태 | 계좌, 주문, 리스크 |
| 신용공여 | 신용융자 | CCRLONDTT001 | LOAN_NO | 융자금, 담보, 이자, 상환, 만기 | 잔고, 예수금, 반대매매 |
| 담보 | 담보평가 | CCLEVLEVB001 | BAS_DT+ACNO | 담보금액, 담보비율, 부족금액 | 신용, 대출, 리스크 |
| 파생 | 파생증거금 | CDRMRGBLB001 | BAS_DT+ACNO+CURR_CD | 위탁/유지/추가증거금, 예탁총액 | 파생주문, 정산, 마진콜 |
| 파생 | 파생정산 | CDRSTLSTT001 | STTL_DT+ACNO+ISU_CD | 정산가격, 정산차금, 평가손익 | 예수금, 리스크 |
| 채권 | 채권이자스케줄 | CBDINTSTS001 | ISU_CD+INT_PAY_SEQ | 이자계산기간, 지급일, 만원당 이자 | 권리, 예수금 |
| 권리 | 권리이벤트 | CRTEVTBSE001 | RIGHT_EVT_NO | 배당, 유/무상증자, 분할, 합병 | 종목, 잔고, 결제 |
| 권리 | 권리배정 | CRTALCDTT001 | RIGHT_EVT_NO+ACNO+ISU_CD | 계좌별 배정수량/금액 | 잔고, 예수금, 세금 |
| 수수료/세금 | 수수료세금규칙 | CFEFTRRLR001 | RULE_ID | 시장/상품/채널/고객등급별 비용 산식 | 주문, 체결, 결제 |
| 채널 | 채널마스터 | CCHMSTBSM001 | CHNL_CD | HTS/MTS/API/영업점/ARS 등 채널 관리 | 주문, 인증, 로그 |
| 조직 | 부점/직원 | CBRMSTBSM001 | BR_CD+EMP_NO | 지점, 영업직원, 승인자, 관리부점 | 고객, 계좌, 감사 |
| 공통 | 공통코드 | CCMCODCDC001/CCMCODDTD001 | GRP_CD+CD | 업무 코드, 유효값, 배포관리 | 전 업무 |
| 인터페이스 | 전문메타 | MIFMSGIFI001 | IF_ID | 송수신 전문, 필드, 매핑, 버전 | 거래소, FEP, 대외기관 |
| 배치 | 배치메타 | MBTJOBBSM001 | JOB_ID | Job, Step, 선후행, 재처리 정책 | 일마감, 월마감, 운영 |
| 감사 | 감사로그 | LAULOGLGL001 | LOG_ID | 조회/변경/승인/접속 이력 | 보안, 감사, 컴플라이언스 |
...
> v0.5.2에서는 각 핵심 테이블의 컬럼 메타를 12자리 표준테이블명 `STD_TBL_NM` 기준으로 재정렬했습니다.  
> 기존 `TB_*`, `MT_*` 명칭은 절 제목의 Legacy 정보 또는 `PHYS_TBL_NM`/`LEGACY_TBL_NM` 속성으로만 관리합니다.  
> 회사별 표준 약어, DBMS 타입, 파티션 정책, 개인정보 암호화 정책에 따라 컬럼 물리명과 타입은 조정할 수 있으나, 테이블 참조 기준은 `STD_TBL_NM`을 우선합니다.

### 05.00 컬럼 메타의 테이블명 표기 기준

| 표기 항목 | 의미 | 적용 기준 |
| --- | --- | --- |
| `STD_TBL_NM` | 12자리 표준테이블명 | 컬럼 메타의 공식 참조 테이블명 |
| `LOGIC_TBL_NM` | 논리테이블명 | 업무 설명 및 화면 표시용 |
| `PHYS_TBL_NM` | 실제 DB 물리명 | 운영 DB에서 실제 생성된 테이블명. 전환 전에는 Legacy와 같을 수 있음 |
| `LEGACY_TBL_NM` | 기존/구명칭 | 영향도 분석, 이행, 레거시 연계 확인용 |

컬럼 메타 절 제목은 `논리테이블명` + `STD_TBL_NM` + `Legacy` 순서로 표기한다. 본문에서 다른 테이블을 참조할 때는 `STD_TBL_NM`을 우선 사용하고, 기존명을 설명해야 할 때만 Legacy를 병기한다.
...
| 기본 | 물리테이블명 | Y | 실제 DB 테이블명. 예: `CORORDBST001` |
...
### 05.1 고객기본 `STD_TBL_NM: CCUMSTBSM001` / Legacy: `TB_CUST_MST`
...
### 05.2 고객KYC정보 `STD_TBL_NM: CCUKYCBSM001` / Legacy: `TB_CUST_KYC`
...
### 05.3 고객주소연락처 `STD_TBL_NM: CCUADRDTM001` / Legacy: `TB_CUST_ADDR`
...
### 05.4 계좌기본 `STD_TBL_NM: CACMSTBSM001` / Legacy: `TB_ACCT_MST`
...
### 05.5 계좌제한 `STD_TBL_NM: CACRSTBSM001` / Legacy: `TB_ACCT_RSTR`
...
### 05.6 종목기본 `STD_TBL_NM: CISMSTBSM001` / Legacy: `TB_ISU_MST`
...
### 05.7 종목시세스냅샷 `STD_TBL_NM: CMKQUOSNB001` / Legacy: `TB_MKT_QUOTE`
...
### 05.8 주문원장 `STD_TBL_NM: CORORDBST001` / Legacy: `TB_ORD_MST`
...
### 05.9 주문이력 `STD_TBL_NM: CORORDHSH001` / Legacy: `TB_ORD_HIST`
...
### 05.10 체결내역 `STD_TBL_NM: CEXEXCDTT001` / Legacy: `TB_EXEC_DTL`
...
### 05.11 종목잔고 `STD_TBL_NM: CBAPOSBLB001` / Legacy: `TB_BAL_POS`
...
### 05.12 예수금잔고 `STD_TBL_NM: CCACSHBLB001` / Legacy: `TB_CASH_BAL`
...
### 05.13 결제예정 `STD_TBL_NM: CSTSCHSTS001` / Legacy: `TB_STL_SCHD`
...
### 05.14 현금입출금내역 `STD_TBL_NM: CCACSHDTT001` / Legacy: `TB_CASH_IO_DTL`
...
### 05.15 신용약정 `STD_TBL_NM: CCRAGRBSM001` / Legacy: `TB_CRD_AGR`
...
### 05.16 신용융자내역 `STD_TBL_NM: CCRLONDTT001` / Legacy: `TB_CRD_LOAN`
...
### 05.17 담보평가내역 `STD_TBL_NM: CCLEVLEVB001` / Legacy: `TB_COLL_EVAL`
...
### 05.18 파생종목기본 `STD_TBL_NM: CDRDRVBSM001` / Legacy: `TB_DERIV_ISU_MST`
...
### 05.19 파생증거금 `STD_TBL_NM: CDRMRGBLB001` / Legacy: `TB_DERIV_MARGIN`
...
### 05.20 파생정산내역 `STD_TBL_NM: CDRSTLSTT001` / Legacy: `TB_DERIV_SETTL_DTL`
...
### 05.21 채권발행기본 `STD_TBL_NM: CBDISSBSM001` / Legacy: `TB_BOND_ISS_MST`
...
### 05.22 채권이자스케줄 `STD_TBL_NM: CBDINTSTS001` / Legacy: `TB_BOND_INT_SCHD`
...
### 05.23 권리이벤트 `STD_TBL_NM: CRTEVTBSE001` / Legacy: `TB_RIGHT_EVT`
...
### 05.24 권리배정내역 `STD_TBL_NM: CRTALCDTT001` / Legacy: `TB_RIGHT_ALOC`
...
### 05.25 수수료세금규칙 `STD_TBL_NM: CFEFTRRLR001` / Legacy: `TB_FEE_TAX_RULE`
...
### 05.26 공통코드그룹 `STD_TBL_NM: CCMCODCDC001` / Legacy: `TB_CODE_GRP`
...
### 05.27 공통코드상세 `STD_TBL_NM: CCMCODDTD001` / Legacy: `TB_CODE_DTL`
...
### 05.28 인터페이스메타 `STD_TBL_NM: MIFMSGIFI001` / Legacy: `TB_IF_MSG_META`
...
### 05.29 인터페이스필드메타 `STD_TBL_NM: MIFFLDDTD001` / Legacy: `TB_IF_FIELD_META`
...
### 05.30 배치Job메타 `STD_TBL_NM: MBTJOBBSM001` / Legacy: `TB_BATCH_JOB_META`
...
### 05.31 배치실행로그 `STD_TBL_NM: LBTRUNLGL001` / Legacy: `TB_BATCH_RUN_LOG`
...
### 05.32 감사로그 `STD_TBL_NM: LAULOGLGL001` / Legacy: `TB_AUDIT_LOG`
...
### 05.33 메타테이블 `STD_TBL_NM: MMTTBLBSM001` / Legacy: `MT_TABLE`
...
### 05.34 메타컬럼 `STD_TBL_NM: MMTCOLBSM001` / Legacy: `MT_COLUMN`
...
| CODE | VARCHAR2 | 20~30 | MKT_DVSN_CD | 표준 코드상세 `CCMCODDTD001` 유효값 참조 | 코드그룹 필수 |
...
| 분류 | 표준 | 예시 | 설명 |
| --- | --- | --- | --- |
| 표준 테이블명 포맷 | `SDDBBBOOTVNN` | `CORORDBST001` | 12자리 고정 길이 위치 기반 코드 체계 |
| 시스템 구분 | 1자리 | `C`, `M`, `I`, `S`, `L` | Core, Meta, Interface, Stage, Log 등 |
| 업무영역 | 2자리 | `OR`, `EX`, `BA`, `CA` | 주문, 체결, 잔고, 예수금 등 업무 대분류 |
| 업무객체 | 3자리 | `ORD`, `EXC`, `POS`, `CSH` | 업무영역 내 관리 객체 |
| 세부처리구분 | 2자리 | `BS`, `DT`, `HS`, `BL` | 기본, 상세, 이력, 잔고 등 |
| 테이블 성격 | 1자리 | `M`, `T`, `H`, `B`, `S`, `L` | Master, Transaction, History, Balance, Schedule, Log |
| 버전/세대 | 1자리 | `0`, `1`, `A` | 구조 버전 또는 세대 |
| 일련번호 | 2자리 | `01`~`99` | 동일 분류 내 순번 |
| 실제 물리명 | `PHYS_TBL_NM` | `CORORDBST001` | 기존 운영 DB의 물리명. 표준명과 분리 관리 |
| 레거시명 | `LEGACY_TBL_NM` | `CORORDBST001` | 이행/영향도 분석용 기존 명칭 |

#### 08.1.1 폐기된 테이블명 규칙

아래 방식은 v0.5.2부터 공식 표준 테이블명 규칙으로 사용하지 않는다. 다만 기존 시스템과의 호환을 위해 `PHYS_TBL_NM` 또는 `LEGACY_TBL_NM`으로만 관리한다.

| 폐기 규칙 | 예시 | 대체 기준 |
| --- | --- | --- |
| `TB_` Prefix | `TB_ORD_MST` | `STD_TBL_NM = CORORDBST001` |
| `MT_` Prefix | `MT_COLUMN` | `STD_TBL_NM = MMTCOLBSM001` |
| `STG_` Prefix | `STG_KRX_EXEC` | 시스템구분 `S` 또는 `I`를 사용하는 12자리 표준명 |
| `_MST`, `_DTL`, `_HIST` Suffix | `CORORDHSH001` | 9번째 테이블성격 코드 `M/T/H/B/S/L` 사용 |
...
| --- | --- | --- | --- |
| 주문접수 | 채널 주문을 접수하고 계좌/종목/주문가능금액 검증 | CORORDBST001, CCACSHBLB001, CBAPOSBLB001 | 계좌상태 정상, 주문제한 없음, 종목 거래가능, 호가단위 적합 |
| 주문전송 | 주문을 FEP/거래소로 송신 | CORORDBST001, CORORDHSH001 | 중복주문 방지, 대외주문번호 매핑 |
| 체결수신 | 거래소 체결을 수신하고 주문에 매핑 | CEXEXCDTT001, CORORDBST001 | 체결 중복 방지, 원주문 존재, 체결수량 누계 검증 |
| 비용계산 | 수수료/세금 산정 | CFEFTRRLR001, CEXEXCDTT001 | 시장/상품/채널별 규칙 적용 |
| 결제예정생성 | 체결 기준 T+N 결제 예정 생성 | CSTSCHSTS001 | 매수/매도 결제일, 금액 부호, 비용 반영 |
| 잔고/예수금반영 | 일중 또는 일마감 기준 잔고/예수금 갱신 | CBAPOSBLB001, CCACSHBLB001 | 잔고 음수 방지, D1/D2 예수금 대사 |
| 일마감 | 거래일 최종 잔고, 예수금, 결제 예정 확정 | LBTRUNLGL001 | 주문/체결/결제/잔고 대사 |

### 09.2 신용공여 처리 기준

| 구분 | 처리 기준 | 관리 테이블 | 비고 |
| --- | --- | --- | --- |
| 약정 | 고객/계좌별 신용거래 약정, 한도, 만기, 등급 관리 | CCRAGRBSM001 | KYC, 투자자구분, 사고계좌 여부 확인 |
| 융자매수 | 매수 체결 후 융자 원장 생성 | CCRLONDTT001, CBAPOSBLB001 | 대출일자 기준으로 잔고 분리 |
| 담보평가 | 현금, 주식, 대용가격, 평가비율 기반 담보 평가 | CCLEVLEVB001, CISSUBBLB001 | 종목별 대용비율 적용 |
| 이자계산 | 대출일수와 이자율 기준 발생이자 산정 | CCRLONDTT001 | 일할 계산, 연체 가산 가능 |
| 상환 | 매도상환 또는 현금상환 처리 | CCRLONDTT001, CCACSHBLB001 | 상환 순서 기준 필요 |
| 반대매매 | 담보부족/미수/연체 발생 시 강제청산 주문 생성 | CORORDBST001, CCRLONDTT001 | 사전통지, 보류/예외 승인 관리 필요 |

### 09.3 파생상품 처리 기준

| 구분 | 처리 기준 | 관리 테이블 | 비고 |
| --- | --- | --- | --- |
| 종목관리 | 결제월, 만기일, 행사가격, 승수 관리 | CDRDRVBSM001 | 만기 도래 시 최근월물 갱신 |
| 주문증거금 | 신규 주문 가능 여부 판단 | CDRMRGBLB001 | 위탁증거금, 주문가능금액 검증 |
| 미결제약정 | 체결 후 포지션 집계 | CDRPOSBLB001 | 매수/매도, 롱/숏, 계약수 관리 |
| 일일정산 | 정산가격 기준 평가손익 및 정산차금 계산 | CDRSTLSTT001 | 예수금 반영, 정산차금 대사 |
| 마진콜 | 유지증거금 미달 여부 판단 | CDRMRGBLB001 | 추가증거금, 강제청산 대상 관리 |
| 만기결제 | 최종거래일/만기일 청산 처리 | CDRSTLSTT001 | 현금결제/실물인수도 여부 구분 |

### 09.4 채권 처리 기준

| 구분 | 처리 기준 | 관리 테이블 | 비고 |
| --- | --- | --- | --- |
| 발행정보 | 발행일, 만기일, 표면금리, 이자주기 관리 | CBDISSBSM001 | 발행기관, 신용등급 포함 |
| 매매 | 장내/장외 채권 매매 체결 | CORORDBST001, CEXEXCDTT001 | 수량 단위와 가격 단위 주식과 상이 |
| 경과이자 | 매매일 기준 경과이자 산정 | CBDINTSTS001 | 세금 및 결제금액 반영 |
| 이자지급 | 이자지급일 권리자 확정 및 예수금 입금 | CRTEVTBSE001, CRTALCDTT001 | 원천징수 처리 필요 |
| 만기상환 | 만기일 원금 상환 | CSTSCHSTS001, CCACSHBLB001 | 상환금액, 세금, 수수료 반영 |
...
| 2 | EXCH_EXEC_NO | 거래소체결번호 | VARCHAR | 30 | Y | CEXEXCDTT001 | EXCH_EXEC_NO | 거래소 원천 체결번호 |
| 3 | EXCH_ORD_NO | 거래소주문번호 | VARCHAR | 30 | Y | CORORDBST001 | EXCH_ORD_NO | 원 주문 매핑 키 |
| 4 | ISU_CD | 종목코드 | VARCHAR | 12 | Y | CEXEXCDTT001 | ISU_CD | 체결 종목 |
| 5 | BUY_SELL_DVSN_CD | 매매구분코드 | CHAR | 1 | Y | CEXEXCDTT001 | BUY_SELL_DVSN_CD | 매도/매수 |
| 6 | EXEC_QTY | 체결수량 | NUMBER | 18,6 | Y | CEXEXCDTT001 | EXEC_QTY | 체결 수량 |
| 7 | EXEC_UPRC | 체결단가 | NUMBER | 18,6 | Y | CEXEXCDTT001 | EXEC_UPRC | 체결 단가 |
| 8 | EXEC_DTTM | 체결일시 | TIMESTAMP | 6 | Y | CEXEXCDTT001 | EXEC_DTTM | 체결 시각 |
| 9 | TRD_NO | 거래번호 | VARCHAR | 30 | N | CEXEXCDTT001 | TRD_NO | 거래소 거래 식별자 |
...
| --- | --- | --- | --- | --- | --- | :---: | --- | --- |
| BAT_ISU_SYNC | 종목마스터동기화 | 영업일 06:00 | - | 거래소/정보벤더 파일 | CISMSTBSM001 | Y | 변경분 롤백 가능 | 종목 신규/변경/상폐 반영 |
| BAT_QUOTE_BASE | 기준가생성 | 영업일 07:00 | BAT_ISU_SYNC | 전일시세 | CMKQUOSNB001 | Y | 전일 데이터 재적재 | 기준가, 상하한가, 호가단위 생성 |
| BAT_ORD_RECON | 주문대사 | 장종료 후 | - | CORORDBST001, FEP 로그 | 대사리포트 | Y | 미매칭 수동 확인 | 주문 접수/거래소 수신 대사 |
| BAT_EXEC_RECON | 체결대사 | 장종료 후 | BAT_ORD_RECON | CEXEXCDTT001, 거래소 체결 | 대사리포트 | Y | 중복/누락 재처리 | 체결 원장 대사 |
| BAT_STL_SCHD | 결제예정생성 | 장종료 후 | BAT_EXEC_RECON | CEXEXCDTT001 | CSTSCHSTS001 | Y | 기존 예정 삭제 후 재생성 | T+N 결제 예정 생성 |
| BAT_BAL_EOD | 일마감 잔고생성 | 일 1회 | BAT_STL_SCHD | 체결/결제/전일잔고 | CBAPOSBLB001 | Y | 스냅샷 보관 후 재수행 | 계좌별 종목 잔고 생성 |
| BAT_CASH_EOD | 일마감 예수금생성 | 일 1회 | BAT_STL_SCHD | 예수금/결제/출납 | CCACSHBLB001 | Y | 차이 검증 리포트 | D0/D1/D2 예수금 생성 |
| BAT_CRD_INT | 신용이자계산 | 일 1회 | BAT_BAL_EOD | CCRLONDTT001 | CCRLONDTT001 | Y | 이자 재계산 가능 | 융자 발생이자 산정 |
| BAT_COLL_EVAL | 담보평가 | 일 1회 | BAT_BAL_EOD | 잔고/대용가격 | CCLEVLEVB001 | Y | 부족계좌 재산정 | 담보비율 및 부족금액 산정 |
| BAT_FORCE_SELL | 반대매매대상생성 | 영업일 08:00 | BAT_COLL_EVAL | CCLEVLEVB001 | CORORDBST001 후보 | 제한적 | 승인 후 주문생성 | 미수/담보부족 강제청산 대상 |
| BAT_DERIV_EOD | 파생일일정산 | 일 1회 | 장종료 | 정산가격/포지션 | CDRSTLSTT001 | Y | 정산차금 대사 | 정산가격 기준 평가손익 및 증거금 반영 |
| BAT_BOND_INT | 채권이자계산 | 일 1회 | BAT_BAL_EOD | 채권스케줄/잔고 | 권리/예수금 | Y | 지급대상 재확정 | 이표채 이자 지급 및 세금 반영 |
| BAT_RIGHT_APPLY | 권리배정반영 | 이벤트별 | 권리승인 | CRTEVTBSE001 | CRTALCDTT001 | 제한적 | 승인 후 rollback | 배당/증자/분할 권리 반영 |
| BAT_CODE_DEPLOY | 공통코드배포 | 변경시 | 승인완료 | MMTCODDTD001 | 각 시스템 코드캐시 | Y | 이전 버전 재배포 | 공통코드 동기화 |
| BAT_META_EXPORT | 메타배포 | 변경시 | 승인완료 | `MMT*` 표준 메타 테이블군 | DW/운영포털 | Y | 승인본만 재배포 | 테이블/컬럼/코드/전문 메타 배포 |
...
| DQ_COMMON_003 | 코드 유효값 검증 | 코드 컬럼 | CCMCODDTD001 사용 가능 코드만 허용 | High | 오류 리포트 및 보정 |
...
| --- | --- | --- | --- | --- | --- |
| DQ_ORD_001 | 주문 | 주문수량 양수 검증 | CORORDBST001 | ORD_QTY > 0 | 주문 거부 또는 보정 |
| DQ_ORD_002 | 주문 | 주문상태-체결수량 정합성 | CORORDBST001 | 전량체결이면 ORD_QTY = EXEC_ACML_QTY | 주문/체결 대사 |
| DQ_EXEC_001 | 체결 | 체결 중복 검증 | CEXEXCDTT001 | EXCH_EXEC_NO 중복 없음 | 중복 수신 제거 |
| DQ_EXEC_002 | 체결 | 체결금액 검증 | CEXEXCDTT001 | EXEC_AMT = EXEC_QTY × EXEC_UPRC 허용오차 이내 | 비용 전 금액 재계산 |
| DQ_STL_001 | 결제 | 체결-결제예정 대사 | CEXEXCDTT001/CSTSCHSTS001 | 체결별 결제예정 1건 이상 | 누락 생성 |
| DQ_BAL_001 | 잔고 | 잔고 음수 검증 | CBAPOSBLB001 | HOLD_QTY >= 0 | 업무 예외 확인 |
| DQ_CASH_001 | 예수금 | D2 예수금 검증 | CCACSHBLB001 | D2 = 현재 + 결제예정 반영 | 예수금 재계산 |
| DQ_CRD_001 | 신용 | 만기일 검증 | CCRLONDTT001 | MTRT_DT >= LOAN_DT | 약정/융자 오류 확인 |
| DQ_COLL_001 | 담보 | 담보비율 검증 | CCLEVLEVB001 | 담보금액 / 융자금액 산식 검증 | 반대매매 대상 재산정 |
| DQ_DERIV_001 | 파생 | 정산차금 검증 | CDRSTLSTT001 | 전일정산가 대비 손익 계산 | 정산 재처리 |
| DQ_BOND_001 | 채권 | 이자스케줄 연속성 | CBDINTSTS001 | 이전 종료일과 다음 시작일 정합성 | 스케줄 보정 |
| DQ_RIGHT_001 | 권리 | 권리배정 대상 검증 | CRTALCDTT001 | 기준일 잔고 존재 | 권리 대상 재확정 |

### 12.3 대사 기준

| 대사명 | 원천 | 대상 | 기준 | 허용 오차 |
| --- | --- | --- | --- | --- |
| 주문 대사 | FEP/거래소 주문로그 | CORORDBST001 | 주문번호, 대외주문번호, 상태 | 0건 불일치 |
| 체결 대사 | 거래소 체결파일 | CEXEXCDTT001 | 체결번호, 수량, 단가, 금액 | 금액 1원 이내 정책 가능 |
| 결제 대사 | 체결내역 | CSTSCHSTS001 | 결제일, 종목, 금액, 수량 | 0건 누락 |
| 잔고 대사 | 전일잔고+당일체결 | CBAPOSBLB001 | 수량, 매입금액 | 업무별 허용오차 |
| 예수금 대사 | 전일예수금+출납+결제 | CCACSHBLB001 | 통화별 금액 | 1원 단위 |
| 파생 정산 대사 | 거래소 정산가/포지션 | CDRSTLSTT001 | 정산차금, 증거금 | 1원 단위 |
...
CREATE TABLE CORORDBST001 (
    ...
    CONSTRAINT PK_CORORDBST001 PRIMARY KEY (ORD_DT, ORD_NO)
)
PARTITION BY RANGE (ORD_DT) (
    PARTITION P_MAX VALUES LESS THAN (MAXVALUE)
);

CREATE INDEX IX_CORORDBST001_01 ON CORORDBST001 (ACNO, ORD_DT);
CREATE INDEX IX_CORORDBST001_02 ON CORORDBST001 (ISU_CD, ORD_DT);
CREATE INDEX IX_CORORDBST001_03 ON CORORDBST001 (EXCH_ORD_NO);
```

### 16.2 메타 컬럼 DDL 예시

```sql
CREATE TABLE MMTCOLBSM001 (
    ...
    CHG_USER_ID         VARCHAR2(50),
    CHG_DTTM            TIMESTAMP(6),
    CONSTRAINT PK_MMTCOLBSM001 PRIMARY KEY (COLUMN_ID),
    CONSTRAINT UK_MMTCOLBSM001_01 UNIQUE (TABLE_ID, PHYSICAL_COLUMN_NM)
);
```
