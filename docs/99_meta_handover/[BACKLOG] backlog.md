...
- 가장 확인하고 싶었던 `entry_score>=0.65` chronic 차단 사례
  (`009420`, `180640`)는 가격 데이터 부재(`009420`, `181710`)나
  종가 미확정(장 진행 중)으로 대부분 사후 성과를 계산할 수 없었다.
- **판정: 미확정.** 표본이 극히 작고 방향도 층위별로 엇갈려
  "과잉 차단"도 "정상 작동"도 지지되지 않는다 — 표본이 쌓인 뒤
  재검증이 필요하다. R2 allocation 관련 기존 결론(§13.2.13/§13.2.14)
  과는 별개 축이라 충돌하지 않는다.
