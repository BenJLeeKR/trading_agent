...
### 1.2 실증적으로 뒷받침된 근거 — `000227` / 2026-06-23 KST 자연 발생 부분체결 표본

`tot_ccld_qty`(총체결수량)가 **"이 주문이 지금까지 누적으로 체결된 수량"**을 의미한다는 가정은, 이후 별도 조사 턴에서 확보한 자연 발생 부분체결 표본으로 **실증적으로 뒷받침됐다**.

- 표본: `000227` 종목, 2026-06-23 KST에 실제로 부분체결 상태(약 10:40~12:24 KST)를 겪은 매도 주문 1건.
- 근거: 이 주문번호에 대해 `fill_history_sync.py`가 **당시(2026-06-23) 실시간으로** 적재해 둔 `broker_fill_snapshots.filled_quantity`를, 이후 시점에 관측 시각순으로 조회한 결과, 같은 주문번호에 대해 **`0 → 11 → 259`(=요청수량 전량)로 단조 증가하는 staircase**가 확인됐다.
- 이 패턴은 "매 관측마다 지금까지의 누적 체결량을 알려준다"는 해석과 정확히 일치한다. 만약 `tot_ccld_qty`가 "이번에 새로 체결된 증분"이었다면 11, (259-11)=248처럼 서로 다른 개별 수량이 나왔어야 하는데, 그렇지 않고 매번 "그 시점까지의 누적 총량"이 나왔다.
- 이로써 **누적→증분 해석 전략(3.2절 안 A + 안 C) 자체를 보류할 이유는 크게 줄었다.**

**이 근거의 정확한 성격과 한계(과장 방지)**:
- 이는 **당시 `fill_history_sync.py`가 캡처해 둔 `broker_fill_snapshots` 시계열 관측값을 통한 간접 실증**이다 — 같은 상황(부분체결 진행 중인 살아있는 주문)에 대해 **오늘 시점에 raw `inquire-daily-ccld`를 직접 여러 번 재호출해 staircase를 실시간으로 재현한 것은 아니다**(해당 주문은 이미 2개월 이상 지나 조회 대상 기간을 벗어나, 오늘 재조회하면 0 rows가 반환된다 — 이는 보존 기간 관련 별개 현상이며 1.3절에서 분리해 다룬다).
- 이 표본에서는 `avg_prvs`(평균단가)가 모든 관측 시점에 걸쳐 동일한 값이었다 — 즉 이 표본은 "수량이 누적되는가"는 명확히 검증했지만, 서로 다른 가격대의 체결이 섞였을 때의 **가중평균 분해 공식**(3.2절 `_infer_delta_price`)까지 검증한 것은 아니다.
- 자연 발생 부분체결 표본은 이번까지 포함해 총 1건(다단계 관측 기준)이다. 이는 "특정 주문 유형에서만 확인됐다"는 뜻이 아니라(이 설계는 주문 유형별로 검증 범위를 나누지 않는다), **표본 수 자체가 아직 많지 않다**는 뜻이다.

### 1.3 미확정 영역 (이 설계가 반드시 보호장치로 다뤄야 하는 부분)

주문 유형(시장가/지정가)은 이 설계의 검증 축이 아니다 — 이 설계는 시장가/지정가를 별도 경로로 다루지 않으므로, "어느 주문 유형에서 확인됐는가"는 남겨야 할 미확정 항목이 아니다. 실제로 남겨야 할 미확정 항목은 다음과 같다:

- **정정/취소가 섞인 경우의 해석** — `cancel_yn`/`rvse_yn`이 관측될 때 delta 계산이 실제로 안전하게 anomaly로 분리되는지는 아직 실제 정정/취소 표본으로 검증되지 않았다(7절 참고).
- **live(실전) 환경 검증** — 같은 TR 계열(`TTTC0081R`)의 응답이 paper(`VTTC0081R`)와 같은 필드명/케이싱/누적 의미론을 쓰는지 **전혀 확인하지 않았다**. 이번 조사는 전부 paper 계좌로만 진행됐다.
- **같은 조건에서의 raw 응답 직접 장기 재관측 부재** — 1.2절의 누적 패턴 확인은 `broker_fill_snapshots`라는 간접 경로를 통한 것이며, 부분체결이 진행되는 동안 `inquire-daily-ccld` raw 응답 자체를 직접, 여러 시점에 걸쳐 재호출해 관측한 것은 아직 없다. 다음 자연 발생 부분체결 표본이 생기면 이 직접 관측을 추가로 확보하는 것이 바람직하다.
- 오래된(2개월 이상) 주문의 조회가 0 rows를 반환하는 현상의 정확한 원인(보존 기간 정책인지, 다른 이유인지) — **미확정**. 이건 이번 설계의 핵심 문제(누적/증분 해석)와는 별개 현상으로 분리해서 취급한다.

> **이 설계에서 명시적으로 강조할 점**: 확인되지 않은 것은 더 이상 "누적값인가 증분값인가"라는 핵심 가정 자체가 아니다 — 이제 초점은 **"정정/취소가 섞인 상황에서도 이 해석이 안전한가"**, **"live 환경에서도 같은 필드 규칙이 성립하는가"**, **"shadow 관측 기간 동안 anomaly 비율이 실제로 낮게 유지되는가"**로 옮겨갔다.
...
이 채택은 처음 설계 시점에는 "누적값일 가능성이 높다"는 가설에 의존했지만, 1.2절의 `000227`/2026-06-23 KST 자연 발생 부분체결 표본(staircase `0→11→259`)으로 **핵심 가정이 실증적으로 보강되었다** — 안 A/안 C를 채택한 근거는 이전보다 약해진 것이 아니라 **더 강해졌다.** 다만 이 표본은 시장가/지정가 구분과 무관하게 "누적 스냅샷" 해석 자체를 검증한 것이며, 정정/취소 혼재·live 환경 검증(1.3절)은 여전히 별도로 확보해야 한다.
...
- **live 전개 전 필수 검증**(구현 이후, 이 설계 문서가 요구하는 최소 조건):
  1. live(실전) 계좌로 최소 1건의 완전체결 read-only 조회를 실행해, paper와 같은 후보 키 순서로 필드가 해석되는지 확인. — **미충족**.
  2. 자연 발생하는 부분체결 주문을 최소 1건 이상, 2회 이상의 서로 다른 누적치 관측으로 확보해 "누적 체결량이 실제로 새 체결분과 일치하는지" 확인. — **부분 충족**: `000227`/2026-06-23 KST 표본으로 `broker_fill_snapshots` 시계열(간접 경로)을 통해 `0→11→259` staircase를 확인했다. 다만 부분체결이 진행되는 동안 `inquire-daily-ccld` raw 응답을 직접 여러 시점에 재호출해 관측한 사례는 아직 없다 — 다음 자연 발생 표본에서 이 직접 관측을 추가로 확보하는 것을 권장한다.
  3. 정정(정정주문, `rvse_yn='Y'`)이 섞인 표본을 최소 1건 확보해 anomaly 분기가 올바르게 동작하는지 확인. — **미충족**.
