...
#### 실전 latency / prompt budget 원칙

v1의 3개 Provider AI Agent(EI / AR / FDC)는 모두 같은 latency budget으로 취급하면 안 된다.
실전 매매에서는 **이벤트를 길게 읽는 것보다, 이미 정리된 context를 제한 시간 안에 판단하는 것**이 더 중요하다.

공통 원칙:

- external event 수집과 매매 판단은 분리한다.
- 공시/뉴스 원문을 주문 직전 경로에서 처음 읽게 하지 않는다.
- 판단 시점에는 provenance가 붙은 **요약된 고신호 event context**만 사용한다.
- prompt 길이가 늘어 latency budget을 침식하면, event 수를 줄이거나 narrative를 더 압축한다.
- timeout은 “정상 기대 시간”이 아니라 **최악 지연을 견디기 위한 상한선**으로 본다.
- timeout 또는 provider 지연 시 deterministic backend / fallback output이 안전하게 이어받아야 한다.

권장 v1 budget:

| Agent | 역할 성격 | 권장 soft budget | 권장 hard timeout | 비고 |
|---|---|---:|---:|---|
| Event Interpretation Agent | slowest of the three, event summarisation | 5-15s | 30-45s | event provenance 해석 담당, raw event를 직접 가장 많이 읽음 |
| AI Risk Agent | medium latency, risk opinion | 3-10s | 20-30s | position/cash/risk snapshot과 event summary 결합 |
| Final Decision Composer | fastest of the three, final intent synthesis | 2-8s | 15-20s | 실전 판단 체인의 마지막 AI 계층이므로 가장 짧아야 함 |

운영 해석:

- `soft budget`은 정상 시 기대 응답 시간이다.
- `hard timeout`은 provider 지연/혼잡을 고려한 절대 상한이다.
- 실전에서는 FDC가 가장 latency-sensitive 하므로, 가장 짧은 prompt와 timeout을 가져야 한다.
- v1 구현에서 timeout이 이보다 크게 설정되어 있더라도, 그것은 **운영 목표가 아니라 방어적 상한**으로 해석해야 한다.

#### v1 prompt slimming 원칙

prompt 품질을 높이기 위해 정보를 더 넣을 수는 있지만, 실전에서는 아래 slimming 원칙을 유지해야 한다.

1. **event는 raw dump가 아니라 ranked summary만 전달**
   - symbol-filtered recent events만 사용
   - non-tradable / irrelevant event는 가능한 한 upstream에서 제외
   - event line당 provenance tag는 유지하되, body narrative는 짧게 제한

2. **Agent별로 읽는 정보량을 다르게 제한**
   - EI는 event provenance를 가장 많이 읽는다
   - AR은 event summary + risk snapshot을 읽되, raw event 재서술은 최소화한다
   - FDC는 EI/AR structured output을 우선 읽고, raw recent_events는 압축된 형식만 본다

3. **default / 무의미 필드는 생략**
   - `severity=medium`, `direction=neutral` 같은 default 정보는 prompt에서 생략
   - fresh event에는 `STALE` tag를 붙이지 않는다
   - null issuer / empty body 같은 low-signal 필드는 넣지 않는다

4. **event count cap 유지**
   - event retention window는 넓혀도 prompt event line 수는 cap을 둔다
   - v1에서는 prompt body가 event를 무한히 늘리지 않도록 제한해야 한다

5. **AI가 수치 계산을 대신하지 않게 한다**
   - position limit, exposure, sizing limit, cash buffer는 서술형으로 길게 설명하지 말고
     이미 계산된 fact만 넣는다
   - 수치 판단은 deterministic backend가 담당한다

6. **downstream 전파 시 provenance를 보존하되 재중복은 피한다**
   - EI에서 provenance-rich event line을 만들었으면 AR/FDC에도 가능한 한 같은 형식을 유지
   - 그러나 같은 정보를 여러 paragraph로 중복 서술하지 않는다
...
**실전 latency / slimming 기준**:

- 3개 Agent 중 event raw context를 가장 많이 읽으므로 가장 느려질 가능성이 높다.
- 따라서 EI는 **event provenance를 풍부하게 유지하되**, event 수 cap과 body summary 길이 제한을 함께 둬야 한다.
- retention window가 72h로 늘어나더라도 prompt line 수는 제한하여 latency를 통제한다.
- stale/freshness는 구조화 tag로 짧게 표현하고, 긴 설명 문장은 downstream으로 넘기지 않는다.
...
- AR은 EI보다 적은 event를 읽어야 하며, raw event보다 **EI structured output + risk snapshot fact**를 우선 본다.
- raw recent events를 다시 prompt에 넣더라도 provenance-rich short line만 사용하고, narrative 중복은 피한다.
- 포지션/현금/노출/손실 한도는 이미 계산된 fact만 전달하고, 장황한 배경 설명은 넣지 않는다.
- 실전에서는 EI보다 더 짧은 soft budget을 가져야 한다.
...
- FDC는 v1 체인에서 가장 latency-sensitive한 AI 계층이다.
- 가능한 한 **EI structured output + AR structured output**을 주 입력으로 삼고,
  raw recent_events는 provenance-rich short line만 보조적으로 사용한다.
- 동일 정보의 재서술을 최소화하고, 최종 방향/이유/집행 선호를 빠르게 구조화하는 데 집중한다.
- 실전 운영 목표상 3개 Agent 중 가장 짧은 soft budget과 hard timeout을 가져야 한다.
