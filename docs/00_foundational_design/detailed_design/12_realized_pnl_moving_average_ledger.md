...
> - **summary endpoint(Admin UI N+1 제거용)도 구현 완료** — `GET /performance/realized-pnl/summary`(`account_id`+`start_date`+`end_date` 필수, `instrument_id` 선택). `RealizedPnlDailyAggregateRepository.list_by_account()`를 최소 추가해(신규 migration 없음) 계좌의 모든 종목의 일자 집계를 **단일 조회**로 가져온 뒤 종목별/전체로 단순 합산한다 — Admin UI가 종목 "전체" 조회 시 종목마다 `daily`를 반복 호출하던 N+1을 없애기 위한 endpoint다. `recompute_required`는 `PositionCostBasisStateRepository`의 기존 `get()`/`list_by_account()`를 그대로 쓴다(추가 확장 없음). 단위 테스트는 `tests/api/test_inspection.py`의 `TestRealizedPnl`(summary 관련 4건). **Admin UI는 아직 이 endpoint로 전환되지 않았다** — `RealizedPnlView.tsx`는 여전히 기존 `positions`+`daily` N+1 경로를 쓴다(후속 과제).
