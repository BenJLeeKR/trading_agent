...
> - **아직 연결되지 않음**: 대규모 backfill CLI(수천 계좌×종목 순회), Admin UI 화면 구현. **Admin UI는 화면 설계서 작성이 먼저 필요하며, 그 단계로 넘어가기 전에는 사용자에게 먼저 알린다.**
