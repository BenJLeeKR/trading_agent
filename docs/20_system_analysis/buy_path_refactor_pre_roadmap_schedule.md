...
- [x] `relative_activity` 역할 1차 분류 완료 — **[2026-08-03 KST
  ...
  닫혀 다음 턴은 B안 코드 초안 착수 가능. **[동일자 3차 갱신]**
  §13.4.3 — B안 원안(activity 하드 플로어 전체 제거)은 기존 테스트가
  exercise하는 "관측 범위 내 dead" 분기라 보류하고, 대신 signal
  체크로 인해 구조적으로 도달 불가능한 `signal_feature_snapshot is
  None` 분기만 제거했다(동작 무변화, 테스트 26건 그대로 통과)
...
- [x] `entry_score`에서 제거해야 할 비-alpha 보정항이 무엇인가 —
   ...
   **[2026-08-03 KST 9차 갱신, R2 상태: B안 후보 계수별 영향 실측
   완료]** `buy_path_variable_gate_matrix.md` §13.2.9 — `B1(-0.10)`
   /`B2(-0.05)`의 신규 통과 표본(전체 이력 297건/2,047건)은 전부
   `bullish_trend`뿐이고, 실제 override 사례(`011070`)는 `B2`부터
...
- R3는 **[2026-08-03 KST 갱신]** 역할 분리 판정(13.3.1)과 유일한 정리
  후보였던 `allocation_budget_ok` 이중 확인 제거(13.3.2, 동작 무변화)
  까지 마치고 트랙을 닫았다. R4가 아직 열려 있으므로 2단계 전체 완료로
  닫지 않는다.
...
- 두 번째 항목(R3)은 §13.3.1로 닫혔다 — 대체로 정당한 역할 분리로
  판정됐고, 유일한 정리 후보였던 `allocation_budget_ok` 코드 레벨
  재확인은 **[2026-08-03 KST 갱신]** §13.3.2로 제거 적용까지 마쳤다
  (동작 무변화).
...
- [x] 제거 후 운영 데이터 재실측 완료 **[2026-08-05 KST 갱신]**
- [x] authoritative 게이트 판정 이동폭 운영 재집계 완료 **[2026-08-05 KST 갱신]**

**[2026-08-05 KST 갱신, 장후 실측 완료]** `signal_predictive_power_
validation.md` §35.2에서 2026-08-05(수) 장후(EOD) 데이터로 재실측
완료 — §13.2.3의 "판정 A(영향 미미)"는 **정정**한다. `buy_candidate`
(`0.65`) 게이트는 오늘 표본에서 3개 종목(`008930`/`051900`/`078930`)
이 allocation 제거로만 매 사이클 `WATCH`에 머무는 재현 가능한 영향이
확인됐다(과거 3거래일/1개월 표본에는 없었으나 오늘 표본에는 존재).
`risk_off_exception_eligible`(authoritative 하드 게이트, `0.28`)은
예상대로 뒤집힘 후보가 0건으로 영향 없음을 확인했다.

판정:

- R2는 **재실측까지 포함해 완료**로 본다.
- 단, `buy_candidate` 게이트 영향에 대한 최초 판정(A: 영향 미미)은
  오늘 실측으로 정정됐다는 점을 이력으로 남긴다 — 게이트 코드/
  threshold 자체는 무변화이며, 입력값(`entry_score`) 변화가 실제
  운영 population에서 관측 가능한 판정 이동을 만든 것으로 확인됐다.

**[2026-08-06 KST 재갱신, 원인 규명 완료]** `buy_path_variable_
gate_matrix.md` §13.2.13 — §13.2.3의 "C=0" 판정을 **동일 방법론**으로
2026-08-05(수) 표본에 재적용해도 C=0이 아님(216행/4종목, `buy_
candidate` 기준으로 좁히면 162행/3종목)을 재확인했다. 원인은 측정
오류가 아니라 **유니버스 회전**이다 — 오늘 boundary-dependent 4종목
(`008930`/`051900`/`073240`/`078930`)은 §13.2.3 측정 기간(06-20~
07-31)에 `trade_decisions`에 전혀 등장하지 않는다(0건, read-only
확인). 즉 §13.2.3의 "영향 미미"는 **그 시점 표본에 한정된 관찰**이며,
유니버스가 회전할 때마다 재검증이 필요한 결론으로 재분류한다(판정
B: 일부 정정). 다음 1순위는 이 4종목의 `entry_score`가 경계에 몰린
것이 우연인지 다른 리팩터링의 계통적 영향인지 규명하는 것이나, 이는
R2 자체의 완료 여부를 바꾸지 않는다.
...
- [x] R3 `portfolio_allocation` 역할 분리 검토 시작 — **[2026-08-03
  ...
  **A(무변화 구조 분리, 작고 선택적)**로 좁혔다. **[2026-08-03 KST
...
- [x] AI downgrade 경로 점검 — **[2026-08-03 KST 갱신]** 6개 하류
  파일(decision_orchestrator/expected_value_gate/decision_factory/
  ...
  **[동일자 재갱신]** `decision_orchestrator.py::_check_ai_buy_
  override_gate()` 내부 `evaluate_action_envelope` 재확인(R5-f)도
  §13.5.3으로 정리 완료 — 호출 순서·5개 source_type 전수 검토로
  구조적으로 100% 도달 불가능함을 확정, 동작 무변화
- [x] EV gate 경로 점검 — 매핑에 포함(위와 동일 갱신), R5-b로 추가
  정리 완료
- [x] submit translation 경로 점검 — R5-a 적용으로 일부 정리 완료
- [x] 하류 contract 독립 정리 필요 여부 판정 — R5-a/R5-b/R5-f 전부
  무변화 구조 정리로 닫혔다. 남은 R5 후보는 이번 턴 범위 밖(대화
  이력 R5 원본 매핑 참고)
...
- [x] R1 정의 고정
- [x] R2 경계 확인(운영 재실측 포함) **[2026-08-05 KST 갱신]** — §8.2
  참고, 재실측 완료(판정 A 정정 포함)
- [ ] Roadmap 작성 착수
...
**[2026-08-04 KST 교차참조]** R1~R5로 이어진 이 BUY 경로 리팩터링
트랙은 `[DESIGN] signal_predictive_power_validation.md`의 `SPPV-2`
후기 구간(§2.107~§2.159)이 사실상 전개된 결과이기도 하다 — 그
문서의 §29에서 이 구간을 종료 선언하고, 이후 검증은 재정의된
`SPPV-3`로 넘어간다는 구조를 정리했다.
