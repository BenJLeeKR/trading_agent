...
### 21.5 표준안 시범 적용 — 계산 가능한 범위(illustrative, 강한 결론 아님)

08-03/08-04 결정일은 T+1/T+2 종가가 이미 존재해 위 표준안을
**지금 바로 시범 적용**할 수 있다(08-05/08-06 결정은 08-06/08-07
종가 미도착으로 여전히 계산 불가 — §19와 동일한 horizon 미도착
사유). `trading.instrument_status_snapshots` 종가로 계산했다.

| `alignment_status` | 결정일 | 종목 수 | T+1 평균 | T+2 평균 |
|---|---|---|---|---|
| `downgraded` | 08-03 | 2(`001450`,`073240`) | +1.73%(혼조: -1.34%/+4.80%) | +4.60%(혼조: -1.61%/+10.81%) |
| `suppressed` | 08-03 | 11 | +2.88% | +3.83% |
| `suppressed` | 08-04 | 12 | +0.71%(T+1만, T+2는 08-06 미도착) | 계산 불가 |

**이 표에서 말할 수 있는 것(factual)**: 계산 가능한 범위에서
`downgraded`/`suppressed` 표본의 T+1/T+2 평균은 **음수가 아니라
양수**였다 — 즉 이번 시범 표본에서는 "하향/차단된 종목이 실제로
하락했다"는 방향은 나타나지 않았다.

**이 표에서 말할 수 없는 것(과잉해석 방지)**: (1) 표본이
13~23건, 종목이 2~13개뿐이라 통계적으로 유의한 결론이 아니다.
(2) `matched`(비교군) 표본의 같은 기간 T+1/T+2를 아직 계산하지
않았다 — "하향된 쪽이 통과한 쪽보다 더 좋았다/나빴다"는 비교
자체를 이번 턴에서는 하지 않았다(다음 착수 턴 1순위 작업으로
남긴다, §21.6). (3) look-ahead caveat(§21.4)가 그대로 적용된다.
(4) horizon 불일치(15분~2시간 vs 24~48시간)도 그대로 적용된다.
**"하향 게이트가 좋은 진입을 놓쳤다"는 결론을 내리지 않는다** —
이 표는 방법론이 실행 가능함을 보여주는 시범일 뿐이다.
...
- **factual**: (1) 가상 진입가 방법론은 §34(2026-08-04)에서
  ...
  축에서 실제로 쓰인 적이 없다 — 두 축 모두 종가 기준 단순 T+1/
  T+2 근사만 썼다. (4) §21.4 표준안으로 08-03/08-04 결정일 표본을
  시범 계산한 결과, T+1/T+2 평균이 양수였다.
- **해석**: 과거 설계(§34)의 통계 계층(Newey-West/quintile)은
  현재 표본 규모와 맞지 않아 그대로 재활용하기 어렵다 — 대신
  A/C축이 실제로 써온 가벼운 근사 방식을 B축에도 통일 적용하는
  것이 축 간 비교 가능성을 지킨다. §21.5의 양(+) 평균은 표본이
  너무 작아 "하향 게이트가 실은 좋은 진입을 놓치고 있다"는
  결론으로 이어질 근거가 아니다 — 다만 최소한 "차단 = 나쁜
  진입"이라는 가정도 이 표본만으로는 뒷받침되지 않는다.
- **미확정**: (1) `matched` 비교군의 동기간 T+1/T+2(다음 착수
  턴 1순위). (2) look-ahead 왜곡의 실제 크기(결정 시각별 분포
  확인 필요). (3) 경계 고득점 구간(`entry_score>=0.60`)만 좁힌
  B축 표본의 사후 성과. (4) chronic 교차 종목(`073240`/`051900`/
  `008930`)의 A/B/C축 결과가 서로 일관되는지.

### 21.7 후속 검증 연결(다음 착수 턴 1순위)

1. `matched` 비교군의 동일 기간(08-03/08-04 결정일) T+1/T+2를
   §21.5와 동일한 방법으로 계산해 직접 비교표를 완성한다.
2. 같은 종목·같은 날에 `alignment_status`가 갈린 사이클(`073240`
   08-03, `008930`/`051900` 08-06)을 우선 짝지어 베타 통제
   비교를 시도한다.
