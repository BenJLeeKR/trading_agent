...
- **D) 설계 목적 침식 여부**: §18.3에서 확인한 **상류 candidate
  ...
  하류에서 걸러진다"는 해석에 더 가까운 **구조적 증거**다. 다만
  이 간극이 "과도한 억제"인지 "의도된 이중 필터"인지는 사후
  성과 데이터 없이는 최종 판정할 수 없다.

**확신 수준: "의심이 커짐"**(3단계 중 중간). "아직 미확정"보다는
근거가 강화됐다(§17의 관찰적 상관 수준에서, 이번 절의 공식 분해로
"왜"라는 메커니즘 수준까지 내려갔다) — 상류/하류 기준선 간극이
구체적 수치로 확인됐기 때문이다. 그러나 "운영상 괴리 가능성
큼"(최상위 확신)으로는 올리지 않는다 — 그 판정에는 이 간극을
넘지 못한 진입들이 실제로는 사후 기대값이 음(-)이었다는(즉
게이트가 옳았다는) 반증이 아직 없다는 것과 동시에, 그 간극을
넘었다면 양(+)이었을 것이라는 긍정 증거도 없다는 **양방향
불확실성**이 남아 있기 때문이다.
...
- **factual**: (1) 하방/비용/슬리피지 공식은 종목별 실측 지표
  ...
  시점(16:44 KST)에 데이터 미도착으로 계산 불가.
- **미확정**: (1) 이 간극이 설계상 의도된 이중 필터인지 우연한
  미조정인지. (2) 개별 파라미터(ATR 상한, 거래량 구간 임계,
  `minimum_required_edge_bps=10`)의 사후 성과 기준 적정성. (3)
