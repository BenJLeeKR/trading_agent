...
1. 독립적으로 비교 가능한 `(symbol, date)` 쌍이 최소 표본
   (문서 전반에서 반복 사용한 기준 n≈10)에 미달한다.
2. 비교 가능한 관측일 사이에서 방향(어느 쪽이 더 나은 성과를
   냈는지)이 반복적으로 뒤집힌다.
3. 통과군/차단군(또는 `matched`/`downgraded`·`suppressed`)의
   성과 차이가 무작위 수준(수 %p 이내)과 구분되지 않는다.
이 세 기준 중 하나라도 충족하면 "완화 후보"로 명시할 근거는
있다고 본다 — 다만 **"완화 후보"는 "지금 제거해야 한다"가
아니라 "제거해도 되는지 계속 확인해야 한다"는 뜻**이다.
...
| 4(큼) | `suppressed`/`downgraded` 전체 완화(하향 임계 자체를 낮춤) | 예: downstream 하향이 트리거되는 조건 자체를 완화 | 신규 진입 억제 총량 감소 | **가장 위험** — §24.2에서 확인했듯 방향이 날짜마다 정반대라, 지금 전체를 완화하면 "우연히 나쁜 진입을 더 많이 허용"하는 결과로 이어질 수 있다 | 비교 가능한 관측일이 최소 10일 이상 쌓이고 방향이 안정적으로 한쪽(하향 쪽이 계속 낮음)으로 수렴해야 착수 가능 | **현재 증거로는 착수 근거가 없다** — 방향이 정반대인 2일만으로 전체 완화를 논의하지 않는다 |
...
| 순위 | 후보명 | 완화 방식 | 기대 효과 | 위험 | 장 종료 후 확인 조건(진전) | 보류/철회 조건 |
|---|---|---|---|---|---|---|
| 1(최소) | near-miss soft handling | `edge_after_cost_bps`가 `minimum_required_edge_bps`에 근소(예: 5bps 이내) 부족한 사이클만 즉시 `WATCH`/`HOLD` 대신 "관찰 유지 후 재평가"로 전환. 구조적 실패(16.5bps 이상 부족)는 전혀 건드리지 않음 | 근소 실패만 좁혀 완화하므로 방어 약화 범위가 가장 작음 | **이미 있는 유일한 실증 사례(`001450`, 2.36bps 부족)가 사후 음(-) 성과(T+1 -1.34%/T+2 -1.61%)를 보였다** — 이 완화의 유일한 근거 사례가 오히려 완화에 반하는 신호다. 이 사실을 숨기지 않고 명시한다 | `008930`/`051900`의 T+1이 양(+)으로 나와도 이들은 near-miss가 아니라 구조적 실패라 이 후보의 근거가 되지 못한다 — 이 후보를 진전시키려면 **새로운 near-miss 사례**(부족분 5bps 이내)가 추가로 관측되고 그 사후 성과가 양(+)이어야 한다 | 추가 near-miss 사례가 계속 음(-) 성과를 보이면 이 후보는 **철회** — 이미 n=1에서 반하는 신호가 있으므로 착수보다 관찰 우선 |
...
| 5 | 완화 후보 유지/강화/철회 판정 | §24.3/§24.5 각 후보의 "진전 조건"/"보류·철회 조건"에 실측값을 대입 | 조건을 충족한 후보만 다음 턴에 구체적 설계(코드 변경 없이 shadow 계측 설계까지)로 진행 |

### 24.7 factual / 해석 / 미확정

- **factual**: (1) 08:54 KST 기준 1차/파생 소스 모두 `2026-08-06`
  이 최신이며 `2026-08-07` 종가는 존재하지 않는다(장 미마감). (2)
  B축은 비교 가능일 2일의 방향이 정반대, C축은 독립 비교 쌍
  1개의 성과가 거의 동일 — 둘 다 §24.1의 완화 후보 기준을
  충족한다. (3) C축의 유일한 near-miss 완화 후보 근거 사례
  (`001450`)는 이미 사후 음(-) 성과를 보였다 — 완화에 반하는
  기존 신호가 있다.
- **해석**: 두 축 모두 "완화 후보"이지만 아직 "완화해야 한다"는
  결론은 아니다. B축은 재분해(어느 guard가 하향을 만드는지)가
  완화 논의의 선행 조건이고, C축은 작은 후보(near-miss)일수록
  오히려 기존 실증 근거가 약하다는 역설적인 상황이다 — 이는
  표본이 극히 작을 때 흔히 나타나는 패턴이며, 표본이 늘어나기
  전에는 후보 우선순위를 확정하지 않는다.
- **미확정**: (1) B축의 guard별 재분해 결과. (2) `008930`/
  `051900`/`001800`/`035420`의 08-07 이후 실제 성과. (3) 위
