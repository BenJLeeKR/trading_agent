...
| `strategy_policy_mismatch` 지배적(사이클 과반) | 1(`035420`, 58.5%) | -8.30% | -8.30% | 0% | n=1, 일반화 불가 |
| `strategy_policy_mismatch` 비지배적 | 5(`001450`/`073240`/`181710`/`008930`/`051900`) | **+5.03%** | +2.93% | 80.0% | |
...
### 30.5 `strategy_policy_mismatch`의 보조 설명력

사이클 과반(50% 초과)에서 `strategy_policy_mismatch` 태그가
붙은 단위는 6개 중 1개(`035420`, 58.5%)뿐이다 — 나머지 5개는
이 태그가 소수 사이클에만 나타난다(§29.2 Step2에서 확인한 대로
`risk_tone==risk_off`가 `allowed_strategies`를 구조적으로
좁히지만, 그렇다고 매 사이클마다 AI가 이 태그를 명시적으로
쓰는 것은 아니다). **이 태그 하나가 `risk_tone`과 별개로 강한
독립적 설명력을 갖는다는 근거는 이번 절에서도 확인되지 않았다**
— 유일하게 이 태그가 지배적이었던 `035420`이 최저 성과(-8.30%)
를 보인 것은 참고할 만하지만 n=1이라 일반화하지 않는다.
...
- **factual**: (1) `risk_off + downgraded(BUY_CANDIDATE)` 표본
  ...
  하다. (5) `strategy_policy_mismatch`가 지배적인 단위는 1개뿐
  이고 그 성과가 최저였다(n=1).
