...
아직 나오지 않았고, `181710`/`009420`은 이 기간 내내 가격 데이터
자체가 없어(read-only 확인, `instrument_status_snapshots`에 해당
`instrument_id` 행 없음) 계산 자체가 불가능하다 — 표본에서 **제외한
것이 아니라 원천적으로 관측 불가**임을 구분해 기록한다.
...
**판정**: 전체 population과 경계 구간(`entry_score>=0.60`)의 결과
방향이 서로 다르고, 경계 구간·차단군의 계산 가능 표본이 n=1~2
수준이라 **어느 쪽으로도 결론을 내릴 수 없다**. 특히 실제로 가장
궁금한 대상(`009420`, `180640` 같은 `entry_score>=0.65` chronic
차단 사례)의 사후 성과는 데이터 부재로 대부분 확인 불가능했다.
"과잉 차단"도 "정상 작동"도 이번 실측만으로는 지지되지 않는다 —
**표본 축적 후 재검증이 필요한 상태로 남긴다.**

**미확인 사항**: (1) `009420`의 사후 성과는 08-07 이후 종가가
나와야 계산 가능하다. (2) `181710`의 가격 데이터 공백 원인(수집
누락인지 다른 사유인지)은 규명하지 않았다. (3) T+5/T+20은 최소
