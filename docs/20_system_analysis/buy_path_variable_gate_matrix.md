...
## 24. B축/C축 완화 후보 설계 초안(장 종료 전 준비, 2026-08-07 KST)

**이번 절의 성격**: `2026-08-07` 장 종료 후에만 가능한 종가
반영 재계산(T+1/T+2)은 이번 턴에서 수행하지 않는다(조회 시각
08:54 KST 기준 1차/파생 소스 모두 여전히 `2026-08-06`이 최신 —
이번 턴에서 재확인, §23과 동일). 이번 절은 **실측이 아니라 설계
정리**다 — §17~§23의 factual 근거 위에서 B축/C축 각각의 완화
후보를 작은 완화→큰 완화 순으로 세분화하고, 장 종료 후 실측
결과가 각 후보를 어느 방향으로 밀지 미리 정해둔다. **코드/정책
변경은 이번 절에서도 하지 않는다.**

### 24.1 설명력이 약하다고 판단하는 기준(공통, B/C축 동일 적용)

**최상위 판단 철학(2026-08-07 KST 재정정)**: 이 시스템의 목표는
개별 거래 무손실이 아니라 **감내 가능한 손실 제약 아래 전체
기대수익률 극대화**다(`AGENTS.md`). 따라서 아래 세 기준은 물론이고
이후 §24.3/§24.5의 모든 완화 후보 판정에서 **"손실 사례가
있었는가"/"어떤 종목에서 음(-)이 났는가"를 판정 기준으로 쓰지
않는다.** 판정 기준은 항상 (a) 전체 기대수익률에 미치는 영향,
(b) 좋은 기회를 과도하게 막아 생기는 기회비용, (c) 감내 가능한
손실 범위 안에서의 분산/평균이다. 단일 사례(예: 특정 종목 1건의
음(-) 성과)는 **참고 사례일 뿐 완화 반대 근거가 아니다** — n=1
표본은 어느 방향의 결론도 뒷받침하지 못한다는 뜻이며, "이미 나쁜
신호가 있다"는 서술로 쓰지 않는다.

1. 독립적으로 비교 가능한 `(symbol, date)` 쌍이 최소 표본
   (문서 전반에서 반복 사용한 기준 n≈10)에 미달한다 — 이 경우
   판정은 "미확정"이며, 있는 유일한 사례의 손익 방향으로 대신
   결론 내지 않는다.
2. 비교 가능한 관측일 사이에서 방향(어느 쪽이 더 나은 **평균**
   성과를 냈는지)이 반복적으로 뒤집힌다 — 개별 종목 단위가 아니라
   버킷 평균 단위로 판단한다.
3. 통과군/차단군(또는 `matched`/`downgraded`·`suppressed`)의
   **평균** 성과 차이가 무작위 수준(수 %p 이내)과 구분되지 않는다.
이 세 기준 중 하나라도 충족하면 "완화 후보"로 명시할 근거는
있다고 본다 — 다만 **"완화 후보"는 "지금 제거해야 한다"가
아니라 "제거해도 되는지 계속 확인해야 한다"는 뜻**이다. 완화
여부의 최종 판단은 항상 **누적 평균 기대수익률과 기회비용**으로
내리고, 개별 사례의 손익 부호로 자동 기각/승인하지 않는다.

### 24.2 B축(downstream 하향) — 지금까지의 근거 재요약

| 항목 | factual | 설명력이 약하다고 의심하는 이유 |
|---|---|---|
| 날짜별 방향 | 08-03: 하향(`suppressed`+3.68%/`downgraded`+1.73%) > `matched`(+1.19%), T+2도 동일 방향. 08-05: `matched`(+5.16%) >> `downgraded`(-0.31%) | 비교 가능한 2일의 방향이 **정반대**(§24.1 기준 2) |
| 대표 라벨 분포 | 08-04/08-06은 지배적 `suppressed`/`downgraded`가 0~1건뿐, 나머지는 거의 전부 `matched` | 비교 표본 자체가 극히 드묾(기준 1) |
| 오염도 경계 사례 | `000810`(08-03, 46.5%)·`073240`(08-03, 47.2%)·`008930`(08-06, 40.4%) | 40% 임계치 경계에 걸린 사례는 라벨 자체가 불안정(사이클이 조금만 달랐어도 반대 라벨) |
| qualitative 근거 | `risk_opinion`/`evidence_strength`는 판별력 부족이 이미 확인됨(§13.2.14) | 어느 하향이 이 qualitative 필드에만 의존하는지는 아직 구조적으로 재분해되지 않음(`[PRIORITY_MAP]` "다음 2순위" 미착수) |
| guard 체인 | 하향은 7개 guard 체인(§13.5의 R5 매핑) 중 어디서든 발생할 수 있음 | **어느 guard가 실제로 하향을 만들었는지 이번 4거래일 표본에서 재분해한 적이 없다** — 완화 대상을 특정할 근거 자체가 아직 없음 |

### 24.3 B축 완화 후보(작은 완화 → 큰 완화)

| 순위 | 후보명 | 완화 방식 | 기대 효과 | 위험 | 장 종료 후 확인 조건(진전) | 보류/철회 조건 |
|---|---|---|---|---|---|---|
| 1(최소) | 오염도 경계 사례 개별 재검토 | 코드 변경 없음 — `000810`/`073240`/`008930`처럼 오염도 35~45%인 사례만 별도로 추적하고, 라벨 배정 신뢰도를 낮춰 표시(문서·리포팅 수준) | 불안정한 라벨이 평균에 과도한 영향을 주는 것을 방지 | 거의 없음(코드 변경 없음, 표시만 바뀜) | 경계 사례 수가 표본이 쌓여도 계속 40% 근처에 몰리면 임계치 자체를 재검토할 근거가 쌓인다 | 경계 사례가 드물어지고 대부분 0~20%/80~100%로 뚜렷이 갈리면 이 후보는 불필요 |
| 2(작음) | qualitative-only 하향 경로 우선 재분해(완화 전 단계) | 코드 변경 없음 — `override_applied`/`reason_codes`/`expected_value_gate.passed`/`risk_check_passed` 조합으로 각 하향 사례가 구조적 근거(EV/risk/compliance)인지 qualitative 근거(`risk_opinion`/`evidence_strength`)만으로 낮춰진 것인지 재분해한다(`[PRIORITY_MAP]` 기존 "다음 2순위" 과제) | qualitative-only 경로가 실제로 존재하고 비중이 크면, 그 경로만 좁혀서 완화 논의를 시작할 수 있다 | 재분해 자체는 위험 없음(read-only) — 다만 재분해 결과가 나오기 전에는 "qualitative-only 완화"를 실행할 수 없다 | 재분해 결과 qualitative-only 하향이 유의미한 비중(예: 30% 이상)으로 확인되면, 그 경로만의 forward return을 별도로 추적하는 다음 단계로 진행 | 재분해 결과 qualitative-only 경로가 거의 없으면(대부분 EV/risk 등 구조적 근거) 이 후보는 폐기 |
| 3(중간) | 특정 guard(신규 진입 억제 방향)만 완화 후보화 | 아직 코드 변경 없음 — 2번의 재분해가 끝난 뒤, 신규 진입을 막는 방향의 guard(예: `_check_buy_eligibility_upgrade_guard`, `_check_watch_candidate_upgrade_guard`)만 분리해 그 guard가 실제로 만든 하향 사례의 forward return을 별도 추적 | 특정 guard 하나만 좁혀서 완화하면 나머지 churn-control 장치(§7~§10, 보유 후 매도 억제)는 그대로 유지된다 | 아직 "어느 guard가 실제로 발동했는지"조차 이번 4거래일 표본에서 특정되지 않았다 — 이 후보는 2번이 끝나기 전에는 **착수 조건 미충족** | 2번 재분해 후 특정 guard가 반복적으로 낮은 forward return과 연관되면(즉 그 guard의 하향이 실제로 좋은 진입을 막았다는 신호) 완화 설계를 구체화 | 특정 guard와 forward return 사이에 일관된 관계가 없으면 이 후보는 보류 |
| 4(큼) | `suppressed`/`downgraded` 전체 완화(하향 임계 자체를 낮춤) | 예: downstream 하향이 트리거되는 조건 자체를 완화 | **기대수익률/기회비용 관점**: 신규 진입 억제 총량이 줄어 좋은 기회를 더 많이 열 수 있다는 잠재적 이점은 있으나, 지금 근거(비교 가능일 2일, 방향 정반대)로는 **전체 기대수익률이 실제로 개선되는지 판단할 수 없다** — 개별 손실 유무가 아니라 이 판단 불가능성 자체가 착수를 보류하는 이유다 | 비교 가능한 관측일이 최소 10일 이상 쌓이고 **평균** 방향이 안정적으로 한쪽(하향 쪽 평균이 계속 낮음, 즉 하향이 실제로 좋은 기회를 막고 있음)으로 수렴해야 착수 가능 | **현재 증거로는 착수 근거가 없다** — 방향이 정반대인 2일만으로는 전체 기대수익률에 대한 효과를 어느 쪽으로도 판단할 수 없어 전체 완화를 논의하지 않는다 |

**건드리면 안 되는 범위(명시)**: `held_position` 이후의 매도/재매수
억제 계층(§7~§10, holding profile/reverse trade hysteresis)은
이번 완화 후보 논의의 대상이 **아니다** — B축은 신규 진입 단계의
하향만 다루며, churn-control의 본래 목적(보유 후 매도/재매수
억제)을 담당하는 계층은 4거래일 표본으로 사후 성과를 평가할 수
있는 상태조차 아니다(§20, `held_position` 경로 0건). 트레이딩
안전 불변식(risk gate, sell guard 등, `AGENTS.md`)도 이 논의의
대상이 아니다.

### 24.4 C축(EV gate) — 지금까지의 근거 재요약

| 항목 | factual | 설명력이 약하다고 의심하는 이유 |
|---|---|---|
| 평가 표본 자체 | 4거래일 동안 EV 게이트가 실제로 평가한(`buy`/`approve`) `(symbol,date)`는 4개뿐 | 표본이 너무 적어 어떤 방향으로도 결론 낼 근거가 부족(§24.1 기준 1) |
| 독립 비교 쌍 | 진짜 독립적인 비교는 `001450`(통과, T+1 -1.34%) vs `035420`(차단, T+1 -1.31%) 1쌍뿐 | 두 성과가 거의 동일 — 게이트가 실제로 구분한다는 근거가 없음(기준 3) |
| 차단 원인 분해 | `051900`은 하방 추정(`expected_downside_bps`) 자체가 원인, `008930`은 비용+슬리피지 합이 원인(§19.2) | 하위 구성요소별로 원인이 다르다 — "EV 게이트 전체"가 약한 것이 아니라 **특정 구성요소**가 약할 가능성 |
| 상류/하류 기준선 간극 | 상류 candidate 기준(≈0.65)과 하류 실질 통과 기준(변동성 종목 0.8~0.9대) 사이 간극(§18.3) | 상류·하류가 서로 맞춰 설계되지 않았을 가능성 — 다만 이 간극이 "의도된 이중 필터"인지 "미조정"인지는 미확정 |
| ATR 포화 | 관측된 모든 후보(ATR>4%)에서 하방/슬리피지의 ATR 성분이 상한(30bps/15bps)에 포화(§18.2) | ATR 상한이 "변동성이 큰 후보 전체를 동일하게 취급"해 후보 간 차별력을 낮춤 |
| 민감도 | `008930`/`051900`은 세 파라미터(슬리피지 -5, 임계치 반토막, 하방 -10bps)를 동시에 완화해도 여전히 4.54~10.35bps 부족(§19.3) — `001450`의 근소 실패만 항목 하나로 뒤집힘 | **전역 threshold 완화는 이미 효과가 작다는 근거가 있다** — 큰 완화일수록 오히려 근거가 약하다 |

### 24.5 C축 완화 후보(작은 완화 → 큰 완화)

| 순위 | 후보명 | 완화 방식 | 기대 효과 | 위험 | 장 종료 후 확인 조건(진전) | 보류/철회 조건 |
|---|---|---|---|---|---|---|
| 1(최소) | near-miss soft handling | `edge_after_cost_bps`가 `minimum_required_edge_bps`에 근소(예: 5bps 이내) 부족한 사이클만 즉시 `WATCH`/`HOLD` 대신 "관찰 유지 후 재평가"로 전환. 구조적 실패(16.5bps 이상 부족)는 전혀 건드리지 않음 | **기대수익률 관점**: 근소 부족 사이클을 즉시 차단하지 않으면, 실제로는 사후 기대값이 양(+)인 근소 사례들을 진입시킬 기회가 열린다 — 목적은 개별 손실을 피하는 것이 아니라 근소 사례 **모집단의 평균 기대값**이 감내 가능한 손실 범위 안에서 양(+)인지 확인하는 것이다 | **기회비용 관점**: 현재 근소 부족 사이클은 전부 즉시 차단돼, 그 사이클이 실제로 좋은 기회였는지와 무관하게 기회 자체가 사라진다 — 이 기회비용의 크기는 아직 계측되지 않았다. **감내 가능한 손실 범위와의 관계**: 유일하게 관측된 참고 사례(`001450`, T+1 -1.34%/T+2 -1.61%)는 단일 사이클 기준으로 감내 가능한 손실 범위 안에 있는 것으로 보이며, **이 한 건의 손익 방향으로 정책의 기대수익률 효과를 판단하지 않는다** — n=1은 참고 사례일 뿐, 완화의 찬성/반대 근거가 되지 못한다 | 근소 부족 사례가 표본으로 축적되고(n≈10 이상) 그 사례들의 **평균** forward return이 감내 가능한 손실 범위 안에서 양(+) 방향으로 나오면 진전 | 표본이 쌓여도(n≈10 이상) **평균**이 뚜렷한 음(-)이거나, 개별 손실의 크기(분산)가 시스템의 손실 감내 범위를 반복적으로 초과하면 보류 — 단일 사례의 부호만으로는 보류하지 않는다 |
| 2(작음) | ATR 상한 포화 계측 확장(완화 아님, 계측만) | 코드 변경 없음 — 향후 EV 게이트 평가 대상 전체에서 ATR 상한 포화 빈도를 계측 항목으로 추가(문서 추적) | 상한이 실제로 "변동성 큰 후보 전체를 동일 취급"하는 정도를 정량화 | 없음(계측만) | 포화 빈도가 높게 유지되면(예: 대부분의 후보가 ATR>4%) 상한 자체의 재보정 필요성 논의로 진행 | 포화가 드물면(다양한 ATR 구간의 후보가 실제로 나타나면) 이 후보는 불필요 |
| 3(중간) | 특정 하위 구성요소만 완화 후보화 | `estimated_round_trip_cost_bps`/`slippage_buffer_bps`의 거래량 구간 임계(10만/30만)를 실제 체결 slippage 이력과 비교해 재검증 — `minimum_required_edge_bps`나 ATR 상한은 건드리지 않음 | 비용/슬리피지 가정이 원인인 사례(`008930`류)에만 국한된 좁은 완화 | 실제 체결 이력 비교 자체가 아직 이뤄지지 않아 착수 조건 미충족(§18.2 미확정) | 실제 체결 slippage 이력을 확보해 현재 가정(20~22bps)과 비교할 수 있게 되면 착수 | 체결 이력이 현재 가정과 비슷하거나 더 크면 이 후보는 폐기 |
| 4(큼) | `minimum_required_edge_bps` 전역 완화(예: 10→5bps) | 진입 임계값 자체를 낮춤 | 신규 진입 확대 | **이미 §19.3에서 효과가 작다는 근거가 있다** — `008930`/`051900`은 임계치를 반토막내도 여전히 통과 못 함. 가장 큰 변경인데 가장 효과가 작아 우선순위 최하위 | 이 후보는 §19.3의 시뮬레이션 결과를 뒤집을 새로운 관측(임계치 완화만으로 통과하는 사례)이 나와야 재검토 | 현재 근거로는 **착수 우선순위 최하위 유지**, 다른 후보가 먼저 소진된 뒤에만 논의 |
| 5(가장 큼) | hard block → soft signal 전환 | EV 게이트를 `WATCH`/`HOLD` 강제(hard block)에서 경고 신호만 남기고 downstream 재량에 맡기는 방식(soft signal)으로 전환 | 설계 목적(§6.3) 자체의 재정의 | **§20 D축(설계 변경 필요성)과 직결되는 가장 큰 변경** — 표본이 극히 작은 현재 상태에서는 시기상조 | D축 자체가 "설계를 바꿔야 한다"는 결론에 도달하고, B/C축 표본이 충분히(n≈10 이상) 쌓여 방향이 안정화된 뒤에만 논의 | **이번 4거래일 표본으로는 논의 자체를 시작하지 않는다** |

**건드리면 안 되는 범위(명시)**: `risk_off_exception_path`
(리스크오프 예외 경로)와 `risk_check_passed`/`compliance_check_
passed` 등 EV 게이트 외부의 하드 게이트는 이번 완화 후보 논의의
대상이 아니다. EV 게이트의 "계산 공식"(§18.2에서 이미 실측
지표 기반임을 확인)을 "임의로 부풀려졌다"는 전제로 되돌리지
않는다 — 완화 후보는 전부 "공식은 맞는데 계수/임계치가 지금
관측된 후보군에 맞는지"를 묻는 것이지 "공식이 틀렸다"를 묻는
것이 아니다.

### 24.6 장 종료 후(2026-08-07 장 마감 이후) 실행 체크리스트

| 순번 | 확인 항목 | 성공/실패 판정 기준 | 다음 액션 |
|---|---|---|---|
| 1 | `2026-08-07` 종가 반영 확인 | 장 마감(15:30 KST) 이후 `signal_feature_snapshots`(`timeframe='1d'`) 20:10 KST 배치 우선 확인, 익일 `2026-08-08` 05:05 KST `instrument_status_snapshots` 반영 후 두 소스 일치 재검증(§22.11/§22.15와 동일 절차) | 불일치 시 두 소스 혼용 정책(§22.11) 재검토, 일치 시 아래 항목 진행 |
| 2 | `2026-08-06` 결정분 B축 T+1 재계산 | `008930`(오염도 40.4% 경계, `downgraded` 지배)·`051900`(`downgraded`)·`001800`(`suppressed`)의 T+1을 §22.4/§22.12와 동일 방법(결정일 종가=진입가)으로 계산 | 방향이 08-03/08-05 중 어느 쪽에 가까운지, 아니면 제3의 방향인지 §23.2 표에 추가 |
| 3 | `2026-08-06` 결정분 C축 T+1 재계산 | `008930`(23사이클 차단)·`051900`(7사이클 차단)의 T+1을 §19.1과 동일 방법으로 계산 | §24.5 각 후보의 "확인 조건" 열에 대입해 진전/보류 판정 |
| 4 | 최근 누적 비교 갱신 | B축: 비교 가능일이 3일(08-03/08-05/08-06)로 늘어나는지, 방향이 어느 쪽으로 수렴하는지. C축: 독립 비교 쌍이 2개로 늘어나는지 | §23의 4거래일 표를 5거래일로 갱신, §24.1 기준(n, 방향 반복, 차이 크기) 재평가 |
| 5 | 완화 후보 유지/강화/철회 판정 | §24.3/§24.5 각 후보의 "진전 조건"/"보류·철회 조건"에 실측값을 대입 — **개별 종목 1건의 손익 부호가 아니라 버킷 평균/기회비용 기준으로 판정**(§24.1) | 조건을 충족한 후보만 다음 턴에 구체적 설계(코드 변경 없이 shadow 계측 설계까지)로 진행 |

### 24.7 factual / 해석 / 미확정

- **factual**: (1) 08:54 KST 기준 1차/파생 소스 모두 `2026-08-06`
  이 최신이며 `2026-08-07` 종가는 존재하지 않는다(장 미마감). (2)
  B축은 비교 가능일 2일의 **평균** 방향이 정반대, C축은 독립
  비교 쌍 1개의 **평균** 성과가 거의 동일 — 둘 다 §24.1의 완화
  후보 기준을 충족한다. (3) C축에서 유일하게 관측된 근소 실패
  참고 사례(`001450`)는 T+1 -1.34%/T+2 -1.61%였다 — n=1이며
  단일 사이클 기준 감내 가능한 손실 범위 안으로 보인다.
- **해석(2026-08-07 KST 재정정)**: 두 축 모두 "완화 후보"이지만
  아직 "완화해야 한다"는 결론은 아니다. B축은 재분해(어느
  guard가 하향을 만드는지)가 완화 논의의 선행 조건이다. **C축의
  `001450` 사례는 완화에 찬성도 반대도 뜻하지 않는 참고 사례**
  다 — n=1로는 근소 부족 사례 모집단의 평균 기대값을 추정할
  수 없고, 판단 기준은 개별 손익 부호가 아니라 **표본이 쌓인
  뒤의 평균 기대수익률과 기회비용**이어야 한다. 두 축 모두
  표본이 극히 작아 어느 방향으로도 결론 낼 수 없는 상태이며,
  이 "판단 불가"를 "완화 반대 근거"로 오독하지 않는다.
- **미확정**: (1) B축의 guard별 재분해 결과. (2) `008930`/
  `051900`/`001800`/`035420`의 08-07 이후 실제 성과. (3)
  근소 부족 사례가 표본으로 쌓였을 때의 평균 기대값과 기회비용
  — 이것이 실제 완화 여부를 가를 핵심 미확정 항목이다. (4) 위
  결과가 나온 뒤에도 각 완화 후보의 우선순위가 이번 절의 순서
  (작은 완화 우선)를 유지할지, 재조정이 필요할지.

## 25. `2026-08-07` 장 종료 후 판정 갱신 — population 확장, forward return은 여전히 불가(read-only)

**이번 절의 성격**: `2026-08-07`(금) 장이 마감(15:30 KST)된 뒤인
17:00 KST에 재확인했다. 대표 라벨 방식(§22의 지배적 상태+오염도)
은 그대로 유지한다. 한국 시장 벤치마크 비교(§22.7~§22.9)는
되살리지 않는다.

### 25.1 종가 확보 — 1차/대체/외부 소스 판단

- **1차 소스**(`instrument_status_snapshots`): `2026-08-07` 종가
  0행 — 다음날(`2026-08-08`) 05:05 KST 배치 이전이라 아직 없다.
- **대체 소스**(`signal_feature_snapshots`, `timeframe='1d'`):
  `2026-08-07` 종가 0행 — 당일 20:10 KST 배치가 아직(17:00
  KST 기준) 실행되지 않았다.
- **그 외 내부 소스 전수 확인**(이번 턴 신규): `market_data_
  snapshots`(0행, 이전 턴에서도 확인된 사실 재확인), `feature_
  snapshots`(0행, 신규 확인) — 이 둘도 08-07 가격을 담고 있지
  않다.
- **외부 시세 API 사용 여부(판단)**: 사용하지 않았다. 이유:
  (1) 20:10 KST 대체 소스 배치까지 남은 시간이 3시간 이내로
  짧고, 그 소스는 이미 이번 조사 전체에서 반복적으로 1차 소스와
  소수점까지 일치함이 검증된 경로다(§13.4.4, §22.11). (2) 외부
  시세 API를 새로 도입하면 종목코드 매핑·형식·정확도를 이번
  조사의 기존 방법론과 같은 수준으로 교차검증할 방법이 없어,
  지금까지 유지해 온 "재현 가능한 소스만 사용" 원칙이 깨진다.
  (3) 사용자가 외부 API 사용을 승인했더라도, 짧은 대기로
  해결되는 상황에서 검증되지 않은 소스를 끌어오는 것은 위험 대비
  이득이 작다고 판단했다. **이 판단은 재검토 가능** — 사용자가
  지금 바로 외부 소스를 강제하길 원하면 후속 턴에서 바로 시도할
  수 있다.
- **결론**: `2026-08-06`/`2026-08-07` 결정분의 forward return은
  이번 턴에도 계산 불가하다(§22.15/§23와 동일한 horizon 미도착
  사유, 하루 더 진행됐을 뿐 구조는 바뀌지 않았다).

### 25.2 B축 — `2026-08-07` population 확장(대표 라벨+오염도)

`(symbol, date, source_type)` 단위로 전수 재확인했다(§22.2/§22.4
와 동일 방법).

| 지배 라벨 | 종목 수 | 오염도 | 비고 |
|---|---|---|---|
| `downgraded` | 6 | 0~19.6% | `008930`(0%)·`180640`(0%)·`111770`(1.8%)·`073240`(3.5%)·`051900`(5.6%)·`196170`(19.6%) — 전부 뚜렷한 다수결 |
| `matched`(경계) | 1 | **49.1%**(`078930`) | 28 vs 29 사이클, 거의 완전한 동전 던지기 — 이번 조사 전체에서 관측된 오염도 중 **최고치**(종전 최고 `008930` 08-06 40.4%) |
| `matched`(나머지) | 13 | 0~7.0% | `001800`/`068270`(0%)·`009240`/`086790`/`090430`/`138040`/`316140`(3.5~7.0%대) |
| `suppressed` | 0 | — | 이번에도 지배적 `suppressed`는 0건(08-04/08-05와 동일 패턴) |

**신규 관측(factual)**: `downgraded` 지배 종목의 `entry_score`
가 이전 3거래일(08-03~06, 대체로 0.60~0.69)보다 **뚜렷이
높다**(`180640`=0.95, `008930`=0.7855, `051900`=0.7971,
`073240`=0.723, `196170`=0.7011, `111770`=0.676) — 즉 오늘은
상대적으로 **더 강한 신호를 가진 후보들이 하향**됐다. 이 자체는
"하향이 틀렸다"는 뜻이 아니다 — entry_score가 높다는 것과
사후 성과가 좋다는 것은 별개 질문이며, 이번 절은 그 질문에
아직 답하지 않는다(§25.1, forward return 불가).

`078930`(오염도 49.1%)은 대표 라벨 방식의 한계를 가장 극명하게
보여주는 사례다 — 사이클 다수결이 1표 차이로 갈렸을 뿐이라,
이 종목을 `matched`로 분류하든 `downgraded`로 분류하든 그
자체로는 어느 쪽에도 강한 근거가 되지 못한다. §24.3(B축 완화
후보 1번, 오염도 경계 사례 개별 재검토)의 대상에 추가한다.

경계 구간(`entry_score>=0.60`, `>=0.65`)은 오늘 population
전체가 이미 대부분 0.58 이상(최저 `055550`=0.58)이라 분리해도
구성이 거의 바뀌지 않는다 — 별도 표를 만들지 않고 이 사실만
기록한다.

### 25.3 C축 — `2026-08-07` population 확장(신규 근소통과·근소실패 사례, 첫 실제 체결)

**신규 통과 사례(2026-08-03 `001450` 이후 두 번째)**: `068270`
(2사이클, `edge_after_cost_bps=10.07`) — 최소 요구치(10.00)를
**0.07bps 차이로 통과**한 극도로 근소한 통과다. `expected_
return_bps=78.07`/`expected_downside_bps=42.00`/`net_expected_
value_bps=36.07`/`estimated_round_trip_cost_bps=8.00`/`slippage_
buffer_bps=18.00`.

**이 사례는 가상 진입가가 아니라 실제 체결이다(2026-08-03+
구간 최초)**: `order_requests`가 `status=filled`, 08-07 08:52:11
KST에 25주 @ 193,200원으로 실제 체결됐다(`broker_fill_
snapshots` 원장 확인). `symbol_trade_states.state`도 `held_
active`로 전환됐다 — §20에서 "`held_active` 0건이라 보유 측
churn-control 계층이 발동 기회조차 없었다"고 기록한 상태가
**이번 절에서 처음으로 바뀌었다.** 다만 이 사실이 D축(설계
변경 필요성) 판단을 바꾸지는 않는다 — 발동 "기회"가 생긴 것과
그 계층이 실제로 기대값을 개선하는지는 별개이며, 후자는 이
포지션이 청산될 때까지 판단할 수 없다.

**신규 근소 실패 사례**: `000240`(53사이클, 전부 실패, 요구치
대비 **1.33~3.33bps 부족** — `001450`의 2.36bps보다도 더 근소한
사례). `expected_return_bps=80.17`(관측된 값 중 최고), `expected_
downside_bps=40~42`, `cost=9.50`, `slippage=22.00`.

**구조적 실패 사례(변동 없는 패턴)**: `051900`/`073240`/`078930`/
`111770`/`196170` — 요구치 대비 10.31~25.69bps 부족, §19.2/§19.3
에서 이미 확인한 "동시 완화해도 못 넘는" 구조와 동일 범주다.

**§24.5(C축 완화 후보 1번, near-miss soft handling)에 대한
갱신**: 근소 실패 참고 사례가 `001450`(deficit 2.36bps, 1개
symbol×date) 1개에서 **`001450`+`000240`(deficit 1.33~3.33bps)
2개로 늘었다.** 아직 n=2로는 "모집단의 평균 기대값"을 추정할
표본이 되지 못한다는 §24.7의 판정은 그대로 유지한다 — 다만
표본이 늘고 있다는 사실 자체는 기록한다. `068270`의 razor-thin
통과와 실제 체결은 이 후보의 **반대쪽 준거점**(통과 후 실제로
어떻게 되는지를 실측할 수 있는 첫 사례)을 제공한다 — 이 포지션의
사후 성과는 이번 턴에서도 계산 불가하다(§25.1).

### 25.4 완화 후보 판정 갱신(구조적 정보만, 방향성 결론 없음)

| 축 | 갱신 전(§24 시점) | 갱신 후(이번 절) | 완화 후보 강도 변화 |
|---|---|---|---|
| B축 전체 | 완화 후보(방향 불안정, n=2일) | population이 08-07까지 3일로 늘었으나 **여전히 forward return 비교 불가**(08-07 결정분은 미래 horizon) — 방향 불안정 판정은 유지, 강도 변화 없음 | 변화 없음(미확정 유지) |
| B축 오염도 경계 재검토 | `000810`/`073240`/`008930` 3건 | `078930`(49.1%) 추가로 **4건** — 이번 조사 최고 오염도 | 강도 소폭 상승(구조적 근거만, 성과 근거 아님) |
| C축 전체 | 완화 후보(표본 극소) | 독립 표본이 4개(symbol×date)에서 **7개**로 늘었고, 통과 사례가 1개에서 2개로 늘었다 | 표본 확대는 판단력을 늘리는 방향이지만, forward return이 없어 **강도 자체는 아직 올리지 않는다** — 미확정 유지 |
| C축 near-miss(1번) | 참고 사례 n=1(`001450`) | 참고 사례 n=2(`001450`/`000240`) | n=2는 "모집단 평균" 추정에 여전히 부족 — **완화 후보 강도는 올리지 않되, 다음 착수 턴에 우선 순위를 유지한다** |

**철학 재확인(2026-08-07)**: 이번 절의 신규 사실(더 높은
`entry_score`의 후보가 하향됨, 더 근소한 실패 사례 추가, 실제
체결 1건 발생)은 모두 개별 종목의 손익 부호와 무관한 **구조적
population 변화**다. 아직 forward return이 없는 상태에서 이
population 변화만으로 완화 후보 강도를 올리거나 내리지 않는다
— "표본이 늘었다"는 "판단할 수 있게 됐다"는 뜻이 아니라 "판단할
날이 가까워졌다"는 뜻이다.

### 25.5 factual / 해석 / 미확정

- **factual**: (1) `2026-08-07` 종가는 1차/대체/기타 내부 소스
  전부에 없다(장 마감 후 정상적인 배치 지연, 데이터 공백 아님).
  (2) 외부 시세 API는 이번 턴에서 사용하지 않았다(§25.1 근거).
  (3) B축: `2026-08-07`에 지배적 `downgraded` 6건(entry_score
  0.68~0.95대로 이전보다 높음), 오염도 49.1%의 극단적 경계
  사례(`078930`) 1건. (4) C축: 신규 통과 사례 1건(`068270`,
  edge=10.07, 실제 체결·`held_active` 전환), 신규 근소 실패
  1건(`000240`, deficit 1.33~3.33bps).
- **해석**: 오늘 population은 이전보다 "판단에 유리한 재료"를
  더 많이 제공한다(더 다양한 entry_score, 더 많은 near-miss,
  최초의 실제 체결) — 하지만 이 재료들은 아직 성과 데이터가
  아니라 **재료일 뿐**이다. `068270`의 실제 체결과 `held_active`
  전환은 §20 D축이 지적한 "보유 측 churn-control이 발동 기회가
  없다"는 상태를 처음으로 깨뜨렸지만, 이것이 D축의 "설계를
  바꿔야 하는가" 판단 자체를 바꾸지는 않는다 — 그 계층이 실제로
  기대값을 개선하는지는 이 포지션의 청산 이후에야 확인된다.
- **미확정**: (1) `068270` 포지션의 실제 청산 시점과 손익. (2)
  `2026-08-06`/`2026-08-07` 결정분 전체의 forward return(가격
  데이터 도착 대기). (3) `078930`의 오염도 49.1% 경계 사례가
  반복되는 패턴인지 일회성인지. (4) 근소 실패 참고 사례(n=2)의
  모집단 평균 추정 가능 시점.

### 25.6 후속 검증 연결

1. `signal_feature_snapshots`의 08-07 20:10 KST 배치(오늘 저녁)
   또는 `instrument_status_snapshots`의 08-08 05:05 KST 배치
   중 먼저 도착하는 소스로 `2026-08-06` 결정분(B축 `008930`/
   `051900`/`001800`, C축 `008930`/`051900`) T+1을 계산한다 —
   §24.6 체크리스트 그대로 유지.
2. `068270`의 실제 체결가(193,200원) 대비 향후 종가로 최초의
   "가상 진입가가 아닌 실제 체결 기준" forward return을 계산한다
   — 이는 이번 조사 전체에서 방법론적으로 더 강한 증거가 된다.
3. `symbol_trade_states.state='held_active'`가 유지되는 동안
   `reverse_trade_hysteresis`/`holding_profile_policy`가 실제로
   개입하는지 관찰한다 — §20 D축이 "평가할 입력이 없다"고 남긴
   전제가 이제 부분적으로 해소됐다.
4. 이 결과를 §20 D축(설계 변경 필요성) 판단에 계속 연결한다 —
   여전히 미확정이며, 이번 절의 population 확장이 D축 판정을
   앞당기지 않는다.

## 26. C축(EV gate) 차단 표본 — `2026-08-07` 종가 기준 손익 시뮬레이션(read-only)

**목적**: EV 게이트가 "얼마나 많이 막았는가"가 아니라 **막힌
표본이 실제로 얼마나 오르거나 내렸는지, 놓친 기회비용이 커
보이는지**를 사후 성과로 추정한다. 정책/코드 변경은 하지
않는다. 시장 벤치마크 비교는 되살리지 않는다(절대수익률만).

### 26.1 가격 소스 확인

- **1차 소스**(`instrument_status_snapshots`): 최신 종가일이
  여전히 `2026-08-06`이다 — `2026-08-07`은 아직 반영되지
  않았다(다음날 05:05 KST 배치 전).
- **대체 소스**(`signal_feature_snapshots`, `timeframe='1d'`):
  `2026-08-07 20:00 KST` 배치가 실행돼 `2026-08-07` 종가
  208건이 반영됐다 — **이번 절의 `2026-08-07` 종가는 전부 이
  대체 소스에서 가져왔다**(1차 소스 미반영이므로 우선순위 원칙
  2번 적용). 두 소스가 겹치는 과거 구간(07-31~08-06)에서 이미
  반복적으로 소수점까지 일치함이 검증됐다(§13.4.4, §22.11,
  §22.15) — 다만 `2026-08-07` 자체는 1차 소스가 아직 없어
  **이번 신규 값 자체의 교차검증은 08-08 05:05 KST 배치 이후에만
  가능**하다(미확인 리스크로 명시).
- 외부 시세 API는 사용하지 않았다 — 내부 대체 소스가 이미
  존재해 사용할 필요가 없었다.

### 26.2 대상 표본과 가상 진입가 기준(canonical 고정)

- **모집단**: `2026-08-03` 이후 `decision_type IN ('buy',
  'approve')`이면서 `expected_value_gate`가 실제로 평가된
  `(symbol, decision_date)` 전수(§19.1/§23.1/§25.3 표본을
  `2026-08-07`까지 확장, 아래 표에 전수 나열).
- **가상 진입가(canonical, 단일 기준)**: **결정일 종가**를
  쓴다(§19.1에서 처음 적용한 규약과 동일). 이유(한 문장): 장중
  tick 가격 소스가 시스템에 없어 결정 시각에 정확히 맞춘 가격을
  만들 수 없고, 종가는 두 소스에서 이미 검증된 방식으로 조회
  가능한 유일한 재현 가능 기준이기 때문이다. 다음 거래일 시가
  대체안은 이번 턴에서 별도 계산하지 않았다(현재 소스 어디에도
  시가 필드가 없어 부록 민감도 체크 자체가 불가능하다 — 이
  한계를 미확인 사항으로 남긴다).
- **중복 제거 기준**: 같은 `(symbol, decision_date, source_type)`
  안에 여러 사이클이 있으면 **그 날의 가상 진입가는 동일**(같은
  결정일 종가)하므로 사이클을 압축하지 않고 대표 1행으로 취급한다
  — forward return은 사이클 수와 무관하게 하나만 계산된다(§21.5.3
  에서 이미 확인한 population 중복 문제와 동일한 이유).
- **horizon**: 계산 가능한 것만 계산한다. `2026-08-07`이 결정일
  자체인 표본(당일 결정)은 진입가=`2026-08-07` 종가라 forward
  return이 **아직 없다**(다음 거래일 `2026-08-08` 필요) — 억지로
  만들지 않는다.

### 26.3 표본 전수와 `2026-08-07` 종가 기준 결과

| 종목 | 결정일 | EV 판정 | edge_after_cost_bps(대표) | 진입가 | `08-07` 종가 | horizon | 수익률 |
|---|---|---|---|---|---|---|---|
| `001450` | 08-03 | **통과**(5/6), 근소실패 1/6 | 15.64~17.64(통과) / 7.64(근소실패, deficit 2.36) | 37,300 | 39,300 | T+4 | **+5.36%** |
| `035420` | 08-05 | 차단(구조적) | -12.97(deficit 22.97) | 229,000 | 210,000 | T+2 | **-8.30%** |
| `008930` | 08-06 | 차단(구조적) | -6.54/-14.54(deficit 16.54/24.54) | 43,800 | 44,650 | T+1 | **+1.94%** |
| `051900` | 08-06 | 차단(구조적) | -20.35(deficit 30.35) | 324,000 | 333,500 | T+1 | **+2.93%** |
| `068270` | 08-07 | **통과**(2/2, 실제 체결) | 10.07(deficit -0.07, 즉 0.07bps 여유로 통과) | 실제 체결가 193,200 | 199,300 | 당일(체결→종가) | **+3.16%**(참고, T+1 아님) |
| `000240` | 08-07 | 차단(근소실패) | 6.67~8.67(deficit 1.33~3.33) | 25,850(당일 종가) | — | T+1 필요(`08-08`) | 계산 불가 |
| `051900` | 08-07 | 차단(구조적) | -4.29(deficit 14.29) | 333,500 | — | T+1 필요 | 계산 불가 |
| `073240` | 08-07 | 차단(구조적) | -5.70/-9.70(deficit 15.70/19.70) | 7,810 | — | T+1 필요 | 계산 불가 |
| `078930` | 08-07 | 차단(구조적) | -3.69~-15.69(deficit 13.69~25.69) | 98,900 | — | T+1 필요 | 계산 불가 |
| `111770` | 08-07 | 차단(구조적) | -14.40(deficit 24.40) | 93,400 | — | T+1 필요 | 계산 불가 |
| `196170` | 08-07 | 차단(구조적) | -7.89/-11.89(deficit 17.89~21.89) | 305,000 | — | T+1 필요 | 계산 불가 |

**forward return이 실제로 계산된 표본은 4건뿐**(`001450`/
`035420`/`008930`/`051900`-08-06) — `2026-08-07` 결정 표본
6건은 전부 진입가=`08-07` 종가 자체라 forward horizon이 아직
없다. 이는 데이터 공백이 아니라 **결정일이 관측 시점과 같은
날**이기 때문이다.

### 26.4 집계 지표

**차단군(구조적 실패, forward return 계산 가능한 것만, n=3)**:
`035420`(-8.30%, T+2) / `008930`(+1.94%, T+1) / `051900`
(+2.93%, T+1).
...
**horizon 혼합 caveat(중요)**: 위 3건은 T+1 2건 + T+2 1건이
섞여 있다 — horizon이 다른 값을 평균 내는 것은 방법론적으로
약하다. **T+1만 분리**하면 `008930`(+1.94%)·`051900`(+2.93%)
2건 모두 양(+), 평균 **+2.44%**, 양수율 **100%**다. T+2는
`035420` 1건(-8.30%)뿐이라 평균을 낼 수 없다.

**근소실패군(deficit≤5bps, forward return 계산 가능한 것만,
n=1)**: `001450`의 근소실패 사이클(deficit 2.36bps)은 그 날의
통과 사이클과 **같은 종목·같은 날**이라 forward return이 동일
(+5.36%, T+4)하다 — 즉 "근소실패군 고유의" 데이터가 아니라
그 날의 전체 성과를 공유한다(population 중복, §21.5.3와 동일
문제). `000240`(deficit 1.33~3.33bps, 이번 조사 최소 deficit)은
forward return이 **아직 없다**(당일 결정). **근소실패군을
전체 차단군과 통계적으로 비교할 수 있는 독립 표본은 이번
턴에도 없다.**

**통과군(참고, n=1 forward 계산 가능 + n=1 당일 참고)**:
`001450`(T+4, +5.36%) / `068270`(당일 체결→종가, +3.16%,
방법론이 달라 직접 비교 불가).

### 26.5 factual / 해석 / 미확정

- **factual**: (1) `2026-08-07` 종가는 대체 소스에만 있고 1차
  소스는 아직 없다 — 이번 신규 값의 1차 소스 교차검증은
  `08-08` 05:05 KST 이후에만 가능하다. (2) forward return이
  실제로 계산된 차단(구조적 실패) 표본은 3건이며 그중 T+1
  2건은 모두 양(+)(+1.94%/+2.93%), T+2 1건은 음(-)(-8.30%)이다.
  (3) 근소실패군은 독립적으로 계산 가능한 표본이 여전히 0건
  이다(`001450`은 population 중복, `000240`은 horizon 미도착).
  (4) `001450`의 T+4 수익률(+5.36%)은 이전에 기록된 T+1(-1.34%)/
  T+2(-1.61%)와 **방향이 바뀌었다** — 08-03→08-05까지 하락한
  뒤 08-06~07에 회복해 양(+)으로 전환됐다.
- **해석**: (1) 계산 가능한 3건의 구조적 차단 표본 중 T+1
  2건이 모두 양(+)이었다는 것은, 이 2건에 한해 **"차단하지
  않았다면 기대값이 더 좋았을 가능성"**을 시사한다 — 다만 n=2
  라는 극히 작은 표본과 아직 이 2건도 시장 국면(상승장) 영향을
  배제할 수 없다는 점에서 이 방향을 일반화하지 않는다. (2)
  `035420`(T+2, -8.30%)은 반대 방향 — "차단이 실제로 손실을
  피하게 했을 가능성"도 동시에 존재한다. 세 건이 두 방향으로
  나뉘어 있어, **"EV 게이트 차단군이 대체로 수익이 났다/손실이
  났다"는 단정을 이번 데이터로는 내리지 않는다.** (3) `001450`
  의 T+4 반전(음→양)은, 짧은 horizon(T+1/T+2)만 봤을 때
  "차단이 옳았다"로 보였을 사례가 더 긴 horizon에서는 반대로
  보일 수 있다는 것을 보여준다 — 이는 근소실패 완화 후보(§24.5
  1번)의 판단에 **horizon 자체의 불안정성**이라는 새로운
  고려사항을 추가한다. (4) `068270`의 실제 체결(당일 +3.16%)은
  통과군이 "일단 나쁘지 않게 출발"했다는 참고 정보이나, 방법론이
  달라(실제 체결가·당일 종가 vs 가상 진입가·익일 종가) 다른
  표본과 직접 비교하지 않는다.
- **미확정**: (1) 구조적 차단 표본 6건(`2026-08-07` 결정, `000240`
  근소실패 포함)의 forward return — `2026-08-08` 종가 필요. (2)
  `001450`의 T+4 반전이 우연인지, 이 시장 국면(전반적 상승) 때문
  인지, 아니면 EV 게이트의 하방 추정이 짧은 horizon에서는 맞고
  긴 horizon에서는 과도한 패턴인지. (3) `068270` 포지션의 실제
  청산 손익. (4) 이번 신규 `2026-08-07` 값의 1차 소스 교차검증.

### 26.6 핵심 질문에 대한 답변

1. **`2026-08-03` 이후 EV 게이트 차단군은 `2026-08-07` 종가
   기준으로 대체로 수익/손실이 났는가**: 계산 가능한 3건 중
   T+1 2건은 양(+)(+1.94%/+2.93%), T+2 1건은 음(-)(-8.30%)이다
   — **한 방향으로 답할 수 없다.** 표본이 3건뿐이라 "대체로"라는
   말 자체를 쓸 수 없는 단계다.
2. **근소실패군은 전체 차단군보다 더 좋은 성과를 보였는가**:
   **비교할 독립 표본이 없어 답할 수 없다.** 유일한 참고
   (`001450`, +5.36%)는 근소실패 고유의 데이터가 아니라 그 날의
   통과 사이클과 공유하는 값이다.
3. **EV 게이트가 막은 종목 중 기대수익률 관점에서 놓친 기회가
   커 보이는가**: T+1 2건(+1.94%/+2.93%)만 보면 "놓친 기회가
   있어 보인다"는 방향의 근거가 **소폭 늘었다** — 다만 T+2
   1건(-8.30%)과 `001450`의 horizon별 반전(음→양)이 동시에
   존재해, "기회비용이 크다"는 결론을 내릴 만큼 근거가 쌓이지
   않았다.
4. **유지/완화/제거 후보 중 어디에 더 가깝게 볼 수 있는가**:
   여전히 **"완화 후보(미확정)"**다 — §24의 판정에서 강도를
   올리거나 내리지 않는다. 이번 절의 새로운 사실(T+1 2건 양(+),
   `001450` T+4 반전)은 "완화를 지지하는 방향의 재료가 조금
   늘었다"고는 말할 수 있으나, n=3~4 수준의 표본으로 정책 판단을
   내리기엔 이전과 마찬가지로 부족하다.

### 26.7 후속 검증 연결

1. `2026-08-08` 05:05 KST 이후 1차 소스가 `2026-08-07` 종가를
   반영하면, 이번 절의 대체 소스 값과 교차검증하고, `2026-08-07`
   결정 표본 6건(근소실패 `000240` 포함)의 T+1을 계산한다 —
   근소실패군의 첫 독립 표본이 될 잠재적 기회다.
2. `035420`의 T+2(-8.30%)와 `008930`/`051900`의 T+1(양(+))이
   같은 방향으로 수렴하는지, 표본이 늘어난 뒤 재확인한다.
3. `001450`의 horizon별 반전(T+1/T+2 음(-) → T+4 양(+))이
...
## 28. B축(downstream 하향) 원인 구조 분해와 사후 성과 검증(2026-08-08 KST, read-only)

**이번 절의 범위 고정**: B축 = `candidate_vs_final.alignment_
status` 기준으로, deterministic이 `BUY_CANDIDATE`(매수 의도)를
냈는데 후단(FDC)이 `WATCH`/`HOLD`/`NO_ACTION`으로 낮춘 경우다.
C축(EV 게이트)은 이번 절의 대상이 아니다. 시장 벤치마크 비교는
하지 않는다(절대수익률만). 관측 population은 `2026-08-03`
이후, horizon은 `2026-08-07` 종가(파생 소스, §22.11에서 이미
검증된 방식)까지 계산 가능한 것만 사용한다.

### 28.1 A축 작업 — 원인 구조 분해

**A-1/A-3. 핵심 발견: `alignment_status`는 코드상 순수 분류
함수의 출력이다** — `decision_factory.py::_classify_alignment_
status()`가 `candidate_intent`(deterministic의 `primary_
candidate`에서 파생)와 `final_intent`(FDC의 `decision_type`에서
파생)만으로 라벨을 결정한다. `risk_opinion`/`evidence_strength`
같은 질적 필드는 이 분류 함수의 **입력이 아니다** — 라벨 자체를
결정하지 않는다. 이를 `2026-08-03` 이후 전체 데이터로 재확인한
결과, **예외 없이(0건 반례)**:
- `downgraded` 전체 → `primary_candidate=BUY_CANDIDATE`(매수
  의도)에서만 발생.
- `suppressed` 전체 → `primary_candidate=WATCH`(관찰 후보)에서만
  발생, 매수 의도와 무관.

**이것은 이번 턴이 고정한 B축 정의와 정확히 일치한다 —
`suppressed`는 애초에 매수 의도가 없었던 종목이 완전히 꺼진
것이라, 엄밀한 B축(매수 의도 하향)의 대상이 아니다.** B-1에서
요청한 대로 비교용으로는 계속 포함하되, 이 구분을 항상 명시한다.

**A-2/A-3. 구조적 필드의 실제 분류력(2026-08-03+ 전체, `primary_
candidate=BUY_CANDIDATE` 사이클 단위로 한정 — `matched`/
`downgraded` 사이클만 비교)**:

| 필드 | `matched`(사이클) | `downgraded`(사이클) | 분류력 |
|---|---|---|---|
| `evidence_strength` | `moderate` 127 + `strong` 10(100%, `weak`/`none` 0건) | `moderate` 348 + `none` 37 + `strong` 53 + `weak` 186 | **강함** — `weak`/`none`은 `matched`에서 단 한 번도 없었다(0% 통과), `moderate`/`strong`도 통과율 약 25.5%뿐(137/538) |
| `risk_opinion` | `allow` 72 + `review` 65(각 52.6%/47.4%) | `allow` 113 + `review` 509(각 18.2%/82.1%) | **중간** — `review`가 `downgraded`에서 압도적으로 많지만 `matched`에도 47%나 있어 단독 판별 지표는 아니다 |
| `event_conflict` | `true` 2.2%(3/137) | `true` 27.3%(182/664) | **중간** — `matched`에서는 거의 없지만 `downgraded`의 다수(72.7%)는 여전히 `false` |
| `source_type`(`core`/`event_overlay`) | `core` 46.8%/`event_overlay` 52.5% | `core` 59.6%/`event_overlay` 40.4% | **약함** — 방향은 있으나(`event_overlay`가 상대적으로 더 잘 통과) 차이가 크지 않다 |
| `detected_event_count=0` | 0건(항상 이벤트 있음) | 37건(소수) | **약함** — 표본이 작고 `downgraded`도 대부분 이벤트가 있다 |

**해석(질적 vs 구조적 재확인)**: `evidence_strength`가 §13.2.14
에서 "판별력 부족"으로 재평가된 바 있으나, 이번에 **동일
population(`BUY_CANDIDATE`)으로 정확히 다시 비교한 결과 실제로는
꽤 강한 분류력**을 보인다 — 다만 이는 "FDC가 실제로 이 필드를
근거로 판단을 내린다"는 것과 "이 필드가 사후 성과(기대값 개선)
를 예측한다"는 것을 구분해야 한다. 전자(분류력)는 이번 절에서
확인됐고, 후자(사후 성과 예측력)는 §28.2에서 별도로 다룬다 —
**"그럴듯해 보인다"와 "실제로 성과를 예측한다"를 섞지 않는다.**

**A-4. 대표 라벨(지배적 상태+오염도) — `primary_candidate=
BUY_CANDIDATE` 사이클만으로 재계산**: 기존 §22의 대표 라벨은
`matched`/`downgraded`/`suppressed`를 구분 없이 전체 사이클로
계산해, `WATCH`/`NO_ACTION` 기원의 `matched`(예: 관찰만 하던
종목이 계속 관찰 상태로 남은 것)가 "매수 의도 유지"로 잘못
집계됐다 — 이를 `primary_candidate=BUY_CANDIDATE` 사이클만으로
다시 계산했다(§21.5.3/§22.4와 동일한 지배적 상태+오염도 방식,
모집단만 좁힘).

| 결정일 | 종목 | `BUY_CANDIDATE` 사이클 | `buy`(매수 의도 유지) | `watch`/`no_action`(하향) | 지배 라벨 | 오염도 |
|---|---|---|---|---|---|---|
| 08-03 | `001450` | 69 | 6 | 63 | **downgraded** | 8.7% |
| 08-03 | `073240` | 38 | 0 | 38 | **downgraded** | 0% |
| 08-05 | `035420` | 54 | 1 | 53 | **downgraded** | 1.9% |
| 08-05 | `181710` | 54 | 0 | 54 | **downgraded** | 0% |
| 08-06 | `008930` | 57 | 23 | 34 | **downgraded**(40.4%, 경계) | 40.4% |
| 08-06 | `051900` | 57 | 7 | 50 | **downgraded** | 12.3% |
| 08-07 | `000240` | 57 | 53 | 4 | **matched** | 7.0% |
| 08-07 | `008930` | 51 | 0 | 51 | **downgraded** | 0% |
| 08-07 | `051900` | 55 | 3 | 52 | **downgraded** | 5.5% |
| 08-07 | `068270` | 2 | 2 | 0 | **matched**(실제 체결) | 0% |
| 08-07 | `073240` | 57 | 2 | 55 | **downgraded** | 3.5% |
| 08-07 | `078930` | 57 | 29 | 28 | **matched**(경계, 거의 동전 던지기) | 49.1% |
| 08-07 | `111770` | 57 | 1 | 56 | **downgraded** | 1.8% |
| 08-07 | `180640` | 52 | 0 | 52 | **downgraded** | 0% |
| 08-07 | `196170` | 53 | 10 | 43 | **downgraded** | 18.9% |

**가장 중요한 factual 발견**: `2026-08-03`부터 `2026-08-06`까지
**4거래일 연속, 매수 의도(`BUY_CANDIDATE`)가 지배적으로 유지된
`(symbol, date)`가 0건**이다 — 이 4일 동안 존재한 `BUY_CANDIDATE`
사이클 6개 단위 전부가 `downgraded`로 귀결됐다. `2026-08-07`
에야 처음으로 3건(`000240`/`068270`/`078930`)이 `matched`로
남았고, 그중 `068270`이 이번 조사 전체에서 유일한 실제 체결
사례다. 08-03~08-07 전체 15개 단위로 보면 매수 의도 유지
3건(20%, 그중 `078930`은 49.1% 오염도의 경계 사례) vs 하향
12건(80%).

### 28.2 B축 작업 — 사후 성과 검증

**B-1/B-2**: 비교 집단은 `matched`(매수 의도 유지)/`downgraded`
(하향)/`suppressed`(참고, watch 기원)로 §28.1의 대표 라벨을
쓴다. 진입가는 canonical 기준(결정일 종가, 1차 소스 우선·
`181710`처럼 1차 소스 결측 시 파생 소스, §22.11의 검증된 방식)
이다.

**B-3. 결정적 한계(먼저 명시)**: `matched`(매수 의도 유지) 3건
전부가 `2026-08-07` 결정이라 **forward return이 단 1건도
계산되지 않는다**(`2026-08-08` 종가 필요, 미도래). 즉 이번
턴에서도 **"매수 의도가 유지됐을 때 실제로 어땠는지"는 전혀
확인할 수 없다** — 유일하게 확인 가능한 것은 "하향된 표본이
실제로 어땠는지"뿐이다. 이는 데이터 공백이 아니라 4거래일
연속 하향이 지배적이었던 구조 자체의 결과다.

**`downgraded`(하향, 계산 가능한 6건, 절대수익률·결정일 종가
진입 기준)**:

| 종목 | 결정일 | T+1 | T+2 | `08-07`까지 누적 |
|---|---|---|---|---|
| `001450` | 08-03 | -1.34% | -1.61% | **+5.36%** |
| `073240` | 08-03 | +4.80% | +10.81% | **+17.27%** |
| `035420` | 08-05 | -1.31% | -8.30% | -8.30%(=T+2) |
| `181710` | 08-05 | +0.70% | -2.34% | -2.34%(=T+2) |
| `008930` | 08-06 | +1.94% | 계산 불가 | +1.94%(=T+1) |
| `051900` | 08-06 | +2.93% | 계산 불가 | +2.93%(=T+1) |

**집계(T+1, n=6)**: 평균 **+1.29%**, 중앙값 +1.32%, 양수율
**66.7%(4/6)**, 합계 +7.72%, 최고 `073240` +4.80%, 최저
`001450` -1.34%. `entry_score`는 전 표본이 이미 `>=0.65`
(`BUY_CANDIDATE` 조건 자체가 0.65 이상을 요구)라 경계 구간
분리는 의미가 없다 — 전부가 이미 경계 이상이다.

**`suppressed`(참고, watch 기원 — 매수 의도와 무관, B축 엄밀
정의 밖)**: §22.4/§25.2에서 이미 계산한 대로 08-03(6건, T+1
평균 +3.68%)·08-06(1건, +3.16%) — 매수 의도가 아예 없었던
종목들의 성과이므로 B축 판정과 직접 연결하지 않는다.

**B-5. 기회비용 관점 질문에 대한 답**:
1. **하향된 종목들이 실제로 더 많이 올랐는가**: 계산 가능한
   6건 중 4건(66.7%)이 상승했고 평균은 양(+1.29%)이다 — "더
   많이 올랐다"고 단정하진 않지만, **하락보다 상승이 더
   흔했다.**
2. **하향되지 않은 종목이 더 나았는가**: **답할 수 없다** —
   비교할 `matched` 표본의 forward return이 전무하다(§28.2
   결정적 한계).
3. **날짜별 방향이 뒤집히는가**: T+1 기준 08-03(`001450` -1.34%/
   `073240` +4.80%, 1승1패)·08-05(`035420` -1.31%/`181710`
   +0.70%, 1승1패)·08-06(`008930` +1.94%/`051900` +2.93%, 2건
   모두 상승)로, **날짜마다 완전히 갈리지는 않지만 08-03/08-05는
   혼조, 08-06만 뚜렷한 양(+)**이다 — C축에서 본 것 같은 완전한
   정반대 반전은 아니고, 전체적으로 양(+) 쪽에 약하게 기운
   혼조세다.
4. **특정 종목/`source_type`에 몰린 현상인가**: 6건이 서로 다른
   6개 종목(`001450`/`073240`/`035420`/`181710`/`008930`/
   `051900`)에 걸쳐 있고 `source_type`은 `core`/`event_overlay`
   혼재라 특정 종목·유형에 국한된 현상은 아니다.
5. **안정적 설명력인가, 흔들리는가**: 3개 관측일 중 2개(08-03/
   08-05)는 종목 단위로 1승1패 혼조, 1개(08-06)만 2건 모두
   양(+)이다 — 날짜 단위로는 **안정적이라 부르기 어렵다.**
   다만 6건 전체 평균(+1.29%)과 양수율(66.7%)은 양(+) 쪽으로
   기울어 있어, "하향이 실제로 나쁜 진입을 막았다"는 근거는
   이번 데이터에서 나타나지 않았고 오히려 반대 방향(기회비용)의
   근거가 더 많이 쌓였다 — 다만 n=6이라는 표본 규모로는 이
   방향성을 "확정"으로 올리지 않는다.

### 28.3 정책 후보 판단

**작업 D의 4가지 옵션 중 판단**: 근거 강도에 맞춰 **"구조
재설계 후보"**로 판정한다(2번 "부분 완화 후보"보다 강하고, 4번
"제거 후보"보다는 약함). 근거:

- **재설계 후보로 올리는 이유**: (1) `2026-08-03~06` **4거래일
  연속, 매수 의도가 지배적으로 유지된 사례가 0건**이라는 사실은
  단일 날짜의 우연이 아니라 **반복되는 구조적 패턴**이다. (2)
  `evidence_strength`가 `moderate`/`strong`이어도 통과율은
  25.5%뿐이다 — 즉 FDC의 보수적 판단이 "근거가 약한 경우"에만
  국한되지 않고 **전반적으로 광범위**하다. (3) 계산 가능한
  하향 표본(n=6)의 사후 성과가 평균 양(+)·양수율 66.7%로,
  하향이 기대값을 개선했다는 근거가 나타나지 않았다.
- **"제거"까지는 가지 않는 이유**: (1) C축(EV 게이트)처럼
  "가장 좁은 지점 1곳"을 가리킬 수 있는 하드 게이트가 아니다 —
  B축은 FDC(AI 4단 체인)의 자체 판단 결과이므로, 코드 한
  지점을 고치는 문제가 아니라 **AI 판단 편향 자체를 다시
  설계해야 하는 문제**다. (2) `matched`(통과) 표본의 forward
  return이 전무해 **직접 비교가 아직 한 번도 이뤄지지 않았다**
  — "재설계가 필요해 보인다"와 "지금 당장 무엇을 어떻게 바꿔야
  하는지 확정됐다"는 다른 질문이다. (3) `evidence_strength`
  분류력 자체는 실제로 존재해(§28.1), 이 필드에 근거한 보수적
  판단이 전부 "잘못된 판단"이라고 볼 근거도 아직 없다.
- **유지가 아닌 이유**: 4거래일 연속 100% 하향은 "가끔 있는
  일"이 아니라 이 시스템의 신규매수 경로에서 **B축이 C축(§27
  에서 이미 완화)보다 더 강하게, 더 반복적으로 작동하는 남은
  주요 병목**이라는 뜻이다 — 유지 판정은 이 반복성과 맞지
  않는다.

### 28.4 factual / 해석 / 미확정

- **factual**: (1) `downgraded`는 100% `BUY_CANDIDATE` 기원,
  `suppressed`는 100% `WATCH` 기원 — 코드(`decision_factory.
  py`)와 데이터 양쪽에서 예외 없이 확인. (2) `evidence_strength`
  `weak`/`none`은 `matched`(`BUY_CANDIDATE` 기준)에서 0건이었다
  — 통과율 0%. `moderate`/`strong`도 통과율 25.5%뿐. (3)
  `2026-08-03~06` 4거래일 연속 매수 의도 유지 사례 0건, `08-07`
  에 처음 3건(그중 1건 실제 체결). (4) 계산 가능한 하향 표본
  (n=6) 사후 성과 평균 +1.29%(T+1), 양수율 66.7%. (5) `matched`
  (통과) 표본의 forward return은 08-07 결정 3건뿐이라 전무하다.
- **해석**: `evidence_strength`는 실제 분류력을 갖지만(FDC가
  이 근거로 판단한다는 뜻), 그 판단이 사후 성과를 개선했다는
  근거는 아직 없다 — 오히려 하향된 표본이 평균 상승했다는
  방향의 근거가 쌓였다. 4거래일 연속 100% 하향은 "가끔 과도하게
  보수적이다"보다 **"거의 항상 보수적으로 작동한다"**는 진단에
  더 가깝다. C축의 신규매수 hard block이 이미 완화된 지금(§27),
  B축이 남은 주요 신규매수 병목이라는 맥락도 이 판단을 뒷받침
  한다.
- **미확정**: (1) `matched`(통과) 표본의 실제 사후 성과 — 아직
  단 1건도 계산할 수 없다. (2) 이 패턴이 5거래일·10거래일로
  계속돼도 반복되는지. (3) `evidence_strength`/`risk_opinion`
  기반 FDC 판단 중 실제로 "옳았던" 사례가 있는지(현재는 반대
  방향 근거만 있음, n=6). (4) B축을 구체적으로 어떻게
  재설계해야 하는지(이번 절은 방향을 제시할 근거만 확인했고,
  구체 설계안은 다루지 않았다).

### 28.5 후속 검증 연결

1. `2026-08-08` 이후 `000240`/`068270`/`078930`(08-07 매수
   의도 유지 3건)의 실제 forward return을 계산 — 이번 조사
   전체에서 **최초의 "매수 의도 유지" 사후 성과** 데이터가
   된다.
2. 매수 의도 유지 사례가 앞으로도 계속 드물게만 나타나는지,
   아니면 08-07이 전환점이었는지 관찰한다.
3. `evidence_strength`/`risk_opinion` 기반 FDC 판단 편향의
   구체적 재설계안은 이번 턴에서 다루지 않았다 — 다음 착수
   턴 과제로 남긴다(코드 변경 전에 `matched`/`downgraded`
   사후 성과 표본이 최소 10건 이상 쌓이는 것이 선행 조건).
...
## 30. `risk_off + downgraded(BUY_CANDIDATE)` 표본의 `08-07` 종가
고정 horizon 사후 성과(2026-08-08 KST, read-only)

**목적**: §29에서 확인한 `risk_tone` 비대칭(통과율 `risk_off`
11.7% vs `risk_on` 93.2%)의 대상 표본만 따로 뽑아, "그날 막힌
채로 `2026-08-07` 종가까지 들고 있었다면 어땠는가"를 **고정
종착점(fixed T_end) 방식**으로 계산한다. `matched`(통과) 비교는
§29에서 이미 다뤘고 여전히 `08-10`(월) 종가가 필요해 이번 절
에서도 계산하지 않는다 — 이번 절은 `downgraded` 표본 자체의
성과만 본다. 시장 벤치마크 비교는 하지 않는다(절대수익률만).

### 30.1 표본 정의와 소스

`decision_json->'candidate_vs_final'->>'primary_candidate' =
'BUY_CANDIDATE'` AND `alignment_status='downgraded'` AND
`deterministic_trigger.metadata.risk_tone='risk_off'`을
`2026-08-03~07` 전체에서 조회했다. 대표 라벨(지배적 상태)은
이미 §28.1/§29.1에서 확정한 것과 정확히 동일한 13개 단위다 —
전부 예외 없이 `regime_label='bullish_trend'`였다(신규 factual,
아래 30.4에서 다룬다).

- **결정일 종가가 필요한 6개 단위**(`2026-08-06` 이전 결정,
  `08-07` 종가까지 실제 고정-horizon 계산이 가능): `001450`
  (08-03)·`073240`(08-03)·`035420`(08-05)·`181710`(08-05)·
  `008930`(08-06)·`051900`(08-06).
- **`08-07` 결정 7개 단위**(진입가=평가가=`08-07` 종가이므로
  수익률이 항상 0%로 정보력이 없다, §30.3에서 별도 표기):
  `008930`·`051900`·`073240`·`078930`·`111770`·`180640`·
  `196170`.

**가격 소스**: 1차 소스(`instrument_status_snapshots`) 우선,
없을 때만 대체 소스(`signal_feature_snapshots` 파생 종가) 사용
— §22.11에서 이미 검증된 방식.

| 종목 | 결정일 종가 소스 | `08-07` 종가 소스 |
|---|---|---|
| `001450` | 1차(37,300) | 대체(39,300) |
| `073240` | 1차(6,660) | 대체(7,810) |
| `035420` | 1차(229,000) | 대체(210,000) |
| `181710` | **대체(42,800)** — 1차 소스에 이 종목 전체 이력이 없음(구조적 공백, §13.4.4에서 이미 확인) | 대체(41,800) |
| `008930` | 1차(43,800) | 대체(44,650) |
| `051900` | 1차(324,000) | 대체(333,500) |

`08-07` 종가는 1차 소스가 아직 `2026-08-06`까지만 반영돼(§29.1)
6건 전부 대체 소스를 썼다 — 미확인 리스크(1차 소스와의 교차
검증은 `08-08` 05:05 KST 이후 가능)는 §22.11/§25.1/§29.1과
동일하게 유지된다.

### 30.2 핵심 집계(6개 단위, `08-07` 종가 고정 horizon)
...
| 양수율 | 66.7%(4/6) |
| 합계 | +16.87% |
| 최고 | `073240`(08-03) **+17.27%** |
| 최저 | `035420`(08-05) **-8.30%** |

개별 값: `001450` +5.36% / `073240` +17.27% / `035420` -8.30%
/ `181710` -2.34% / `008930` +1.94% / `051900` +2.93%(§28.2와
동일한 값 — 계산 방식이 같으므로 재확인일 뿐 새 결론이 아니다).

### 30.3 하위 버킷 분해

| 버킷 | n | 평균 | 중앙값 | 양수율 | 비고 |
|---|---|---|---|---|---|
| 전체(§30.2와 동일) | 6 | +2.81% | +2.44% | 66.7% | — |
| `regime_label='bullish_trend'` | 6 | +2.81% | +2.44% | 66.7% | **6건 전부 `bullish_trend`** — 비교할 비-`bullish_trend` 표본이 없다(30.4) |
| `source_type='core'` | 4(`001450`/`073240`/`035420`/`181710`) | **+2.99%** | +1.51% | 50.0% | |
| `source_type='event_overlay'` | 2(`008930`/`051900`, 08-06) | **+2.44%** | +2.44% | 100.0% | n이 작아 참고용 |
| `strategy_policy_mismatch`(전체) | 6/6 | +2.81% | +2.44% | 66.7% | **[2026-08-08 KST 정정] 6건 전부 이 태그가 최소 1사이클 이상 존재한다 — "있음/없음"으로 나눌 수 있는 분해 축이 아니다. §30.5 참고 |

**`08-07` 결정 7건(정보력 약함, 참고 표기만)**: 진입가=평가가라
전부 수익률 0.00%다 — 이 7건은 §30.2/§30.3의 핵심 집계에서
**제외**했다. 사후 성과를 보려면 `08-10`(월) 이후 재계산이
필요하다(§29.6에서 이미 다음 과제로 명시).

### 30.4 신규 factual 발견 — `regime_label`이 전부 `bullish_trend`

이번 표본 13개 단위(`(symbol, date)` 기준 6+7=13건) **전부**
`regime_label='bullish_trend'`였다 — `risk_
off + downgraded(BUY_CANDIDATE)` 조합에서 다른 추세 국면
(`bearish_trend`/`range_bound`/`event_driven_unstable`)은
`2026-08-03~07` 구간에 **단 한 번도 관측되지 않았다.** 즉
§29.4(B)에서 제안한 "`risk_off`+`bullish_trend` 조합을 분리해
보라"는 요청에는 **분리할 대상 자체가 없다** — 이 기간의 `risk_
off + downgraded` 표본은 이미 전부 `bullish_trend`다. 이는
"이 조합이 특별히 나쁘다"는 뜻이 아니라, **이 구간 자체가
`bullish_trend` 국면이었을 가능성**을 시사한다 — 국면 자체의
빈도는 이번 절에서 확인하지 않았다(미확정으로 남긴다).

### 30.5 `strategy_policy_mismatch`에 대한 정정(2026-08-08 KST 재검증)

**정정**: 이 절의 최초 버전은 "사이클 과반(50% 초과)에서 이
태그가 붙은 단위는 6개 중 1개(`035420`)뿐이고 나머지 5개는
아니다"라고 서술했다 — 이는 **사실 오류**다. 존재 여부(사이클
1개 이상 태그가 붙었는지)를 다시 확인한 결과, **계산 가능한
6건 전부(그리고 대표 표본 13건 전부)** `strategy_policy_
mismatch` 태그가 최소 1사이클 이상 존재한다(§30.1 population
전수 재확인, 각 단위별 태그 붙은 사이클 수: `001450` 9/63,
`073240`(08-03) 13/38, `035420` 31/53, `181710` 16/54,
`008930`(08-06) 1/34, `051900`(08-06) 12/50 — 전부 0보다
크다). 이전 버전의 "과반 기준 지배적 1건/비지배적 5건" 분해와
그에 따른 성과 차이(§30.3 이전 표, -8.30% vs +5.03%)는 **철회
한다.**

**정정된 해석**: `strategy_policy_mismatch`는 이번 `risk_off +
downgraded(BUY_CANDIDATE)` 표본에서 **공통 태그에 가깝다** —
붙는 사이클의 비율(1.9%~58.5%로 넓게 분포)은 종목마다 다르지만,
"이 태그가 있는 단위 vs 없는 단위"로 나눌 수 있는 **분해 축으로
기능하지 못한다.** 따라서 **이 태그의 독립 설명력은 이번 집계
로는 판단할 수 없다** — "설명력이 약하다"고 단정하는 것도,
"설명력이 있다"고 단정하는 것도 이번 데이터로는 성립하지 않는다.
`risk_off`/`bullish_trend`/`strategy_policy_mismatch`가 구조적
으로 함께 나타나는 묶음일 가능성은 있으나(§29.2 Step4에서 확인한
`strategy_selection.py`의 `risk_tone==risk_off` 분기가 이 태그의
근원일 가능성), 이 묶음 내부에서 어느 것이 원인이고 어느 것이
결과인지에 대한 인과 해석은 이번 절에서 하지 않는다.

### 30.6 factual / 해석

- **factual**: (1) `risk_off + downgraded(BUY_CANDIDATE)` 표본
  중 `08-07` 종가까지 계산 가능한 6건의 평균 수익률은 **+2.81%,
  양수율 66.7%(4/6)**다. (2) 이 6건 중 최고(+17.27%)와 최저
  (-8.30%)는 24.97%p 차이로 편차가 크다. (3) `source_type`별
  (`core` +2.99%, `event_overlay` +2.44%)로 나눠도 방향(양(+))
  은 유지된다. (4) 표본 13개 단위 전부가 `regime_label=bullish_
  trend`라 이 국면을 분리한 비교는 이번 구간 데이터로는 불가능
  하다. (5) `strategy_policy_mismatch`는 계산 가능한 6건·대표
  표본 13건 **전부**에 최소 1사이클 이상 존재해 이 표본 내부의
  분해 축으로 쓸 수 없다(§30.5, 2026-08-08 정정 — 이전 버전의
  "지배적 1건" 서술은 사실 오류였다).
- **해석**: 계산 가능한 표본에서 `risk_off`로 하향된 종목들의
  사후 성과는 **평균적으로 양(+)이었고 다수(4/6)가 상승**했다
  — 이는 "`risk_off` 하향이 기대수익률을 개선했다"는 근거로
  이어지지 않는다. 다만 "상승 종목이 많았다"가 곧 "이 하향이
  틀렸다"는 뜻도 아니다 — 최저 성과(-8.30%)도 함께 존재하고
  표본이 6건뿐이라 방향성의 안정성은 아직 확정할 수 없다.
  `risk_tone`의 비대칭 자체(11.7% vs 93.2% 통과율, §29)가 매우
  강한 구조적 신호라는 사실과, 그 비대칭이 실제로 기대값을
  개선했다는 근거가 없다는 사실은 **서로 다른 층위의 결론**이며
  이번 절에서도 후자의 근거는 나타나지 않았다.
- **미확정**: (1) `08-07` 결정 7건의 실제 성과(`08-10` 이후).
  (2) `risk_off + downgraded` 표본이 왜 전부 `bullish_trend`
  였는지(이 구간의 시장 국면 자체 특성인지, 표본 편향인지). (3)
  n=6을 넘는 표본에서도 이 방향(양(+) 편향)이 유지되는지.
- **`risk_tone` 완화 후보 판단**: §29의 "완화 후보"·"구조
  재설계 후보" 판정은 **유지한다** — 이번 절의 결과가 그 근거를
  약화시키지 않았다(오히려 계산 가능한 표본의 사후 성과가 하향을
  지지하지 않는다는 방향을 재확인했다). 다만 표본이 아직 n=6
