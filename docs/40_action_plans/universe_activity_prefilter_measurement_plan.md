...
3. **"신호는 괜찮지만 activity만 낮은 종목"인가, 전반적으로 약한 종목인가?** → **혼재돼 있다, 단일 유형이 아니다.** 두 하위 그룹이 구분된다 — (a) 만성 콜드형(`000810`/`081660`/`021240`/`042700`/`402340`/`329180`/`196170`/`055550`/`090430`): block_rate 82~100%, `relative_activity` p50이 매우 낮음(0.009~0.09 수준 다수). (b) 경계선 변동형(`001450`/`316140`/`004370`/`009240`/`008930`): block_rate 26~50%로 대부분은 통과하고, `relative_activity` p50이 오히려 1.10 이상(1.27~1.63)인 경우도 있다 — 가끔만 경계선 아래로 떨어진다. `entry_score`는 두 그룹 모두 0.27~0.78 범위로 겹쳐, `entry_score` 하나로 두 그룹을 가르지 못한다.
...
5. **원인이 어디에 더 가까운가?**
   ...
   - **부분적으로 참: 선정 기준이 activity와 독립적이다.** 만성 콜드형 그룹의 존재가 이를 뒷받침하지만, 경계선 변동형 그룹(`001450` 등)은 대체로 activity가 정상이라 "선정 기준 자체"보다는 "차단된 뒤에도 안 빠지는 것"이 더 핵심으로 보인다.
