...
- `TOT_CCLD_QTY`가 누적값이라는 핵심 가정은 `000227`/2026-06-23 KST 자연 발생 부분체결 표본으로 **실증적으로 보강됐다** — 이 쟁점을 이유로 구현을 보류할 단계는 아니었고, 실제로 이 가정을 전제로 안 A + 안 C 구현이 완료됐다(5절). 다만 이 근거는 `broker_fill_snapshots` 시계열을 통한 간접 확인이므로, shadow 모드(`KIS_FILL_INCREMENTAL_APPEND_ENABLED=false`) 기간 동안 raw 응답을 직접 재관측해 재확인한 뒤에만 실제 append(`=true`)로 전환해야 한다.
