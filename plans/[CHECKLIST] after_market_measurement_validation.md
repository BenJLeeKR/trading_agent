...
## 운영 원본과 수동 재실행본 구분 메모

- 장후 자동 배치가 만든 운영 원본 로그는 덮어쓰지 않고 유지한다.
- 수동 재실행으로 만든 검증용 산출물은 `_rerun` suffix를 붙여 별도 보관한다.

예시:

- 운영 원본:
  - `logs/trigger_proxy_attribution_2026-07-08.json`
- 수동 재실행본:
  - `logs/trigger_proxy_attribution_2026-07-08_rerun.json`

구분 이유:

- 운영 원본은 당시 실제 스케줄러가 만든 감사용 산출물이다.
- 수동 재실행본은 코드 보정 후 검증 결과이므로 성격이 다르다.
- 두 파일을 분리 보존해야
  - 당시 운영 결과
  - 보정 후 재산출 결과
  를 비교할 수 있다.
...
- 운영 장애 분석, 감사, 회귀 비교에는 원본 파일을 우선 사용한다.
- 신규 shadow 필드, 보정 로직, 실험적 리포트 검증에는 `_rerun` 파일을 사용한다.
- 동일 날짜에 대해 여러 번 수동 검증한 경우에도 운영 원본은 수정하지 않는다.
