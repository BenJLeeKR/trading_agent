...
- **2.23순위(완료, 2026-07-15 — 새 alpha 상위군과 기존 차단 축 결합
  효과 검증, 진짜 병목 재발견)**: `regime_conditional_signal`을 새
...
- **3순위(보류 유지, 형태 재정의 — 우선순위 재조정)**: **`entry_
  ...
  **§2.23~§2.24에서 결합 사용 시 가장 빈번하게 걸리는 축이 regime
  관련 축이 아니라 활동성 필터(`eligibility_low_relative_activity`)
  임이 확인됐으나, 이 필터가 과잉 억제인지·완화가 실제로 기대수익률을
  개선하는지는 아직 확정되지 않았다(Watch, 추가 threshold 스윕
  필요)**. SPPV-3의 다음 착수 항목은 이 완화 방향(1.10→1.00 근방)의
  추가 검증이며, 운영 코드 반영은 Conditional Go 이상이 확보된 뒤
