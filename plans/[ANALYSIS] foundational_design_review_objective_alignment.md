...
- **3순위(보류 유지, 형태 재정의)**: **`entry_score`와 BUY funnel
  ...
  재현으로는 착수하지 않는다. §2.15의 판정에 따라 다음 착수 형태는
  기존 sub-component 조합의 재현이 아니라 **`regime_switch_v1` 아이디어
  를 국면 분기형 entry 설계 원형으로 삼는 것**으로 재정의된다. 착수
  전제(1차 게이트 `TRIGGERED` 전환 또는 shadow 설계 우선 착수 여부)는
  사용자 확인 필요. 착수 시 regime/allocation/strategy를 복원하고
  signal, `risk_off_penalty`, eligibility block의 중복 억제를
  ablation한다. Virtual BUY별
