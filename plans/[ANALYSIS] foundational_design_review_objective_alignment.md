...
- **3순위(보류 유지, 형태 재정의 — 우선순위 재조정)**: **`entry_
  ...
  **§2.23에서 결합 사용 시 진짜 병목이 regime 관련 축이 아니라
  활동성 필터(`eligibility_low_relative_activity`)임이 새로
  확인됐다.** SPPV-3의 다음 최우선 조사 대상은 이 활동성 필터
  재검토다. 1차 게이트(§21 모니터링)가 `TRIGGERED`로 전환되는 즉시
