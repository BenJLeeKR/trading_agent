...
- **3순위(보류 유지, 형태 재정의 — 우선순위 재조정)**: **`entry_
  ...
  **§2.23~§2.26에서 결합 사용 시 가장 빈번하게 걸리는 축이 regime
  관련 축이 아니라 활동성 필터(`eligibility_low_relative_activity`)
  임이 확인됐으나, 이 필터가 과잉 억제인지·완화가 실제로 기대수익률을
  개선하는지는 여전히 미확정이다(Watch — threshold sweep + 기간
  분할 검증에서도 재현성이 확인되지 않아 격상 근거 없음)**. SPPV-3의
  다음 착수 항목은 전반부·후반부가 왜 반대 방향을 보이는지 원인
  규명이며, 운영 코드 반영은 Conditional Go 이상이 확보된 뒤
