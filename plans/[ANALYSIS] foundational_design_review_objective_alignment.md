...
- **2.5순위(완료, 2026-07-14 — SPPV-2.5)**: quintile spread 정체 진단 —
  spread 자체의 Newey-West 유의성 검정 + 국면 내부(within-regime) 분해.
  **결과: `overall_score` pooled spread(T+20, t_NW=2.30)는 유의하나, 국면
  내부(bullish_trend t=1.55, range_bound t=1.63, bearish_trend t=0.38)
  어디서도 재현되지 않음 — pooled 유의성이 국면 혼입(regime-mix) 착시일
  가능성이 높다.** 종목 선택 알파가 아니라 국면(상승/하락) 자체를 반영한
  것일 수 있다는 뜻이다. 상세: `plans/[DESIGN] signal_predictive_
  power_validation.md` §11.
- **3순위(조건부 보류 유지)**: **`entry_score`와 BUY funnel 재현** — 2.5순위
  결과 국면 내부 유의성이 확보되지 않아 착수하지 않는다. 표본 확장
  (기간 1→2~3년 또는 유니버스 확대)으로 국면 내부 유의성이 재확인되거나,
  아니면 현재 backbone 신호 feature 구성 자체의 재검토로 트랙을 전환해야
  한다(§11.5). 확인/재설계가 끝나면 regime/allocation/strategy를 복원하고
  signal, `risk_off_penalty`, eligibility block의 중복 억제를 ablation한다.
  Virtual BUY별 T+1/T+3/T+5/T+10/T+20, MFE/MAE, 비용 차감 수익률과
