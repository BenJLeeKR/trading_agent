...
> 보정 (최신)**: SPPV-2.34의 "구성효과 8개 창 중 6개 음(-)"은
...
0. **BUY 주문 0건 근본 복구 (2026-07-14 신설, 최우선)**
  ...
  이후 §2.24/§14 ablation으로 보정됨)**: `regime_conditional_
   ...
   - **SPPV-2.35(완료, 2026-07-16, 분기3 스왑 집중일 세부 진단 +
     ...
     `plans/[DESIGN] regime_conditional_entry_signal_v1.md` §25.
   - **SPPV-3(다음 착수: 2025-02-12~13 연속 악재일의 이벤트/실적
     연관성 외부 데이터로 검증 + "대형 스왑일=양(+)/소규모 스왑일
     =음(-)" 패턴의 다른 창 재현 여부 확인 + R3b 구성 효과와 활동성
     필터의 상호작용 확인 + 더 긴 표본으로 재평가 + §3 전제조건
     ...
     진단은 이 반례를 더 정밀화했다 — 대형 스왑일은 순기여 양(+)
     이고, 문제는 나머지 다수 소규모 스왑일의 완만한 음(-)
     누적이다(§25).** 한편 **§2.23~§2.26에서 결합 사용
