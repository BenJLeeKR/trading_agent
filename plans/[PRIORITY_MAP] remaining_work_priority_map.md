  ...
  이후 §2.24/§14 ablation으로 보정됨)**: `regime_conditional_
   ...
   - **SPPV-2.31(완료, 2026-07-16, R3b 엄격 재검증 + R3 실패 구간
     ...
     **핵심 발견 — overlap 진단: R3(전체 universe 기준)는 R0와
     ...
     2026-07-16.json`. 상세: `plans/[DESIGN] regime_conditional_
     entry_signal_v1.md` §21.
   - **SPPV-3(다음 착수: R3b를 §3 공식에 정식 반영할지 사용자
     확인 + R3b 거래 빈도 감소 영향 검토 + §3 전제조건 충족 후
     ...
     분기 재현성 검증에서 무너졌으나, §2.31에서 candidate 내부
     기준 변형(R3b)이 더 엄격한 검증을 통과해 새로운 유력 후보로
     격상됐다(확정 Go는 아님).** 한편 **§2.23~§2.26에서 결합 사용
     ...
     단계에 머문다.** 다음 착수 형태는 R3b를 §3 공식에 정식
     반영할지 사용자 확인, R3b의 거래 빈도 감소가 자본 회전에
     미치는 영향 검토, alpha 교체의 §3 전제조건(§21 1차 게이트
