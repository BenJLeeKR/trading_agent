...
> 동일성 검증 + "거의 해소" 표현 재점검 (최신)**: 직전 턴(SPPV-2.53,
...
0. **BUY 주문 0건 근본 복구 (2026-07-14 신설, 최우선)**
  ...
  이후 §2.24/§14 ablation으로 보정됨)**: `regime_conditional_
   ...
   - **SPPV-3(다음 착수: §21 게이트 정기 재모니터링 + 게이트 충족
     ...
     경로 리스크(MAE)·손절 정책 설계 검토(신규, MAE 평균 -11%·
     심각 손실 12.8% 확인됨) + T+5 리스크 20일판·60일판 진짜
     페어드 비교(선택 사항, 20일판을 1048건 부분집합으로 제한
