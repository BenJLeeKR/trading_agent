  ...
  이후 §2.24/§14 ablation으로 보정됨)**: `regime_conditional_
   ...
   - **SPPV-2.34(완료, 2026-07-16, R3b pooled 우위 날짜 집중도
     ...
     **결과 1(날짜 집중도): 스왑 상위 10% 거래일을 제거해도 8개 창
     ...
     2026-07-16.json`. 상세: `plans/[DESIGN] regime_conditional_
     entry_signal_v1.md` §24.
   - **SPPV-3(다음 착수: 분기3의 스왑 상위 10% 거래일 구체 나열해
     특정 사유 존재 여부 확인 + R3b 구성 효과와 활동성 필터의
     상호작용 확인 + 더 긴 표본으로 재평가 + §3 전제조건 충족 후
     alpha 교체 재검증 + "국면 조건부 활동성 threshold" 설계 검토
     여부 사용자 확인)**:
     ...
     판정을 유지할 근거로 남겼다.** 한편 **§2.23~§2.26에서 결합 사용
     ...
     단계에 머문다.** 다음 착수 형태는 분기3의 스왑 상위 10%
     거래일을 구체적으로 나열해 특정 사유 존재 여부 확인, R3b 구성
     효과와
