...
> 이상(통상적 유의 수준)인 창은 2개(2차 1.96, 전반부 2.07)뿐이고
> 나머지는 1.0~1.9의 marginal한 값이었다. **R0 vs R3(전체
...
0. **BUY 주문 0건 근본 복구 (2026-07-14 신설, 최우선)**
  ...
  이후 §2.24/§14 ablation으로 보정됨)**: `regime_conditional_
   ...
   - **SPPV-2.32(완료, 2026-07-16, R3b 대응표본 검증 — overlap
     ...
     창은 2개(2차, 전반부)뿐이고 나머지는 marginal했다. **R0 vs
     ...
     signal_v1.md` §22.
   - **SPPV-3(다음 착수: R3b aggregate 우위와 대체쌍 성과 불일치
     원인 규명 + 더 긴 표본으로 재평가 + §3 전제조건 충족 후 alpha
     교체 재검증 + "국면 조건부 활동성 threshold" 설계 검토 여부
     사용자 확인)**:
     ...
     R3b도 다시 Watch로 하향됐다.** 한편 **§2.23~§2.26에서 결합 사용
     ...
     단계에 머문다.** 다음 착수 형태는 R3b의 aggregate 우위와
     대체쌍 성과(분기3 음수) 불일치 원인 규명, 더 긴 표본·더 많은
     교체 발생일 축적 후 재평가, alpha 교체의 §3 전제조건(§21 1차
