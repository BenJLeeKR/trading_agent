...
> 영향 ablation (최신)**: §42(SPPV-2.53)가 §38 보조 잔여 조건에
...
0. **BUY 주문 0건 근본 복구 (2026-07-14 신설, 최우선)**
  ...
  이후 §2.24/§14 ablation으로 보정됨)**: `regime_conditional_
   ...
   - **SPPV-3(다음 착수: §21 게이트 정기 재모니터링 + 게이트 충족
     (또는 별도 승인) 시 entry_score 코드 반영 절차 설계 +
     포지션 사이징 등 exit 외 리스크 관리 수단 검토(신규, 낮은
     우선순위, 실거래 계좌 상태 필요) + T+5 리스크 20일판·60일판
     진짜 페어드 비교(선택 사항, 20일판을 1048건 부분집합으로 제한
     재계산) + 국면 혼합도 감지·대응 설계 검토(강한 구조적 정합
     증거 확인됨, 선택 사항이나 우선순위 상향 권장) +
