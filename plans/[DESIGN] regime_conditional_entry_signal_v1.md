...
보다 강함 — "T+5 리스크"는 "부분 완화"에서 "거의 해소"로 격상.
다만 이 검증으로 MAE(경로 리스크) 평균 -11.08%, 심각 손실(-20%
이하) 비율 12.8%, 강제 손절 임계값 부재가 새로 드러나 §38 보조
잔여 조건에 "경로 리스크·손절 정책 부재"를 신규 추가. Conditional
Go 유지, 방향성 반전 아님, 운영 코드 변경 없음.**
