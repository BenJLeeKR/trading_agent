...
| 항목 | 기준 | 비고 |
|------|------|------|
| Submit 성공 | ✅ `broker_native_order_id` (ODNO) 발급 | 최소 성공 조건 |
| Sync loop 실행 | ✅ `last_synced_at` 갱신 | 1회 이상 sync cycle이 정상 실행되어야 함 |
| `order_state_events` 기록 | ✅ 상태 전이 이력 DB 저장 | `submitted → reconcile_required` 경로도 정상 |
| `broker_status = reconcile_required` | ✅ **허용 (정상)** | Paper mock의 `inquire-daily-ccld`가 `output: []` 반환하므로 예상된 결과 |
| FILLED / CANCELLED / REJECTED 수렴 | ❌ **Paper mock에서 기대 불가** | Live 환경 전용 검증 항목 |

**핵심 원칙**: Paper 환경에서는 "sync pipeline이 정상 동작하는가"를 검증하고, "terminal status가 무엇으로 수렴하는가"는 검증하지 않습니다. 후자는 Live 환경에서만 검증 가능합니다.
...
**Reconciliation loop의 Live 경로 사전 검증 불가**
...
| 3 | **Snapshot freshness** | 최근 snapshot sync 5분 이내 완료 | [`live_verification_prerequisites.md#3-snapshot-freshness-확인`](plans/live_verification_prerequisites.md) |
| 4 | **Sync loop 경로** | `run_post_submit_sync_loop.py` 존재, syncable orders 확인 | [`live_verification_prerequisites.md#4-sync-loop-실행-경로-확인`](plans/live_verification_prerequisites.md) |
...
**Live 검증 전 체크리스트 준비 완료 → Live 환경 전환 대기**

1. [`live_verification_prerequisites.md`](plans/live_verification_prerequisites.md)의 4가지 사전 조건이 모두 충족될 때까지 대기 (장중 시간)
2. 조건 충족 시 Live submit 실행 (별도 절차)
3. submit 후 post-submit sync 실행하여 3가지 logging 제거 조건 확인
4. 조건 모두 충족 시 instrumentation logging 제거 (`git checkout -- rest_client.py`)
...
**장중 Live 최초 submit 시 instrumentation logging으로 응답 구조 검증**

1. Live KIS 계정 준비 후 첫 장중 세션에서 submit 실행
2. Post-submit sync 실행
3. DEBUG logging에서 `inquire-daily-ccld: output_count=N, first_item_fields=...` 출력 확인
4. 3가지 제거 조건 충족 시 logging 제거
