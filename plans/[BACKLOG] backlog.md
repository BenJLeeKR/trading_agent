...
- **BUY 주문 0건 근본 복구 — 신호 예측력·`entry_score`·전체 주문 funnel 재설계**
  ...
  - **SPPV-2.32(완료, 2026-07-16, R3b 대응표본 검증 — overlap
    ...
    뒤집혔다.** t_NW가 통상 유의 수준(1.96) 이상인 창은 2개(2차,
    전반부)뿐이고 나머지는 marginal했다. **R0 vs R3(전체
  ...
  - **SPPV-3(다음 착수: R3b aggregate 우위와 대체쌍 성과 불일치
    ...
    다시 Watch로 하향됐다.** 한편 **§2.23~§2.26에서 결합 사용
    시 가장 빈번하게 걸리는 축이 활동성 필터임이 확인됐고, 완화
    효과의 반전이 국면·유동성 구조 차이 때문임을 규명했으나, 정적
    완화(1.10→1.00)가 기대수익률을 실제로 개선하는지는 여전히
    Watch(격상 근거 없음) 단계에 머문다.** 다음 착수 형태는 R3b의
    aggregate 우위와 대체쌍 성과(분기3 음수) 불일치 원인 규명,
    더 긴 표본·더 많은 교체 발생일 축적 후 재평가, alpha 교체의 §3
    전제조건(§21
