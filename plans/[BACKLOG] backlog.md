...
- **BUY 주문 0건 근본 복구 — 신호 예측력·`entry_score`·전체 주문 funnel 재설계**
  ...
  - **SPPV-2.31(완료, 2026-07-16, R3b 엄격 재검증 + R3 실패 구간
    ...
    때문으로 판단한다.** **판정: R3b를 유력한 재보정 후보로 신규
  ...
  - **SPPV-3(다음 착수: R3b를 §3 공식에 정식 반영할지 사용자 확인
    + R3b 거래 빈도 감소 영향 검토 + §3 전제조건 충족 후 alpha
    ...
    Watch(격상 근거 없음) 단계에 머문다.** 다음 착수 형태는 R3b를
    §3 공식에 정식 반영할지 사용자 확인, R3b의 거래 빈도 감소가
    자본 회전에 미치는 영향 검토, alpha 교체의 §3 전제조건(§21
