...
따라서 아래 3개 축을 정식 백로그로 올린다.

1. **주문 실행 파이프라인 리팩토링**
2. **KIS Sync 신뢰성 리팩토링**
3. **Event Interpretation(EI) output contract 리팩토링**
...
| SYNC-005 | P1 | **positions / cash / orderable_amount sync cadence 분리** | 장중 submit에 중요한 `cash + orderable_amount`와 positions를 같은 cycle에 강하게 결합하지 말 것. 중요도와 실패 허용도를 분리하고, 핵심 값은 더 짧은 주기로 재조회 가능하게 해야 함. |
| SYNC-006 | P1 | **snapshot sync를 scheduler cadence로부터 더 독립적으로 운영** | 설정은 5분이어도 실제로는 상위 scheduler 한 바퀴에 종속되어 8~12분처럼 보이는 문제가 있었음. 실질 cadence 보장과 재조회 간격 단축이 필요함. |
...
## 3B. EI Output Contract 백로그

| ID | 우선순위 | 작업 | 이유 / 기대효과 |
|---|---|---|---|
| EI-001 | P1 | **EI output contract 재설계 (`detected_event_count` vs `interpreted_events[]`)** | 현재는 `aggregate_view.event_count`와 `events[]`가 서로 다른 truth를 말할 수 있음. 감지된 이벤트 수와 LLM이 해석한 이벤트 목록을 분리해야 함. |
| EI-002 | P1 | **EI degraded mode 도입** | provider/parse/fallback 문제로 `events[]`를 못 만들더라도, `interpretation_incomplete`, `detected_event_count`, `summary_basis` 같은 상태를 명시적으로 남겨야 함. |
| EI-003 | P1 | **fallback 시 deterministic minimal event reconstruction 도입** | 입력 recent_events가 있는데 LLM 구조화가 실패하면, 최소 필드(headline/source/published_at/preview) 기반의 deterministic `events[]`를 복원할 수 있어야 summary와 화면이 일관됨. |
| EI-004 | P1 | **EI summary 생성 규칙 재작성** | 현재 `_build_ei_summary()`는 `events=[]`이면 무조건 “유의미한 신규 이벤트 없음”으로 감. `event_count>0`, degraded mode, interpreted events 존재 여부를 함께 보고 summary를 생성해야 함. |
| EI-005 | P1 | **EI provider raw / parsed / final / DB 저장값 audit 강화** | 입력 events는 있는데 output이 0건이 되는 문제는 raw response, parsed output, final output, DB 저장값을 같은 run 기준으로 비교해야만 추적 가능함. |
| EI-006 | P1 | **AgentsRuns/Admin UI에 EI degraded 상태 표시** | 운영 화면에서 “진짜 이벤트 없음”과 “이벤트는 감지됐으나 해석이 불완전”을 구분할 수 있어야 함. |
| EI-007 | P2 | **EI schema/parse failure class 분리** | provider zero-events, parse failure, fallback output, deterministic reconstruction 사용 여부를 reason code로 구분해 추후 통계와 회귀 분석이 가능해야 함. |
...
### 4.2A-1 KIS Sync cadence 목표

1. 현재처럼 `decision_submit_gate` timeout(약 610~620초)에 끌려 **실제 9~12분 간격**으로 밀리는 구조를 해소
2. snapshot sync는 최소한 **설정된 5분 cadence를 실제로 보장**할 것
3. 가능하면 장중 핵심 값은 cadence를 차등화
   - `cash + orderable_amount`: 1~3분대 검토
   - `positions`: 3~5분대 검토
4. timeout 한 번이 다음 재조회 시점을 10분 이상 밀어버리지 않게 할 것

### 4.2B EI output contract 착수 순서

1. `EI-001` EI output contract 재설계
2. `EI-002` EI degraded mode 도입
3. `EI-004` EI summary 생성 규칙 재작성
4. `EI-003` deterministic minimal event reconstruction 도입
5. `EI-005` raw / parsed / final / DB audit 강화
6. `EI-006` Admin UI degraded 상태 표시
...
5. scheduler의 장기 실행/timeout과 무관하게 snapshot 재조회 cadence를 독립적으로 보장한다.
6. 리팩토링 후 실제 재조회 간격은 현재 9~12분대에서 **최소 5분 수준 이하로 단축**하는 것을 목표로 한다.
...
- snapshot sync는 scheduler top-level cycle의 부속 작업이 아니라, **독립 cadence를 가진 truth refresh job**으로 재정의할 것
- cadence 보장은 설정값만이 아니라 **실제 관측 간격(metrics/log/DB 기준)** 으로 검증할 것

## 4.7 EI output contract 리팩토링 원칙

EI는 단순히 `events[]` 배열 하나를 채우는 agent가 아니라, **입력 이벤트 감지 truth / LLM 해석 truth / 운영 summary truth**를 일관되게 보존하는 계층으로 다뤄야 한다.

### 현재 문제

1. 입력 이벤트가 있어도 provider/parse/fallback 문제로 `events=[]`, `event_count=0`이 반복될 수 있다.
2. fallback/guard가 `event_count`만 보정하고 `events[]`를 비워두면 summary가 여전히 “유의미한 신규 이벤트 없음”으로 왜곡된다.
3. 운영 화면에서는 “이벤트 없음”과 “이벤트 감지됐지만 해석 불완전”이 구분되지 않는다.
4. raw response / parsed output / final output / DB 저장값 사이 차이를 한 번에 추적하기 어렵다.

### 리팩토링 목표

1. `detected_event_count`와 `interpreted_events[]`를 구분해 저장한다.
2. provider/parse failure가 나도 degraded mode로 truth를 보존한다.
3. summary는 `events[]`만 보지 않고, degraded 상태와 detected count를 함께 반영한다.
4. 운영자는 EI run 하나만 보고도
   - 진짜 이벤트 없음
   - 이벤트 감지됨
   - 해석 불완전
   를 구분할 수 있어야 한다.
...
- degraded mode 필드 도입
  ...
  - `summary_basis`
  - `degraded_reason`
- fallback 시 deterministic minimal event reconstruction 허용
- `/workspace/agent_trading/logs`에 run별로
  - input events count
  - provider raw output 요약
  - parsed output
  - final output
  - DB persist 결과
  를 연결해서 남길 것
...
7. EI에서 `event_count > 0`인데 summary가 “유의미한 신규 이벤트 없음”으로 남는 모순이 존재함
8. 운영자가 EI 결과를 보고 “진짜 이벤트 없음”과 “해석 불완전”을 구분할 수 없음
9. snapshot sync 설정값이 5분이어도 실제 재조회 간격이 여전히 9~12분대로 관측됨
