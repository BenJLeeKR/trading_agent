...
이 저장소의 궁극적 목표는 AI 기반 멀티 에이전트 트레이딩 시스템을 구축하여, 감내 가능한 손실 제약 아래 최고의 기대수익률을 추구하는 것이다. 기대수익률은 보장 결과가 아니며, 위험조정 성과·운영 안정성·감사 가능성·재현성을 함께 만족해야 한다.

## 최상위 설계 기준

시스템의 목적, 아키텍처, 에이전트 역할, 브로커 경계, 리스크·감사·운영 원칙은 [`docs/00_foundational_design/ENTERPRISE_TRADING_SYSTEM_DESIGN.md`](docs/00_foundational_design/ENTERPRISE_TRADING_SYSTEM_DESIGN.md)를 최상위 설계 기준으로 삼는다.

설계 또는 구현 판단 전에는 작업 주제와 관련된 장을 확인한다. 매매·주문·리스크는 §6~§12, 데이터·API·보안은 §9·§17·§18, 운영·검증은 §14·§20을 우선 확인한다. 전체 문서를 매 작업마다 읽도록 요구하지는 않는다.

## 문서 우선순위와 역할

- Foundational Design은 우리가 무엇을 만들며 어떤 제약을 지켜야 하는지를 정의한다.
- 이 파일과 하위 경로의 `AGENTS.md`는 AI가 저장소에서 어떻게 작업해야 하는지를 정의한다.
- Harness 문서와 `scripts/harness/run.sh`는 변경분이 요구사항을 충족했음을 어떻게 검증하고 증명하는지를 정의한다.
- 구현, 설계 문서, 하네스 계약이 어긋나면 임의로 한쪽에 맞추지 말고 사실·영향·권고안을 분리해 보고한다.

## Harness Engineering 역할

Harness Engineering은 제품 목표 자체가 아니라, 멀티 에이전트 트레이딩 시스템을 안전하고 재현 가능하게 개발·검증·배포하기 위한 방법론과 실행 계약이다. 하네스 통과만을 목표로 매매 의미론, 리스크 정책, 주문 흐름을 축소하거나 왜곡하지 않는다.
...
- [`docs/80_harness_engineering/definition_of_done.md`](docs/80_harness_engineering/definition_of_done.md): AI가 완료를 주장할 수 있는 최소 조건.
...
- 문서, 보고, 설명, 사용자 응답에는 중국어와 일본어를 사용하지 않는다.(한국어 사용)
- 코드 식별자, 명령어, 로그 원문, 에러 메시지, 외부 API 필드명, 설정 키, 파일 경로는 원문 그대로 유지할 수 있다.
- 사용자 보고, 문서, 설명, 계획, 완료 보고에서는 `diff`를 단독 용어로 쓰지 않는다. 기본 용어는 문맥에 따라 `변경분`, `변경 차이`, `전후 차이`, `수정 내역`을 사용한다.
- `git diff`, GitHub `Files changed`, patch/diff 포맷처럼 원문 보존이 필요한 경우에만 `diff`를 backtick으로 감싸고, 바로 뒤에 한국어 설명을 붙인다.
...
- 이 시스템의 목표는 손실을 0으로 만드는 것이 아니라 감내 가능한 손실 제약 아래에서 최고의 기대수익률을 추구하는 것이다. 차단 장치(eligibility gate, downstream override 등)의 적정성은 차단 빈도가 아니라 사후 성과(백테스트)로 기대값을 실제로 개선했는지로 판단하고, 주문 요청이 0건에 수렴하는 상태를 안전성의 증거로 보지 않는다. 상세 판단 기준은 `docs/10_signal_research_sppv/[PRIORITY_MAP] remaining_work_priority_map.md`의 공통 판단 원칙과 `docs/99_meta_handover/agent_workspace_guide.md`를 따른다.
...
- 우회 행동 금지 기준은 `docs/80_harness_engineering/no_bypass_policy.md`를 따른다. 명확한 안전 위반은 실패로 보고, 맥락 판단이 필요한 항목은 검토 대상으로 카운트한다.
...
- AI가 완료를 주장할 수 있는 조건은 `docs/80_harness_engineering/definition_of_done.md`를 따른다.
- 오류 메시지, API 오류 응답, 운영 로그를 보강할 때는 `docs/80_harness_engineering/ai_friendly_error_message_contract.md`를 따른다.
