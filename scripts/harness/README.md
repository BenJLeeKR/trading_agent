...
- `fdc_quota_postgres_relevant`의 file-change 판정 로직은 `scripts/harness/detect_fdc_quota_postgres_relevance.sh`(GitHub Actions 컨텍스트에 의존하지 않는 순수 bash)로 분리돼 있다 — workflow 스텝은 이벤트별 base ref(`pull_request`→PR base SHA, `push`→직전 push SHA, `workflow_dispatch`→해당 없음)만 계산해 이 스크립트에 넘긴다(2026-08-25 2차 보정, `HEAD^`만 보던 1차 구현이 "관련 커밋 뒤에 문서만 고친 커밋을 추가 push하면 relevant=0으로 잘못 판정"하는 결함이 있어 PR/push 전체 범위 비교로 교체). base ref를 확인할 수 없으면(얕은 checkout, 최초 push의 all-zero SHA 등) fail-open(`relevant=0`)하지 않고 보수적으로 `relevant=1`을 출력한다. `bash scripts/harness/test_detect_fdc_quota_postgres_relevance.sh`(`test_docker_compose_env_git_sha.sh`와 동일한 관례 — fixture git 저장소를 만들어 실제 `git diff`로 검증)로 4개 필수 시나리오(PR 범위에 관련 파일 있고 마지막 커밋은 무관/문서 전용 → relevant=1, 관련 파일 전혀 없음 → relevant=0, workflow_dispatch → relevant=1) + base ref 부재 시 보수적 fallback을 검증한다.
- `postgres_fixture_loop_scope_integration` job(2026-08-26 신설, 3차 보정)은 `tests/conftest.py`의 공용 `postgres_repos`/`seeded_postgres_data` fixture와 이를 소비하는 로컬 async fixture(예: `test_postgres_fill_events.py`/`test_postgres_broker_orders.py`의 로컬 `seeded_postgres_data` override + 체이닝된 하위 fixture)가 `pyproject.toml`의 `asyncio_default_fixture_loop_scope="module"`과 test 기본 loop scope("function") 불일치로 실제 PostgreSQL 앞에서 "attached to a different loop" 오류를 내는지 재현·검증한다 — FDC quota 전용 job과 역할을 분리하되, relevance 판정은 `scripts/harness/detect_fdc_quota_postgres_relevance.sh`를 5번째 인자(pattern override)로 재사용한다. 대상은 `test_postgres_fill_events.py`/`test_postgres_broker_orders.py`(로컬 fixture override 케이스) + `test_external_events.py`(공용 fixture만 쓰는 대조군) 3개 파일로 좁혔다. **판정 기준(3차 보정)**: 3개 파일 전부 `exit_status=0`이고 `skipped_count=0`·`failed_count=0`·`error_count=0`이어야 job이 통과한다 — 하나라도 어긋나면 그 즉시 실패 처리하며, "attached to a different loop" 오류 문자열 검사는 원인 진단용 보조 지표로만 남기고 성공/실패 판정에는 쓰지 않는다(이전 버전은 이 문자열의 부재만으로 통과시켜, `instruments.exchange_code`/`market_segment` NOT NULL 위반이라는 별개의 사전 존재 결함이 남아 있는데도 job을 초록으로 표시하는 은폐가 있었다 — 그 결함은 `test_postgres_fill_events.py`/`test_postgres_broker_orders.py`의 로컬 `seeded_postgres_data` fixture 데이터만 보정해(schema/migration/도메인 계약은 무변경) 함께 해소했다). `safe`/`heavy`/`fdc_quota_postgres_integration` job의 범위·timeout·실행 조건은 이 job으로 전혀 바뀌지 않는다.
