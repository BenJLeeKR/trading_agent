...
- `postgres_fixture_loop_scope_integration` job(2026-08-26 신설)은 `tests/conftest.py`의 공용 `postgres_repos`/`seeded_postgres_data` fixture와 이를 소비하는 로컬 async fixture(예: `test_postgres_fill_events.py`/`test_postgres_broker_orders.py`의 로컬 `seeded_postgres_data` override + 체이닝된 하위 fixture)가 `pyproject.toml`의 `asyncio_default_fixture_loop_scope="module"`과 test 기본 loop scope("function") 불일치로 실제 PostgreSQL 앞에서 "attached to a different loop" 오류를 내는지 재현·검증한다 — FDC quota 전용 job과 역할을 분리하되, relevance 판정은 `scripts/harness/detect_fdc_quota_postgres_relevance.sh`를 5번째 인자(pattern override)로 재사용한다. 대상은 `test_postgres_fill_events.py`/`test_postgres_broker_orders.py`(로컬 fixture override 케이스) + `test_external_events.py`(공용 fixture만 쓰는 대조군) 3개 파일로 좁혔고, 파일별로 skip/오류를 모두 확인해 하나라도 skip이거나 실패하면 job을 실패 처리한다. `safe`/`heavy`/`fdc_quota_postgres_integration` job의 범위·timeout·실행 조건은 이 job 추가로 전혀 바뀌지 않는다.
