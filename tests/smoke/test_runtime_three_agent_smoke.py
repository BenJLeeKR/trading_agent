"""Smoke tests: real 3-agent chain through the runtime path.

Verifies that:
* ``build_default_runtime()`` creates real ``EventInterpretationAgent``,
  ``AIRiskAgent``, and ``FinalDecisionComposerAgent`` when provider
  credentials are configured (DeepSeek or OpenAI), and falls back to
  ``None`` (stub) for all three when they are missing.
* ``orchestrator.assemble()`` executes the full EI → AR → FDC chain
  via real provider calls, records all 3 runs, and assembles a valid
  ``AIDecisionInputs`` contract on ``OrderIntent``.
* Partial chains (EI-only real, EI+AR real) correctly mix real and stub
  agents, with the recorder and ``AIDecisionInputs`` reflecting the
...
class TestRuntimeThreeAgentFallback:
    """Runtime returns ``None`` (stub) for all three agents when provider
    ...
    def test_default_runtime_all_stub_when_no_credential(
        ...
        """All three agent slots are ``None`` (stub fallback) when no
    ...
    async def test_orchestrator_assemble_all_stub(
        ...
        """``assemble()`` succeeds with all stubs — recorder stores 3 runs
...
# ---------------------------------------------------------------------------
# Real 3-agent full chain — requires provider config (DeepSeek or OpenAI)
# ---------------------------------------------------------------------------
...
class TestRuntimeThreeAgentSmoke:
    """Real provider call — full EI → AR → FDC chain through
    ...
    * ``orchestrator.assemble()`` is called once; the result is shared
    ...
    Total: **1 real ``assemble()`` call** (3 internal provider calls) for
    the entire class.
    """
    ...
    def test_runtime_creates_real_agents(
        ...
        """All three agent slots hold real agent instances."""
    ...
    async def assemble_result(
        ...
        This executes the full EI → AR → FDC chain with real provider
        calls (3 internal provider calls, 1 real API invocation).
        """
    ...
    async def test_orchestrator_assemble_with_real_agents(
        ...
        """``assemble()`` returns ``OrderIntent`` — recorder stores 3 real
        ...
        for i, expected_agent in enumerate(
            ...
            assert run.structured_output_json is not None, (
                f"Run {i} ({expected_agent}): structured_output_json is None"
            )
            assert (
                run.structured_output_json.get("schema_version") == "v1"
            ), f"Run {i}: schema_version mismatch"
            assert run.structured_output_json.get("agent_name") == expected_agent, (
                f"Run {i}: agent_name mismatch"
            )
    ...
    async def test_ai_backend_inputs_assembled(
        ...
        schema versions from all three agents."""
        ai = assemble_result.ai_backend_inputs
        assert isinstance(ai, AIDecisionInputs)

        # Metadata: source_agent_names from _run_agents() assembly
        assert len(ai.source_agent_names) == 3, (
            f"Expected 3 source_agent_names, got {len(ai.source_agent_names)}"
        )
        for name in ("event_interpretation", "ai_risk", "final_decision_composer"):
            assert name in ai.source_agent_names, (
                f"Missing {name!r} in source_agent_names"
            )

        # Metadata: schema_versions — each agent has a (name, version) tuple
        assert len(ai.schema_versions) == 3, (
            f"Expected 3 schema_versions, got {len(ai.schema_versions)}"
        )
        for agent_name in (
            ...
            assert matching[0][1] == "v1", (
                f"Unexpected schema_version for {agent_name!r}: "
                f"{matching[0][1]}"
            )
...
# ---------------------------------------------------------------------------
# Partial real-agent chains — requires provider config (DeepSeek or OpenAI)
# ---------------------------------------------------------------------------
...
class TestRuntimeThreeAgentPartialChain:
    ...
    * ``test_ei_only_real_rest_stub`` — 1 ``assemble()`` (1 real provider
      call: EI only; AR and FDC are stubs).
    * ``test_ei_and_ar_real_fdc_stub`` — 1 ``assemble()`` (2 real provider
      calls: EI + AR; FDC is stub).
    Total: **2 real ``assemble()`` calls** (3 internal provider calls) for
    the entire class.
    """
    ...
    async def test_ei_only_real_rest_stub(
        ...
        """Real EI + stub AR + stub FDC — recorder shows 1 real + 2 stub
        ...
        # Recorder: 3 runs — 1 real EI, 2 stubs
    ...
    async def test_ei_and_ar_real_fdc_stub(
        ...
        """Real EI + real AR + stub FDC — recorder shows 2 real + 1 stub
        runs; ``AIDecisionInputs`` metadata reflects both real agents."""
        orchestrator = DecisionOrchestratorService(
            repos=repos,
            event_interpretation_agent=real_ei,
        ...
        # Recorder: 3 runs — 2 real (EI + AR), 1 stub (FDC)
        ...
        # EI and AR have real schema_version entries; FDC entry is from stub
        ei_ar_versions = [
            v
            for v in ai.schema_versions
            if v[0] in ("event_interpretation", "ai_risk")
        ]
