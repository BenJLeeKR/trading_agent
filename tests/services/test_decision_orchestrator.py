...
class TestWatchCandidateUpgradeGuard:
    ...
    async def test_market_overlay_buy_override_blocked_by_symbol_state_reentry_cooldown(
        ...
        assert "ai_override_reverse_feature_change_blocked" in intent.ai_backend_inputs.reason_codes
        assert (
            intent.ai_backend_inputs.validator_rule_set_version
            == "decision_policy_validator_v1"
        )
        assert intent.ai_backend_inputs.validator_stop_reason == "ai_override_gate"
        assert "ai_override_reverse_feature_change_blocked" in (
            intent.ai_backend_inputs.validator_blocking_rule_codes
        )

    @pytest.mark.asyncio
    async def test_market_overlay_buy_override_allows_reentry_when_three_axis_hysteresis_passes(
        self, sample_request: SubmitOrderRequest
    ) -> None:
        repos = build_in_memory_repositories()
        now = datetime.now(timezone.utc)
        ...
        context = DecisionContextEntity(
            ...
            config_version_id=uuid4(),
            market_timestamp=now,
            correlation_id="corr-market-overlay-reentry-pass",
            signal_feature_snapshot_id=uuid4(),
            created_at=now,
        )
        await repos.decision_contexts.add(context)
        ...
        service._run_agents = AsyncMock(  # type: ignore[method-assign]
            return_value=AgentExecutionBundle(
                ai_inputs=AIDecisionInputs(
                    ...
                    final_trade_score=Decimal("0.80"),
                    minimum_required_edge_bps=Decimal("10.00"),
                    edge_after_cost_bps=Decimal("35.00"),
                    estimated_round_trip_cost_bps=Decimal("10.00"),
                    slippage_buffer_bps=Decimal("10.00"),
                    expected_value_gate_passed=True,
                    reason_codes=("fdc_overlay_entry",),
        ...
        assert intent.ai_backend_inputs.decision_type == "APPROVE"
        assert "ai_override_gate" not in intent.ai_backend_inputs.reason_codes
        expected_value_anchor = intent.request.metadata["expected_value_anchor"]
        assert expected_value_anchor["anchor_passed"] is True
        assert expected_value_anchor["decision_type"] == "APPROVE"
        assert expected_value_anchor["source_type"] == "market_overlay"
...
def test_build_expected_value_anchor_metadata_tracks_exit_edge_deltas(
    service: DecisionOrchestratorService,
) -> None:
    symbol_state = SymbolTradeStateEntity(
        ...
        metadata_json={
            "holding_profile_policy": {
                "last_entry_edge_after_cost_bps": "30",
                "last_reduce_edge_after_cost_bps": "18",
                "last_exit_edge_after_cost_bps": "20",
            }
        },
    ...
    payload = service._build_expected_value_anchor_metadata(
        ai_inputs=AIDecisionInputs(
            decision_type="APPROVE",
            expected_return_bps=Decimal("70.00"),
            expected_downside_bps=Decimal("20.00"),
    ...
    assert payload["anchor_passed"] is True
    assert payload["last_entry_edge_after_cost_bps"] == "30"
    assert payload["last_reduce_edge_after_cost_bps"] == "18"
    assert payload["last_exit_edge_after_cost_bps"] == "20"
    assert payload["edge_vs_last_entry_delta_bps"] == "5.00"
    assert payload["edge_vs_last_reduce_delta_bps"] == "17.00"
    assert payload["edge_vs_last_exit_delta_bps"] == "15.00"
    assert payload["reentry_edge_improved_vs_last_exit"] is True

    @pytest.mark.asyncio
    async def test_held_position_exit_hysteresis_blocks_early_reduce_without_strong_reason(
        self, sample_request: SubmitOrderRequest
    ) -> None:
        repos = build_in_memory_repositories()
        now = datetime.now(timezone.utc)
        ...
        context = DecisionContextEntity(
            ...
            config_version_id=uuid4(),
            market_timestamp=now,
            correlation_id="corr-held-exit-gate",
            created_at=now,
        )
        await repos.decision_contexts.add(context)
        position_snapshot = PositionSnapshotEntity(
        ...
        service._run_agents = AsyncMock(  # type: ignore[method-assign]
            return_value=AgentExecutionBundle(
                ai_inputs=AIDecisionInputs(
                    ...
                    confidence=0.77,
                    conviction=0.74,
                    edge_after_cost_bps=Decimal("30"),
                    expected_return_bps=Decimal("50"),
                    expected_downside_bps=Decimal("15"),
                    net_expected_value_bps=Decimal("35"),
                    final_trade_score=Decimal("0.70"),
                    minimum_required_edge_bps=Decimal("10"),
                    estimated_round_trip_cost_bps=Decimal("10"),
        ...
        assert intent.ai_backend_inputs.decision_type == "WATCH"
        assert "held_position_exit_hysteresis_blocked" in intent.ai_backend_inputs.reason_codes

    @pytest.mark.asyncio
    async def test_held_position_exit_hysteresis_allows_early_reduce_on_edge_collapse(
        self, sample_request: SubmitOrderRequest
    ) -> None:
        repos = build_in_memory_repositories()
        now = datetime.now(timezone.utc)
        ...
        context = DecisionContextEntity(
            ...
            config_version_id=uuid4(),
            market_timestamp=now,
            correlation_id="corr-held-exit-pass",
            created_at=now,
        )
        await repos.decision_contexts.add(context)
        position_snapshot = PositionSnapshotEntity(
        ...
        service._run_agents = AsyncMock(  # type: ignore[method-assign]
            return_value=AgentExecutionBundle(
                ai_inputs=AIDecisionInputs(
                    ...
                    confidence=0.77,
                    conviction=0.74,
                    edge_after_cost_bps=Decimal("5"),
                    expected_return_bps=Decimal("30"),
                    expected_downside_bps=Decimal("25"),
                    net_expected_value_bps=Decimal("5"),
                    final_trade_score=Decimal("0.55"),
                    minimum_required_edge_bps=Decimal("10"),
                    estimated_round_trip_cost_bps=Decimal("10"),
                    slippage_buffer_bps=Decimal("10"),
                    expected_value_gate_passed=True,
        ...
        assert intent.ai_backend_inputs.decision_type == "REDUCE"
        assert "held_position_exit_hysteresis_blocked" not in intent.ai_backend_inputs.reason_codes
...
class TestAssembleAndCreateOrderFullFlow:
    ...
    async def test_assemble_and_create_order_full_flow(
        ...
        """Full flow: assemble → recorder 4 runs → create_order → audit log."""
        repos = build_in_memory_repositories()
        now = datetime.now(timezone.utc)
        ...
        # ── Step 1: assemble() → recorder should have 4 agent runs ──
        intent = await service.assemble(
            sample_request,
        ...
        assert len(runs) == 4, (
            f"Expected 4 agent runs (EI, AR, AC, FDC), got {len(runs)}"
        )
        ...
        assert len(await service._agent_recorder.list_all()) == 4  # unchanged
...
async def test_assemble_creates_decision_context_when_not_provided(
    ...
    # Verify 4 agent runs recorded (recorder has no repo guard → all persisted)
    runs = await service._agent_recorder.list_all()
    assert len(runs) == 4, (
        f"Expected 4 agent runs (EI, AR, AC, FDC), got {len(runs)}"
    )
...
async def test_assemble_fail_open_when_account_missing(service, sample_request):
    ...
    runs = await service._agent_recorder.list_all()
    assert len(runs) == 4, (
        f"Expected 4 agent runs even on fail-open, got {len(runs)}"
    )
...
class TestHeldPositionReduceSkipShadowObservation:
    ...
    reduce 하위 구간만 별도 key(shadow_held_position_reduce_skip)로
    관측하며, 어떤 실행 경로에서도 decision_type/side를 바꾸지 않는다."""

    def _make_decision(self, *, decision_type=DecisionType.REDUCE, side=OrderSide.SELL):
        return TradeDecisionEntity(
            trade_decision_id=uuid4(),
            decision_context_id=uuid4(),
            decision_type=decision_type,
    ...
    async def test_disabled_flag_is_a_full_noop(self) -> None:
        ...
        await service._record_held_position_reduce_skip_shadow_observation(
            trade_decision_id=decision.trade_decision_id,
            assembled_context=assembled_context,
            ar_output=AIRiskOutput(risk_opinion="reduce", risk_score=0.7),
            composer_output=FinalDecisionComposerOutput(decision_type="REDUCE", side="sell"),
            fdc_raw_decision_type="REDUCE",
        )
        ...
        updated = await repos.trade_decisions.get(decision.trade_decision_id)
        assert updated is not None
        assert updated.decision_type == DecisionType.REDUCE
        assert updated.side == OrderSide.SELL
        assert "shadow_held_position_reduce_skip" not in updated.decision_json

    @pytest.mark.asyncio
    async def test_reduce_opinion_agrees_with_actual_reduce(self) -> None:
        """risk_opinion=reduce인 REDUCE_CANDIDATE에서 FDC가 실제로도
        reduce를 냈다면 shadow도 override 재적용으로 reduce가 나와
        agreement=True여야 한다(concentration flag 없어 REDUCE 유지)."""
        repos = build_in_memory_repositories()
        service = DecisionOrchestratorService(
            repos=repos, use_subprocess_isolation=False,
            held_position_reduce_skip_shadow_enabled=True,
        )
        decision = self._make_decision(decision_type=DecisionType.REDUCE, side=OrderSide.SELL)
        await repos.trade_decisions.add(decision)
        ...
        ar_output = AIRiskOutput(risk_opinion="reduce", risk_score=0.7, risk_flags=())
        composer_output = FinalDecisionComposerOutput(decision_type="REDUCE", side="sell")
        ...
        shadow = updated.decision_json["shadow_held_position_reduce_skip"]
        assert shadow["rule_set_version"] == "held_position_reduce_skip_shadow_v1"
        assert shadow["primary_candidate"] == "REDUCE_CANDIDATE"
        assert shadow["risk_opinion"] == "reduce"
        assert shadow["risk_score"] == 0.7
        assert shadow["shadow_skip_candidate"] is True
        assert shadow["shadow_decision_type"] == "HOLD"
        assert shadow["shadow_final_decision_type"] == "REDUCE"
        assert shadow["shadow_final_side"] == "SELL"
        assert shadow["actual_fdc_raw_decision_type"] == "REDUCE"
        assert shadow["actual_final_decision_type"] == "REDUCE"
        assert shadow["held_position_override_applied"] is True
        assert shadow["agreement"] is True
        assert shadow["agreement_decision_only"] is True
        assert shadow["agreement_execution_meaning"] is True
        assert shadow["provider_rate_limit_observed"] is False

    @pytest.mark.asyncio
    async def test_label_mismatch_but_execution_meaning_agrees(self) -> None:
        """FDC가 EXIT를 냈지만 override 시뮬레이션은 concentration flag
        없어 REDUCE로 나오는 경우: 라벨은 다르지만(agreement_decision_
        only=False) 둘 다 실제 매도 시도이므로 agreement_execution_
        meaning=True여야 한다."""
        repos = build_in_memory_repositories()
        service = DecisionOrchestratorService(
            repos=repos, use_subprocess_isolation=False,
        ...
        # risk_flags에 concentration/exposure/over 신호가 없으므로
        # override는 REDUCE를 고른다(EXIT가 아님).
        ar_output = AIRiskOutput(risk_opinion="reduce", risk_score=0.65, risk_flags=())
        composer_output = FinalDecisionComposerOutput(decision_type="EXIT", side="sell")
        ...
        assert shadow["shadow_final_decision_type"] == "REDUCE"
        assert shadow["actual_final_decision_type"] == "EXIT"
        assert shadow["agreement_decision_only"] is False
        assert shadow["agreement_execution_meaning"] is True

    @pytest.mark.asyncio
    async def test_review_opinion_is_not_a_shadow_target(self) -> None:
        """risk_opinion=review는 관측 대상이 아니다(reject/reduce만
        본다) — REDUCE_CANDIDATE 전체는 실제 disagreement가 낮지 않아
        더 보수적으로 좁혔다."""
        repos = build_in_memory_repositories()
        service = DecisionOrchestratorService(
            repos=repos, use_subprocess_isolation=False,
        ...
        await service._record_held_position_reduce_skip_shadow_observation(
            trade_decision_id=decision.trade_decision_id,
            assembled_context=assembled_context,
            ar_output=AIRiskOutput(risk_opinion="review", risk_score=0.7),
            composer_output=composer_output,
            fdc_raw_decision_type="HOLD",
        )
    ...
    @pytest.mark.asyncio
    async def test_watch_primary_candidate_is_not_a_shadow_target(self) -> None:
        """WATCH는 이 관측(REDUCE/SELL_CANDIDATE 전용)의 대상이 아니다
        — 기존 shadow_held_position_fdc_skip이 담당하는 영역이다."""
        repos = build_in_memory_repositories()
        service = DecisionOrchestratorService(
            repos=repos, use_subprocess_isolation=False,
            held_position_reduce_skip_shadow_enabled=True,
        ...
        await service._record_held_position_reduce_skip_shadow_observation(
            trade_decision_id=decision.trade_decision_id,
            assembled_context=assembled_context,
            ar_output=AIRiskOutput(risk_opinion="reduce", risk_score=0.9),
            composer_output=composer_output,
            fdc_raw_decision_type="WATCH",
        )
    ...
    @pytest.mark.asyncio
    async def test_sell_candidate_with_reject_opinion_is_recorded(self) -> None:
        """SELL_CANDIDATE + risk_opinion=reject도 관측 대상이다."""
        repos = build_in_memory_repositories()
        service = DecisionOrchestratorService(
            repos=repos, use_subprocess_isolation=False,
            held_position_reduce_skip_shadow_enabled=True,
        ...
        ar_output = AIRiskOutput(
            risk_opinion="reject", risk_score=0.9,
            risk_flags=("concentration_over_limit",),
        )
        composer_output = FinalDecisionComposerOutput(decision_type="EXIT", side="sell")
        ...
        updated = await repos.trade_decisions.get(decision.trade_decision_id)
        shadow = updated.decision_json["shadow_held_position_reduce_skip"]
        assert shadow["primary_candidate"] == "SELL_CANDIDATE"
        assert shadow["risk_opinion"] == "reject"
        assert shadow["shadow_final_decision_type"] == "EXIT"
        assert shadow["agreement"] is True
    ...
    async def test_repository_write_failure_is_logged_not_raised(self) -> None:
        ...
        repos.trade_decisions.sync_shadow_held_position_reduce_skip_observation = AsyncMock(
            side_effect=RuntimeError("boom")
        )
