...
class TestFDCPositionConcentration:
    """FDC _build_user_prompt() must include Position Concentration section."""

    def _build_fdc_prompt_with_context(
        self,
        *,
        position_qty: Decimal | None = None,
        position_avg_price: Decimal | None = None,
        risk_limit_nav: Decimal | None = None,
        cash_total_asset: Decimal | None = None,
    ) -> str:
        """Helper to build an FDC user prompt with given context snapshots."""
        ...
        request = AgentExecutionRequest(
            decision_context_id=uuid4(),
            correlation_id="test-fdc-concentration",
            context=context,
        )
        agent = FinalDecisionComposerAgent(provider_client=MagicMock())
        return agent._build_user_prompt(request)

    def test_fdc_prompt_contains_concentration(self) -> None:
        """FDC prompt contains 'Position Concentration' section with key fields."""
        prompt = self._build_fdc_prompt_with_context(
            position_qty=Decimal("1000"),
            position_avg_price=Decimal("50000"),
            risk_limit_nav=Decimal("100000000"),
        )
        assert "Position Concentration" in prompt
        assert "Over-concentrated" in prompt
        assert "NAV" in prompt
        assert "REDUCE (partial reduction)" in prompt or "REDUCE" in prompt

    def test_fdc_concentration_over_reduce_policy(self) -> None:
        """Over-concentrated prompt includes REDUCE policy text."""
        prompt = self._build_fdc_prompt_with_context(
            position_qty=Decimal("1000"),
            position_avg_price=Decimal("50000"),
            risk_limit_nav=Decimal("100000000"),
        )
        # 50% concentration → over-concentrated
        assert "Concentration: 50.0% of NAV" in prompt
        assert "Over-concentrated: Yes" in prompt
        # REDUCE policy must be mentioned
        assert "REDUCE" in prompt
        assert "Decision Policy" in prompt
        assert "suppress BUY/APPROVE" in prompt

    def test_fdc_concentration_normal_no_reduce(self) -> None:
        """Normal concentration (5%) does NOT force REDUCE policy mentions
        that imply over-concentrated state."""
        prompt = self._build_fdc_prompt_with_context(
            position_qty=Decimal("100"),
            position_avg_price=Decimal("50000"),
            risk_limit_nav=Decimal("100000000"),
        )
        # 5% concentration → normal
        assert "Concentration: 5.0% of NAV" in prompt
        assert "Over-concentrated: No" in prompt
        # The Decision Policy section is always present, but the
        # over-concentrated-specific directives are conditional on the data.
        # The section header should still be there.
        assert "Decision Policy" in prompt
        # Normal range policy
        assert "maintain existing policy" in prompt

    def test_fdc_concentration_no_position(self) -> None:
