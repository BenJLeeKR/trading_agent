...
| v0.5 | 2026-06-21 07:44 | ChatGPT | 테이블명명규칙 v0.1의 12자리 고정 길이 코드형 표준명 체계와 컬럼명명규칙 v0.1의 컬럼 메타 Rule을 통합 반영. 핵심 테이블 메타에 표준테이블명 매핑, 컬럼 메타에 도메인·Suffix·Audit·품질검증 Rule 적용 기준 보강 | 데이터설계총괄 |
...
- [04. 핵심 기간계 테이블 메타](#04-핵심-기간계-테이블-메타)
- [04A. 테이블 표준명 적용 기준](#04a-테이블-표준명-적용-기준)
...
| 업무영역 | 논리테이블명 | 물리테이블명 | 유형 | 주요키 | 보관주기 | PII | 파티션 기준 | 설명 |
| --- | --- | --- | --- | --- | --- | :---: | --- | --- |
| 고객 | 고객기본 | TB_CUST_MST | Master | CUST_NO | 준영구/법정 | Y | 없음 | 고객번호, 고객구분, 실명확인, 고객상태 |
| 고객 | 고객KYC정보 | TB_CUST_KYC | Master | CUST_NO | 법정 | Y | 없음 | 투자성향, 위험등급, 적합성/적정성 판단 정보 |
| 고객 | 고객주소연락처 | TB_CUST_ADDR | Master | CUST_NO+ADDR_TP_CD | 법정 | Y | 없음 | 주소, 휴대폰, 이메일, 통지 선호 수단 |
| 계좌 | 계좌기본 | TB_ACCT_MST | Master | ACNO | 준영구/법정 | Y | 없음 | 계좌번호, 고객번호, 계좌상품구분, 개설/폐쇄 상태 |
| 계좌 | 계좌제한 | TB_ACCT_RSTR | Master | ACNO+RSTR_CD | 법정 | Y | 없음 | 압류, 사고, 미수동결, 주문제한, 출금제한 |
| 상품 | 종목기본 | TB_ISU_MST | Master | ISU_CD | 준영구 | N | 없음 | 표준코드, 단축코드, 시장, 상품유형, 상장상태 |
| 상품 | 주식종목기본 | TB_STK_ISU_MST | Master | ISU_CD | 준영구 | N | 없음 | 상장주식, 우선주, 관리종목, 대용가능 여부 |
| 상품 | 해외상품기본 | TB_FRGN_ISU_MST | Master | EXCH_CD+SYMB | 준영구 | N | 없음 | 해외거래소 심볼, 통화, 거래단위, 현지시장 |
| 상품 | 채권발행기본 | TB_BOND_ISS_MST | Master | ISU_CD | 준영구 | N | 없음 | 발행일, 만기일, 표면금리, 이자지급주기 |
| 상품 | 파생종목기본 | TB_DERIV_ISU_MST | Master | ISU_CD | 준영구 | N | 없음 | 선물/옵션 표준코드, 만기, 행사가격, 승수 |
| 시세 | 종목시세스냅샷 | TB_MKT_QUOTE | Snapshot | MKT_DT+ISU_CD | 10년 | N | MKT_DT | 현재가, 전일종가, 기준가, 상하한가, 거래정지 |
| 주문 | 주문원장 | TB_ORD_MST | Transaction | ORD_DT+ORD_NO | 10년 이상 | Y | ORD_DT | 주문 접수, 정정, 취소, 주문상태, 주문수량/가격 |
| 주문 | 주문이력 | TB_ORD_HIST | History | ORD_DT+ORD_NO+HIST_SEQ | 10년 이상 | Y | ORD_DT | 주문 상태변경, 정정취소 이력, 거부 사유 |
| 체결 | 체결내역 | TB_EXEC_DTL | Transaction | EXEC_DT+EXEC_NO | 10년 이상 | Y | EXEC_DT | 거래소 체결번호, 체결수량/단가, 수수료/세금 |
| 잔고 | 종목잔고 | TB_BAL_POS | Balance | BAS_DT+ACNO+ISU_CD+BAL_TP_CD+LOAN_DT | 10년 이상 | Y | BAS_DT | 보유수량, 매입금액, 평가금액, 신용구분별 잔고 |
| 예수금 | 예수금잔고 | TB_CASH_BAL | Balance | BAS_DT+ACNO+CURR_CD | 10년 이상 | Y | BAS_DT | 예수금, 주문가능금, 출금가능금, 미수금, D1/D2 예수금 |
| 결제 | 결제예정 | TB_STL_SCHD | Schedule | STL_DT+ACNO+ISU_CD+STL_SEQ | 10년 이상 | Y | STL_DT | 매수/매도 결제예정 금액 및 수량 |
| 출납 | 현금입출금내역 | TB_CASH_IO_DTL | Transaction | TR_DT+IO_NO | 10년 이상 | Y | TR_DT | 입금, 출금, 대체, 환전, 수수료 출금 |
| 신용 | 신용약정 | TB_CRD_AGR | Master | ACNO | 법정 | Y | 없음 | 신용거래 약정, 한도, 만기, 상태 |
| 신용 | 신용융자내역 | TB_CRD_LOAN | Transaction | LOAN_NO | 10년 이상 | Y | LOAN_DT | 융자금, 담보, 이자, 상환상태, 만기 |
| 담보 | 담보평가내역 | TB_COLL_EVAL | Balance | BAS_DT+ACNO | 10년 이상 | Y | BAS_DT | 담보평가금액, 담보비율, 부족금액, 반대매매 대상 |
| 파생 | 파생증거금 | TB_DERIV_MARGIN | Balance | BAS_DT+ACNO+CURR_CD | 10년 이상 | Y | BAS_DT | 예탁총액, 위탁증거금, 유지증거금, 추가증거금 |
| 파생 | 파생정산내역 | TB_DERIV_SETTL_DTL | Transaction | STTL_DT+ACNO+ISU_CD | 10년 이상 | Y | STTL_DT | 정산가격 기준 평가손익 및 정산차금 |
| 채권 | 채권이자스케줄 | TB_BOND_INT_SCHD | Schedule | ISU_CD+INT_PAY_SEQ | 준영구 | N | 없음 | 회차별 이자지급일, 이자계산기간, 만원당 이자 |
| 권리 | 권리이벤트 | TB_RIGHT_EVT | Event | RIGHT_EVT_NO | 준영구 | N | EVT_DT | 배당/증자/감자/분할/합병 등 이벤트 |
| 권리 | 권리배정내역 | TB_RIGHT_ALOC | Transaction | RIGHT_EVT_NO+ACNO+ISU_CD | 10년 이상 | Y | BAS_DT | 계좌별 권리 배정 수량/금액 |
| 수수료 | 수수료세금규칙 | TB_FEE_TAX_RULE | Rule | RULE_ID | 준영구 | N | 없음 | 상품·시장·채널별 수수료/세금 산식 |
| 공통 | 공통코드그룹 | TB_CODE_GRP | Code | GRP_CD | 준영구 | N | 없음 | 코드 그룹 정의 |
| 공통 | 공통코드상세 | TB_CODE_DTL | Code | GRP_CD+CD | 준영구 | N | 없음 | 코드별 유효값, 정렬순서, 사용여부, 유효기간 |
| 인터페이스 | 인터페이스메타 | TB_IF_MSG_META | Interface | IF_ID | 준영구 | N | 없음 | 송수신 시스템, 전문ID, 포맷, 주기, 버전 |
| 인터페이스 | 인터페이스필드메타 | TB_IF_FIELD_META | Interface | IF_ID+FIELD_SEQ | 준영구 | N | 없음 | 전문 필드명, 타입, 길이, 필수여부, 매핑컬럼 |
| 배치 | 배치Job메타 | TB_BATCH_JOB_META | Batch | JOB_ID | 준영구 | N | 없음 | Job명, 스케줄, 선후행, 재처리 가능 여부 |
| 배치 | 배치실행로그 | TB_BATCH_RUN_LOG | Log | RUN_ID | 10년 | N | START_DTTM | 배치 시작/종료, 상태, 오류코드, 처리건수 |
| 감사 | 감사로그 | TB_AUDIT_LOG | Log | LOG_ID | 10년 이상 | Y | LOG_DTTM | 조회/변경/승인/접속 로그 |

### 04.2 META 시스템 자체 테이블

| 논리테이블명 | 물리테이블명 | 유형 | 주요키 | 설명 |
| --- | --- | --- | --- | --- |
| 메타업무영역 | MT_BIZ_DOMAIN | Meta | BIZ_DOMAIN_CD | 고객/계좌/주문/체결 등 업무영역 정의 |
| 메타엔터티 | MT_ENTITY | Meta | ENTITY_ID | 업무 엔터티 정의, 담당부서, 소유자 |
| 메타테이블 | MT_TABLE | Meta | TABLE_ID | 논리/물리 테이블명, 유형, 보관주기, 소유 시스템 |
| 메타컬럼 | MT_COLUMN | Meta | COLUMN_ID | 논리/물리 컬럼명, 타입, 키, NULL, 도메인 |
| 메타표준단어 | MT_STD_WORD | Meta | WORD_ID | 표준 단어, 영문명, 약어, 금칙어 |
| 메타표준용어 | MT_STD_TERM | Meta | TERM_ID | 표준 용어, 구성 단어, 약어 조합 |
| 메타도메인 | MT_DOMAIN | Meta | DOMAIN_ID | 데이터 타입 도메인, 길이, 정밀도, 검증 규칙 |
| 메타코드그룹 | MT_CODE_GRP | Meta | GRP_CD | 코드 그룹, 업무영역, 담당자, 배포 대상 |
| 메타코드상세 | MT_CODE_DTL | Meta | GRP_CD+CD | 코드값, 코드명, 유효기간, 사용여부 |
| 메타관계 | MT_RELATION | Meta | REL_ID | 테이블 간 관계, FK, 업무 관계 |
| 메타인터페이스 | MT_IF_MSG | Meta | IF_ID | 전문 ID, 송수신 시스템, 방식, 버전 |
| 메타인터페이스필드 | MT_IF_FIELD | Meta | IF_ID+FIELD_SEQ | 필드명, 타입, 길이, 필수여부, 매핑 컬럼 |
| 메타배치Job | MT_BATCH_JOB | Meta | JOB_ID | Job명, 스케줄, 선후행, 재처리 정책 |
| 메타품질규칙 | MT_DQ_RULE | Meta | RULE_ID | 품질 규칙, 검증 SQL, 임계치, 담당자 |
| 메타변경요청 | MT_CHANGE_REQ | Meta | CHG_REQ_ID | 메타 변경 요청, 승인 상태, 배포 상태 |
| 메타승인이력 | MT_APPROVAL_HIST | Meta | CHG_REQ_ID+APPR_SEQ | 승인자, 승인일시, 승인 의견 |
