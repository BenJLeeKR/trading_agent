...
| v0.5.3 | 2026-06-21 | ChatGPT | 테이블 표준명 12자리 구성요소별 코드 사전 보강. 1번째 시스템구분, 2~3번째 업무영역, 4~6번째 업무객체, 7~8번째 세부처리구분, 9번째 테이블성격 코드의 의미·적용 기준·예외 기준 추가 | 데이터설계총괄 |
...
- [04. 테이블 표준명 적용 기준](#04-테이블-표준명-적용-기준)
  - [04.1 테이블명 12자리 포맷](#041-테이블명-12자리-포맷)
  - [04.2 위치별 코드 사전](#042-위치별-코드-사전)
  - [04.3 표준테이블명 매핑 예시](#043-표준테이블명-매핑-예시)
...
표준테이블명은 다음 12자리 고정 길이 구조를 사용합니다.
...
| 위치 | 자리수 | 구성요소 | 의미 | 예시 |
| ---: | ---: | --- | --- | --- |
| 1 | 1 | `S` 시스템구분 | Core, Meta, Interface, Stage, Work, Log 등 시스템/저장소 성격 | `C`, `M`, `I`, `S`, `W`, `L` |
| 2~3 | 2 | `DD` 업무영역 | 고객, 계좌, 주문, 체결, 잔고 등 업무 대분류 | `CU`, `AC`, `OR`, `EX`, `BA` |
| 4~6 | 3 | `BBB` 업무객체 | 업무영역 내 관리 객체 | `MST`, `ORD`, `EXC`, `POS` |
| 7~8 | 2 | `OO` 세부처리구분 | 기본, 상세, 이력, 잔고, 정산, 평가, Rule 등 처리 성격 | `BS`, `DT`, `HS`, `BL` |
| 9 | 1 | `T` 테이블성격 | Master, Transaction, Detail, History, Balance 등 테이블 유형 | `M`, `T`, `D`, `H`, `B` |
| 10 | 1 | `V` 버전/세대 | 테이블 구조 세대 | `0`, `1`, `2`, `A`, `B` |
| 11~12 | 2 | `NN` 일련번호 | 동일 분류 내 순번 | `01`~`99` |
...
| 분해 | 값 | 의미 |
| --- | --- | --- |
| `S` | `C` | Core, 기간계 핵심 업무 테이블 |
| `DD` | `OR` | 주문 업무영역 |
| `BBB` | `ORD` | 주문 객체 |
| `OO` | `BS` | 기본/대표 원장 |
| `T` | `T` | Transaction, 거래 원장 |
| `V` | `0` | 최초 표준 버전 |
| `NN` | `01` | 주문 기본 원장 테이블 |

따라서 `CORORDBST001`은 **기간계 주문 업무의 주문 기본 거래원장 테이블**을 의미합니다.

---

### 04.2 위치별 코드 사전

#### 04.2.1 1번째 자리: 시스템구분 코드 `S`

1번째 자리는 테이블이 어느 시스템 성격 또는 저장소 계층에 속하는지를 나타냅니다.

| 코드 | 영문명 | 한글명 | 적용 대상 | 적용 기준 | 예시 |
| --- | --- | --- | --- | --- | --- |
| `C` | Core | 기간계/Core | 고객, 계좌, 주문, 체결, 잔고, 결제 등 핵심 업무 원장/마스터 | 실제 업무 트랜잭션의 원천 또는 업무 기준 데이터 | `CORORDBST001`, `CEXEXCDTT001` |
| `M` | Meta | META | 테이블, 컬럼, 도메인, 코드, Rule, 변경요청 등 메타 관리 테이블 | META 시스템 자체가 관리하는 기준정보 | `MMTTBLBSM001`, `MMTCOLBSM001` |
| `I` | Interface | 인터페이스 | 대외기관, 거래소, FEP, 채널 송수신 전문 및 연계 처리 | 전문 원문, 전문 헤더, 송수신 결과, 매핑 전후 데이터 | `IIFMSGIFI001` |
| `S` | Stage | 스테이징 | 외부 원천 적재, 이행 적재, 정합성 검증 전 데이터 | 정식 원장 반영 전 임시 적재/검증 계층 | `SISSTKDTW001` |
| `W` | Work | 작업/Work | 배치 중간 작업, 집계, 재처리, 정렬, 산출 임시 테이블 | 운영 배치 과정에서 생성되는 중간 산출물 | `WBAPOSWKB001` |
| `L` | Log | 로그/감사 | 감사로그, 접근로그, 변경로그, 배치실행로그 | 행위 추적, 장애 분석, 감사 대응 목적 | `LAULOGLGL001` |
| `R` | Report | 리포트/조회 | 조회 성능용 집계, 통계, 리포트 테이블 | 원천 원장이 아니라 조회 최적화/집계 목적 | `RBAPOSSNB001` |
| `B` | Backup | 백업/보정전 | 보정 전 스냅샷, 이행 백업, 복구용 보관 | 원본 복구 또는 이행 검증 목적의 보관 테이블 | `BBAPOSBKB001` |

> 주의: `C`는 Customer가 아니라 **Core**입니다. 고객 업무영역은 2~3번째 자리 `CU`로 표현합니다.  
> 주의: `M`은 Master가 아니라 **Meta**입니다. Master 여부는 9번째 자리 `M`으로 표현합니다.

#### 04.2.2 2~3번째 자리: 업무영역 코드 `DD`

2~3번째 자리는 업무 대분류를 나타냅니다. 시스템구분 코드와 독립적으로 사용됩니다.

| 코드 | 업무영역 | 설명 | 대표 객체 | 적용 예 |
| --- | --- | --- | --- | --- |
| `CU` | 고객 | 고객기본, KYC, 주소, 연락처, 고객상태 | `MST`, `KYC`, `ADR` | `CCUMSTBSM001` 고객기본 |
| `AC` | 계좌 | 계좌기본, 계좌권한, 계좌제한, 계좌속성 | `MST`, `AUT`, `RST` | `CACMSTBSM001` 계좌기본 |
| `IS` | 상품/종목 | 주식, ETF, ETN, 채권, 파생, 해외상품 | `MST`, `STK`, `BND`, `DRV` | `CISMSTBSM001` 종목기본 |
| `MK` | 시세 | 현재가, 호가, 기준가, 지수, 환율 | `QUO`, `IDX`, `FXR` | `CMKQUOSNB001` 종목시세스냅샷 |
| `OR` | 주문 | 주문접수, 정정, 취소, 주문상태 | `ORD`, `RJT` | `CORORDBST001` 주문원장 |
| `EX` | 체결 | 체결내역, 부분체결, 거래소 체결결과 | `EXC` | `CEXEXCDTT001` 체결내역 |
| `BA` | 잔고 | 종목잔고, 상품잔고, 평가잔고 | `POS` | `CBAPOSBLB001` 종목잔고 |
| `CA` | 예수금/현금 | 예수금, 주문가능금, 출금가능금, 입출금 | `CSH`, `IO` | `CCACSHBLB001` 예수금잔고 |
| `ST` | 결제 | 결제예정, 결제완료, 결제대사 | `SCH`, `RCN` | `CSTSCHSTS001` 결제예정 |
| `CR` | 신용공여 | 신용약정, 신용융자, 상환, 이자 | `AGR`, `LON` | `CCRAGRBSM001`, `CCRLONDTT001` |
| `CL` | 담보 | 담보평가, 담보부족, 반대매매 | `EVL` | `CCLEVLEVB001` 담보평가 |
| `DR` | 파생 | 선물옵션 종목, 증거금, 정산, 미결제약정 | `MRG`, `STL` | `CDRMRGBLB001` 파생증거금 |
| `BD` | 채권 | 채권발행, 이자스케줄, 상환, 경과이자 | `ISS`, `INT` | `CBDISSBSM001` 채권발행기본 |
| `RT` | 권리 | 배당, 유상증자, 무상증자, 분할, 합병 | `EVT`, `ALC` | `CRTEVTBSE001` 권리이벤트 |
| `FE` | 수수료/세금 | 수수료율, 세율, 비용 산정 규칙 | `FTR` | `CFEFTRRLR001` 수수료세금규칙 |
| `CH` | 채널 | HTS, MTS, API, 영업점, ARS | `MST`, `USR` | 채널마스터 |
| `BR` | 조직/직원 | 부점, 직원, 권한, 담당자 | `MST`, `EMP` | 부점/직원 |
| `CM` | 공통 | 공통코드, 공통도메인, 공통 설정 | `COD`, `DOM` | `CCMCODCDC001` 공통코드그룹 |
| `IF` | 인터페이스 | 전문, 필드, 송수신, 매핑 | `MSG`, `FLD` | `MIFMSGIFI001` 메타인터페이스전문 |
| `BT` | 배치 | 배치 Job, Step, 스케줄, 실행로그 | `JOB`, `RUN` | `MBTJOBBSM001` 메타배치Job |
| `DQ` | 품질 | 품질규칙, 검증결과, 예외승인 | `RUL`, `RST` | `MDQRULRLQ001` 메타품질규칙 |
| `AU` | 감사 | 감사로그, 접근로그, 변경이력 | `LOG` | `LAULOGLGL001` 감사로그 |
| `MT` | 메타 | 테이블, 컬럼, 용어, 단어, 도메인, 관계 | `TBL`, `COL`, `TRM`, `WRD`, `REL` | `MMTCOLBSM001` 메타컬럼 |

#### 업무영역 코드 적용 원칙

1. 업무영역은 회사 조직명이 아니라 **데이터의 업무 의미**를 기준으로 부여합니다.
2. 동일 테이블이 여러 업무에서 사용되더라도, 원천 소유 업무영역을 우선합니다.
3. `MT`는 업무영역 코드이고, 1번째 자리 `M`은 시스템구분입니다. 예: `MMTCOLBSM001` = Meta 시스템 + Meta 업무영역 + Column 객체.
4. 고객 계좌처럼 복합 의미가 있는 경우, 주 식별자보다 **테이블의 주 업무 책임 영역**을 기준으로 합니다. 예: 계좌기본은 고객번호를 포함하더라도 `AC`입니다.

#### 04.2.3 4~6번째 자리: 업무객체 코드 `BBB`

4~6번째 자리는 업무영역 내 세부 관리 객체를 나타냅니다.

| 업무영역 | 객체코드 | 객체명 | 의미 | 예시 |
| --- | --- | --- | --- | --- |
| `CU` | `MST` | 고객기본 | 고객 식별, 실명, 고객상태 | `CCUMSTBSM001` |
| `CU` | `KYC` | 고객KYC | 고객확인, 투자성향, 위험등급 | `CCUKYCBSM001` |
| `CU` | `ADR` | 고객주소 | 주소, 연락처, 통지수단 | `CCUADRDTM001` |
| `AC` | `MST` | 계좌기본 | 계좌번호, 계좌상태, 계좌상품구분 | `CACMSTBSM001` |
| `AC` | `RST` | 계좌제한 | 사고, 압류, 거래정지, 출금제한 | `CACRSTBSM001` |
| `IS` | `MST` | 종목기본 | 상품 공통 종목정보 | `CISMSTBSM001` |
| `MK` | `QUO` | 시세/현재가 | 현재가, 기준가, 상하한가, 호가 | `CMKQUOSNB001` |
| `OR` | `ORD` | 주문 | 주문접수, 정정, 취소, 상태 | `CORORDBST001` |
| `OR` | `RJT` | 주문거부 | 주문 거부 및 오류 사유 | 주문거부 |
| `EX` | `EXC` | 체결 | 체결내역, 부분체결, 거래소 체결번호 | `CEXEXCDTT001` |
| `BA` | `POS` | 보유잔고 | 계좌별 상품 보유수량 및 평가 | `CBAPOSBLB001` |
| `CA` | `CSH` | 현금/예수금 | 예수금, 주문가능금, 출금가능금 | `CCACSHBLB001` |
| `ST` | `SCH` | 결제예정 | T+N 결제 예정 수량 및 금액 | `CSTSCHSTS001` |
| `ST` | `RCN` | 결제대사 | 체결, 결제, 예수금 대사 | 결제대사 |
| `CR` | `AGR` | 신용약정 | 신용거래 약정 및 한도 | `CCRAGRBSM001` |
| `CR` | `LON` | 신용융자 | 융자금, 상환, 이자, 만기 | `CCRLONDTT001` |
| `CL` | `EVL` | 담보평가 | 담보평가금액, 담보비율 | `CCLEVLEVB001` |
| `DR` | `MRG` | 증거금 | 위탁/유지/추가증거금 | `CDRMRGBLB001` |
| `DR` | `STL` | 파생정산 | 정산가격, 정산차금, 평가손익 | `CDRSTLSTT001` |
| `BD` | `ISS` | 채권발행 | 발행일, 만기일, 표면금리 | `CBDISSBSM001` |
| `BD` | `INT` | 채권이자 | 이자스케줄, 경과이자 | `CBDINTSTS001` |
| `RT` | `EVT` | 권리이벤트 | 배당, 증자, 분할, 합병 이벤트 | `CRTEVTBSE001` |
| `RT` | `ALC` | 권리배정 | 계좌별 권리 배정 내역 | `CRTALCDTT001` |
| `FE` | `FTR` | 수수료세금 | 수수료율, 세율, 산식 | `CFEFTRRLR001` |
| `CM` | `COD` | 코드 | 코드그룹, 코드상세 | `CCMCODCDC001` |
| `CM` | `DOM` | 도메인 | 데이터 타입 도메인 | 공통도메인 |
| `MT` | `TBL` | 테이블메타 | 테이블 정의 메타 | `MMTTBLBSM001` |
| `MT` | `COL` | 컬럼메타 | 컬럼 정의 메타 | `MMTCOLBSM001` |
| `MT` | `TNR` | 테이블명규칙 | 테이블명 구조 Rule | `MMTTNRRLM001` |
| `MT` | `CNR` | 컬럼명규칙 | 컬럼명 Rule | `MMTCNRRLM001` |
| `MT` | `SNM` | 표준명매핑 | 표준명/물리명/레거시명 매핑 | `MMTSNMMPM001` |

#### 업무객체 코드 적용 원칙

1. 업무객체 코드는 3자리 고정입니다.
2. 동일 업무영역 내에서 동일 객체코드는 동일 의미로만 사용합니다.
3. `MST`는 모든 업무영역에서 “기본/마스터 객체”로 사용할 수 있습니다.
4. 객체코드가 부족한 경우 임의 생성하지 않고 `MT_TBL_NM_CODE`에 등록·승인 후 사용합니다.
5. 객체코드는 테이블성격이 아니라 **업무상 관리 대상**을 의미합니다. 예: 주문 객체 `ORD`는 원장, 이력, 오류 테이블 모두에서 사용할 수 있습니다.

#### 04.2.4 7~8번째 자리: 세부처리구분 코드 `OO`

7~8번째 자리는 같은 업무객체 안에서 기본, 상세, 이력, 잔고, 평가, Rule 등 세부 처리 성격을 나타냅니다.

| 코드 | 영문명 | 한글명 | 설명 | 대표 테이블성격 |
| --- | --- | --- | --- | --- |
| `BS` | Basic | 기본 | 대표 속성, 기본 원장, 기본 마스터 | `M`, `T` |
| `DT` | Detail | 상세 | 상세내역, 하위내역, 세부 항목 | `D`, `T` |
| `HS` | History | 이력 | 변경이력, 상태이력, 변경 전후 추적 | `H` |
| `BL` | Balance | 잔고 | 기준일 잔고, 수량/금액 스냅샷 | `B` |
| `ST` | Settlement | 결제/정산 | 결제, 정산, 확정 처리 | `S`, `T` |
| `EV` | Evaluation | 평가 | 평가, 산정, 계산 결과 | `B`, `T` |
| `RL` | Rule | 규칙 | 업무 규칙, 산식, 정책 | `R` |
| `CD` | Code | 코드 | 코드그룹, 코드값 | `C` |
| `DM` | Domain | 도메인 | 데이터 타입/값 범위 도메인 | `M` |
| `MP` | Mapping | 매핑 | 표준명-물리명, 전문-컬럼 매핑 | `M`, `D` |
| `RQ` | Request | 요청 | 신청, 접수, 변경요청 | `A`, `T` |
| `RS` | Response | 응답 | 응답, 결과 회신 | `T`, `D` |
| `RC` | Reconcile | 대사 | 정합성 비교, 대사 결과 | `T`, `Q` |
| `ER` | Error | 오류 | 오류, 예외, 거부 | `X`, `L` |
| `LG` | Log | 로그 | 처리 로그, 실행 로그, 감사 로그 | `L` |
| `AP` | Approval | 승인 | 승인, 결재, 승인 이력 | `A`, `H` |
| `WK` | Work | 작업 | 중간처리, 작업용 데이터 | `W` |
| `BK` | Backup | 백업 | 백업, 보정 전 보관 | `B`, `W` |
| `SN` | Snapshot | 스냅샷 | 특정 시점 복제/스냅샷 | `B` |
| `IF` | Interface | 인터페이스 | 전문 송수신, 연계 처리 | `I` |

#### 세부처리구분과 테이블성격 조합 권장 Matrix

| 세부처리구분 | 권장 테이블성격 | 예시 |
| --- | --- | --- |
| `BS` | `M`, `T` | 고객기본 `CCUMSTBSM001`, 주문원장 `CORORDBST001` |
| `DT` | `D`, `T` | 체결내역 `CEXEXCDTT001`, 권리배정내역 `CRTALCDTT001` |
| `HS` | `H` | 주문이력 `CORORDHSH001` |
| `BL` | `B` | 종목잔고 `CBAPOSBLB001`, 예수금잔고 `CCACSHBLB001` |
| `ST` | `S`, `T` | 결제예정 `CSTSCHSTS001`, 파생정산내역 `CDRSTLSTT001` |
| `RL` | `R`, `Q` | 수수료세금규칙 `CFEFTRRLR001`, 품질규칙 `MDQRULRLQ001` |
| `CD` | `C`, `D` | 공통코드그룹 `CCMCODCDC001`, 공통코드상세 `CCMCODDTD001` |
| `LG` | `L` | 감사로그 `LAULOGLGL001` |
| `RQ` | `A`, `T` | 메타변경요청 `MMTCHGRQA001` |
| `MP` | `M`, `D` | 표준명매핑 `MMTSNMMPM001` |

#### 04.2.5 9번째 자리: 테이블성격 코드 `T`

9번째 자리는 테이블의 데이터 성격을 직접 표현합니다. 이 자리가 운영 정책, 보관주기, Audit, 파티션, 변경 가능 여부를 결정하는 핵심 기준입니다.

| 코드 | 영문명 | 한글명 | 의미 | 설계/운영 기준 |
| --- | --- | --- | --- | --- |
| `M` | Master | 마스터 | 기준정보, 속성정보, 현재 상태 | `USE_YN`, `DEL_YN`, `DATA_VER_NO` 권장 |
| `T` | Transaction | 거래/원장 | 주문, 체결, 입출금처럼 발생성 데이터 | 물리삭제 지양, 변경 이력/Audit 필수 |
| `D` | Detail | 상세 | 상위 데이터의 하위 상세 | 상위 PK + 순번 구조 권장 |
| `H` | History | 이력 | 변경 전후, 상태변경, 이력 추적 | 원천 PK + `HIST_SEQ` 또는 변경일시 |
| `B` | Balance | 잔고 | 기준일자별 수량/금액 상태 | `BAS_DT` 파티션 권장 |
| `S` | Schedule | 스케줄/예정 | 결제예정, 이자스케줄, 처리예정 | 예정일자 기준 관리 |
| `E` | Event | 이벤트 | 권리, 기업행위, 공시성 이벤트 | 이벤트번호/이벤트일자 관리 |
| `R` | Rule | 규칙 | 수수료, 세금, 산식, 업무 정책 | 유효기간, 승인상태 필수 |
| `C` | Code | 코드 | 코드그룹, 코드상세 | 유효기간, 사용여부 필수 |
| `L` | Log | 로그 | 실행로그, 감사로그, 접근로그 | Append-only, 변경/삭제 제한 |
| `W` | Work | 작업 | 중간처리, 임시작업 | 보관주기 짧음, 재생성 가능 |
| `I` | Interface | 인터페이스 | 전문, 송수신, 원문 | 원천추적 컬럼 필수 |
| `Q` | Quality | 품질 | 품질규칙, 검증결과 | Rule ID, 결과상태 관리 |
| `A` | Approval | 승인 | 결재, 변경요청, 승인흐름 | 승인자/승인일시/상태 필수 |
| `X` | Exception | 예외 | 오류, 거부, 예외처리 | 오류코드/메시지/재처리상태 필수 |

#### 테이블성격별 기본 설계 기준

| 테이블성격 | 주요 키 기준 | Audit | 파티션 | 변경 기준 |
| --- | --- | --- | --- | --- |
| `M` Master | 업무 식별자 | 등록/변경 필수 | 보통 없음 | UPDATE 가능, 이력 권장 |
| `T` Transaction | 업무일자 + 거래번호 | 등록/변경/원천추적 필수 | 업무일자 | UPDATE 제한, 취소/정정 이력 |
| `D` Detail | 상위 PK + 순번 | 등록/변경 필수 | 상위 기준 | 상위 생명주기 종속 |
| `H` History | 원천 PK + 이력순번 | 생성 Audit 필수 | 원천 업무일자 | Append-only |
| `B` Balance | 기준일자 + 계좌/상품 | 배치추적 필수 | 기준일자 | 재산출/보정 이력 |
| `S` Schedule | 예정일자 + 업무키 | 처리상태 필수 | 예정일자 | 처리상태 변경 |
| `E` Event | 이벤트번호 | 등록/변경 필수 | 이벤트일자 | 이벤트 정정 이력 |
| `R` Rule | Rule ID + 유효기간 | 승인/Audit 필수 | 보통 없음 | 승인 후 변경 제한 |
| `C` Code | 코드그룹 + 코드 | 등록/변경 필수 | 보통 없음 | 유효기간 관리 |
| `L` Log | 로그ID 또는 실행ID | 생성 Audit | 일시 | 변경 금지 |
| `I` Interface | IF ID + MSG ID | 원천추적 필수 | 수신일시 | 재처리 상태 관리 |
| `Q` Quality | Rule ID 또는 검증ID | 등록/변경 필수 | 기준일자 | 검증결과 보관 |
| `A` Approval | 요청ID + 승인순번 | 승인 Audit 필수 | 요청일자 | 승인흐름 변경 이력 |
| `X` Exception | 오류ID 또는 원천거래ID | 오류추적 필수 | 발생일자 | 재처리 가능 |

#### 04.2.6 10번째 자리: 버전/세대 코드 `V`

| 코드 | 의미 | 적용 기준 |
| --- | --- | --- |
| `0` | 최초 표준 | 최초 설계 버전 |
| `1` | 1차 개편 | PK, 파티션, 주요 업무 식별체계 변경 |
| `2` | 2차 개편 | 추가 구조 개편 |
| `A` | 차세대 A안 | 차세대 병행 운영 A안 |
| `B` | 차세대 B안 | 차세대 병행 운영 B안 |
| `Z` | 이행/전환 | 마이그레이션, 전환, 임시 병행 |

버전 코드는 단순 컬럼 추가로 변경하지 않습니다. PK 구조, 파티션 기준, 업무 식별 체계, 테이블성격이 바뀌는 수준의 변경에서만 세대 변경을 검토합니다.

#### 04.2.7 11~12번째 자리: 일련번호 `NN`

| 범위 | 용도 | 설명 |
| --- | --- | --- |
| `01` | 기본 테이블 | 해당 조합의 대표 테이블 |
| `02`~`09` | 동일 객체 추가 테이블 | 동일 업무객체의 확장 테이블 |
| `10`~`49` | 업무 확장 | 업무 기능 확장 |
| `50`~`79` | 채널/제휴/대외 특화 | 특정 채널, 기관, 제휴 전용 |
| `80`~`89` | 운영/보정/예외 | 보정, 재처리, 운영 지원 |
| `90`~`99` | 이행/임시/폐기예정 | 마이그레이션, 임시, 폐기 예정 |
